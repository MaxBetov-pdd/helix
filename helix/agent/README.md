# Helix Agent Harness (`helix.agent`)

Drive the running Helix backend from **any AI harness** — Claude Code, Codex, a
Tauri-embedded agent, a sidecar, CI, or a plain script — over Helix's HTTP API.
No MCP required.

## Why this exists

The Helix MCP server (`helix.mcp_server`) is a thin **stdio wrapper** over the
...
HTTP API directly, so **anything that can make an HTTP request can use Helix** —
with **zero third-party dependencies** (Python stdlib only).

```
        ┌───────────────────────────────────────────────┐
        │ Backend: helix.api:app  (uvicorn :8003)        │
        │ REST /api/...  +  scheduler / gauntlet / DB      │
        └───────────────┬─────────────────────────────────┘
                        │  HTTP (:8003/api)
   Tauri/Svelte UI ─────┼───── MCP server (stdio) ───── helix.agent (this)
...
- Backend running on `:8003` (`start_all.ps1` supervises it). Verify: `python -m helix.agent health`.
- Optional auth (only if `:8003` is exposed beyond localhost): set `HELIX_API_KEY` /
  `HELIX_OPERATOR_KEY` (env) or pass `--api-key/--operator-key`. Local calls need none.
- Override the origin with `HELIX_API_URL` or `--base-url`.
...
python -m helix.agent context --out .tmp/ctx.json     # context is large; save it
python -m helix.agent skills --regime range_bound
python -m helix.agent list --status paper
python -m helix.agent strategy S02545                  # full container
python -m helix.agent gate-report S02545               # why it is/isn't promotable
python -m helix.agent status S02545,S02604             # {stage,status} for polling
python -m helix.agent runs --limit 10
python -m helix.agent result <result_id>

# write / lifecycle
python -m helix.agent create-session --label hunt --objective "find paper strats"
python -m helix.agent register --file /abs/path/strat.py --session ADZ-0001
python -m helix.agent backtest --strategy S02545 --dataset BTC/USDT-1h --compact
python -m helix.agent backtest --strategy S02545 --dataset ADA/USDT-1h \
    --trade-mode short_only --params '{"base_horizon":48}' --compact
python -m helix.agent optimize --strategy S02545 --dataset BTC/USDT-1h --n-trials 30
python -m helix.agent verdict  --strategy S02545 --dataset BTC/USDT-1h --tests walk_forward,cost_stress
python -m helix.agent promote  --strategy S02550 --to gauntlet --from quick_screen
...
python -m helix.agent wait-paper --strategies S02545,S02604 --timeout 1800 --interval 90
```
...
fc = HelixAgentClient()                       # http://127.0.0.1:8003 by default
...
# write a strategy .py to helix/strategies/custom/, then:
...
1. **Design** a `.py` strategy (extend `helix.strategies.base.BaseStrategy`;
...
- `quick_screen` judges **both** the IS and OOS windows: PF ≥ 1.05 (aim ≥ 1.3),
  ...
  (`HelixAgentClient.quick_screen(compact)` pre-checks this offline.)
