...
"""Helix agent CLI — drive the backend from any shell. JSON in, JSON out.
...
Examples:
    ...
    python -m helix.agent context --out .tmp/ctx.json      # context is large
...
from .client import HelixAgentClient, HelixAPIError
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="helix-agent", description="Drive the Helix backend over HTTP.")
    p.add_argument("--base-url", default=None, help="backend origin (default env HELIX_API_URL or http://127.0.0.1:8003)")
...
def main(argv=None) -> int:
    ...
    fc = HelixAgentClient(base_url=args.base_url, api_key=args.api_key,
