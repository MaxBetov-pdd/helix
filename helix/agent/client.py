"""Zero-dependency HTTP client + high-level harness for the Helix backend.
...
Endpoint map mirrors helix/mcp_server/server.py (the proven set):
  GET  /api/health
...
class HelixAgentClient:
    """Blocking, stdlib-only client for the Helix REST API.

    Args:
        base_url: backend origin (default env HELIX_API_URL or :8003).
        api_key / operator_key: optional auth headers (env HELIX_API_KEY /
            HELIX_OPERATOR_KEY). Not needed for local calls; required only if
    ...
    def __init__(
        ...
        self.base_url = (base_url or os.environ.get("HELIX_API_URL") or DEFAULT_BASE_URL).rstrip("/")
        self.api_key = api_key or os.environ.get("HELIX_API_KEY") or ""
        self.operator_key = operator_key or os.environ.get("HELIX_OPERATOR_KEY") or ""
    ...
    def _request(self, method: str, path: str, params: dict | None = None,
        ...
        try:
            ...
            raise HelixAPIError(exc.code, exc.read().decode(errors="replace"), method, path) from None
    ...
    def get_gate_report(self, strategy_id: str) -> dict:
        ...
        Mirrors the MCP helix_get_gate_report; use to diagnose why a strategy
