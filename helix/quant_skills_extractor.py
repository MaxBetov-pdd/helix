...
from helix.quant_skills import (
    QuantSkill,
    read_skill,
    store_hypothesis,
    update_skill,
...
log = logging.getLogger("helix.quant_skills_extractor")
...
def _extract_and_store(backtest_result: dict) -> None:
    ...
    try:
        from helix.vectordb import search_quant_skills, upsert_quant_skill
        ...
        if insight["action"] == "update_skill" and insight.get("skill_name"):
            ...
            if h.count >= PROMOTION_THRESHOLD:
                from helix.quant_skills import promote_hypothesis
