...
class DataHub:
    ...
    def status(self) -> dict[str, object]:
        ...
        from helix.dataeng.source import get_source_registry
        from helix.dataeng.stream import get_stream_manager
    ...
    def trades(self, symbol: str, *, start: object | None = None, end: object | None = None, source: str = "binance") -> pd.DataFrame:
        from helix.dataeng.microstructure import read_micro_rows
...
def _normalize_projected_frame(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    from helix.data import _normalize_ohlcv_frame
...
def _available_enrichment_specs(
    ...
    from helix import data_manager
...
def _quality_from_path(path: Path, symbol: str, timeframe: str) -> dict[str, object]:
    from helix.data import _freshness_for, _timeframe_to_ms, _to_iso
