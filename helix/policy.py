...
from helix.db import create_approval, get_db, kv_get, kv_set, log_activity, log_gate_rejection

from helix.gauntlet.legitimacy import validate_robustness_payload
from helix.util import normalize_stage

log = logging.getLogger("helix.policy")
...
def load_pipeline_config() -> dict:
    ...
    config = kv_get("helix:pipeline_thresholds")
    ...
    if isinstance(config, dict) and config != normalized:
        kv_set("helix:pipeline_thresholds", normalized)
    return normalized

def save_pipeline_config(config: dict):
    """Save pipeline thresholds to KV store."""
    kv_set("helix:pipeline_thresholds", _normalize_pipeline_config(config))
...
def _load_pipeline_settings() -> dict:
    ...
    raw = kv_get("helix:pipeline:settings")
    if not isinstance(raw, dict):
        raw = {}
    # Merge with defaults so new keys are always present
    from helix.api_core import _DEFAULT_PIPELINE_SETTINGS
...
def verify_backtest_persisted(strategy_id: str) -> tuple[bool, str, dict]:
    ...
    from helix.vectordb import search_backtest_results
...
def verify_backtest_exists_for_stage_transition(strategy_id: str, target_stage: str) -> tuple[bool, str]:
    ...
    from helix.db import get_db as _get_db
...
def _evaluate_source_divergence_gate(strategy_id: str, general_settings: Any) -> tuple[bool, str]:
    ...
    ``helix-source-reconciliation`` job and read here cache-only via ``kv_get``
    ...
    payload = kv_get(f"helix:data:divergence:{symbol}:{timeframe}")
...
def _paper_slot_competition_enabled(settings: object = None) -> bool:
    ...
    unchanged. Wired setting: helix:settings.paper_slot_competition_enabled.
    """
    if settings is None:
...
def evaluate_promotion(strategy_id: str, from_stage: str, to_stage: str) -> tuple[bool, str]:
    ...
    general_settings = kv_get("helix:settings")
    ...
    if normalized_to not in {"archived", "rejected"}:
        ...
        if not current_sym or current_sym == "GENERIC":
            ...
            from helix.db import auto_assign_best_symbol
            ...
            if row:
                ...
                if symbol:
                    try:
                        ...
                        from helix.regime import peek_cached_regime
...
def _dethrone_cooldown_key(strategy_id: str) -> str:
    return f"helix:dethrone:cooldown:{strategy_id}"
        ...
        for victim_id, victim_sharpe in members[1:]:
            ...
            if not dry_run:
                try:
                    ...
                    _logging.getLogger("helix.policy").exception(
                        "dedupe_capital_slots: failed to archive %s", victim_id
                    )
                    continue
            archived.append(entry)
    if archived:
        _logging.getLogger("helix.policy").info(
...
def _maybe_auto_apply_dethrone(approval_id: int | None, settings: object) -> None:
    ...
    if not auto_dethrone:
        try:
            from helix.brain import _auto_approve_promotions_enabled
...
def _implausible_metrics_reason(metrics: dict, config: dict) -> str | None:
    ...
    # +/-10 is the backtest clamp (helix.strategies.backtest._MAX_ABS_RISK_RATIO):
...
def _evaluate_gauntlet_gate(strategy_id: str, config: dict) -> tuple[bool, str]:
    ...
    if bool(rob_thresholds.get("deflated_sharpe_gate_enabled", False)):
        try:
            from helix.gauntlet.deflated_sharpe import compute_strategy_dsr
