...
H-Op1: long-running helix processes (lab worker, bot runner, API server)
...
def setup_rotating_file_logger(
    ...
    *,
    ...
    max_bytes: rotation threshold (HELIX_LOG_MAX_BYTES env override).
    backup_count: number of rotated files to keep
                  (HELIX_LOG_BACKUP_COUNT env override).
    ...
    """
    resolved_max = _env_int("HELIX_LOG_MAX_BYTES", max_bytes or DEFAULT_MAX_BYTES)
    resolved_backup = _env_int("HELIX_LOG_BACKUP_COUNT", backup_count or DEFAULT_BACKUP_COUNT)
    ...
    try:
        from helix.correlation import RequestIdLogFilter
    ...
    try:
        from helix.redact import RedactingLogFilter
