...
def _expected_lab_db_path() -> Path:
    return Path(config.HELIX_LAB_DB).resolve()
...
@contextmanager
def get_lab_db():
    """Get isolated lab DB connection (never touches production helix.db)."""
    config.ensure_dirs()
    conn = sqlite3.connect(str(config.HELIX_LAB_DB), timeout=5)
