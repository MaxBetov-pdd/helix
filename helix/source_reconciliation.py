...
The promotion gate (``helix.policy.evaluate_promotion``) must never fetch market
...
result to KV under ``helix:data:divergence:{SYMBOL}:{TIMEFRAME}``. The gate later
...
from helix.data import get_dataset_source, load_parquet, reconcile_close_prices
from helix.db import get_db, kv_set_best_effort
...
def divergence_key(symbol: str, timeframe: str) -> str:
    ...
    return f"helix:data:divergence:{str(symbol).strip().upper()}:{str(timeframe).strip().lower()}"
