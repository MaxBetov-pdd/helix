...
from helix.db import _now
from helix.market_data import fetch_hyperliquid_candles

log = logging.getLogger("helix.api")

_REMOTE_DATA_ROOT_ENV = "HELIX_REMOTE_ENGINE_DATA_ROOT"
_REMOTE_DATA_ALLOWED_ROOT_ENV = "HELIX_REMOTE_ENGINE_ALLOWED_ROOT"
...
def _normalize_dataset_rows(raw_rows: list[dict]) -> list[dict[str, object]]:
    from helix.data import classify_dataset_asset_class, dataset_market_type
...
def _scan_remote_data_root_datasets(remote_root: str) -> list[dict[str, object]]:
    from helix.data import _dataset_from_file
...
def get_data_ingestion_runs(
    ...
    from helix.data import get_active_ingestion_runs, scan_datasets
...
def get_cached_data_ingestion_runs(
    ...
    from helix.data import get_active_ingestion_runs, peek_cached_datasets
...
def post_data_ingestion_submit(
    ...
    if remote_enabled and not remote_skip:
        ...
        try:
            ...
            from helix.data import _ingestion_runs, _ingestion_runs_lock
    ...
    from helix.data import submit_ingestion
...
def post_fetch_data(
    ...
    try:
        from helix.data import fetch_ohlcv_chunked
...
def get_datasets_stub(remote_skip: bool = False):
    ...
    try:
        from helix.data import scan_datasets
...
def get_cached_datasets_stub() -> list[dict[str, object]]:
    try:
        from helix.data import peek_cached_datasets
...
def get_dataset_detail_stub(symbol: str, timeframe: str, remote_skip: bool = False):
    ...
    from helix.data import get_dataset_detail
...
def delete_dataset_stub(symbol: str, timeframe: str, remote_skip: bool = False):
    ...
    from helix.data import delete_dataset
...
def get_data_quality(symbol: str, timeframe: str, remote_skip: bool = False):
    ...
    from helix.data import compute_data_quality
...
def get_quality_reports(limit: int = 100) -> list[dict]:
    ...
    from helix.data import DATA_DIR
...
def get_data_health():
    ...
    :func:`helix.data.compute_data_health` is preserved so the frontend
    ...
    """
    from helix.data import compute_data_health
    from helix.data_manager import _now_iso, data_manager_stats
...
def get_collection_health() -> dict:
    ...
    from helix.health_monitor import DATA_STREAM_FAILURE_RED, data_health_score
...
def get_data_activity(limit: int = 200) -> dict:
    ...
    try:
        from helix.data import get_active_ingestion_runs
...
def post_scan_orphans() -> dict:
    ...
    from helix.data import scan_parquet_orphans
...
def post_cleanup_orphans() -> dict:
    ...
    from helix.data import cleanup_parquet_orphans
...
def get_data_engine_status() -> dict:
    from helix.dataeng.hub import get_data_hub
...
def post_data_engine_backfill_plan() -> dict:
    from helix.dataeng.catalog import Catalog
...
def execute_data_engine_catchup(
    ...
    endpoint and the scheduled ``helix-data-engine-catchup`` auto-drain job can
    ...
    from helix.data import backfill_ohlcv_gaps
    ...
    try:
        from helix.data import _log_data_action
...
def post_upload_csv_preview(content: bytes):
    from helix.data import preview_csv
...
def post_upload_csv(
    ...
    from helix.data import process_csv_upload
...
def get_dataset_export(symbol: str, timeframe: str, format: str = "csv") -> tuple[bytes, str, str]:
    from helix.data import export_dataset_bytes
...
def get_symbol_export(symbol: str, format: str = "csv") -> tuple[bytes, str]:
    from helix.data import export_symbol_zip
...
def _build_ohlcv_response(symbol: str, timeframe: str = "1h", limit: int = 100) -> dict:
    ...
    from helix.api_domains.trading import _coerce_iso_timestamp, _normalize_asset_key
...
def get_dataset_ohlcv(symbol: str, timeframe: str, limit: int = 100, remote_skip: bool = False):
    ...
    from helix.data import dataset_ohlcv
...
def get_sources_stub():
    try:
        from helix.data import list_data_sources
...
def get_source_symbols_stub(source: str, query: str | None = None, exchange: str | None = None):
    try:
        from helix.data import search_source_symbols
...
def get_stream_health(symbol: str) -> dict:
    ...
    try:
        from helix.data import load_parquet, symbol_to_fs
...
def get_stream_rows(
    ...
    try:
        from helix.data import symbol_to_fs
        from helix.data_manager import FUNDING_DIR, OI_DIR, _load_stream_parquet, data_manager
        ...
        from helix.util import sanitize_json_floats
...
def get_coverage() -> dict:
    ...
    """
    from helix.data import DATA_DIR, coverage_entry, prune_coverage_cache
    from helix.data_manager import FUNDING_DIR, OI_DIR
