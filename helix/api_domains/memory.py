...
from helix.vectordb import search_narratives as _search_narratives_sync
...
def _chroma_dir() -> Path:
    return Path(getattr(cfg, "HELIX_HOME", Path.home() / ".helix")) / "chromadb"
...
def _chroma_health(limit: int = 6) -> dict[str, Any]:
    from helix.vectordb import _in_process_chroma_disabled

    if _in_process_chroma_disabled():
        # The vector layer is off on this host (HELIX_DISABLE_CHROMA_IN_PROCESS,
        ...
        return {
            ...
            "summary": "Vector memory disabled on this host (HELIX_DISABLE_CHROMA_IN_PROCESS)",
            "count": 0,
            "collections": [],
