...
from helix.db import get_db, kv_get, next_container_id
from helix.exchange import books as books_mod
...
def _live_vault_for_trade(trade: dict) -> str | None:
    ...
    """
    from helix.scanner import _resolve_trade_vault_address
...
def _live_open(
    ...
    testnet = _live_testnet()
    from helix.exchange.hyperliquid import get_account_value, market_order
    ...
    try:
        from helix.scanner import _update_trade_fill
...
def _open_trade_db_safe(
    *, strategy_id, asset, direction, entry, size, risk_pct, leverage, signal_data,
    ...
    from helix.scanner import _open_trade_db
...
def _place_live_protective(kind: str, trade: dict, price: float, vault_address: str | None = None):
    ...
    testnet = _live_testnet()
    from helix.exchange.hyperliquid import place_protective_stop, place_take_profit
