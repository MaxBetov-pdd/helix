...
from helix.research_sources.url_ingest import fetch_preview
from helix.system_mode_policy import is_manual_mode
...
def _compute_signals_safely(hypothesis_id: str) -> dict[str, Any] | None:
    ...
    try:
        from helix.hypothesis_verdict import compute_verdict_signals
...
def get_ranked_data_gap_payload(limit: int = 20) -> dict[str, Any]:
    ...
    if items:
        from helix.db import get_data_gap_requesters
...
def force_revisit_payload(hypothesis_id: str) -> dict[str, Any]:
    ...
    if not existing:
        raise HTTPException(status_code=404, detail="hypothesis not found")
    from helix.hypothesis_revisit import force_revisit
    try:
...
def cleanup_evidence_payload(*, dry_run: bool = False) -> dict[str, Any]:
    from helix.hypothesis_cleanup import cleanup_stale_hypotheses
    return cleanup_stale_hypotheses(dry_run=dry_run)


def cleanup_triage_payload(*, batch_size: int = 10) -> dict[str, Any]:
    from helix.hypothesis_cleanup import run_triage_loop
...
def _research_contract_for(hypothesis: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from helix.research_contract import build_research_contract, get_effective_research_settings
