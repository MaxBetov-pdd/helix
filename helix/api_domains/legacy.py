...
from helix.routers import agents as agents_routes
from helix.routers import auth as auth_routes
from helix.routers import strategies as strategies_routes
from helix.routers import websockets as websockets_routes

log = logging.getLogger("helix.legacy_api")
...
def apply_legacy_response_headers(response: Response, route_path: str) -> None:
    ...
    response.headers["X-Helix-Legacy-Route"] = route_path
    log.warning(
...
def legacy_helix_get(legacy_path: str, request: Request, limit: int = 50):
    """Legacy compatibility layer for older callers using `/api/helix/*`."""
    ...
    raise HTTPException(status_code=404, detail=f"Legacy endpoint not supported: /api/helix/{normalized_path}")


def post_brain_chat_legacy(body):
    from helix.routers import system as system_routes
