...
:mod:`helix.agents.tools_core`). The existing inline blocklist there
...
log = logging.getLogger("helix.sandbox.shell_guard")
...
def is_strict_mode_enabled() -> bool:
    ...
    (managed by ``helix.api_core.get_settings``). Falls back to False if
    settings can't be loaded — fail-open by design.
    """
    try:
        from helix.api_core import get_settings  # noqa: PLC0415
