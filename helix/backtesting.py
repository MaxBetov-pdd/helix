"""Helix backtesting client — connects Helix agents to the backtesting control plane.

By default the backtesting service runs on 127.0.0.1:8003, but this
is configurable with HELIX_BACKTEST_API.
...
log = logging.getLogger("helix.backtesting")

# ── Configuration ────────────────────────────────────────────────────

_BACKTEST_API_ENV = "HELIX_BACKTEST_API"
_DEFAULT_HELIX_BASE = "http://127.0.0.1:8003"
_DEFAULT_HELIX_API = f"{_DEFAULT_HELIX_BASE}/api"
...
def _resolve_backtesting_api_base_url(base_url: str | None = None) -> str:
    ...
    if not raw:
        ...
        client_base = str(os.getenv("HELIX_CLIENT_BASE") or "").strip()
        if client_base:
            raw = client_base
    if not raw:
        port = str(os.getenv("HELIX_PORT") or "").strip()
        if port.isdigit():
            raw = f"http://127.0.0.1:{port}"
    if not raw:
        # Fall back to settings remote_engine_url when env var is unset.
        try:
            from helix.db import kv_get as _kv
...
HELIX_API = _resolve_backtesting_api_base_url()
HELIX_BASE = _resolve_backtesting_service_url(HELIX_API)
...
def _build_api_headers() -> dict[str, str]:
    ...
    api_key = str(os.getenv("HELIX_API_KEY") or "").strip()
    if api_key:
        headers["x-api-key"] = api_key
    operator_key = str(os.getenv("HELIX_OPERATOR_KEY") or "").strip()
    if operator_key:
...
def get_backtesting_api_base() -> str:
    ...
    port = str(os.environ.get("HELIX_PORT", "")).strip()
    if port.isdigit():
        return _normalize_api_base(f"http://127.0.0.1:{port}") or _DEFAULT_HELIX_API
...
class BacktestingClient:
    ...
    def create_strategy(
        ...
        if params is not None:
            ...
            # This mirrors the fix in helix/routers/backtesting.py to support
