...
from helix.config import HELIX_HOME, ensure_dirs, load_config
from helix.control_plane.queue_processing import (
...
from helix.db import create_pending_task, kv_get, kv_set, kv_set_best_effort
from helix.model_routing import get_default_model_for_provider
from helix.notification_policy import DEFAULT_RESPONSE_CHANNEL_ALIASES
from helix.notification_renderers import summarize_discord_text
from helix.ai import _is_rate_limit_exception, is_transient_provider_exception, normalize_provider_and_model
from helix.task_timeouts import coerce_stale_recovery_minutes

log = logging.getLogger("helix.bot")
...
def _bot_owns_runtime_loops() -> bool:
    ...
    """
    return os.environ.get("HELIX_BOT_OWNS_RUNTIME", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _load_respond_channels() -> set[str]:
    ...
    preferences = kv_get("helix:notification_preferences", {}) or {}
...
def _render_operational_discord_reply(response: str, *, source: str = "", task_message: str = "") -> str:
    ...
    compact = summarize_discord_text(response, limit=420, max_lines=3) or _single_line(response) or "Helix update."
...
def get_bot_token() -> str:
    ...
    if not token:
        raise ValueError(
            ...
            "or add 'discord_token' to ~/.helix/config.json"
        )
    return token
...
def _build_default_agents() -> list[dict]:
    ...
    pipeline = kv_get("helix:pipeline_thresholds", {})
    ...
    return [
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Quant Researcher for Helix. Your focus is benchmarking, market structure research, data-gap discovery, and data integrity.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Simulation Agent for Helix. Your focus is Strategy Container Validation.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Risk Manager for Helix. Your focus is Live Strategy Container Oversight and Capital Allocation.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Execution Trader for Helix. You are the ONLY agent with exchange access.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Strategy Developer for Helix. You own the full hypothesis-to-strategy loop.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Full-Stack Engineer for Helix — the operator-facing triage and diagnosis agent.\n"
        ...
        },
        {
            "agent_id": "brain",
            "name": "Brain",
            "role": "Central orchestrator and decision layer for Helix. Delegates work and arbitrates between agents.",
            "model": "openai",
            "model_id": get_default_model_for_provider("openai"),
            "visibility": "visible",
            "instructions": (
                "You are the Brain for Helix. Your focus is Strategy Container Lifecycle Management.\n"
...
class HelixBot(commands.Bot):
    """Helix Discord bot — gateway to the AI brain."""
    ...
    async def setup_hook(self):
        ...
        if not self.agent_id and _bot_owns_runtime_loops():
            ...
            log.info(
                ...
                "(set HELIX_BOT_OWNS_RUNTIME=1 to override)."
            )
    ...
    async def on_ready(self):
        ...
        from helix.scheduler import reset_scheduler_job_locks
    ...
    async def _maybe_recover_stale_tasks(self, source: str) -> None:
        ...
        async with self._stale_recovery_lock:
            ...
            from helix.db import STALE_RECOVERY_FAIL_AGENTS, recover_stale_running_tasks
    ...
    async def _bootstrap(self):
        ...
        """
        from helix.db import get_db, log_activity
            ...
            if should_queue_bootstrap_brain_cycle():
                create_pending_task(
                    ...
                    "brain_invoke",
                    {
                        "source": "bootstrap",
                        "message": (
                            "Helix just started. You are the Brain — the sole orchestrator.\n\n"
        ...
        boot_msg = f"HELIX BOOTSTRAP COMPLETE\n{summary}"
        log.info(boot_msg)
        ...
        if self.agent_id:
            ...
            async with message.channel.typing():
                try:
                    ...
                    from helix.db import log_activity
    ...
    async def _generate_agent_response(self, content: str, channel_id: str, channel_name: str) -> str:
        ...
        from helix.ai import call_ai
        from helix.context import build_agent_context
        from helix.vectordb import store_narrative
        from helix.workspace import append_workspace, read_workspace
        ...
        preamble = (
            f"You are {agent_name}, a specialist agent in the Helix team.\n"
            f"Your role: {agent_role or 'Specialist operator.'}\n"
            "Stay in this persona and never answer as Helix (the Brain)."
        )
        context = preamble + "\n\n---\n\n" + context
        ...
        context += (
            ...
            f"If an action request reaches you without being queued, tell Judder to use `!assign {agent_id}: <task>` on the main Helix bot.\n"
    ...
    async def _generate_response(self, content: str, channel_id: str, channel_name: str, is_dm: bool) -> str:
        ...
        if history:
            context += "\n\n---\n\n# RECENT CONVERSATION IN THIS CHANNEL\n"
            for msg in history[-10:]:
                role = "Judder" if msg["role"] == "user" else "Helix"
                context += f"\n**{role}**: {msg['content'][:300]}\n"
        ...
        from helix.workspace import append_workspace, today_memory_path
        try:
            append_workspace(
                today_memory_path(),
                f"\n### Discord #{channel_name} — {datetime.now(timezone.utc).strftime('%H:%M UTC')}\n"
    ...
    async def _process_pending_tasks(self):
        ...
        from helix.db import get_db, claim_pending_tasks
        from helix.context import build_brain_context
        ...
        if not rows:
            ...
            try:
                from helix.runtime_worker import ensure_brain_keepalive
        ...
        for task in rows:
            ...
            try:
                ...
                if source == "bootstrap":
                    from helix.brain import assign_research_cycle
                ...
                if is_chat:
                    ...
                    from helix.context import build_chat_context
                    context = build_chat_context()
                    ...
                    _chat_tool_names = {
                        ...
                        # Backtesting tools
                        "helix_run_backtest", "helix_list_datasets", "helix_get_results",
                    }
                    ...
                    if chat_tools:
                        context += (
                            ...
                            "- **helix_run_backtest**: Run a backtest via the Helix pipeline\n"
                            "- **helix_list_datasets**: List available datasets\n"
                            "- **helix_get_results**: Retrieve backtest results\n\n"
                    ...
                    context += (
                        ...
                        "## Backtesting (Helix)\n"
                        "- **helix_list_datasets**: List available backtesting datasets\n"
                        "- **helix_create_strategy**: Create a strategy on Helix Backtesting\n"
                        "- **helix_run_backtest**: Run a backtest with realistic fees\n"
                        "- **helix_run_optimization**: Run parameter optimization\n"
                        "- **helix_run_verdict**: Validate a strategy\n"
                        "- **helix_get_results**: Get detailed backtest results\n\n"
                    ...
                    try:
                        ...
                        store_narrative(
                            ...
                            metadata={"type": "brain_cycle", "source": "helix"},
                        )
                    except Exception:
                        pass
                ...
                if isinstance(e, asyncio.TimeoutError):
                    from helix.db import requeue_brain_task
...
_bot: HelixBot | None = None


def get_bot() -> HelixBot:
    """Get or create the singleton bot instance."""
    global _bot
    if _bot is None:
        _bot = HelixBot(agent_id=None)
    return _bot
...
async def send(channel_name: str, message: str, channel_id: str | None = None, agent_id: str | None = None):
    ...
    async def _send_with_bot(selected_bot: HelixBot):
...
async def send_thread(channel_name: str, title: str, message: str, channel_id: str | None = None, embeds: list[dict] | None = None, agent_id: str | None = None):
    ...
    async def _send_thread_with_bot(selected_bot: HelixBot):
        await selected_bot.wait_until_ready_custom()
        channel = selected_bot.get_channel(int(target_id))
        if not channel:
            channel = await selected_bot.fetch_channel(int(target_id))
        ...
        try:
            with httpx.Client(timeout=request_timeout_seconds) as client:
                ...
                if view_resp.status_code == 200:
                    ...
                    if probe_send:
                        probe_msg = (
                            f"[Helix Discord audit] {actor} probe at "
...
async def start_bot():
    ...
    try:
        ...
        from helix.model_selection import ensure_enforcement_armed
...
def run_bot():
    ...
    if not _acquire_bot_lock():
        ...
        log.warning("Another bot instance is already running%s; skipping duplicate start.", pid_suffix)
        try:
            from helix.db import init_db, log_activity
    ...
    try:
        log.info("Bot process forcing subprocess-only/disabled ChromaDB access for stability on this host")
        log.info("Starting Helix gateway (bot + scheduler + task processor)")
