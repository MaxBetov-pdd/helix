...
Helix is run from a git checkout (users clone the repo and launch it with
...
- We track a fixed remote/branch (default ``origin``/``main``) — configurable
  via ``HELIX_UPDATE_REMOTE`` / ``HELIX_UPDATE_BRANCH``.
...
log = logging.getLogger("helix.self_update")
...
def _repo_root() -> Path:
    # self_update.py lives at <repo>/helix/self_update.py
...
def _target_remote() -> str:
    value = _get_env("HELIX_UPDATE_REMOTE", "origin")
    if not _GIT_REMOTE_RE.match(value):
        raise RuntimeError(f"Invalid HELIX_UPDATE_REMOTE: {value!r}")
    return value


def _target_branch() -> str:
    value = _get_env("HELIX_UPDATE_BRANCH", "main")
    if not _GIT_REF_RE.match(value):
        raise RuntimeError(f"Invalid HELIX_UPDATE_BRANCH: {value!r}")
    return value
...
def check_for_update(*, fetch: bool = True) -> dict[str, Any]:
    ...
    """
    current_version = getattr(helix, "__version__", "")
