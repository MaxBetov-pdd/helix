...
from helix.config import get_execution_mode, load_config
from helix.circuit_breaker import hl_price_breaker, hl_trade_breaker, hl_account_breaker
from helix.db import kv_get

log = logging.getLogger("helix.exchange.hl")
...
# increasing nonces. Helix signs everything with ONE master key (vault-routed
...
class HyperliquidInfoClient(Protocol):
    """Subset of HyperLiquid info methods used by Helix."""
...
def _assert_execution_allowed(testnet: bool) -> None:
    ...
    ``HELIX_ALLOW_MAINNET=1`` opt-in, otherwise we refuse. This collapses the
    ...
    if testnet:
        return
    if _is_truthy(os.environ.get("HELIX_ALLOW_MAINNET")):
        return
    raise RuntimeError(
        "Refusing to place a MAINNET order: resolved testnet=False but "
        "HELIX_ALLOW_MAINNET is not set. This is the stable-release safety guard — "
        "set HELIX_ALLOW_MAINNET=1 only when you intentionally trade real funds."
    )


def _hl_encryption_disabled() -> bool:
    env_value = os.environ.get("HELIX_HL_DISABLE_ENCRYPTION")
...
def _load_creds_from_helix_settings() -> dict:
    try:
        settings = kv_get("helix:settings", {}) or {}
        secrets = kv_get("helix:settings:secrets", {}) or {}
    except Exception:
        return {}
...
def _load_creds_from_env() -> dict:
    ...
    env_map = {
        "HL_API_SECRET": _read("HELIX_HL_API_SECRET") or _read("HL_API_SECRET"),
        "HL_API_KEY": _read("HELIX_HL_API_KEY") or _read("HL_API_KEY"),
        "HL_WALLET_ADDRESS": _read("HELIX_HL_WALLET_ADDRESS") or _read("HL_WALLET_ADDRESS"),
        "USE_TESTNET": _read("HELIX_HL_USE_TESTNET") or _read("USE_TESTNET"),
    }
...
def _get_creds_path() -> str:
    ...
    path = os.environ.get("HELIX_HL_CREDS_PATH")
    if not path:
        cfg = load_config()
...
def _get_creds() -> dict:
    ...
    settings_creds = _load_creds_from_helix_settings()
    if settings_creds:
    ...
    if not settings_file.exists():
        raise FileNotFoundError(
            ...
            "or set HELIX_HL_WALLET_ADDRESS/HELIX_HL_API_SECRET env vars, "
    ...
    if not key_file.exists():
        ...
        raise FileNotFoundError(
            ...
            "(set HELIX_HL_DISABLE_ENCRYPTION=1 for plaintext settings)"
        )
...
def _get_account_info_client(testnet: bool = True) -> tuple[HyperliquidInfoClient, str]:
    ...
    if not wallet:
        raise RuntimeError(
            ...
            "Set Settings > HyperLiquid wallet address or HELIX_HL_WALLET_ADDRESS."
        )
    ...
    *,
    ...
    if not pk:
        raise RuntimeError(
            ...
            "Set Settings > HyperLiquid private key or HELIX_HL_API_SECRET."
        )
...
def market_order(
    ...
    if is_sim_active():
        from helix.sim.mock_exchange import sim_market_order
...
def close_position(
    ...
    from helix.sim.clock import is_sim_active
    if is_sim_active():
        from helix.sim.mock_exchange import sim_close_position
...
def get_positions(testnet: bool = True, *, account_address: str | None = None) -> dict:
    ...
    from helix.sim.clock import is_sim_active
    if is_sim_active():
        from helix.sim.mock_exchange import sim_get_positions
...
def get_account_value(
    ...
    from helix.sim.clock import is_sim_active
    if is_sim_active():
        from helix.sim.mock_exchange import sim_get_account_value
...
def get_all_mids(testnet: bool = True) -> dict[str, float]:
    ...
    from helix.sim.clock import is_sim_active
    if is_sim_active():
        from helix.sim.mock_exchange import sim_get_all_mids
