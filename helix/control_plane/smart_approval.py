...
from helix.model_routing import get_auxiliary_routing

log = logging.getLogger("helix.control_plane.smart_approval")
...
def _build_prompt(approval: Mapping[str, Any]) -> str:
    return (
        "You classify Helix approval-queue items for an autonomous trading research platform. "
        "Helix is paper-trading only (no live money). The operator wants safe routine items "
...
def _call_aux_llm(prompt: str, routing: Mapping[str, Any]) -> str:
    ...
    loop context we're in. Mirrors ``helix.recall._call_aux_llm`` so tests
