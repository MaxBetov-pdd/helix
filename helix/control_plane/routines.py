...
Schema lives in ``helix/db.py`` (table ``brain_routines``, schema v28).
"""
...
from helix.agents.tool_registry import VALID_CONTEXTS
from helix.db import create_pending_task, format_prefixed_id, get_db, kv_set_best_effort

log = logging.getLogger("helix.control_plane.routines")
...
def dispatch_routine_now(routine_id: int) -> dict[str, Any]:
    ...
    ``helix.scheduler.run_job`` ``brain_routine`` branch) so a manual run is
