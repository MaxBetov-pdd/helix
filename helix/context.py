...
log = logging.getLogger("helix.context")

from helix.db import get_open_trades, get_recent_trades, get_strategies, kv_get, list_approvals
from helix.strategy_diversity import filter_recall_records_for_diversity, render_strategy_diversity_guard
...
SYSTEM_PREAMBLE = """\
You are Helix — an autonomous trading intelligence system built by Judder.

CRITICAL BEHAVIORAL RULES:
- You ARE Helix. Do not talk about "reading files", "sessions", "context windows", "system prompts", "tokens", or any implementation details. You simply know things because you are Helix.
...
- End every message with your signature line: a short "— Helix | <model>" where <model> is the AI model you're running on (you can infer this from context or just say the model name if known).
- When you don't know your current model, just sign "— Helix".
...
def _format_brain_lessons(limit: int = 10, min_confidence: float = 0.5) -> str:
    ...
    """
    try:
        from helix.brain_lessons import list_lessons
...
- End every message with your signature line: a short "— Helix"
...
async def store_conversation(
    ...
    Args:
        ...
        source: Conversation origin. Defaults to ``'ui_chat'`` for the in-app
            ...
            ``helix/bot.py`` caller keeps working without an edit.
    """
    try:
        ...
        summary = f"{prefix} Judder: {user_msg[:200]} | Helix: {ai_response[:300]}"
...
def build_agent_context(
    ...
    identity = read_workspace("IDENTITY.md", optional=True)
    if identity:
        parts.append(f"# HELIX — IDENTITY & RULES\n{identity}")
...
def _get_learned_skills_context() -> str:
    ...
    """
    try:
        from helix.quant_skills import get_ideation_context

        regime: str | None = None
        try:
            from helix.regime import TRACKED_ASSETS, peek_cached_regime
...
def get_brain_learning_injection(query: str = "", *, n_results: int = 6, n_lessons: int = 8) -> str:
    ...
    try:
        from helix import brain_lessons
...
def _format_market_regime() -> str:
    ...
    try:
        from helix.regime import format_regime_summary
