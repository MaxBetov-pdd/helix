...
log = logging.getLogger("helix.health_monitor")
...
# helix:settings.staleness_thresholds.
...
def check_scheduler() -> ComponentStatus:
    ...
    try:
        ...
        from helix.scheduler import get_enabled_jobs
        ...
        try:
            from helix.scheduler import get_last_tick_at
...
def check_bots() -> list[ComponentStatus]:
    ...
    results = []
    try:
        from helix.db import get_running_bots
...
def check_data_collector() -> ComponentStatus:
    ...
    try:
        ...
        jobs = get_enabled_jobs()
        data_job_ids = {
            "helix-data-ohlcv-keepalive", "helix-data-funding-collect",
            "helix-data-lsr-collect", "helix-data-taker-collect",
            "helix-data-liquidation-collect", "helix-data-fng-collect",
            "helix-data-macro-collect", "helix-data-btcdom-collect",
        }
...
def _stream_staleness_sla_minutes(stream: str) -> float:
    ...
    Operator-overridable via helix:settings.staleness_thresholds (minutes per
...
def check_lab_worker() -> ComponentStatus:
    ...
    try:
        ...
        from helix.lab_worker_service import WORKER_STATUS_META_KEY
...
def _kill_lab_worker_processes() -> int:
    ...
    try:
        ...
        pid_file = HELIX_HOME / "lab" / "lab_worker.pid"
        if pid_file.exists():
...
async def _attempt_recovery(state: HealthState, name: str, status: ComponentStatus) -> None:
    ...
    try:
        if name.startswith("bot:"):
            ...
            from helix.lab_worker_service import _reconcile_orchestrator_state
...
def check_candle_freshness() -> list[DataCheck]:
    ...
    results = []
    try:
        from helix.db import get_running_bots, get_db
        running = get_running_bots()
        if not running:
...
def check_ai_providers() -> ComponentStatus:
    ...
    """
    try:
        from helix.provider_runtime_health import get_provider_health_runtime
...
def check_pipeline_throughput() -> ComponentStatus:
    ...
    """
    try:
        from helix.lab_db import get_lab_meta, list_lab_jobs, LabJobState
        from helix.lab_worker_service import PIPELINE_PROGRESS_META_KEY, WORKER_STATUS_META_KEY
