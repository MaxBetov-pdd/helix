...
from helix.ai import normalize_provider_and_model
from helix.codex_responses import is_openai_oauth_token
from helix.model_routing import (
    get_default_model_for_provider,
    get_model_routing_snapshot,
    update_model_routing,
)
from helix.config import AUTH_FILE, HELIX_HOME, is_beta_build
from helix.agents.manager import create_agent, delete_agent, inspect_agent, update_agent
from helix.auth.store import (
    delete_profile,
    get_profile,
    get_token,
    is_profile_opaque,
...
from helix.auth.callback_listener import LoopbackCallbackListener
...
from helix.scheduler import (
    ...
    reconcile_helix_jobs,
    seed_helix_jobs,
)
from helix.secret_storage import decrypt_secret, encrypt_secret
from helix import strategy_lifecycle as lifecycle_service
from helix.workspace import read_workspace, write_workspace
from helix.util import generate_pkce, generate_state, normalize_stage

log = logging.getLogger("helix.api")
_BACKTEST_RESULTS_REMOTE_API_ENV = "HELIX_BACKTEST_RESULTS_REMOTE_API"
...
def _optimization_executor_workers() -> int:
    raw = str(os.environ.get("HELIX_OPTIMIZATION_MAX_WORKERS", "2") or "").strip()
    try:
        parsed = int(raw)
    except Exception:
        parsed = 2
...
class HelixV1CompatMiddleware(BaseHTTPMiddleware):
	"""Allow frontend calls that still use `/api/helix/*`."""

	async def dispatch(self, request, call_next):
		path = request.scope.get("path", "")
		legacy_path = path if path.startswith("/api/helix") else None
		if path.startswith("/api/helix/"):
			request.scope["path"] = path.replace("/api/helix", "/api", 1)
		elif path == "/api/helix":
		...
		if legacy_path:
			...
			response.headers.setdefault("X-Helix-Legacy-Route", legacy_path)
		return response
    ...
    with _SCHEDULER_BOOTSTRAP_LOCK:
        ...
        try:
            ...
            if not existing_jobs:
                ...
                reconciliation = reconcile_helix_jobs()
...
async def _on_startup():
    ...
    try:
        from helix.db import recover_dangling_runtime_tasks
        from helix.system_mode_policy import reconcile_manual_mode_backlog
    ...
    try:
        from helix.data_manager import assert_data_root_consistent
        ...
        # HELIX_DATA_DIR / HELIX_HOME are aligned.
        if not assert_data_root_consistent():
            log.error(
                ...
                "UNRELIABLE until HELIX_DATA_DIR / HELIX_HOME are aligned "
...
def _discover_provider_models(provider: str, force_refresh: bool = False) -> tuple[list[dict], str | None]:
    ...
    if provider == "openrouter":
        ...
        # useful set for Helix's agent loop), auto-updating as the free roster
...
def _build_auth_provider_payload(provider: str) -> dict:
    ...
    refresh_command = (
        ...
        if provider == "lmstudio"
        else f"helix auth refresh {provider}"
    )
    ...
    # refuses to call it.
    try:
        from helix import model_selection
...
def _get_auth_providers_compat() -> dict:
    return {
        ...
        "configure_command": "helix auth status",
        "status_command": "helix auth status",
...
def _resolve_exchange_testnet() -> bool:
    from helix.api_domains.trading import _resolve_exchange_testnet as _domain_resolve_exchange_testnet
...
_SETTINGS_STORAGE_KEY = "helix:settings"
_SETTINGS_SECRET_STORAGE_KEY = "helix:settings:secrets"
_SETTINGS_API_KEYS_STORAGE_KEY = "helix:settings:api-keys"
_SETTINGS_PIPELINE_STORAGE_KEY = "helix:pipeline:settings"
...
_DEFAULT_SETTINGS_PAYLOAD = {
    ...
    # Strict mode for the agent run_shell command guard (helix.sandbox.shell_guard).
...
# Only Polygon is wired to a consumer (helix.config.get_polygon_api_key ->
...
_DEFAULT_PIPELINE_SETTINGS = {
    ...
    # Consumed by helix.maintenance.run_db_maintenance via this same payload.
    ...
    # Aligned with the canonical gate store (helix:pipeline_thresholds /
...
def _default_data_engine_settings_payload() -> dict:
    from helix.dataeng.settings import default_data_engine_settings_payload

    return default_data_engine_settings_payload()


def _merge_data_engine_settings_payload(value) -> dict:
    from helix.dataeng.settings import merge_data_engine_settings_payload
...
def _default_research_settings_payload() -> dict:
    from helix.research_contract import default_research_settings
...
def _apply_settings_section(section: str, payload: dict) -> dict:
    ...
    if section == "exchange":
        ...
        if "trading_mode" in payload:
            ...
            try:
                from helix.config import set_execution_mode
        ...
        # Promotion-safety gates (read top-level from helix:settings by
        # helix.policy.evaluate_promotion and helix.hypothesis_graduation).
        ...
        # The live trade gate (helix.regime.is_strategy_allowed -> helix.config
        ...
        if "discord_bot_token" in payload:
            ...
            if bot_token:
                ...
                from helix.config import load_config, save_config
                cfg = load_config()
        ...
        # (helix:notification_preferences), which resolve_notification_policy
        ...
        if _notif_pref_updates or "notification_level" in payload:
            try:
                from helix.notifications import (
    ...
    try:
        import os as _os
        from helix.config import load_config as _load_cfg
        _cfg = _load_cfg()
    ...
    if section == "bot-operations":
        try:
            from helix.scheduler import apply_runtime_scheduler_overrides
    ...
    if section == "hyperliquid" and bool(updates.get("hyperliquid_has_key")):
        try:
            from helix.exchange.hyperliquid import _get_creds
            _get_creds()
...
def _build_openai_oauth_start() -> dict[str, object]:
    from helix.auth import openai as openai_auth
...
def _build_minimax_oauth_start() -> dict[str, str]:
    from helix.auth import minimax as minimax_auth
...
def _complete_openai_oauth(state: str, code: str, code_verifier: str | None) -> None:
    ...
    from helix.auth import safe_extract_chatgpt_account_id
...
def _delete_backtest_record(result_id: str):
    ...
    if not result_id:
        return
    try:
        from helix.vectordb import get_collection
...
def _result_data_dirs() -> list[str]:
    ...
    candidates = [
        ...
        os.path.join(str(HELIX_HOME), "data", "results"),
    ]
    out: list[str] = []
    seen: set[str] = set()
...
def _chroma_backtest_records():
    ...
    from helix.vectordb import _check_chroma_available
...
async def post_brain_chat_direct(body: BrainChatBody):
    ...
    from helix.agents.runner import (
        AGENT_TOOLS,
        BACKTESTING_TOOLS,
        BRAIN_TOOLS,
        _call_with_tools,
    )
    from helix.agents.tool_definitions import CHAT_ASK_TOOL_NAMES
    from helix.brain import resolve_brain_provider_model
    from helix.context import build_chat_context, store_conversation
    ...
    try:
        ...
        from helix.ai import _is_rate_limit_exception
...
def put_pipeline_settings(body: PipelineSettingsUpdateBody):
    ...
    if threshold_updates:
        from helix.policy import load_pipeline_config, save_pipeline_config
...
# blob (default 30, enforced by helix.exchange.risk reading the blob directly)
...
def get_settings():
    ...
    try:
        ...
        pass
    try:
        from helix.policy import load_pipeline_config, pipeline_thresholds_for_display
    ...
    try:
        from helix.policy import _normalize_pipeline_config, pipeline_thresholds_for_display
    ...
    try:
        from helix import config as _regime_cfg
    ...
    try:
        from helix.notifications import get_notification_preferences
...
def get_settings_discord_audit(send_probe: bool = False):
    try:
        from helix.bot import run_discord_audit
...
def test_settings_api_key(source: str):
    ...
    if source == "polygon":
        try:
            from helix.polygon_client import PolygonClient
...
def get_pipeline_config():
    ...
    from helix.policy import load_pipeline_config
    return load_pipeline_config()

def update_pipeline_config(config: dict):
    """Save pipeline thresholds using policy module."""
    from helix.policy import save_pipeline_config
...
def upsert_auth_provider(provider: str, body: AuthProviderProfileBody):
    ...
    try:
        from helix.model_selection import mark_provider_connected
...
def delete_auth_provider(provider: str):
    ...
    try:
        from helix.model_selection import unmark_provider_connected
...
def post_agent_test_discord(agent_id: str, payload: AgentDiscordTestBody | None = None):
    ...
    if not token:
        try:
            from helix.bot import get_bot_token
    ...
    from helix.bot import CHANNELS
    from helix.reporter import AGENT_CHANNEL_MAP
    ...
    tested_at = _now()
    message = (
        f"[Helix Settings] Gateway Discord test message for {agent_name} ({normalized_agent_id}) at {tested_at}.\n"
...
def update_strategy_default_params(
    ...
    *,
    ...
    from helix.strategies.certification import certify_execution_strategy
    from helix.brain import update_strategy_params
    ...
    try:
        from helix.db import append_strategy_event
        ...
        with get_db() as conn:
            ...
            if pin_to_store:
                ...
                if pin_row is not None:
                    from helix.strategy_lifecycle import _extract_symbol_timeframe_from_config
...
def _try_research_recovery_on_edit(strategy_id: str):
    ...
    try:
        from helix.db import get_db as _gdb, kv_get as _kvg, kv_set as _kvs
        ...
        debounce_key = f"helix:recert_debounce:{strategy_id}"
        ...
        if result.get("promoted"):
            try:
                from helix.api_domains.live_ws import ws_manager
                import asyncio
                try:
        ...
        import logging
        logging.getLogger("helix.api_core").warning(
...
def get_backtest_chart_context(result_id: str, remote_skip: bool = False):
    ...
    try:
        from helix.strategies import backtest as backtest_mod
...
# â”€â”€ Phase 2: Helix backtesting endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _require_existing_strategy_row(strategy_id: str) -> dict:
    ...
    try:
        from helix.strategies.registry import _TYPE_MAP, discover
        discover()
...
def _persist_completed_backtest_run(
    *,
    ...
    try:
        from helix.vectordb import store_backtest_result
...
def post_backtest_preview(body: BacktestPreviewBody):
    ...
    try:
        from helix.strategies.backtest import preview_strategy_signals
        ...
        total_bars = 0
        try:
            from helix.strategies.backtest import load_backtest_candles
...
def post_backtest_preview_chart(body: PreviewChartBody) -> dict:
    ...
    try:
        from helix.strategies.backtest import build_strategy_preview_chart_context
...
async def post_nl_to_spec(body: NlToSpecBody) -> dict:
    ...
    from helix.strategies.nl_spec_gen import nl_to_rule_spec
...
def register_manual_backtest_strategy(body: ManualStrategyBody) -> dict:
    ...
    try:
        from helix.sandbox.ast_guard import scan_source
    ...
    try:
        from helix.selfheal import validate_strategy_code
    ...
    try:
        from helix.strategies.registry import _TYPE_MAP, discover, reset
        discover()
        if type_name in _TYPE_MAP:
            existing_module = str(getattr(_TYPE_MAP[type_name], "__module__", ""))
            our_module = f"helix.strategies.custom.manual_{type_name}"
...
def send_manual_strategy_to_forge(body: SendToForgeBody) -> dict:
    ...
    """
    from helix.strategies.registry import _TYPE_MAP, discover
    from helix.db import create_strategy_container
    ...
    if mode == "visual":
        ...
        try:
            from helix.strategies.builtin.rule_engine import validate_rule_spec
...
def _execution_profile_parity_warnings(controls: dict | None, leverage: float | None = None) -> list[str]:
    ...
    warnings: list[str] = []
    try:
        from helix.exchange.risk import _get_risk_limits
...
def _resolve_local_backtest_execution_params(
    ...
    *,
    ...
    from helix.strategies.certification import (
        EXECUTION_CERTIFIED_FAMILIES,
        certify_execution_strategy,
    )
    from helix.strategies.params import extract_execution_params_from_rule_blobs
...
def post_backtest_submit(body: BacktestSubmitBody, *, skip_auto_trash: bool = False):
    ...
    from helix.strategies.backtest import backtest_strategy
...
def post_optimization_submit(body: OptimizationSubmitBody):
    ...
    def _run_optimization_background() -> None:
        try:
            from helix.strategies.optimizer import optimize_strategy
    ...
    from helix.db import set_user_active
    set_user_active()
...
def _backtest_task_title_prefix(request_source: str) -> str:
    ...
    if normalized in {"agent_tool", "helix_agent_tool", "strategy_developer_tool"}:
...
def _agent_backtest_source(request_source: str) -> bool:
    ...
    normalized = str(request_source or "").strip().lower()
    return (
        normalized in {"agent_tool", "helix_agent_tool", "strategy_developer_tool"}
...
def _create_inline_backtest_task(
    *,
    ...
    from helix.db import get_db as _get_db, next_container_id
...
def _finalize_inline_backtest_task(
    *,
    ...
    if task_id is None:
        return
    from helix.db import append_task_audit_event, get_db as _get_db
...
def post_backtesting_run(body: dict):
    ...
    try:
        ...
        if "strategy_id" in body and "dataset_id" in body:
            from helix.backtesting import get_client
            client = get_client()
            ...
            # Strip Helix dataset prefix (e.g., "dataset-26-" from "dataset-26-BTC/USDT-1h")
        ...
        if is_remote:
            ...
            from helix.brain import assign_task
            trace_lines = []
            if template_id:
            ...
            task_id = assign_task(
                ...
                description=(
                    ...
                    "Use helix_list_datasets to find data, then helix_create_strategy "
                    "and helix_run_backtest to iterate."
                ),
                priority=10,
                source="user",
            )
...
async def websocket_endpoint(ws: WebSocket):
    from helix.api_domains import live_ws
