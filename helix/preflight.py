...
Verify the running interpreter actually satisfies helix's DECLARED dependencies
...
imported only 4 modules (helix.api / uvicorn / websockets / multipart), so a
...
- **Import-light**: only stdlib at module load (``packaging`` is imported lazily
  inside the checker), so `python -m helix.preflight` runs even from a source
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not problems:
        print("helix preflight: all declared dependencies satisfied")
        return 0

    print("helix preflight: MISSING or INCOMPATIBLE dependencies:", file=sys.stderr)
    for p in problems:
    ...
    if "--install" in argv:
        ...
        print(f"helix preflight: pip install -e . into {sys.executable}", file=sys.stderr)
        rc = subprocess.call([sys.executable, "-m", "pip", "install", "-e", str(repo)])
        if rc != 0:
            print("helix preflight: pip install failed", file=sys.stderr)
            return rc
        remaining = check_dependencies()
        if remaining:
            print("helix preflight: STILL unsatisfied after install:", file=sys.stderr)
            for p in remaining:
                print(f"  - {p}", file=sys.stderr)
            return 1
        print("helix preflight: dependencies satisfied after install")
        return 0

    print(
        "\nFix: run `pip install -e .` in your helix checkout "
