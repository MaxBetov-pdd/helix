...
from helix.lab_matrix_engine import MATRIX_JOB_TYPE, run_matrix_job
from helix.lab_orchestrator import (
...
from helix.lab_regime_engine import (
...
log = logging.getLogger("helix.lab_worker_service")
...
LAB_WORKER_STARTUP_LOG = "helix_lab_worker.log"
LAB_WORKER_STARTUP_ERR_LOG = "helix_lab_worker.err.log"
...
def _pid_file_path() -> Path:
    return HELIX_HOME / "lab" / LAB_WORKER_PID_FILE
...
def _reconcile_orchestrator_state() -> bool:
    ...
    from helix.lab_db import set_lab_job_state, LabJobState, set_lab_meta
    ...
    if state == "failed":
        ...
        if should_recover:
            ...
            if config.get("enabled"):
                from helix.lab_orchestrator import _now_iso
...
def _load_cpu_gate_config() -> dict[str, int]:
    ...
    from helix.lab_orchestrator import get_orchestrator_config
    config = get_orchestrator_config()
...
def get_lab_worker_log_candidates() -> list[Path]:
    ...
    return [
        repo_root / ".helix_home" / "lab" / LAB_WORKER_LOG,
...
def start_lab_worker_process() -> dict[str, Any]:
    ...
    process = subprocess.Popen(
        [sys.executable, "-m", "helix", "lab", "worker"],
        **popen_kwargs,
    )
    return {
        "status": "started",
        ...
        try:
          while True:
            try:
              ...
              try:
                  from helix.lab_orchestrator import get_orchestrator_config as _get_orch_config
