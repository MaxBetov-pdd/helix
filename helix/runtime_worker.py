...
from helix.config import HELIX_HOME

log = logging.getLogger("helix.runtime_worker")
...
try:
    BOT_TASK_WORKER_STALE_SECONDS = int(os.environ.get("HELIX_BOT_TASK_WORKER_STALE_SECONDS", "120") or 120)
except (TypeError, ValueError):
    BOT_TASK_WORKER_STALE_SECONDS = 120
try:
    API_TASK_WORKER_STALE_SECONDS = int(os.environ.get("HELIX_API_TASK_WORKER_STALE_SECONDS", "600") or 600)
except (TypeError, ValueError):
...
def _mark_agent_callback_reviewed(payload: dict) -> list[int]:
    from helix.brain import mark_agent_tasks_reviewed
...
def _resolve_agent_task_timeout_seconds(task: dict) -> int:
    ...
    from helix.task_timeouts import resolve_agent_task_timeout_seconds

    try:
        from helix.db import kv_get

        raw_settings = kv_get("helix:settings", {}) or {}
    except Exception:
        raw_settings = {}
    settings = dict(raw_settings) if isinstance(raw_settings, dict) else {}
    env_default = os.environ.get("HELIX_AGENT_TASK_TIMEOUT_SECONDS")
...
def _recover_durable_completed_develop_candidate_tasks() -> int:
    ...
    from helix.db import append_task_audit_event, get_db, log_activity
...
async def _run_crucible_planner_backtest_task(task: dict, payload: dict) -> dict:
    ...
    from helix.db import append_task_audit_event, get_db, log_activity
    from helix.evolution import run_backtest_validation
...
def _write_api_task_worker_heartbeat(
    ...
    *,
    ...
    try:
        from helix.db import kv_set_best_effort
...
def _headless_task_processing_allowed() -> bool:
    ...
    try:
        ...
        from helix.system_pause import is_autonomy_paused
...
async def _run_brain_task(task: dict) -> None:
    ...
    from helix.context import build_brain_context, build_chat_context
    from helix.db import get_db
    ...
    if is_chat:
        ...
        from helix.agents.tool_definitions import CHAT_ACT_TOOL_NAMES
        ...
        try:
            from helix.context import store_conversation
    ...
    from helix.agents.tool_registry import VALID_CONTEXTS, filter_tools_for_context
...
def acquire_runtime_worker_lock(lock_name: str = "api_runtime_worker.lock") -> bool:
    ...
    lock_path = HELIX_HOME / lock_name
...
async def process_agent_tasks_once(concurrency: int = 5) -> int:
    ...
    async with _agent_claim_lock:
        ...
        active_task_ids = [
            int(getattr(task, "_helix_agent_task_id", 0) or 0)
            for task in _active_agent_tasks
            if not task.done() and int(getattr(task, "_helix_agent_task_id", 0) or 0)
        ]
        ...
        if terminal_task_ids:
            ...
            for task in list(_active_agent_tasks):
                task_id = int(getattr(task, "_helix_agent_task_id", 0) or 0)
        ...
        if available_slots <= 0:
            return 0
        active_agent_ids = {
            str(getattr(task, "_helix_agent_id", "") or "")
            for task in _active_agent_tasks
            if not task.done()
        }
        ...
        def _persist_task_failure(task_id: int, agent_id: str, error_text: str, exc_type: str) -> None:
            try:
                with get_db() as conn:
                    ...
                    try:
                        from helix.db import append_task_audit_event
                        append_task_audit_event(
                            conn,
                            int(task_id or 0),
                            "failed",
            ...
            async with sem:
                ...
                try:
                    ...
                    if not isinstance(exc, asyncio.TimeoutError):
                        from helix.ai import is_transient_provider_exception
        ...
        for agent, task in claimed:
            ...
            setattr(active, "_helix_agent_id", str(agent.get("id") or ""))
            setattr(active, "_helix_agent_task_id", int(task.get("id") or 0))
...
def _is_credential_failure(exc: Exception, error_text: str) -> bool:
    ...
    try:
        from helix.auth.store import CredentialError
...
def _maybe_pause_routine_on_credential_failure(
    ...
    try:
        ...
        from helix.control_plane.routines import set_routine_enabled
...
def ensure_brain_keepalive() -> bool:
    ...
    """
    try:
        from helix.db import create_pending_task, get_db
        from helix.system_pause import is_autonomy_paused
...
async def process_brain_tasks_once(limit: int | None = None) -> int:
    ...
    from helix.ai import _is_rate_limit_exception, is_transient_provider_exception
    from helix.db import claim_pending_tasks, get_db, requeue_brain_task
