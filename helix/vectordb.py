...
log = logging.getLogger("helix.vectordb")
...
_upsert_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="helix-chroma")
_HEALTH_COLLECTION_NAME = "helix_health_check"
_HEALTH_DOCUMENT_ID = "hc"
_HEALTH_METADATA = {"source": "helix_health_check"}
_DISABLE_IN_PROCESS_ENV = "HELIX_DISABLE_CHROMA_IN_PROCESS"
...
def store_backtest_result(
    ...
    *,
    ...
    try:
        from helix.quant_skills_extractor import maybe_extract
        maybe_extract({
            "strategy_id": strategy_id,
            "strategy_name": canonical_strategy_name,
            "strategy_type": strategy_type,
...
def upsert_quant_skill(skill) -> None:
    ...
    Accepts a ``helix.quant_skills.QuantSkill`` instance.
    """
    doc = (
        f"{skill.description}. "
