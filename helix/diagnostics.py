"""Helix diagnostics — health checks & system snapshot.

Used by:
- ``helix doctor`` CLI (T10)
...
log = logging.getLogger("helix.diagnostics")
...
def check_database() -> CheckResult:
    ...
    try:
        from helix.db import SCHEMA_VERSION, get_db_best_effort
...
def check_scheduler_freshness() -> CheckResult:
    ...
    """
    try:
        from helix.db import get_db_best_effort
        with get_db_best_effort(timeout_seconds=1.0) as conn:
            row = conn.execute(
...
def check_resumable_tasks() -> CheckResult:
    ...
    try:
        from helix.task_progress import list_resumable_tasks
...
def check_brain_fence_strips() -> CheckResult:
    ...
    try:
        from helix.sanitize import fence_strip_count
...
def check_brain_cache_hit_rate() -> CheckResult:
    ...
    try:
        from helix.brain_inject import cache_hit_rate_snapshot
...
def check_helix_home() -> CheckResult:
    """HELIX_HOME exists and is writable."""
    try:
        from helix.config import HELIX_HOME
        path = HELIX_HOME
        if not path.exists():
            return CheckResult("helix_home", FAIL, f"missing: {path}", {"path": str(path)})
        if not os.access(str(path), os.W_OK):
            return CheckResult("helix_home", FAIL, f"not writable: {path}", {"path": str(path)})
        return CheckResult("helix_home", PASS, f"{path} OK", {"path": str(path)})
    except Exception as exc:
        return CheckResult("helix_home", FAIL, f"check failed: {exc}", {"error": str(exc)})
