...
from helix.model_routing import get_primary_provider_model
...
from helix.hypotheses import require_hypothesis
from helix.workspace import (
    append_workspace,
    today_memory_path,
)
from helix.util import normalize_stage
from helix.policy import evaluate_promotion, load_pipeline_config
from helix.strategies.certification import certify_execution_strategy

try:
    from helix.policy import verify_backtest_exists_for_stage_transition
except ImportError:
...
log = logging.getLogger("helix.brain")
...
def _gauntlet_entry_guardrails(strategy_id: str, metrics: dict) -> tuple[bool, str]:
    ...
    from helix.metrics_integrity import check_metrics_integrity, data_quality_hold_reason
...
def _auto_approve_promotions_enabled(
    ...
    try:
        ...
        pipeline_settings = kv_get("helix:pipeline:settings", {}) or {}
...
def transition_stage(
    ...
    with get_db() as conn:
        ...
        if (not force) and normalized_target in {"paper", "deployed", "live_graduated"}:
            try:
                from helix.strategies.registry import runtime_unloadable_reason
        ...
        if normalized_target == "paper":
            from helix.policy import _paper_slot_competition_enabled
        ...
        if (not force) and normalized_target not in _wip_skip_stages:
            try:
                from helix.lab_features import check_stage_wip_capacity
        ...
        if normalized_target in ("quick_screen", "gauntlet"):
            try:
                from helix.gauntlet.store import WORKFLOW_DEFINITION_VERSION
    ...
    if normalized_target in ("archived", "rejected") and event_reason:
        ...
        try:
            from helix.vectordb import store_post_mortem
            store_post_mortem(
    ...
    # transition itself.
    try:
        from helix.brain_decisions import backfill_outcome_for_strategy
    ...
    if not skip_outcome_closure:
        ...
        if outcome_kind is not None:
            try:
                from helix.skill_outcomes import record_outcome as _record_outcome
...
def execute_brain_actions(
    ...
    *,
    ...
    """
    from helix.brain_decisions import (
        link_agent_task,
        record_decision,
        update_action_taken,
    )
...
async def invoke(
    ...
    try:
        from helix.context import get_brain_learning_injection
    ...
    # (P1-T05).
    if message:
        from helix.sanitize import sanitize_operator_input
    ...
    from helix.brain_inject import (
        build_user_message,
        compute_prompt_hash,
...
def _build_cycle_prompt() -> str:
    ...
    return (
        "You are the Brain ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ the boss of the Helix trading operation.\n\n"
...
def run_strategy_review():
    ...
    """
    from helix.strategies.fitness import get_promotion_candidates
...
def process_task_queue():
    ...
    from helix.db import claim_pending_tasks
...
def run_gauntlet_backtest_migration():
    ...
    Guarded by KV flag ``helix:migration:gauntlet_backtest_demotion_done`` so it runs once.
    Snapshots each affected strategy before demotion.
    """
    from helix.db import save_migration_snapshot

    flag_key = "helix:migration:gauntlet_backtest_demotion_done"
    if kv_get(flag_key):
        return
