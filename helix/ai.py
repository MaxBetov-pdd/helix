...
import httpx
from helix.model_routing import get_model_routing_snapshot
...
log = logging.getLogger("helix.ai")
...
def _record_call_health(provider: str, error: Exception | None = None) -> None:
    ...
    try:
        from helix.provider_runtime_health import record_call_failure, record_provider_ok
...
def _record_fallback_event(primary: str, used: str) -> None:
    ...
    try:
        from helix.provider_runtime_health import record_provider_event
...
async def _call_openai(
    ...
    if provider_label == "openai" and is_openai_oauth_token(token):
        from helix.codex_responses import call_codex
