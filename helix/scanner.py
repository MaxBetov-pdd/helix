...
from helix.db import format_prefixed_id, get_db, init_db, kv_get, kv_set, log_activity, next_container_id
...
from helix.strategies.params import canonicalize_params_with_metadata, resolve_strategy_family
from helix.trade_state import close_trade_record, mark_trade_pending_close_reconcile, parse_trade_signal_data

log = logging.getLogger("helix.scanner")
...
def _book_account_equity(account_address: str | None) -> float | None:
    ...
    if not account_address:
        return None
    try:
        from helix.exchange.hyperliquid import get_account_value
...
def _opposite_book_would_cross(asset: str, open_book: str) -> tuple[bool, str | None]:
    ...
    try:
        ...
        try:
            from helix.exchange.hyperliquid import get_open_orders
...
def _get_account_equity() -> float:
    ...
    from helix.sim.clock import is_sim_active, sim_kv_key
...
def _load_live_price_cache() -> tuple[dict[str, float], float | None]:
    ...
    from helix.sim.clock import is_sim_active
    try:
        if is_sim_active():
        ...
        pass
    try:
        from helix.exchange.hyperliquid import get_all_mids
        prices = get_all_mids()
        if prices:
            return prices, 0.0
...
def _queue_trade_execution_intent(intent: dict) -> tuple[bool, str | None, str | None]:
    ...
    try:
        from helix.brain import assign_task_direct
...
def _paper_test_mode_enabled() -> bool:
    ...
    try:
        from helix.api_domains.paper import _apply_paper_test_settings
...
def fetch_candles(coin: str, bars: int = 300, interval: str = "1h") -> pd.DataFrame:
    ...
    from helix.sim.clock import is_sim_active, get_now
    if is_sim_active():
        end_ms = int(get_now().timestamp() * 1000)

        # Try pre-fetch cache first
        from helix.sim.data_pump import get_cached_candles
...
def _enrich_scan_frame(df: pd.DataFrame, asset: str, timeframe: str) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        return df
    try:
        from helix.strategies.backtest import _enrich_with_market_data
...
def get_signal(
    ...
    if strategy_instance is None:
        try:
            from helix.strategies.registry import _TYPE_MAP, get_active, resolve_runtime_type
...
def _report_execution_failure(strategy_id: str | None, action: str, trade_id: str | None, reason: str | None = None) -> None:
    ...
    try:
        from helix.brain import handoff_execution_failure_to_developer
...
def _resolve_hyperliquid_testnet() -> bool:
    ...
    from helix.exchange.hyperliquid import resolve_configured_testnet
...
def _execute_direct(
    ...
    from helix.exchange.hyperliquid import close_position, market_order
    from helix.sim.clock import is_sim_active
    ...
    if action == "open":
        ...
        if not is_sim_active():
            from helix.exchange.risk import is_trading_allowed
        ...
        if not is_sim_active():
            from helix.exchange.hyperliquid import set_leverage
    ...
    if action == "close":
        ...
        try:
            from helix.exchange.hyperliquid import get_positions as _get_positions_for_funding
...
def _resolve_trade_vault_address(trade_id, *, strict: bool = False) -> str | None:
    ...
    try:
        from helix.exchange import books
        from helix.db import kv_get as _kv_get
        with get_db() as conn:
        ...
        settings = _kv_get("helix:settings", {})
        if not isinstance(settings, dict):
            settings = {}
    ...
    for strat_id, strat in strategy_items:
        try:
            ...
            if _last_ts is not None:
                ...
                try:
                    _max_bars = float((kv_get("helix:settings", {}) or {}).get("scanner_max_candle_staleness_bars", 2) or 2)
                except Exception:
                    _max_bars = 2.0
        ...
        for future in as_completed(futures):
            ...
            for item in scanned:
                try:
                    ...
                    try:
                        from helix.db import record_signal_result
...
def run_scan(*, execute_positions: bool = True) -> dict:
    ...
    try:
        from helix.strategies.registry import discover, get_active
        discover()
    ...
    asset_regimes: dict[str, object] = {}
    try:
        from helix.regime import detect_regime
...
def sweep_pending_close_reconcile() -> dict:
    ...
    """
    from helix.db import get_db, log_activity, kv_set
    from helix.trade_state import (
    ...
    def _exchange_open_size(target_asset: str, vault: str | None) -> float | None:
        ...
        if snap is None:
            try:
                from helix.exchange.hyperliquid import get_positions
    ...
    for row in trades:
        ...
        if outcome == "closed_locally_paper_local":
            ...
            try:
                from helix.exchange.hyperliquid import close_position
...
def record_scan_slo(
    ...
    from helix.db import kv_set, log_activity
