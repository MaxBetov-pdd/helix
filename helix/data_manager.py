...
from helix.binance_vision import bv_client

log = logging.getLogger("helix.data_manager")
...
# HELIX_HOME in packaged installs). Previously _BASE_DIR hardcoded a repo-relative
...
from helix.data import data_root as _data_root
...
def assert_data_root_consistent() -> bool:
    ...
    if ohlcv_root != stream_root:
        log.warning(
            ...
            "on zeros. Align HELIX_DATA_DIR / HELIX_HOME.",
            ohlcv_root,
            stream_root,
        )
        return False
...
def _save_stream_parquet(df: pd.DataFrame, path: Path, stream: str, symbol: str) -> None:
    """Atomic write of a stream DataFrame to parquet with helix metadata.
    ...
    try:
        ...
        meta.update({
            b"helix_source": b"binanceusdm",
            b"helix_stream": stream.encode(),
            b"helix_symbol": symbol.encode(),
            b"helix_updated_at": _now_iso().encode(),
        })
...
def _data_engine_collect_enabled() -> bool:
    try:
        from helix.data import _data_engine_read_enabled
...
def _fetch_stream_via_source_registry(
    ...
    *,
    ...
    from helix.dataeng.ccxt_source import CcxtSource
    from helix.dataeng.errors import NoData
    from helix.dataeng.identity import to_ref
    from helix.dataeng.source import Stream, get_source_registry
...
class FundingCollector:
    ...
    def collect(self, symbol: str) -> int:
        ...
        from helix.data import symbol_to_fs, symbol_to_ccxt
...
class DataManager:
    ...
    def _normalize_keepalive_symbol(self, symbol: str | None, *, require_dataset: bool = False) -> str | None:
        ...
        """
        from helix.data import DATA_DIR, symbol_to_fs
    ...
    def _select_keepalive_pairs(
        ...
        if not (max_pairs_per_run and max_pairs_per_run > 0) or len(pairs) <= max_pairs_per_run:
            return list(pairs)
        from helix.data import parquet_path
    ...
    def collect_liquidations(self) -> dict[str, Any]:
        ...
        auth-gated/deprecated. Set HELIX_ENABLE_LIQUIDATIONS=1 to opt in."""
        # Env-gated early return: do NOT record a run when disabled (T22).
        if os.environ.get("HELIX_ENABLE_LIQUIDATIONS") != "1":
    ...
    def _backfill_ohlcv(self, fs_sym: str, bv_symbol: str) -> dict:
        from helix.data import DATA_DIR, load_parquet, save_parquet, _get_dataset_lock
        out: dict = {}
...
# Existing callers — `from helix.data_manager import data_manager` and
