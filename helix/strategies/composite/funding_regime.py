...
class FundingRegimeStrategy(BaseStrategy):
    ...
    def _funding_extreme(self, df: pd.DataFrame) -> tuple[bool, float]:
        ...
        if "funding_rate" not in df.columns:
            return False, 0.0
        from helix.scanner import funding_rate_zscore
    ...
    def generate_signal(self, df: pd.DataFrame) -> Signal:
        if len(df) < 10:
            return Signal()
        try:
            from helix.scanner import atr as calc_atr, oi_price_divergence
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            from helix.scanner import oi_price_divergence
