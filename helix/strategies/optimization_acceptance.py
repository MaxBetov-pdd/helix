...
* ``helix.gauntlet.tasks.run_apply_optimized_defaults``
* ``helix.evolution`` ``apply_best_params``
...
log = logging.getLogger("helix.strategies.optimization_acceptance")
...
def _inert_axes() -> frozenset[str]:
    ...
    try:
        from helix.strategies.optimizer import _NEVER_SIMULATED_RISK_AXES
...
def _decide(baseline_wfa: dict, candidate_wfa: dict) -> AcceptanceDecision:
    ...
    Both inputs are :func:`helix.strategies.backtest.walk_forward` results run
