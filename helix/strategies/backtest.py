...
from helix.db import create_strategy_container, get_db, init_db, next_container_id


from helix.regime import HIGH_VOL, RANGE_BOUND, TREND_DOWN, TREND_UP, _classify, resolve_regime_gate


from helix.scanner import (
...
from helix.strategies.base import BaseStrategy, DirectionalSignals, TradeMode


from helix.strategies.params import canonicalize_params, resolve_strategy_family





log = logging.getLogger("helix.strategies.backtest")
...
def _should_use_process_isolation() -> bool:
    override = str(os.getenv("HELIX_BACKTEST_PROCESS_ISOLATION", "") or "").strip().lower()
...
def _isolated_backtest_worker(
    ...
    """
    try:
        from helix.strategies.registry import discover
        discover()
...
def _validate_backtest_execution_parity(
    ...
    *,
    ...
    from helix.strategies.certification import EXECUTION_CERTIFIED_FAMILIES
    ...
    *,
    ...
    try:
        ...
        with get_db() as conn:
            ...
            from helix.brain import stage_is_param_locked
            ...
            from helix.metrics_integrity import DATA_QUALITY_FLAGS_KEY, check_metrics_integrity
...
def _check_data_requirements(strategy_type: str, asset: str, timeframe: str, bars: int) -> str | None:
    ...
    try:
        ...
        from helix.data import load_parquet, fetch_ohlcv_chunked, symbol_to_fs
...
def _resolve_strategy_class(strategy_type: str | None):
    ...
    try:
        ...
        from helix.strategies.registry import _TYPE_MAP, discover, resolve_runtime_type
    ...
    try:
        ...
        from helix.strategies import custom
        ...
        for _importer, modname, _ispkg in pkgutil.iter_modules(custom.__path__):
            ...
            try:
                ...
                from helix.strategies.registry import assert_custom_module_safe

                assert_custom_module_safe(modname)
                module = importlib.import_module(f"helix.strategies.custom.{modname}")
...
def _enrich_with_market_data(df: pd.DataFrame, asset: str) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        return df
    try:
        from helix.market_data_collector import get_funding_rate_series, get_open_interest_series
        ...
        try:
            from helix.market_data_collector import ensure_funding_history
...
def load_backtest_candles(
    ...
    *,
    ...
    try:
        ...
        from helix.data import load_parquet
...
def _load_remote_chart_frame(
    ...
    *,
    ...
    try:
        ...
        from helix.data import _timeframe_to_ms, fetch_ohlcv_chunked, load_parquet
...
def _load_local_chart_frame(
    ...
    *,
    ...
    try:
        ...
        from helix.data import load_parquet, parquet_path
...
def _build_rule_engine_chart_indicators(
    ...
    """
    from helix.strategies.builtin.rule_engine import build_series_table, validate_rule_spec
    from helix.strategies import indicators as _ind
...
def _build_chart_indicators(frame: pd.DataFrame, strategy_type: str, params: dict | None) -> tuple[list[dict], list[dict], list[str]]:
    ...
    if normalized_type not in _CHART_SUPPORTED_TYPES:
        ...
        if normalized_type:
            ...
            try:
                from helix.strategies.params import is_known_runtime_type as _is_known_rt
            ...
            if not _rt_known:
                warnings.append(
                    ...
                    "helix/strategies/custom/ or archive it."
                )
            else:
...
def build_strategy_preview_chart_context(
    *,
    ...
    """
    from helix.strategies.builtin.rule_engine import (
        RuleEngineStrategy,
        validate_rule_spec,
        _spec_min_bars,
    )
...
def _precompute_indicators(df: pd.DataFrame, strategy_type: str, params: dict) -> pd.DataFrame:
    ...
    if strategy_type == "rsi_momentum":
        ...
        from helix.scanner import stochastic as compute_stochastic
        ...
        from helix.scanner import atr as compute_atr
...
def _compute_adx_filter(df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    if "adx_val" not in df.columns:
        ...
        from helix.scanner import adx as calc_adx
...
def _vectorized_signals(df: pd.DataFrame, strategy_type: str, params: dict) -> tuple:
    ...
    if strategy_type == "rsi_momentum":
        ...
        from helix.strategies.builtin.donchian import donchian_bands, resolve_donchian_period
        ...
        from helix.strategies.builtin.parabolic_sar import _resolve_psar_params, parabolic_sar_series
...
def _hours_per_bar(timeframe: str) -> float:
    ...
    try:
        from helix.data import _timeframe_to_ms
...
from helix.strategies import sizing as _sizing
...
def _run_remote_backtest(
    ...
    api_key = os.environ.get("HELIX_COMPUTE_API_KEY", "").strip()
...
def backtest_strategy(
    ...
    from helix.api_core import get_settings
    ...
    if strategy_cls is None:
        from helix.strategies.params import is_known_strategy_family as _is_known_family
        ...
        if not _is_known_family(family_strategy_type) and not _is_known_family(original_strategy_type):
            return {
                "error": (
                    ...
                    "helix/strategies/custom/ or archive it. Running this "
            ...
            if persist_legacy_run:
                ...
                if run_id:
                    ...
                    try:
                        ...
                        from helix.api_core import _persist_backtest_result_row
    ...
    if family_strategy_type == "funding":
        ...
        from helix.strategies.sentiment import get_funding_for_backtest
    ...
    # strategy code cannot freeze the main Helix backend.
    ...
    try:
        from helix.data import get_dataset_source
    ...
    if run_id:
        ...
        try:
            ...
            from helix.api_core import _persist_backtest_result_row, _write_backtest_result_artifacts
    ...
    if run_id:
        ...
        try:
            ...
            from helix.strategies.fitness import compute_fitness_score
...
def _compute_basic_metrics(trades: list[dict], total_bars: int, *, timeframe: str = "1h", symbol: str | None = None) -> dict:
    ...
    if symbol:
        try:
            from helix.asset_constants import get_bars_per_year
            from helix.symbol_mapping import detect_asset_class
...
def _detect_entry_regime(window) -> str:
    ...
    try:
        ...
        from helix.scanner import rsi as calc_rsi, adx as calc_adx
...
def walk_forward(
    ...
    if total_bars is None:
        ...
        from helix.api_core import _timeframe_to_minutes, stage_backtest_duration_days
    ...
    try:
        from helix.policy import load_pipeline_config as _load_wfa_config
