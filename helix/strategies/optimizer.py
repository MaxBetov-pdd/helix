...
from helix.strategies.backtest import (
...
log = logging.getLogger("helix.strategies.optimizer")

# B-4: Risk fields the backtest engine does NOT enforce when they appear inside
# a strategy's ``params`` blob (warn-only — see
# helix.strategies.backtest._UNSUPPORTED_BACKTEST_RISK_FIELDS). The optimizer
...
def optimize_strategy(
        ...
        if not is_known_runtime_type(strategy_type):
            return {
                "error": (
                    ...
                    "helix/strategies/custom/, or archive the strategy."
                )
            }
        return {
            "error": (
                ...
                "helix/strategies/optimizer.py:_get_param_space defaults."
            )
        }
    ...
    from helix.strategies.backtest import execution_controls_from_params
...
def optimize_all_deployed() -> list[dict]:
    ...
    from helix.db import get_strategies
...
def _resolve_strategy(strategy_id: str) -> tuple[str, str, dict]:
    ...
    # Try registry
    try:
        from helix.strategies.registry import get as registry_get
...
def _get_param_space(strategy_id: str, strategy_type: str, base_params: dict) -> dict:
    ...
    if not space:
        try:
            from helix.strategies.params import resolve_strategy_family
