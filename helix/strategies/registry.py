...
from helix.strategies.base import BaseStrategy
from helix.strategies.custom_catalog import custom_strategy_status, include_archived_custom_strategies
from helix.strategies.params import canonicalize_params_with_metadata, resolve_strategy_family

log = logging.getLogger("helix.strategies.registry")
...
def discover(include_custom: bool = True):
    """Auto-discover strategy classes in helix.strategies.builtin and custom.
    ...
    if not _builtin_discovered:
        _registry.clear()
        try:
            from helix.strategies import builtin
...
def _load_builtin_strategy_module(modname: str) -> None:
    module = importlib.import_module(f"helix.strategies.builtin.{modname}")
