...
log = logging.getLogger("helix.strategies.intake")
...
def _intended_timeframe(stored_params: object) -> str:
    ...
    try:
        from helix.market_data import INTERVAL_TO_MS
...
def _extract_embedded_hypothesis_id(path: Path) -> str | None:
    ...
    patterns = (
        r'^\s*HELIX_HYPOTHESIS_ID\s*=\s*["\']([^"\']+)["\']',
        r'^\s*#\s*HELIX_HYPOTHESIS_ID\s*:\s*(\S+)',
...
def scan_custom_strategies(*, register: bool = False) -> dict:
    ...
    from helix.strategies.custom_catalog import (
    ...
    for _importer, modname, _ispkg in pkgutil.iter_modules(custom.__path__):
        ...
        fqn = f"helix.strategies.custom.{modname}"
        ...
        if banned:
            report.errors.append(IntakeError(
                ...
                error=(
                    ...
                    "instead. See helix/strategies/STRATEGY_TEMPLATE.md."
                ),
                file_name=file_name,
            ))
            continue
        ...
        from helix.strategies.lookahead_probe import detect_lookahead
...
def register_custom_strategy_file(
    *,
    ...
    from helix.strategies import registry
    from helix.strategies.certification import certify_execution_strategy
    from helix.db import create_strategy_container, get_db, log_activity
    from helix.ai_dropzone_sessions import (
    ...
    if banned:
        raise ValueError(
            ...
            "See helix/strategies/STRATEGY_TEMPLATE.md."
        )
...
def _resolve_targeted_custom_module(
    *,
    ...
    if normalized_module.startswith("helix.strategies.custom."):
