...
_RUNTIME_FINGERPRINT_FILES = (
    "helix/daemon.py",
    "helix/control_plane/status.py",
    "helix/api_domains/trading.py",
    "helix/exchange/risk.py",
    "helix/exchange/hyperliquid.py",
    "helix/scanner.py",
)
...
def read_daemon_lock_pid(lock_path: Path | None = None) -> int | None:
    path = Path(lock_path or (HELIX_HOME / "daemon.lock"))
    if not path.exists():
        return None
    try:
...
def remove_stale_daemon_lock(expected_pid: int | None = None) -> bool:
    lock_path = Path(HELIX_HOME) / "daemon.lock"
