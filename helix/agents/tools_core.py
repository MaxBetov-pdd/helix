...
from helix.security.env_allowlist import build_subprocess_env
...
# H-S3: opt-in strict allowlist. When HELIX_SHELL_STRICT_ALLOWLIST=1, only
...
def _shell_strict_allowlist_enabled() -> bool:
    return str(os.environ.get("HELIX_SHELL_STRICT_ALLOWLIST", "")).strip().lower() in {"1", "true", "yes", "on"}
...
def _shell_tool_enabled() -> bool:
    ...
    """
    return str(os.environ.get("HELIX_ENABLE_SHELL_TOOL", "")).strip().lower() in {"1", "true", "yes", "on"}
...
async def _tool_run_shell(command: str) -> str:
    ...
    # operator must opt in with HELIX_ENABLE_SHELL_TOOL=1 (and should also set
    # HELIX_SHELL_STRICT_ALLOWLIST=1). See security audit 2026-04-23.
    if not _shell_tool_enabled():
        return (
            "Blocked: the run_shell tool is disabled by default. Set "
            "HELIX_ENABLE_SHELL_TOOL=1 to enable it (at your own risk; consider "
            "also HELIX_SHELL_STRICT_ALLOWLIST=1). Use structured tools "
    ...
    try:
        from helix.sandbox.shell_guard import evaluate_for_run_shell
    ...
    # compatibility; opt in via HELIX_SHELL_STRICT_ALLOWLIST=1.
        ...
        for prog in programs:
            if prog and prog not in _PROGRAM_ALLOWLIST:
                return (
                    ...
                    f"(H-S3 strict mode). Set HELIX_SHELL_STRICT_ALLOWLIST=0 to disable."
                )
...
@register_tool(
    name="read_file",
    description="Read a file from the Helix workspace (~/.helix/workspace/). Provide path relative to workspace root.",
    input_schema={
        "type": "object",
        "properties": {
...
def _tool_read_file(path: str) -> str:
    ...
    from helix.workspace import WorkspacePathError, safe_workspace_path
    try:
...
@register_tool(
    name="write_file",
    description=(
        "Write or append to a file in the Helix workspace (~/.helix/workspace/). "
...
def _tool_write_file(path: str, content: str, append: bool = True) -> str:
    ...
    if append:
        ...
        from helix.workspace import write_workspace
...
async def _tool_search_memory(query: str) -> str:
    ...
    from helix.vectordb import search_narratives
...
async def _tool_store_memory(content: str, category: str | None = None) -> str:
    ...
    from helix.vectordb import _in_process_chroma_disabled, store_narrative
    ...
    store_narrative(content, metadata={"type": category or "agent_finding", "source": "helix_agent"})
...
def _tool_store_chroma(collection: str, doc_id: str, content: str, metadata: dict | None = None) -> str:
    ...
    from helix.vectordb import _in_process_chroma_disabled, _upsert
    if _in_process_chroma_disabled():
...
@register_tool(
    ...
    description="List OHLCV datasets available in the local Helix storage. Returns symbol, timeframe, and row count.",
    input_schema={
        "type": "object",
        "properties": {
...
def _tool_list_local_datasets(symbol_filter: str | None = None) -> str:
    """List OHLCV datasets available in local Helix storage."""
