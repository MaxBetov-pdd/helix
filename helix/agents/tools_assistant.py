...
from helix.agents.context import _current_agent_id_var  # noqa: F401 (kept for parity/future use)
from helix.agents.tool_registry import register_tool
...
def _tool_get_portfolio_status() -> str:
    from helix.db import get_open_trades, kv_get
...
def _tool_get_pipeline_status() -> str:
    from helix.context import _format_evolution_status
...
def _tool_assistant_create_strategy(
    ...
    from helix.brain import create_strategy, resolve_brain_provider_model
...
# canonical backtest over the configured Backtest window). Mirrors helix/agent/client.py.
_QUICK_SCREEN = {
...
@register_tool(
    ...
    description=(
        ...
        "helix/strategies/custom/) into the 'quick_screen' stage so it can be "
    ...
    input_schema={
        "type": "object",
        "properties": {
            "file_path": {"type": "string", "description": "Absolute path to the .py in helix/strategies/custom/"},
...
def _tool_assistant_register_strategy_file(file_path: str, session_id: str | None = None) -> str:
    ...
    if not fp:
        return "Error: file_path is required (absolute path to a .py in helix/strategies/custom/)."
    try:
...
def _tool_assistant_enqueue_candidate(
    ...
    from helix.brain import promote_strategy
