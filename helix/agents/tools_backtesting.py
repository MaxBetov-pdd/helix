...
from helix.verdict_engine import build_strategy_verdict_blob
...
def _persist_agent_backtest(
    *,
    ...
    from helix.db import verify_chroma_persistence
...
def _tool_run_backtest(params: dict) -> str:
    ...
    try:
        ...
        from helix.strategies.fitness import score_strategy
...
def _tool_run_code(code: str) -> str:
    ...
    from helix.sandbox import run_code
...
@register_tool(
    ...
    description=(
        ...
        "backtesting via run_backtest. The code must extend BaseStrategy from helix.strategies.base and "
...
def _tool_register_strategy(params: dict) -> str:
    ...
    from helix.crucible_tasks import validate_candidate_strategy_creation
    ...
    try:
        from helix.strategies.registry import reset, discover, _TYPE_MAP
        from helix.strategies.intake import register_custom_strategy_file
        reset()
...
def _tool_lint_code(code: str) -> str:
    ...
    from helix.sandbox import lint_code
...
# Helix availability cache — avoid repeated health checks that waste time
...
def _check_backtesting_available() -> bool:
    """Check Helix availability with short failure TTL for quick recovery."""
    ...
    from helix.backtesting import is_available
...
_BACKTESTING_FALLBACK_MSG = (
    "Helix Backtesting is not reachable. Do NOT debug connectivity — "
...
def _tool_backtesting(tool_name: str, params: dict) -> str:
    """Execute a Helix Backtesting tool. Routes to the backtesting client."""
    ...
    try:
        ...
        elif tool_name == "helix_run_backtest":
        ...
        elif tool_name == "helix_run_optimization":
        ...
        elif tool_name == "helix_run_verdict":
        ...
        elif tool_name == "helix_get_results":
...
# ── Register Helix Backtesting tools (routed through _tool_backtesting) ──
...
register_tool(
    name="helix_list_datasets",
    description=(
        "List available backtesting datasets on Helix Backtesting. Returns datasets with "
...
)(_make_jbt_handler("helix_list_datasets"))

register_tool(
    name="helix_create_strategy",
    description=(
        "Create a tradable strategy on Helix Backtesting. Any strategy family and params are accepted — "
    ...
    input_schema={
        ...
        "properties": {
            ...
            "strategy_type": {
                "type": "string",
                "description": (
                    "Strategy family name. Prefer executable Helix families such as orb, "
    ...
    permissions={"role:strategy-developer", None},
)(_make_jbt_handler("helix_create_strategy"))

register_tool(
    name="helix_run_backtest",
    description=(
        "Run a backtest on Helix Backtesting with realistic fees (4.5 bps) and slippage (2 bps)."
...
)(_make_jbt_handler("helix_run_backtest"))

register_tool(
    name="helix_run_optimization",
    description=(
        "Run parameter optimization on Helix Backtesting. Finds optimal parameter values using Optuna. "
...
)(_make_jbt_handler("helix_run_optimization"))
...
)(_make_jbt_handler("helix_run_verdict"))

register_tool(
    name="helix_get_results",
    description="Get detailed results from a Helix backtest, including optional trade list and equity curve.",
    input_schema={
        "type": "object",
        "properties": {
...
)(_make_jbt_handler("helix_get_results"))
