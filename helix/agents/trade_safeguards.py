...
machine-specific path (``/home/trestor/.helix/workspace/agents/execution-trader``),
which raised ``ModuleNotFoundError`` on every other machine and silently disabled the
safeguard layer. It now lives in ``helix.agents`` and is imported normally.
...
which regime* is handled upstream in ``helix.regime``; this is a last-line defence on
...
class MarketRegime(str, Enum):
    ...
    aliases understood by :func:`helix.regime.normalize_regime_label`
