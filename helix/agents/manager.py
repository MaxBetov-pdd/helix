...
from helix.config import LEGACY_WORKSPACE_DIR, WORKSPACE_DIR
from helix.ai import normalize_provider_and_model
from helix.db import get_db, init_db, get_agents, normalize_agent_visibility
from helix.workspace import read_workspace, write_workspace

log = logging.getLogger("helix.agents.manager")
...
def _render_soul_md(name: str, role: str) -> str:
    ...
    header = (
        f"# {name} — Identity\n\n"
        f"_You are **{name}**, a Helix sub-agent. Your role: {role}_\n\n"
        f"Everything below is Helix's shared soul — internalize it as your own.\n\n---\n\n"
    )
    return header + base
