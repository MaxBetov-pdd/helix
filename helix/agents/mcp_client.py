...
Helix speaks MCP as a *client* so external servers (filesystem, git,
sqlite, custom user servers) can expose tools that agents call through
the same `tool_registry` flow as native Helix tools.
...
- **No background services.** Sessions are per-call, opened on demand
  ...
  any orphan subprocesses; if Helix dies, MCP children die with it.
...
- **Subprocess env scrubbing.** stdio servers spawn through
  `helix.security.env_allowlist.build_subprocess_env`. Caller-supplied
...
from helix.db import get_db, get_db_best_effort
from helix.security.env_allowlist import build_subprocess_env

log = logging.getLogger("helix.agents.mcp_client")


PROTOCOL_VERSION = "2024-11-05"
CLIENT_INFO = {"name": "Helix", "version": "0.1"}
...
def _timeout(default: float) -> float:
    raw = os.environ.get("HELIX_MCP_TIMEOUT")
    if not raw:
        return default
    try:
        v = float(raw)
...
async def connect(config: MCPServerConfig) -> MCPSession:
    ...
    try:
        if config.transport == "stdio":
            ...
            # only when HELIX_ALLOW_LOCAL_MCP=1 (developer opt-in).
            from helix.security.url_safety import (
                UnsafeUrlError,
                validate_public_url,
            )
            allow_local = bool(os.environ.get("HELIX_ALLOW_LOCAL_MCP", "").strip())
            try:
                if allow_local:
...
def _mcp_tool_name(server: str, tool: str) -> str:
    """Helix-namespaced tool name: ``mcp_<server>_<tool>``.
...
async def register_server_tools(name: str) -> int:
    ...
    """
    from helix.agents.tool_registry import _REGISTRY, ToolDef
