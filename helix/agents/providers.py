...
import httpx
from helix.ai import build_provider_timeout
from helix.auth.store import get_profile
from helix.codex_responses import call_codex, is_openai_oauth_token, stream_codex

log = logging.getLogger("helix.agents.providers")
...
class CodexProvider(OpenAIProvider):
    ...
    ChatGPT subscription OAuth tokens (minted by ``helix.auth.openai``) are
    ...
    ``helix.codex_responses`` converts to Responses ``input`` items. The raw
...
class OpenRouterProvider(OpenAIProvider):
    ...
    ``anthropic/claude-sonnet-4``). Helix's ``provider:model`` routing
...
class AnthropicProvider(ToolCallProvider):
    ...
    Profile (``helix.auth.store.get_profile('anthropic')``) supports
