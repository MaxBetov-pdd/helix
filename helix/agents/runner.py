...
from helix.cost_pricing import estimate_cost_usd
from helix.db import claim_pending_agent_tasks, create_pending_task, format_prefixed_id, get_db, get_task_tool_calls, init_db, is_user_active, kv_get, kv_set, log_activity
from helix.provider_runtime_health import (
    record_call_failure,
    record_provider_event,
    record_provider_ok,
)
from helix.research_context import build_research_context, coerce_research_contract
from helix.task_timeouts import DEFAULT_AGENT_TASK_TIMEOUT_SECONDS, resolve_agent_task_timeout_seconds
...
from .tool_definitions import (
    AGENT_TOOLS,
    BACKTESTING_TOOLS,  # noqa: F401 - legacy re-exported via helix.agents
    BRAIN_TOOLS,  # noqa: F401 - legacy re-exported via helix.agents
    EXCHANGE_TOOLS,  # noqa: F401 - legacy re-exported via helix.agents
    MAX_TOOL_ROUNDS,
...
import helix.agents.tools_core         # noqa: F401
import helix.agents.tools_brain        # noqa: F401
import helix.agents.tools_exchange     # noqa: F401
import helix.agents.tools_backtesting  # noqa: F401
...
log = logging.getLogger("helix.agents.runner")
...
# Backward-compatible re-exports used by helix.agents.__init__ and older tests.
...
def _resolve_tool_call_chain(
    ...
    if agent_id:
        try:
            from helix.model_selection import _policy_slot_fallbacks
...
def _provider_has_credentials(provider: str) -> bool:
    ...
    """
    try:
        from helix.ai import get_token
        get_token(provider)
        return True
    except Exception:
        return False
...
def _first_configured_provider() -> tuple[str, str] | None:
    ...
    """
    try:
        from helix.model_routing import get_default_model_for_provider, get_model_routing
...
def _resolve_backup_provider(primary_provider: str) -> tuple[str, str] | None:
    ...
    """
    try:
        from helix.config import get_backup_ai_model, get_backup_ai_provider
...
async def _call_with_tools(
    ...
    # the manual CLI `helix brain-invoke` path calls call_ai() directly and is not
    ...
    if not _provider_has_credentials(primary_provider):
        ...
        if backup is not None:
            ...
            from helix.auth.store import CredentialError, credential_status
...
async def _call_with_tools_single(
    ...
    """
    from helix.auth.store import get_token
    ...
    for round_num in range(MAX_TOOL_ROUNDS):
        ...
        if round_num > 0:
            from helix.billing_guard import check_daily_cost_cap
        ...
        from helix.model_selection import assert_callable
...
def _error_detail(error: Exception, body_limit: int = 4000) -> str:
    ...
    SECRET-REDACTED at the source (helix.redact): this string is persisted to
    ...
    from helix.redact import redact
...
def _emit_provider_quota_alert(provider: str, detail: str) -> None:
    ...
    cooldown_key = f"helix:provider-quota-alert:{provider_key}"
...
def _research_task_already_performed_first_wave(task_display_id: str) -> bool:
    ...
    attempted_follow_through = any(
        ...
        in {"helix_create_strategy", "register_strategy", "run_backtest", "helix_run_backtest"}
        for call in tool_calls
    )
...
def _hypothesis_target_strategy_count() -> int:
    ...
    """
    from helix.research_contract import get_hypothesis_discipline_settings
    ...
    *,
    ...
    for _, hypothesis in candidates:
        ...
        description = (
            ...
            "1. Use helix_create_strategy or register_strategy to create a linked strategy container.\n"
...
async def run_agent_task(agent: dict, task: dict) -> dict:
    ...
    if not can_run:
        if task_type == "phantom_repair" and strategy_id:
            from helix.phantom_recovery import mark_phantom_recovery_exhausted
    ...
    if task_type == "phantom_repair" and strategy_id:
        from helix.phantom_recovery import mark_phantom_recovery_repair_running
    ...
    try:
        ...
        if not is_trade_execution_task:
            ...
            if _requeue_agent_task(
                ...
                if task_type == "phantom_repair" and strategy_id:
                    from helix.phantom_recovery import mark_phantom_recovery_repair_pending
...
async def _run_agent_task_inner(
    ...
    try:
        ...
        if is_trade_execution_task:
            from helix.scanner import execute_trade_intent
        ...
        from helix.redact import redact_dict
        ...
        if task_type == "phantom_repair" and strategy_id:
            from helix.phantom_recovery import handle_phantom_repair_completion
        ...
        if strategy_id:
            try:
                from helix.brain import NEXT_STAGE, STAGE_TO_AGENT, transition_stage
        ...
        with get_db() as conn:
            ...
            if handoff_target_agent:
                from helix.db import handoff_task
        ...
        try:
            from helix.reporter import broadcast_agent_task
        ...
        from helix.model_selection import UnconfiguredRouteError
