...
from helix import brain_memory as _brain_memory
...
def _suggest_known_families(stype: str, *, n: int = 3) -> list[str]:
    ...
    new family names.
    """
    from helix.strategies.params import SUPPORTED_PARAM_FAMILIES
...
def _tool_create_strategy(params: dict) -> str:
    ...
    from helix.agents.tools_research import assert_hypothesis_spawn_allowed
...
def _tool_factory_reset(params: dict) -> str:
    ...
    from helix.db import factory_reset, FACTORY_RESET_CATEGORIES
...
def _tool_recall_similar_situation(params: dict) -> str:
    """Brain-only recall tool — wraps :func:`helix.recall.recall_similar_situation`."""
    from helix.recall import recall_similar_situation
...
def _tool_propose_skill_update(params: dict) -> str:
    ...
    from helix import quant_skills as qs
    from helix.db import create_approval
...
def _tool_create_routine(params: dict) -> str:
    ...
    """
    from helix.control_plane.routines import RoutineValidationError, _validate_cron, _validate_context, get_routine_by_name
    from helix.db import create_approval
