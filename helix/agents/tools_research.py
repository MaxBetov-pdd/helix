...
from helix.strategy_extrapolation import extrapolate_strategy_spec, record_extrapolation_gaps

try:
    from helix.research_sources.youtube import inspect_youtube_video, search_youtube_videos
...
def _hypothesis_creation_blocker_for_current_task() -> dict[str, Any] | None:
    ...
    if action_kind == "refine_crucible" or "refine crucible" in task_title:
        ...
        guidance = (
            "This task is a backtest task. Use helix_run_backtest for the provided "
            "strategy_id; do not create a new hypothesis."
        )
    else:
        guidance = (
            "This task is bound to an existing crucible. Use helix_create_strategy "
...
def _resolve_source_registry(source_type: str):
    ...
    from helix.research_sources._registry import resolve_registry
...
def _tool_create_hypothesis(params: dict) -> str:
    from helix.system_mode_policy import autonomous_hypothesis_generation_allowed
    from helix.system_pause import get_system_mode
    ...
    # Crypto-only scope: Helix trades crypto (Hyperliquid) for now; stock/forex
    ...
    from helix.symbol_mapping import AssetClass, detect_asset_class
    ...
    if non_crypto:
        return json.dumps({
            ...
            "error": (
                ...
                "Helix currently trades crypto only."
            ),
            "guidance": (
    ...
    if not params.get("derived_from_hypothesis_id"):
        from helix.hypotheses import count_unstarted_active_hypotheses, find_duplicate_hypothesis
...
def _tool_discover_reddit_posts(params: dict) -> str:
    ...
    try:
        from helix.research_sources.reddit import _client, search_reddit_posts
    except ImportError:
...
def _tool_inspect_reddit_thread(params: dict) -> str:
    ...
    if cfg is None:
        return json.dumps({"ok": False, "error": "reddit source is disabled in research_sources settings"})
    try:
        from helix.research_sources.reddit import _client, inspect_reddit_thread
    except ImportError:
        return json.dumps({"ok": False, "error": "reddit research helper unavailable"})
    rate_per_min = _coerce_positive_int(cfg.get("rate_limit_per_min"), 30)
    client = _client(rate_per_min=rate_per_min)
...
def _tool_discover_blog_articles(params: dict) -> str:
    ...
    try:
        from helix.research_sources.blog import _client, search_blog_articles
    except ImportError:
...
def _tool_inspect_blog_article(params: dict) -> str:
    ...
    if cfg is None:
        return json.dumps({"ok": False, "error": "blog source is disabled in research_sources settings"})
    try:
        from helix.research_sources.blog import _client, inspect_blog_article
    except ImportError:
        return json.dumps({"ok": False, "error": "blog research helper unavailable"})
    rate_per_min = _coerce_positive_int(cfg.get("rate_limit_per_min"), 30)
    client = _client(rate_per_min=rate_per_min)
...
def _tool_discover_github_repos(params: dict) -> str:
    ...
    try:
        from helix.research_sources.github import _client, search_github_repos
    except ImportError:
...
def _tool_inspect_github_repo(params: dict) -> str:
    ...
    if cfg is None:
        return json.dumps({"ok": False, "error": "github source is disabled in research_sources settings"})
    try:
        from helix.research_sources.github import _client, inspect_github_repo
    except ImportError:
        return json.dumps({"ok": False, "error": "github research helper unavailable"})
    rate_per_min = _coerce_positive_int(cfg.get("rate_limit_per_min"), 60)
    pat = cfg.get("personal_access_token")
...
def _tool_discover_forum_threads(params: dict) -> str:
    ...
    try:
        from helix.research_sources.forum import _client, search_forum_threads
    except ImportError:
...
def _tool_inspect_forum_thread(params: dict) -> str:
    ...
    if cfg is None:
        return json.dumps({"ok": False, "error": "forum source is disabled in research_sources settings"})
    try:
        from helix.research_sources.forum import _client, inspect_forum_thread
    except ImportError:
        return json.dumps({"ok": False, "error": "forum research helper unavailable"})
    rate_per_min = _coerce_positive_int(cfg.get("rate_limit_per_min"), 20)
    client = _client(rate_per_min=rate_per_min)
