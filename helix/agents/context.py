...
_current_agent_id_var: ContextVar[str | None] = ContextVar("helix_current_agent_id", default=None)
_current_task_display_id_var: ContextVar[str | None] = ContextVar("helix_current_task_display_id", default=None)
_current_strategy_id_var: ContextVar[str | None] = ContextVar("helix_current_strategy_id", default=None)
...
_current_tools_context_var: ContextVar[str | None] = ContextVar("helix_current_tools_context", default=None)
...
def _normalize_legacy_paths(
    ...
    *,
    ...
    # Current runtime home is ~/.helix.
    normalized = re.sub(r"(?<!\w)~/judex(?=[^\w]|$)", "~/.helix", normalized)
    normalized = re.sub(r"(?<!\w)~/\.judex(?=[^\w]|$)", "~/.helix", normalized)

    current_home_alias = home_norm + "/.helix"
    legacy_home_aliases = (
        home_norm + "/judex",
        home_norm + "/.judex",
    )
