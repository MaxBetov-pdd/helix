...
from helix.api_core import HelixV1CompatMiddleware, _on_startup
from helix.api_security import (
    ApiKeyMiddleware,
    assert_auth_keys_configured,
    assert_safe_bind_host,
    get_allowed_cors_origins,
)
from helix.async_utils import spawn
from helix.correlation import CorrelationIdMiddleware, RequestIdLogFilter
from helix.rate_limiting import install_rate_limiter
from helix.control_plane import (
    ConfirmBody,
    ExecutionModeBody,
    post_emergency_halt,
    post_execution_mode,
...
from helix.routers.analytics import router as analytics_router
from helix.routers.approvals import router as approvals_router
from helix.routers.auth import router as auth_router
from helix.routers.agents import router as agents_router
from helix.routers.agent_toolsets import router as agent_toolsets_router
from helix.routers.data import router as data_router
from helix.routers.deepdive import router as deepdive_router
from helix.routers.assistant import router as assistant_router
from helix.routers.jobs import router as jobs_router
from helix.routers.legacy import router as legacy_router
from helix.routers.memory import router as memory_router
from helix.routers.diagnostics import router as diagnostics_router
from helix.routers.mcp import router as mcp_router
from helix.routers.notifications import router as notifications_router
from helix.routers.ops import router as ops_router
from helix.routers.paper import router as paper_router
from helix.routers.status import router as status_router
from helix.routers.strategies import router as strategies_router
from helix.routers.strategy_library import router as strategy_library_router
from helix.routers.system import router as system_router
from helix.routers.tasks import router as tasks_router
from helix.routers.trading import router as trading_router
from helix.routers.websockets import router as websockets_router
from helix.routers.quant_factory import router as quant_factory_router
from helix.routers.routines import router as routines_router
from helix.routers.profile import router as profile_router
from helix.routers.webhooks import router as webhooks_router
from helix.routers.updates import router as updates_router
from helix.routers.backtesting import router as backtesting_router
from helix.routers.lifecycle import router as lifecycle_router
from helix.routers.simulation import router as simulation_router, simulation_api_enabled
from helix.routers.verdict import router as verdict_router
from helix.routers.robustness import router as robustness_router
from helix.routers.gauntlet import router as gauntlet_router
from helix.routers.lab_regime import router as lab_regime_router
from helix.routers.bot_factory import router as bot_factory_router
from helix.routers.brain import router as brain_router
from helix.routers.strategy_guard import router as strategy_guard_router
from helix.routers.skills import router as skills_router
from helix.routers.health import router as health_router
from helix.routers.hypotheses import router as hypotheses_router, data_gap_router
from helix.lab_dormancy import quiesce_regime_lab
from helix.lab_features import regime_lab_enabled
from helix.runtime_worker import (
...
def _spawn_supervised_runtime_thread(
    ...
    *,
    ...
    thread = threading.Thread(target=_runner, name=f"helix-{name}", daemon=True)
    thread.start()
    return thread
...
if sys.platform.startswith("win"):
    ...
    # uvicorn.
    os.environ.setdefault("HELIX_DISABLE_CHROMA_IN_PROCESS", "1")
...
def _configure_runtime_logging() -> None:
    ...
    production launcher (``python -m uvicorn helix.api:app``) the ``__main__``
    ...
    try:
        if home_env:
            ...
            from helix.logging_config import setup_rotating_file_logger

            log_path = _Path(home_env) / "logs" / "api.log"
            setup_rotating_file_logger(
                log_path, level=logging.INFO, fmt=fmt, also_stdout=True
            )
            logging.getLogger("helix.api").info("Runtime logging -> %s", log_path)
...
async def lifespan(_app: FastAPI):
    ...
    # `uvicorn helix.api:app` / Docker run would otherwise only discover a
    ...
    try:
        from helix.preflight import check_dependencies
    ...
    # Packaged installs: seed $HELIX_HOME/data/ohlcv/ from bundled parquets
    ...
    # returns empty. No-op for dev runs (HELIX_HOME unset) and for repeat
    ...
    try:
        from helix.config import ensure_seed_data_bootstrapped
    ...
    try:
        from helix.lifecycle import resume_interrupted_tasks_on_startup
    ...
    # Seed core Helix agents (brain, quant-researcher, simulation-agent,
    ...
    try:
        from helix.model_selection import (
            enable_enforcement,
    ...
    try:
        from helix.workspace import init_workspace
    ...
    try:
        from helix.bot import seed_default_agents
    ...
    try:
        ...
        kv_set("helix:simulation:active", False)
    ...
    try:
        from helix.bot_factory.manager import BotManager
    ...
    try:
        from helix.agents.mcp_client import register_all_enabled_servers
    ...
    try:
        if acquire_runtime_worker_lock():
            ...
            from helix.scheduler import reset_scheduler_job_locks, run_scheduler_loop
            ...
            agent_concurrency = _env_int("HELIX_HEADLESS_AGENT_CONCURRENCY", 3, minimum=1, maximum=8)
            brain_limit = _env_int("HELIX_HEADLESS_BRAIN_LIMIT", 2, minimum=1, maximum=8)
            runtime_thread_mode = _env_bool("HELIX_API_RUNTIME_THREAD_MODE", True)
            runtime_start_delay = _env_float("HELIX_API_RUNTIME_START_DELAY_SECONDS", 5.0, minimum=0.0, maximum=60.0)
            ...
            # (Tauri shell only spawns `python -m helix.api`) still drives the
            # market feed, price ticks, and daemon_state heartbeat. The daemon
            # holds its own file lock so an external `helix daemon start` wins
            # if it's already running.
            try:
                from helix.daemon import run_in_loop as run_daemon_in_loop
                if runtime_thread_mode:
    ...
    try:
        from helix.health_monitor import HealthMonitor, HealthState, set_health_monitor
        ...
        try:
            from helix.db import kv_set
            kv_set("helix:health_monitor:unavailable", True)
        except Exception:
            pass
    else:
        try:
            from helix.db import kv_set
            kv_set("helix:health_monitor:unavailable", False)
        except Exception:
            pass
    ...
    try:
        from helix.daemon import shutdown as _daemon_shutdown_event
    ...
    try:
        from helix.lifecycle import mark_in_flight_tasks_interrupted
...
app.add_middleware(HelixV1CompatMiddleware)
...
# closes that door. HELIX_ALLOWED_HOSTS can extend the list for dev/LAN
# scenarios; packaged builds stick to loopback.
_allowed_hosts_env = os.environ.get("HELIX_ALLOWED_HOSTS", "").strip()
...
from helix.config import is_beta_build as _is_beta_build  # noqa: E402
...
# Serve the prebuilt SvelteKit bundle at "/" when HELIX_FRONTEND_DIR points to
...
_frontend_dir = os.environ.get("HELIX_FRONTEND_DIR")
...
if __name__ == "__main__":
    ...
    # Packaged installs (Tauri sets HELIX_HOME) have no attached console — the
    ...
    # actually read. Dev runs (no HELIX_HOME) keep the existing stdout-only
    # behavior so `helix api` in a terminal stays chatty.
    # Same logging bootstrap the lifespan uses; idempotent via the module guard.
    # Running it here too means a direct `python -m helix.api` launch still
    # configures logging before uvicorn.run() (so bind/startup errors are
    # captured), while `uvicorn helix.api:app` relies on the lifespan call.
    _configure_runtime_logging()

    parser = argparse.ArgumentParser()
    parser.add_argument("--port", type=int, default=None)
    args = parser.parse_args()
    port = args.port if args.port is not None else int(os.environ.get("HELIX_PORT", "8003"))
    ...
    # HELIX_BIND_HOST (or the HELIX_HOST launcher alias) can be set for dev/ops
    ...
    from helix.api_security import resolved_bind_host
