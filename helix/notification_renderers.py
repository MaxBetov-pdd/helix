...
_HELIX_SIGNATURE_RE = re.compile(r"\s+(?:-|[\u2013\u2014])\s*helix\s*$", re.IGNORECASE)
...
def render_discord_message(notification: Mapping[str, Any]) -> str:
    ...
    title = _single_line(notification.get("title")) or "Helix update"
...
def render_discord_thread(notification: Mapping[str, Any]) -> tuple[str, str]:
    ...
    title = _single_line(notification.get("title")) or "Helix digest"
    summary = _single_line(notification.get("summary"))
...
def _normalize_discord_line(raw_line: str) -> str | None:
    ...
    line = re.sub(r"`([^`]+)`", r"\1", line)
    line = _HELIX_SIGNATURE_RE.sub("", line).strip()
    if not line:
        return None
