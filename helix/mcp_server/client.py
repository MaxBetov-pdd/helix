"""Thin auth-aware HTTP client for hitting the running Helix API.

The MCP server is an external process and must cross the wire rather than
reach into helix internals — the user might be running the backend with a
...
class HelixClient:
    """Blocking httpx client bound to a single Helix backend."""
    ...
    def __init__(
        ...
        self.base_url = (base_url or os.environ.get("HELIX_API_URL") or "http://127.0.0.1:8003").rstrip("/")
        ...
        try:
            self.timeout = float(timeout if timeout is not None else os.environ.get("HELIX_MCP_TIMEOUT") or 60.0)
