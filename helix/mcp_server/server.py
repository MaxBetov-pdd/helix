...
"""MCP server that exposes the Helix AI Drop Zone as tools.
...
Tool naming: every tool is prefixed with `helix_` so they do not collide
...
log = logging.getLogger("helix.mcp_server")
...
def build_server(client: HelixClient | None = None) -> FastMCP:
    ...
    """
    helix = client or HelixClient()
    server = FastMCP(
        name="helix",
        instructions=(
            "Helix AI Drop Zone — create, register, and backtest trading "
            "strategies. Start by calling helix_get_context to learn the "
            "system, then (optionally) helix_create_session to group your "
            "work, write strategy .py files to the workspace, register them "
            "with helix_register_strategy_file, and backtest with "
            "helix_run_backtest. Use helix_promote_strategy and "
            "helix_get_gate_report to inspect lifecycle gates without "
            "bypassing them."
        ),
    )
    ...
    def helix_get_context() -> dict[str, Any]:
        return helix.get("/api/ai-dropzone/context")

    @server.tool(
        name="helix_list_sessions",
        description=(
            "List recent AI Drop Zone sessions with strategy counts. Pass "
            "include_closed=false to see only sessions still open."
        ),
    )
    def helix_list_sessions(limit: int = 20, include_closed: bool = True) -> dict[str, Any]:
        return helix.get(
    ...
    @server.tool(
        name="helix_get_session",
        description=(
            "Fetch a session's detail: tagged strategies and recent backtest "
            "runs. Use this to answer 'what did I try in session X?'."
        ),
    )
    def helix_get_session(session_id: str) -> dict[str, Any]:
        return helix.get(f"/api/ai-dropzone/sessions/{session_id}")

    @server.tool(
        name="helix_list_strategies",
        description=(
            "List registered strategies. Filter by status ('active', "
            "'archived', etc.). Use this to see what's in the lab."
        ),
    )
    def helix_list_strategies(status: str | None = None) -> Any:
        return helix.get("/api/strategies", params={"status": status})

    @server.tool(
        name="helix_get_recent_runs",
        description="Return the last N backtest runs across all strategies.",
    )
    def helix_get_recent_runs(limit: int = 20) -> Any:
        return helix.get("/api/backtesting/runs", params={"limit": limit})

    @server.tool(
        name="helix_get_result",
        description=(
            "Fetch a backtest result by result_id — metrics, trades, config. "
            "Use after helix_run_backtest to inspect outcomes."
        ),
    )
    def helix_get_result(result_id: str) -> Any:
        return helix.get(f"/api/results/{result_id}")

    @server.tool(
        name="helix_get_gate_report",
        description=(
    ...
    def helix_get_gate_report(strategy_id: str) -> dict[str, Any]:
        container = helix.get(f"/api/strategies/{strategy_id}/container")
        result_payload: Any = None
        try:
            results = helix.get("/api/results", params={"strategy": strategy_id, "limit": 1})
            rows = results.get("results") if isinstance(results, dict) else results
            if isinstance(rows, list) and rows:
                result_id = rows[0].get("result_id") or rows[0].get("id")
                result_payload = helix.get(f"/api/results/{result_id}") if result_id else rows[0]
        ...
        readiness: Any = None
        try:
            readiness = helix.get(f"/api/lifecycle/strategies/{strategy_id}/readiness")
        except Exception as exc:
    ...
    @server.tool(
        name="helix_close_session",
        description="Mark an AI Drop Zone session closed (idempotent).",
    )
    def helix_close_session(session_id: str) -> dict[str, Any]:
        return helix.post(f"/api/ai-dropzone/sessions/{session_id}/close")

    @server.tool(
        name="helix_register_strategy_file",
        description=(
    ...
    def helix_register_strategy_file(
        file_path: str,
        ...
        return helix.post("/api/strategies/intake/register-file", body)

    @server.tool(
        name="helix_run_backtest",
        description=(
    ...
    def helix_run_backtest(
        ...
        result = helix.post("/api/backtesting/run", body)
        return _compact_backtest_payload(result) if compact else result

    @server.tool(
        name="helix_create_strategy",
        description=(
    ...
    def helix_run_optimization(
        ...
        if end:
            body["end"] = end
        return helix.post("/api/backtesting/optimize", body)

    @server.tool(
        name="helix_run_verdict",
        description=(
    ...
    def helix_promote_strategy(
        ...
        return _with_gate_failures(helix.post(f"/api/strategies/{strategy_id}/promote", body))

    @server.tool(
        name="helix_get_paper_readiness",
        description=(
            "Read current state and latest evidence for paper readiness. This "
            "does not promote; use helix_promote_strategy to attempt paper."
        ),
    )
    def helix_get_paper_readiness(strategy_id: str) -> dict[str, Any]:
        report = helix_get_gate_report(strategy_id)
        report["target_status"] = "paper"
        return report

    @server.tool(
        name="helix_start_paper_session",
        description=(
            "Attempt to promote a gauntlet strategy to paper trading through "
            "the normal lifecycle gate. Defaults to non-forced."
        ),
    )
    def helix_start_paper_session(
        strategy_id: str,
        reason: str = "MCP paper readiness promotion",
        force: bool = False,
    ) -> dict[str, Any]:
        return helix_promote_strategy(
            strategy_id=strategy_id,
    ...
    @server.tool(
        name="helix_run_gauntlet_candidate",
        description=(
    ...
    def helix_run_gauntlet_candidate(
        strategy_id: str,
        dataset_id: str,
        parameters: dict[str, Any] | None = None,
        trade_mode: str | None = None,
        ...
        verdict = None
        if run_verdict:
            verdict = helix_run_verdict(strategy_id, dataset_id, verdict_tests)
        promotion = helix_promote_strategy(
            strategy_id=strategy_id,
