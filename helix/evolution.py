...
from helix.brain import assign_task, transition_stage
...
from helix.policy import evaluate_promotion, score_strategy, compute_live_metrics, check_promotion_readiness

log = logging.getLogger("helix.evolution")
_TESTING_STEP_LOCK = threading.Lock()
_TESTING_STEP_RUNNING_SINCE: float | None = None

# Pipeline Thresholds are dynamically loaded from helix.policy.load_pipeline_config()
...
def _bars_for_validation_timeframe(timeframe: str) -> int:
    ...
    try:
        from helix.api_core import stage_backtest_duration_days
...
def _collect_validation_symbols(primary_symbol: str, params: dict | None = None) -> list[str]:
    ...
    raw_pipeline = kv_get("helix:pipeline:settings", {})
...
def _compute_and_persist_robustness_score(strategy_id: str) -> float:
    ...
    helix.routers.robustness (`_recalculate_robustness_score`) so there is exactly
    ...
    """
    from helix.routers.robustness import _recalculate_robustness_score
...
def _execute_gauntlet_step(
    ...
    if action == "run_timeframe_sweep":
        from helix.api_core import _load_pipeline_settings_payload, post_backtest_submit, BacktestSubmitBody
    ...
    if action == "apply_best_params":
        ...
        from helix.strategies.optimization_acceptance import apply_optimized_params_if_accepted
    ...
    if action == "run_confirmation_backtest":
        from helix.api_core import post_backtest_submit, BacktestSubmitBody
    ...
    if action in ("run_validation_suite", "re_run_validation_suite"):
        ...
        from helix.routers.robustness import (
            CostStressBody,
            MonteCarloBody,
            ParamJitterBody,
            RegimeSplitBody,
...
def _advance_paper_live_readiness(
    ...
    """
    from helix.policy import check_paper_live_readiness
...
def _sweep_pipeline_hygiene() -> dict[str, int]:
    ...
    try:
        ...
        from helix.policy import _evaluate_gauntlet_gate, load_pipeline_config
...
def _check_gauntlet_overflow_alert():
    ...
    from helix.lab_features import GAUNTLET_MAX
    ...
    alert_key = "helix:alert:gauntlet_overflow_start"
