...
Helix runs agents in a `while True` loop that calls paid LLM APIs every round
...
(see helix.cost_pricing.estimate_cost_usd + runner persistence), but until now
...
Configuration (kv key `helix:settings`):
...
from helix.sim.clock import get_today

log = logging.getLogger("helix.billing_guard")

_SETTINGS_KEY = "helix:settings"
