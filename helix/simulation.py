...
from helix.db import get_db, kv_get as kv_get, kv_set
from helix.trade_state import close_trade_record
from helix.market_cache import publish_price_snapshot
from helix.market_data import fetch_hyperliquid_candles
from helix.exchange.risk import update_equity
...
class SimulationRunner:
    ...
    async def run(self):
        ...
        kv_set("helix:simulation:active", True)
        kv_set("helix:simulation:exec_mode", self.config.exec_mode)

        from helix.sim.clock import set_sim_active
        set_sim_active(True)
        ...
        try:
            ...
            from helix.sim.data_pump import prefetch_candles
            ...
            from helix.sim.clock import sim_kv_key
            ...
            for i, bar_time in enumerate(bar_timestamps):
                ...
                from helix.sim.clock import set_sim_time
                ...
                from helix.regime import invalidate_cache
                invalidate_cache()
    ...
    def _finalize(self):
        ...
        kv_set("helix:simulation:exec_mode", None)
        kv_set("helix:simulation:time", None)

        # Clear data pump cache
        try:
            from helix.sim.data_pump import clear_cache
            clear_cache()
        except ImportError:
            pass
