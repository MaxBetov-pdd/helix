"""Token storage, auto-refresh, and migration for Helix auth."""
...
from helix.config import AUTH_FILE, OPENCLAW_AUTH, ensure_dirs
from helix.secret_storage import decrypt_secret, encrypt_secret

console = Console()
log = logging.getLogger("helix.auth.store")
...
def _refresh_openai(profile: dict) -> dict:
    ...
    from helix.auth.openai import CLIENT_ID, TOKEN_URL
...
def _refresh_minimax(profile: dict) -> dict:
    ...
    from helix.auth.minimax import CLIENT_ID, TOKEN_URL
...
def get_token(provider: str) -> str:
    ...
    if _is_expired(profile):
        ...
        if refresher and profile.get("refresh"):
            ...
            raise RuntimeError(f"{provider} token expired and no refresh available. Run: helix auth login {provider}")
    ...
    def __init__(self, provider: str, status: str = "missing"):
        ...
        if status == "opaque":
            ...
            msg = (
                ...
                f"(or run: helix auth login {self.provider})."
            )
        else:  # missing / unknown
            msg = (
                f"{self.provider} has no API credentials configured. Add them in "
                f"Settings > Agents > AI providers (or run: helix auth login {self.provider})."
            )
        super().__init__(msg)
...
def display_status():
    ...
    table = Table(title="Helix Auth Status")
...
def migrate_from_openclaw():
    ...
    # Map OpenClaw provider names to Helix names
    provider_map = {
    ...
    for oc_key, oc_profile in oc_profiles.items():
        ...
        helix_provider = provider_map.get(oc_provider)
        if not helix_provider:
            console.print(f"  [dim]Skipping unknown provider: {oc_provider}[/dim]")
            continue

        helix_key = f"{helix_provider}:default"

        # Build normalized profile
        profile = {"type": oc_profile.get("type", "oauth"), "provider": helix_provider}
        ...
        store["profiles"][helix_key] = profile
        migrated += 1
        console.print(f"  [green]Migrated {helix_provider}[/green]")
...
def run_login(provider: str):
    ...
    flows = {
        "openai": "helix.auth.openai",
        "minimax": "helix.auth.minimax",
    }
    import importlib
