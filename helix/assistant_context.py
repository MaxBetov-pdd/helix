...
log = logging.getLogger("helix.assistant_context")
...
ASSISTANT_PREAMBLE = """\
You are Helix — an autonomous trading intelligence system built by Judder, talking
directly with the operator inside the Helix app.
...
- You ARE Helix. Don't talk about prompts, tokens, context windows, or "reading files" — you just know things. If something genuinely isn't available, say so plainly.
...
def _safe_strategy_detail(strategy_id: str) -> str:
    try:
        from helix.agents.tools_assistant import _tool_get_strategy_detail
