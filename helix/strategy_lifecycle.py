...
log = logging.getLogger("helix.strategy_lifecycle")
...
from helix import phantom_recovery
from helix.strategies.certification import certify_execution_strategy
from helix.strategies.params import canonicalize_params
from helix.util import normalize_stage, sanitize_json_floats
...
class StrategyPromoteBody(BaseModel):
    ...
    # (HELIX_ALLOW_MAINNET) is separate and unaffected.
...
def create_lifecycle_strategy(body: LifecycleCreateBody):
    ...
    from helix.api_core import _resolve_backtesting_strategy_type
    ...
    if certification.unregistered_runtime_type:
        return {
            ...
            "error": (
                ...
                "helix/strategies/custom/ before creating strategies of this type."
            ),
        }
    ...
    if target_stage == "quick_screen":
        try:
            ...
            from helix.gauntlet.store import create_or_get_workflow
            ...
            logging.getLogger("helix.strategy_lifecycle").exception(
...
def _resolve_container_source_code(strategy_type: str, source_ref: str | None) -> dict | None:
    ...
    A code-class strategy's logic lives in a file under ``helix/strategies/custom/``
    ...
    if not type_name:
        return None
    try:
        from helix.strategies import custom, registry
...
def import_strategy_container(payload: object) -> dict:
    ...
    meta = payload.get("helix_export")
    if not isinstance(meta, dict):
        raise HTTPException(
            status_code=400,
            detail="missing helix_export metadata — this is not a Helix strategy export",
        )
...
def transition_lifecycle_strategy(body: LifecycleTransitionBody):
    ...
    try:
        from helix.brain import _USER_ACTORS
