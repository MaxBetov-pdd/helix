...
log = logging.getLogger("helix.security")
...
def _auth_required() -> bool:
    return _env_truthy("HELIX_AUTH_REQUIRED")
...
def resolved_bind_host() -> str:
    ...
    ``HELIX_HOST`` as a ``--host`` fallback, but the fail-closed guard used to
    read only ``HELIX_BIND_HOST`` — so ``HELIX_HOST=0.0.0.0`` with no API key
    ...
    actual bind.
    """
    for name in ("HELIX_BIND_HOST", "HELIX_HOST"):
...
def assert_safe_bind_host(bind_host: str) -> None:
    ...
    we fail closed there: an exposed bind must come with HELIX_API_KEY (or an
    explicit HELIX_AUTH_REQUIRED=true).
    """
    if is_loopback_host(bind_host):
        return
    if _read_env_secret("HELIX_API_KEY") or _auth_required():
        return
    raise RuntimeError(
        f"Refusing to start: the API is bound to {bind_host!r} (reachable beyond "
        "localhost) but HELIX_API_KEY is not set. Set HELIX_API_KEY (and "
        "HELIX_OPERATOR_KEY) in your .env, or bind to 127.0.0.1."
    )


def assert_auth_keys_configured() -> None:
    """Call at app startup. Refuse to start if auth is required but unset."""
    api_key = _read_env_secret("HELIX_API_KEY")
    operator_key = _read_env_secret("HELIX_OPERATOR_KEY")
    required = _auth_required()
    if required and not api_key:
        raise RuntimeError(
            "HELIX_AUTH_REQUIRED=true but HELIX_API_KEY is unset. "
            "Set HELIX_API_KEY in .env or unset HELIX_AUTH_REQUIRED for dev."
        )
    if required and not operator_key:
        raise RuntimeError(
            "HELIX_AUTH_REQUIRED=true but HELIX_OPERATOR_KEY is unset. "
            "Set HELIX_OPERATOR_KEY in .env or unset HELIX_AUTH_REQUIRED for dev."
        )
    if not api_key:
        log.warning(
            "HELIX_API_KEY is unset — API is unauthenticated. "
            "Set HELIX_AUTH_REQUIRED=true to refuse start without keys."
        )
    if not operator_key:
        log.warning(
            "HELIX_OPERATOR_KEY is unset — operator endpoints are unauthenticated. "
            "Set HELIX_AUTH_REQUIRED=true to refuse start without keys."
        )
    # Fail closed if the API is exposed beyond loopback without a key. Use the
    # launcher-aware resolver (HELIX_BIND_HOST → HELIX_HOST → 127.0.0.1) so the
    # HELIX_HOST alias cannot smuggle a public bind past the guard (M6).
    assert_safe_bind_host(resolved_bind_host())


def require_api_access(request: HTTPConnection) -> None:
    expected_api_key = _read_env_secret("HELIX_API_KEY")
...
def require_operator_access(request: HTTPConnection) -> None:
    ...
    expected_operator_key = _read_env_secret("HELIX_OPERATOR_KEY")
...
async def require_api_access_ws(ws: HTTPConnection) -> bool:
    ...
    when no HELIX_API_KEY is set (the default localhost setup), but when a key
    ...
    """
    expected = _read_env_secret("HELIX_API_KEY")
    if not expected:
        return True
...
def get_allowed_cors_origins() -> list[str]:
    configured = _normalize_secret(os.environ.get("HELIX_CORS_ORIGINS"))
    if configured:
        values = configured.split(",")
    else:
        frontend_port = _normalize_secret(os.environ.get("FRONTEND_PORT")) or "5173"
        api_port = _normalize_secret(os.environ.get("HELIX_PORT")) or "8003"
        values = [
            f"http://127.0.0.1:{frontend_port}",
            f"http://localhost:{frontend_port}",
            f"http://127.0.0.1:{api_port}",
            f"http://localhost:{api_port}",
            _normalize_secret(os.environ.get("HELIX_CLIENT_BASE")),
        ]
