...
from helix.config import HELIX_DB, HELIX_HOME, WORKSPACE_DIR
...
log = logging.getLogger("helix.data")


def _resolve_data_dir() -> Path:
    """Pick where OHLCV parquets live.

    Packaged installs set `HELIX_HOME` env var (Tauri's `backend.rs` points it
    at `%LOCALAPPDATA%\\Helix\\`). In that case we use
    `$HELIX_HOME/data/ohlcv/` so data persists across app updates and stays
    ...
    explicit `HELIX_DATA_DIR` env var overrides both, for tests or custom
    setups.
    """
    override = os.environ.get("HELIX_DATA_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    if os.environ.get("HELIX_HOME"):
        return HELIX_HOME / "data" / "ohlcv"
...
def data_root() -> Path:
    ...
    honor HELIX_HOME in packaged installs. This fixes the prior split-brain where
    OHLCV honored HELIX_HOME but funding/OI/derivatives/macro hardcoded a
    ...
    if os.environ.get("HELIX_HOME"):
        return HELIX_HOME / "data"
    return Path(__file__).resolve().parent.parent / "data"
CHUNK_LIMIT = 1000
CATALOG_CACHE_TTL_SECONDS = 30
MARKET_CACHE_TTL_SECONDS = 3600
THREAD_POOL = ThreadPoolExecutor(max_workers=2, thread_name_prefix="helix-data")
...
def load_parquet(symbol: str, timeframe: str, *, as_of: object | None = None) -> pd.DataFrame | None:
    ...
    if as_of is not None:
        try:
            from helix.dataeng.revisions import reconstruct_as_of
...
def save_parquet(df: pd.DataFrame, symbol: str, timeframe: str, source: str = "ccxt") -> None:
    ...
    if _using_pyarrow():
        ...
        meta.update(
            {
                b"helix_source": str(source).encode("utf-8"),
                b"helix_symbol": symbol_to_fs(symbol).encode("utf-8"),
                b"helix_timeframe": str(timeframe).encode("utf-8"),
                b"helix_updated_at": _now_iso().encode("utf-8"),
            }
        )
        table = table.replace_schema_metadata(meta)
        pq.write_table(table, tmp_path, compression="zstd")
    else:
        out.attrs["helix_source"] = str(source)
        out.attrs["helix_symbol"] = symbol_to_fs(symbol)
        out.attrs["helix_timeframe"] = str(timeframe)
        out.attrs["helix_updated_at"] = _now_iso()
...
def _capture_ohlcv_revisions(symbol: str, timeframe: str, new_frame: pd.DataFrame) -> None:
    ...
    try:
        from helix.dataeng.revisions import capture_restatements
...
def get_dataset_source(symbol: str, timeframe: str) -> str | None:
    ...
    ``helix_source`` metadata; None if missing/unknown.
    ...
    try:
        ...
        if _using_pyarrow():
            ...
            raw = keyvals.get(b"helix_source")
            return raw.decode("utf-8", errors="ignore") if raw else None
        frame = pd.read_pickle(path)
        src = frame.attrs.get("helix_source")
...
def _dataset_from_file(path: Path, symbol: str, timeframe: str) -> dict[str, Any]:
    if _using_pyarrow():
        ...
        source = keyvals.get(b"helix_source", b"ccxt")
        ...
        frame = _normalize_ohlcv_frame(pd.read_pickle(path))
        rows = len(frame)
        source_name = str(frame.attrs.get("helix_source") or "ccxt")
        if rows == 0:
            start = None
            end = None
        else:
...
def _fetch_ohlcv_polygon(
    ...
    from helix.polygon_client import PolygonClient, PolygonError
    from helix.symbol_mapping import to_fs as sym_to_fs
