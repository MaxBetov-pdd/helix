...
from helix.model_routing import get_default_model_for_provider
from helix.db import create_pending_task, get_db, init_db, is_user_active, kv_get, kv_set, kv_set_best_effort, log_activity, reap_long_running_agent_tasks, recover_stale_running_tasks
from helix.system_pause import is_autonomy_paused, is_generation_paused
from helix.task_timeouts import coerce_stale_recovery_minutes, recommended_agent_reaper_timeout_minutes, recommended_stale_recovery_minutes
...
_DEFAULT_JOB_IDS = {
    "helix-crucible-planner",
    "helix-crucible-discovery",
    "helix-ideation-daily",
    "helix-testing-cycle",
    "helix-paper-graduation",
    "helix-risk-audit",
    "helix-decay-tracker",
    "helix-weekly-review",
    "helix-regime-update",
    "helix-slippage-monitor",
    "helix-scanner-signal",
    "helix-scanner-hourly",
    "helix-recalibration",
    "helix-daily-learning",
    "helix-orphan-type-scan",
    "helix-decay-kill-switch",
    "helix-reconcile-sweep",
    "helix-source-reconciliation",
    "helix-market-data-collect",
    "helix-funding-history-reconcile",
    "helix-data-ohlcv-keepalive",
    "helix-data-oi-collect",
    "helix-data-funding-collect",
    "helix-data-engine-catchup",
    "helix-data-bv-backfill",
    "helix-overnight-summary",
    "helix-data-lsr-collect",
    "helix-data-taker-collect",
    "helix-data-liquidation-collect",
    "helix-data-fng-collect",
    "helix-data-macro-collect",
    "helix-data-btcdom-collect",
    "helix-quant-skills-consolidation",
    "helix-stale-triage",
    "helix-auto-intake",
    "helix-wal-checkpoint",
    "helix-db-maintenance",
    "helix-capital-slot-dedupe",
    "helix-hypothesis-verdict-loop",
    "helix-hypothesis-promotion-loop",
    "helix-hypothesis-revisit-pass",
    "helix-hypothesis-unstarted-ageout",
    "helix-gauntlet-step-loop",
    "helix-phantom-sweep",
    "helix-param-optimization",
}
_SUPERSEDED_CRUCIBLE_AGENT_JOB_IDS = {
    "helix-ideation-daily",
}
_LEGACY_DEFAULT_JOB_PREFIXES = ("helix-",)
...
_DATA_MANAGER_JOB_PAYLOAD_DEFAULTS: dict[str, dict[str, object]] = {
    ...
    "helix-data-oi-collect": {"kind": "data_manager_collect_oi", "timeout_seconds": 180},
    "helix-data-funding-collect": {"kind": "data_manager_collect_funding", "timeout_seconds": 180},
    "helix-data-lsr-collect": {"kind": "data_manager_collect_lsr", "timeout_seconds": 120},
    "helix-data-taker-collect": {"kind": "data_manager_collect_taker", "timeout_seconds": 120},
    "helix-data-liquidation-collect": {"kind": "data_manager_collect_liquidation", "timeout_seconds": 120},
    "helix-data-fng-collect": {"kind": "data_manager_collect_fng", "timeout_seconds": 120},
    "helix-data-macro-collect": {"kind": "data_manager_collect_macro", "timeout_seconds": 180},
    "helix-data-btcdom-collect": {"kind": "data_manager_collect_btcdom", "timeout_seconds": 120},
}
...
# (helix-scanner-signal / helix-scanner-hourly) are deliberately exempt:
...
def _job_has_live_background_task(job_id: str) -> bool:
    ...
    """
    name = f"helix-scheduler-job:{job_id}"
...
_CURRENT_SCHEDULER_JOB_ID: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "helix_current_scheduler_job_id", default=None
)
...
def _load_runtime_task_timeout_settings() -> dict[str, int]:
    raw_settings = kv_get("helix:settings", {})
...
def _defer_tracker_key(job_id: str) -> str:
    return f"helix:scheduler:deferring:{job_id}"
...
def _apply_runtime_scheduler_overrides() -> int:
    ...
    overrides = {
        ...
        "helix-ideation-daily": ("interval", _runtime_interval_expr(int(tuning["ideation_interval_minutes"]))),
        "helix-testing-cycle": ("interval", _runtime_interval_expr(int(tuning["testing_interval_minutes"]))),
        "helix-paper-graduation": ("interval", _runtime_interval_expr(int(tuning["graduation_interval_minutes"]))),
        "helix-scanner-signal": ("interval", _runtime_interval_expr(int(tuning["scanner_signal_interval_minutes"]))),
        "helix-scanner-hourly": ("interval", _runtime_interval_expr(int(tuning["scanner_execution_interval_minutes"]))),
    }
    ...
    with get_db() as conn:
        for job_id, (schedule_type, schedule_expr) in overrides.items():
            ...
            payload_update = None
            if job_id == "helix-crucible-planner":
...
def migrate_legacy_scanner_cadence() -> bool:
    ...
    with get_db() as conn:
        ...
        conn.execute(
            ...
            ("Live Scanner Execution Worker", "300000", next_run, "helix-scanner-hourly"),
        )
    log.info("Migrated legacy scanner cadence from 60m to 5m (helix-scanner-hourly)")
    return True
...
def migrate_data_engine_catchup_cadence() -> bool:
    ...
    The fresh-seed default is now 1800000 (30m) in seed_helix_jobs, but that path
    ...
    with get_db() as conn:
        ...
        conn.execute(
            ...
            ("1800000", next_run, "helix-data-engine-catchup"),
        )
    log.info("Migrated Data Engine catch-up cadence from 10m to 30m (helix-data-engine-catchup)")
    return True
...
def apply_startup_catchup(*, now: datetime | None = None) -> dict[str, int]:
    ...
    Helix's lifecycle is "open the app → scheduler runs; close the app →
    ...
    try:
        from helix.dataeng.catchup import CatchUpPlanner
        from helix.dataeng.settings import load_data_engine_settings
...
# still pass a tighter timeout. Override via HELIX_DEFAULT_SYNC_JOB_TIMEOUT_SECONDS.
try:
    _DEFAULT_SYNC_JOB_TIMEOUT_SECONDS = float(
        os.environ.get("HELIX_DEFAULT_SYNC_JOB_TIMEOUT_SECONDS", "600") or 600.0
    )
except (TypeError, ValueError):
...
async def _run_sync_job(fn, *args, timeout_seconds: float | None = None, **kwargs):
    ...
    executor = concurrent.futures.ThreadPoolExecutor(
        max_workers=1,
        thread_name_prefix="helix-scheduler-job",
    )
...
def _run_regime_update_job() -> int:
    from helix.regime import TREND_DOWN, TREND_UP, RANGE_BOUND, HIGH_VOL, detect_regime
...
async def run_job(job: dict) -> tuple[str, str | None]:
    ...
    try:
        ...
        if kind == "brain_routine":
            ...
            try:
                from helix.control_plane import routines as control_plane_routines
        ...
        if kind == "scanner_run":
            from helix.scanner import run_scan
            timeout_seconds = _coerce_timeout_seconds(
        ...
        if kind == "scanner_signal_run":
            from helix.scanner import run_signal_scan
            timeout_seconds = _coerce_timeout_seconds(
                payload.get("timeout_seconds"),
                _SCANNER_JOB_TIMEOUT_SECONDS,
            )
        ...
        if kind == "fitness_eval":
            from helix.brain import run_strategy_review
            ...
            if actions:
                ...
                try:
                    from helix.bot import send_sync
                    await _run_sync_job(
                        send_sync,
                        "research",
        ...
        if kind == "daily_learning":
            from helix.jobs.daily_learning import run_daily_learning
            await run_daily_learning()
            return "ok", None

        # Crucible planner - unified broad autonomous work router
        if kind == "crucible_planner":
            from helix.crucible_planner import run_crucible_planner_cycle
        ...
        if kind == "evolution_graduation":
            from helix.evolution import check_paper_graduation
            await _run_sync_job(check_paper_graduation)
            return "ok", None

        if kind == "evolution_review":
            from helix.evolution import run_weekly_review
            await _run_sync_job(run_weekly_review)
            return "ok", None

        if kind == "quant_skills_consolidation":
            from helix.quant_skills import run_consolidation
        ...
        if kind == "param_optimization":
            from helix.strategies.optimizer import optimize_all_deployed
            await _run_sync_job(optimize_all_deployed)
            return "ok", None

        # Risk audit — assign audit task to risk-manager
        if kind == "risk_audit":
            from helix.brain import assign_risk_audit
        ...
        if kind == "decay_tracker":
            ...
            try:
                from helix.monitoring import compute_paper_live_drift
                await _run_sync_job(compute_paper_live_drift)
            except Exception as exc:
                log.warning("decay_tracker: drift snapshot refresh failed: %s", exc)
            return "ok", None

        # Stale quick-screen triage — daily janitor (mirrors `helix strategies triage-stale`)
        if kind == "stale_triage":
            days = int(payload.get("days", 7))
            from helix.brain import transition_stage
        ...
        if kind == "auto_intake":
            from helix.strategies.intake import auto_intake_recent_files
        ...
        if kind == "wal_checkpoint":
            from helix.db import checkpoint_wal
        ...
        if kind == "db_maintenance":
            from helix.maintenance import run_db_maintenance
            raw_settings = kv_get("helix:pipeline:settings", {}) or {}
            settings = raw_settings if isinstance(raw_settings, dict) else {}
        ...
        if kind == "capital_slot_dedupe":
            from helix.policy import dedupe_capital_slots
            result = await _run_sync_job(
        ...
        if kind == "orphan_type_scan":
            from helix.strategies.params import is_known_runtime_type
            from helix.brain import transition_stage
        ...
        if kind == "slippage_monitor":
            from helix.monitoring import run_slippage_monitor
            await _run_sync_job(
        ...
        if kind == "recalibrate":
            try:
                from helix.recalibrator import check_and_recalibrate
        ...
        if kind == "ghost_container_scan":
            from helix.db import run_daily_ghost_detection
        ...
        if kind == "decay_kill_switch":
            from helix.monitoring import run_decay_kill_switch
        ...
        if kind == "reconcile_sweep":
            from helix.scanner import sweep_pending_close_reconcile
        ...
        # that feeds the cache-only promotion gate (helix.policy). Never runs inside
        # a promotion write txn, so it is free to fetch the live venue here.
        if kind == "source_reconciliation":
            from helix.source_reconciliation import run_source_reconciliation_job
            result = await _run_sync_job(
        ...
        if kind == "market_data_collect":
            from helix.market_data_collector import collect_current_snapshot
        ...
        if kind == "funding_history_reconcile":
            from helix.market_data_collector import DEFAULT_FUNDING_TARGET_DAYS, reconcile_funding_history
        ...
        if kind == "data_manager_collect_ohlcv":
            from helix.data_manager import data_manager
            result = await _run_sync_job(
        ...
        if kind == "data_engine_catchup":
            ...
            from helix.api_domains.data import execute_data_engine_catchup
        ...
        if kind == "hypothesis_verdict_loop":
            from helix.hypothesis_verdict import run_verdict_loop
        ...
        if kind == "gauntlet_step_loop":
            from helix.gauntlet.engine import tick_active_gauntlet_workflows
        ...
        if kind == "phantom_sweep":
            from helix.phantom_recovery import run_phantom_recovery_sweep
        ...
        if kind == "hypothesis_promotion_loop":
            from helix.hypothesis_promotion import run_promotion_loop
        ...
        if kind == "hypothesis_revisit_pass":
            from helix.hypothesis_revisit import run_revisit_pass
        ...
        if kind == "hypothesis_unstarted_ageout":
            from helix.hypothesis_cleanup import run_unstarted_ageout_pass
        ...
        if kind == "overnight_summary":
            from helix.jobs.overnight_summary import run_overnight_summary_job
...
def _brain_queue_soft_cap() -> int:
    ...
    try:
        settings = kv_get("helix:settings")
...
async def tick():
    ...
    try:
        from helix.gauntlet.engine import recover_stale_running_steps
    ...
    try:
        from helix.control_plane.routines import DIRTY_FLAG_KEY as _ROUTINES_DIRTY_KEY
    ...
    try:
        from helix.control_plane.approvals import expire_overdue_approvals
...
def _log_resolved_config_snapshot():
    ...
    try:
        settings = kv_get("helix:settings", {})
        pipeline_thresholds = kv_get("helix:pipeline_thresholds", {})
        pipeline_settings = kv_get("helix:pipeline:settings", {})
        runtime_task_timeouts = _load_runtime_task_timeout_settings()

        snapshot = {
            "helix_settings": settings if isinstance(settings, dict) else {},
    ...
    try:
        ...
        with get_db() as conn:
            scanner_job = conn.execute(
                "SELECT schedule_type, schedule_expr FROM scheduler_jobs WHERE id = 'helix-scanner-hourly'"
            ).fetchone()
...
async def run_scheduler_loop(interval_seconds: int = 30):
    ...
    # App-open catch-up: collapse missed cycles into a single run. Helix
    ...
    while True:
        try:
            ...
            if consecutive_errors >= _SCHEDULER_CIRCUIT_BREAKER_TERMINATE:
                from helix.config import is_beta_build
                ...
                try:
                    from helix.notifications import emit_notification
                    emit_notification(
...
def seed_helix_jobs():
    """Create the default Helix 2.0 Continuous Learning scheduler jobs."""
    init_db()
    ...
    add_job(
        job_id="helix-crucible-discovery",
    ...
    add_job(
        job_id="helix-quant-skills-consolidation",
    ...
    add_job(
        job_id="helix-param-optimization",
    ...
    add_job(
        job_id="helix-capital-slot-dedupe",
    ...
    add_job(
        job_id="helix-decay-kill-switch",
    ...
    add_job(
        job_id="helix-source-reconciliation",
    ...
    add_job(
        job_id="helix-market-data-collect",
    ...
    add_job(
        job_id="helix-funding-history-reconcile",
    ...
    add_job(
        job_id="helix-data-ohlcv-keepalive",
    ...
    add_job(
        job_id="helix-data-funding-collect",
    ...
    add_job(
        job_id="helix-data-engine-catchup",
    ...
    add_job(
        job_id="helix-overnight-summary",
    ...
    add_job(
        job_id="helix-data-taker-collect",
    ...
    add_job(
        job_id="helix-data-liquidation-collect",
    ...
    add_job(
        job_id="helix-data-macro-collect",
    ...
    add_job(
        job_id="helix-data-btcdom-collect",
    ...
    add_job(
        job_id="helix-hypothesis-verdict-loop",
    ...
    add_job(
        job_id="helix-hypothesis-promotion-loop",
    ...
    with get_db() as conn:
        conn.execute(
            ...
            UPDATE scheduler_jobs
            SET enabled = 1
            WHERE id = 'helix-hypothesis-promotion-loop'
            """,
        )
    ...
    add_job(
        job_id="helix-hypothesis-revisit-pass",
    ...
    add_job(
        job_id="helix-hypothesis-unstarted-ageout",
    ...
    add_job(
        job_id="helix-gauntlet-step-loop",
    ...
    log.info("Seeded Helix Continuous Learning jobs")


def reconcile_helix_jobs() -> dict[str, int]:
    ...
    the Helix rename. Leaving them enabled makes the same workflow run twice.
...
def ensure_monitoring_jobs() -> int:
    ...
    if "helix-decay-tracker" not in existing_ids:
        add_job(
            job_id="helix-decay-tracker",
            name="Strategy Decay Tracker",
            schedule_type="interval",
    ...
    if "helix-slippage-monitor" not in existing_ids:
        add_job(
            job_id="helix-slippage-monitor",
            name="Execution Slippage Monitor",
            schedule_type="interval",
        ...
        added += 1
    if "helix-recalibration" not in existing_ids:
        add_job(
            job_id="helix-recalibration",
            name="Adaptive Regime Recalibration",
            schedule_type="interval",
        ...
        added += 1
    if "helix-scanner-signal" not in existing_ids:
        tuning = _load_runtime_scheduler_tuning()
        add_job(
            job_id="helix-scanner-signal",
            name="Live Scanner Signal Worker",
...
def migrate_from_openclaw():
    """Import scheduler jobs from OpenClaw — but replace with Helix equivalents."""
    ...
    console.print(f"\n[bold green]Created {len(jobs)} Helix scheduler jobs[/bold green]")
