...
The helix DB grows unbounded — backtest_results (~133K rows / ~940MB),
...
log = logging.getLogger("helix.maintenance")
...
def _resolve_retention(settings: dict | None = None) -> dict[str, int]:
    ...
    if settings is None:
        raw = kv_get("helix:pipeline:settings", {}) or {}
...
def vacuum_db() -> bool:
    ...
    # (which rebind helix.config.HELIX_DB) cover this too — a by-value import
    # captured at module load would VACUUM the live production DB from a test.
    import helix.config as _cfg

    conn = sqlite3.connect(str(_cfg.HELIX_DB), timeout=120, isolation_level=None)
    try:
        conn.execute("VACUUM")
    finally:
        conn.close()
