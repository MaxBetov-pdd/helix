"""Helix Trading Daemon — data ingestion and hard-risk loop.
...
from helix.config import HELIX_HOME, ensure_dirs, get_execution_mode
from helix.db import get_db, get_open_trades, init_db, kv_get, kv_set_best_effort, log_activity
from helix.exchange.hyperliquid import (
    HyperLiquidFeed,
    _get_creds,
    get_account_value,
    get_all_mids,
...
from helix.market_cache import normalize_prices, publish_price_snapshot, publish_candle_snapshot
from helix.market_data import fetch_hyperliquid_candles, dataframe_to_ohlcv_rows
from helix.runtime_health import compute_runtime_code_fingerprint
from helix.system_mode_policy import autonomous_runtime_allowed

log = logging.getLogger("helix.daemon")
...
def _get_testnet() -> bool:
    ...
    try:
        settings = kv_get("helix:settings", {}) or {}
    except Exception:
        settings = {}
...
PRICE_SNAPSHOT_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_PRICE_SNAPSHOT_TIMEOUT_SECONDS", 6)
RISK_ACCOUNT_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_ACCOUNT_TIMEOUT_SECONDS", 10)
RISK_UPDATE_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_RISK_UPDATE_TIMEOUT_SECONDS", 8)
RISK_RECONCILE_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_RECONCILE_TIMEOUT_SECONDS", 20)
...
_RECONCILE_ERROR_ESCALATE_AFTER = int(_env_float("HELIX_DAEMON_RECONCILE_ERROR_ESCALATE_AFTER", 3))
# After a transient error, retry on this short cadence instead of the full 600s.
_RECONCILE_ERROR_RETRY_SECONDS = _env_float("HELIX_DAEMON_RECONCILE_ERROR_RETRY_SECONDS", 45)
...
_RECONCILE_OUTAGE_ESCALATE_SECONDS = _env_float(
    "HELIX_DAEMON_RECONCILE_OUTAGE_ESCALATE_SECONDS", 900
)
...
RISK_CLOSE_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_CLOSE_TIMEOUT_SECONDS", 20)
HEARTBEAT_SEND_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_HEARTBEAT_TIMEOUT_SECONDS", 20)
FALLBACK_MIDS_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_MIDS_TIMEOUT_SECONDS", 8)
CANDLE_FETCH_TIMEOUT_SECONDS = _env_float("HELIX_DAEMON_CANDLE_TIMEOUT_SECONDS", 12)
SHUTDOWN_GRACE_SECONDS = _env_float("HELIX_DAEMON_SHUTDOWN_GRACE_SECONDS", 5)
...
def _sweep_orphan_multiprocessing_workers() -> int:
    ...
    Why: on Windows, taskkill /F or an unclean crash of a helix process does not terminate
...
def get_daemon_lock_status() -> dict[str, object]:
    ...
    lock_path = HELIX_HOME / "daemon.lock"
...
def send_heartbeat(state: dict, prices: dict):
    ...
    sent_msg = ""
    try:
        from helix.strategies.sentiment import analyze_sentiment
    ...
    msg = (
        f"HELIX HEARTBEAT — {datetime.now(timezone.utc).strftime('%H:%M UTC')}\n"
...
def _check_liquidation_distances() -> None:
    ...
    try:
        ...
        from helix.exchange import books
        from helix.exchange.hyperliquid import get_all_mids, get_positions
        from helix.notifications import emit_notification

        s = kv_get("helix:settings", {}) or {}
        try:
...
def _daemon_startup_bookkeeping(install_signal_handlers: bool) -> dict | None:
    ...
    in-process variant used by `helix.api`. Returns the initial state dict
    ...
    log.info("=" * 50)
    log.info("HELIX DAEMON v2.0 — DATA/RISK LOOP STARTING")
    log.info("=" * 50)
...
class DaemonLoopDeclined:
    ...
    is already held by another instance (e.g. a standalone ``helix daemon
...
async def run_in_loop() -> "DaemonLoopDeclined | None":
    ...
    `helix daemon start` process. The file lock still prevents duplicate
