...
from helix.agents.tool_registry import execute_tool
from helix.assistant_db import (
    append_message,
    get_thread,
    list_messages,
    thread_cost_total,
...
from helix.deepdive_session import (
...
log = logging.getLogger("helix.assistant")
...
def _build_assistant_tools(allow_actions: bool) -> list[dict]:
    ...
    from helix.agents.tool_definitions import (
    ...
    _ensure_tools_imported()
    from helix.agents.tool_registry import _REGISTRY
...
def _is_confirm_tool(name: str) -> bool:
    from helix.agents.tool_definitions import CHAT_CONFIRM_TOOL_NAMES
...
async def _invoke_llm_stream(llm_messages: list[dict], system: str, tools: list[dict]):
    ...
    """
    from helix.agents.providers import get_provider
...
async def run_turn(
    ...
    *,
    ...
    from helix.assistant_context import build_assistant_context
...
async def confirm_action(thread_id: str, action_id: str, *, approve: bool) -> dict:
    ...
    """
    from helix.assistant_db import get_message, set_message_status
