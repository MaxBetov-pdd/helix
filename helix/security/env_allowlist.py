...
Hermes-inspired Phase 0: when Helix shells out via run_shell / run_code /
...
- **Allow** a fixed set of operationally-required vars by name (PATH,
  HOME, USER, LANG, locale + temp + Python runtime vars, HELIX_HOME).
...
log = logging.getLogger("helix.security.env_allowlist")
