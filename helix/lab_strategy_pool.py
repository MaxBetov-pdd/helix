...
from helix.strategies.backtest import expand_strategy_trade_modes, validate_backtest_risk_controls
from helix.strategies.base import BaseStrategy
from helix.strategies.registry import build_strategy_from_row
from helix.util import normalize_stage

log = logging.getLogger("helix.lab_strategy_pool")
...
def get_main_db_readonly() -> sqlite3.Connection:
    ...
    db_path = Path(cfg.HELIX_DB).expanduser().resolve()
...
def inspect_strategy_pool(
    *,
    ...
    if not Path(cfg.HELIX_DB).exists():
        return {
...
def _persist_quarantine_events(skipped: list[dict[str, Any]]) -> None:
    ...
    """
    try:
        from helix.lab_db import create_selection_event, existing_quarantine_event_keys
