...
from helix.db import create_task_container, get_db, kv_get, kv_set, log_activity
from helix.notification_policy import merge_notification_preferences, resolve_notification_policy
from helix.notification_renderers import render_discord_message, render_discord_thread

log = logging.getLogger("helix.notifications")

_NOTIFICATION_PREFS_KEY = "helix:notification_preferences"
...
COALESCE(
    ...
    LOWER(
        ...
        COALESCE(NULLIF(TRIM(COALESCE({column_prefix}title, '')), ''), 'helix update')
    )
)
"""
...
def update_notification_preferences(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    bot_module = sys.modules.get("helix.bot")
...
def emit_notification(
    ...
    *,
    ...
    event = {
        ...
        "title": str(title or "Helix update").strip() or "Helix update",
...
def create_notification_repair_task(
    ...
    *,
    ...
    payload = {
        ...
        "title": str(notification.get("title") or "Helix update"),
...
def _notification_group_key(notification: Mapping[str, Any]) -> str:
    ...
    title = str(notification.get("title") or "Helix update").strip().lower() or "helix update"
...
def _render_thread_fallback_message(title: str, message: str) -> str:
    normalized_title = str(title or "Helix update").strip() or "Helix update"
...
def _deliver_thread_notification(
    ...
    *,
    ...
    from helix.bot import send_sync, send_thread_sync
...
def _build_repair_task_title(notification: Mapping[str, Any]) -> str:
    return f"Repair notification #{int(notification['id'])}: {str(notification.get('title') or 'Helix update').strip() or 'Helix update'}"
...
def _build_repair_task_description(notification: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Title: {str(notification.get('title') or 'Helix update')}",
    ]
