...
log = logging.getLogger("helix.lab_features")
...
_UNSET = object()
_PIPELINE_SETTINGS_KEY = "helix:pipeline:settings"
...
def is_pipeline_saturated() -> tuple[bool, int, str]:
    ...
    try:
        ...
        # fall back to helix:settings (set via the frontend settings page),
...
def regime_lab_enabled() -> bool:
    ...
    raw = str(os.getenv("HELIX_ENABLE_REGIME_LAB", "") or "").strip().lower()
...
def brain_research_recovery_enabled() -> bool:
    ...
    raw = str(os.getenv("HELIX_BRAIN_RESEARCH_RECOVERY", "") or "").strip().lower()
    return raw in ENABLED_VALUES
