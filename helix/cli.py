...
from helix.config import ensure_dirs


@click.group()
@click.version_option(package_name="helix")
def cli():
    """Helix — Algorithmic trading operations framework."""
    ensure_dirs()
...
def auth_status():
    ...
    from helix.auth.store import display_status
    display_status()
...
def auth_login(provider):
    ...
    from helix.auth.store import run_login
    run_login(provider)
...
def auth_refresh(provider):
    ...
    from helix.auth.store import force_refresh
...
def auth_migrate():
    ...
    from helix.auth.store import migrate_from_openclaw
...
def configure():
    ...
    from helix.auth.store import interactive_configure
...
def ai_ask(provider, prompt, model):
    ...
    from helix.ai import call_ai_sync
...
def db_migrate():
    ...
    from helix.config import OPENCLAW_WORKSPACE
    from helix.db import init_db, migrate_from_openclaw
...
def db_restore_snapshot(snapshot_id):
    ...
    from helix.db import init_db, restore_migration_snapshot
...
def db_status():
    ...
    from helix.db import init_db, table_counts
...
def workspace_list():
    ...
    from helix.workspace import list_workspace_files
...
def workspace_read(filename):
    ...
    from helix.workspace import read_workspace
...
def trades_list(limit):
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.db import init_db, get_recent_trades
...
def trades_open():
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.db import init_db, get_open_trades
...
def strategies_list(status):
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.db import init_db, get_strategies
...
def bot_status():
    ...
    from helix.bot import get_bot_lock_status
...
def bot_send(channel, message):
    ...
    import asyncio
    from helix.bot import send, get_bot, get_bot_token
...
def scheduler_list():
    ...
    from rich.table import Table
    from helix.db import init_db
    from helix.scheduler import get_jobs

    init_db()
    jobs = get_jobs()
    if not jobs:
        click.echo("No scheduler jobs. Run `helix scheduler migrate` to import from OpenClaw.")
        return
...
def scheduler_migrate():
    ...
    from helix.scheduler import migrate_from_openclaw
    migrate_from_openclaw()
...
def scheduler_enable(job_id):
    ...
    from helix.scheduler import enable_job
...
def scheduler_run(job_id):
    ...
    import asyncio
    from helix.db import init_db
    from helix.scheduler import run_job

    init_db()

    with __import__("helix.db", fromlist=["get_db"]).get_db() as conn:
...
def scanner_status():
    ...
    from helix.db import init_db, kv_get
    init_db()
    state = kv_get("scanner_state", {})
    if not state:
        click.echo("No scanner state found. Run `helix scanner run` first.")
        return
...
def scanner_strategies():
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.scanner import STRATEGIES
...
def scanner_backtest(bars, strategy):
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.strategies.backtest import backtest_strategy, backtest_all, save_backtest_results
    from helix.strategies.fitness import evaluate_all
    from helix.scanner import STRATEGIES
...
def scanner_walkforward(bars, splits, strategy):
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.strategies.backtest import walk_forward
    from helix.scanner import STRATEGIES
...
def daemon_status():
    """Show daemon state."""
    from helix.db import init_db
    from helix.runtime_health import normalize_daemon_state
...
def brain_invoke(provider, model):
    ...
    from helix.brain import invoke_sync
    from helix.model_selection import ensure_enforcement_armed
    ensure_enforcement_armed()
...
def brain_tasks():
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.db import init_db, get_db
...
def risk_status():
    ...
    from helix.db import init_db
    from helix.exchange.risk import get_portfolio_summary, get_risk_status, PORTFOLIO_BUDGET
    ...
    if rs["kill_switch_active"]:
        ...
        click.echo("  All trading halted. Run `helix risk reset` to clear after review.")
...
def risk_reset():
    ...
    from helix.db import init_db
    from helix.exchange.risk import reset_kill_switch, get_risk_status
...
def team_list():
    ...
    from rich.table import Table
    from helix.db import init_db
    from helix.agents.manager import list_agents_with_stats

    init_db()
    agents = list_agents_with_stats()
    if not agents:
        click.echo("No agents configured. Create one with: helix team create")
        return
...
def team_create(agent_id, name, role, model, model_id, schedule, schedule_type):
    """Create a new agent."""
    from helix.agents.manager import create_agent
...
def team_inspect(agent_id):
    ...
    from helix.agents.manager import inspect_agent
...
def team_enable(agent_id):
    """Enable an agent."""
    from helix.agents.manager import update_agent
...
def evolution_status():
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.evolution import get_evolution_status
    from helix.db import init_db
...
def evolution_optimize(strategy_id):
    ...
    from rich.console import Console
    from helix.strategies.optimizer import optimize_strategy, optimize_all_deployed
    from helix.db import init_db
...
def regime_status():
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.regime import detect_all_regimes
    from helix.db import init_db
...
def lab_worker(once, poll_seconds, lease_seconds):
    ...
    from helix.lab_dormancy import ensure_regime_lab_enabled
    from helix.lab_db import init_lab_db
    from helix.lab_worker_service import get_canonical_lab_worker_log_path, run_lab_worker_loop
...
def lab_status():
    ...
    from rich.table import Table
    from helix.lab_db import init_lab_db
    from helix.lab_worker_service import get_lab_worker_status
...
def execution_mode(mode):
    ...
    """
    from rich.console import Console
    from helix.config import get_execution_mode, set_execution_mode
...
def execution_status():
    ...
    from rich.console import Console
    from rich.table import Table
    from helix.config import get_execution_mode
    from helix.db import get_db, init_db
    ...
    try:
        from helix.exchange.hyperliquid import get_positions, get_account_value
...
def execution_reconcile():
    ...
    from rich.console import Console
    from helix.exchange.risk import reconcile_all_books
    from helix.db import init_db
...
def status():
    ...
    # Auth
    from helix.auth.store import get_status_rows
    ...
    try:
        ...
        if any(c > 0 for c in counts.values()):
            ...
            console.print("[dim]Database: empty (run `helix db migrate` to import data)[/dim]")
    except Exception as e:
    ...
    if files:
        ...
        console.print("[dim]Workspace: not initialized (run `helix workspace init`)[/dim]")
...
def fts5_rebuild():
    ...
    """
    from helix.db import init_db, rebuild_fts5_indices
...
def doctor(as_json):
    ...
    from helix.diagnostics import FAIL, PASS, WARN, snapshot
    ...
    console.print(f"\n[bold {color}]helix doctor — overall: {overall.upper()}[/bold {color}]")
    console.print(
...
def market_data_backfill(asset, days):
    ...
    from helix.db import init_db
    from helix.market_data_collector import backfill_all, backfill_funding_history
...
def market_data_coverage(asset):
    ...
    from helix.db import init_db
    from helix.market_data_collector import get_data_coverage

    init_db()
    report = get_data_coverage(asset)
    if not report.get("coverage"):
        click.echo("No market data collected yet. Run: helix market-data backfill")
        return
...
def dump_cmd(out):
    ...
    from helix.db import get_db
    from helix.workspace import read_operator_profile
