...
log = logging.getLogger("helix.routers.robustness")
...
def _robustness_executor_workers() -> int:
    raw = str(os.environ.get("HELIX_ROBUSTNESS_MAX_WORKERS", "5") or "").strip()
    try:
        parsed = int(raw)
    except Exception:
        parsed = 5
...
def _robustness_timeout_seconds() -> int:
    raw = str(os.environ.get("HELIX_ROBUSTNESS_TIMEOUT_SECS", "600") or "").strip()
    try:
        parsed = int(raw)
    except Exception:
        parsed = 600
...
# (HELIX_BACKTEST_PROCESS_ISOLATION defaults ON outside pytest), so N concurrent
# reruns = N child processes, each re-importing helix and holding its own candles.
#
...
def _estimate_rerun_seconds(strategy_type: object, n_bars: int) -> float:
    ...
    """
    try:
        from helix.strategies.backtest import _VECTORIZABLE_TYPES
    except Exception:
...
def _write_payload_artifact(result_id: str, job_id: str, payload: object) -> None:
    from helix.api_core import _ensure_result_data_dir, _safe_result_artifact_key
...
def _load_payload_artifact(result_id: str, config: dict, result_type: str):
    from helix.api_core import _load_result_json_artifact
...
def _run_cost_stress_analysis(body: CostStressBody) -> dict:
    ...
    try:
        from helix.api_core import _estimate_backtest_bars, stage_backtest_duration_days
...
def _run_regime_split_analysis(body: RegimeSplitBody) -> dict:
    from helix.strategies.backtest import _detect_entry_regime, load_backtest_candles
...
def _validation_row_passed(
    ...
    *,
    ...
    from helix.gauntlet.legitimacy import validate_robustness_payload
...
def _recalculate_robustness_score(strategy_id: str) -> None:
    ...
    try:
        ...
        from helix.gauntlet.models import ROBUSTNESS_STEP_KEYS, normalize_step_key
        from helix.gauntlet.settings import normalize_required_tests
...
def _reconcile_stage_after_validation(strategy_id: str) -> None:
    from helix.brain import transition_stage, try_research_recovery
    from helix.db import get_db
...
def _run_inline_result(
    *,
    ...
    from helix.api_core import _now
