...
def _webhook_secret() -> str:
    return _get_env("HELIX_GITHUB_WEBHOOK_SECRET")


def _target_branch() -> str:
    return _get_env("HELIX_GITHUB_WEBHOOK_BRANCH", "main")


def _target_remote() -> str:
    return _get_env("HELIX_GITHUB_WEBHOOK_REMOTE", "origin")


def _target_repo_path() -> str:
    default_path = str(_repo_root())
    return _get_env("HELIX_GITHUB_WEBHOOK_REPO", default_path)


def _post_pull_command() -> str:
    return _get_env("HELIX_GITHUB_WEBHOOK_POST_PULL_CMD")


def _delivery_cache_window_seconds() -> int:
    raw = _get_env("HELIX_GITHUB_WEBHOOK_MAX_AGE_SECONDS", "300")
    try:
...
def _git_pull(repo_path: str, remote: str, branch: str) -> dict[str, Any]:
    safe_remote = _validate_git_remote(remote, field="HELIX_GITHUB_WEBHOOK_REMOTE")
    safe_branch = _validate_git_ref(branch, field="HELIX_GITHUB_WEBHOOK_BRANCH")
...
def _run_post_pull(command: str, repo_path: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Invalid HELIX_GITHUB_WEBHOOK_POST_PULL_CMD: {exc}") from exc
...
async def github_webhook(request: Request):
    ...
    if not secret:
        raise HTTPException(
            status_code=503,
            detail="Webhook disabled: HELIX_GITHUB_WEBHOOK_SECRET is not configured",
        )
