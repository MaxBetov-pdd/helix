"""SQLite database — single source of truth for all Helix state."""
...
_FACTORY_RESET_KV_KEYS = {
    "settings": (
        "helix:settings",
        "helix:pipeline:settings",
        "helix:model-routing",
    ),
    "secrets": (
        "helix:settings:secrets",
    ),
    "api-keys": (
        "helix:settings:api-keys",
    ),
}
...
def backup_db(destination: str | Path) -> Path:
    ...
    src = sqlite3.connect(str(HELIX_DB), timeout=30)
    try:
...
def init_db():
    ...
    with get_db() as conn:
        ...
        try:
            from helix.gauntlet.store import init_gauntlet_schema
        ...
        # any new-style migrations registered in helix.migrations.
        conn.execute("SAVEPOINT named_migrations")
        try:
            from helix.migrations import apply_pending, record_applied
...
def recover_dangling_runtime_tasks() -> dict[str, int]:
    ...
    """
    from helix.system_mode_policy import initial_queue_status_for_source, normalize_task_source
...
def _run_migrations(conn: sqlite3.Connection):
    ...
    #
    # mcp_servers — config for each external MCP server Helix can connect to
...
def kv_get(key: str, default=None):
    ...
    if row:
        ...
        if isinstance(raw_value, (str, bytes, bytearray)):
            ...
            payload = raw_value
        if key == "helix:settings:secrets" and isinstance(payload, dict):
            try:
...
_USER_ACTIVE_KEY = "helix:user_active"
...
def log_activity(
    ...
    try:
        from helix.correlation import get_request_id
        rid = get_request_id()
    ...
    try:
        ...
        if _is_sqlite_lock_error(exc):
            logger = logging.getLogger("helix.db")
            logger.warning(
...
def create_approval(
    ...
    expires_at = None
    try:
        from helix.control_plane.approval_modes import get_deadline_hours
        from datetime import datetime, timedelta, timezone
    ...
    if conn is None:
        try:
            from helix.control_plane.approval_modes import (
                get_mode,
                is_off_allowed,
            )
            ...
            if mode == "smart":
                from helix.control_plane.smart_approval import apply_smart_decision
                ...
                from helix.control_plane.approvals import post_approve_approval
                from helix.control_plane.models import ApprovalDecisionBody
                try:
...
def claim_pending_agent_tasks(agent_id: str, limit: int | None = None) -> list[dict]:
    ...
    """
    from helix.system_mode_policy import SYSTEM_SOURCE, USER_SOURCE, is_manual_mode
...
def reap_long_running_agent_tasks(timeout_minutes: int = 30) -> int:
    ...
    from helix.task_timeouts import REAPER_GRACE_MINUTES, resolve_agent_task_timeout_seconds
...
def _factory_reset_wipe_chroma_collections(collections: list[str]) -> None:
    if not collections:
        return
    try:
        from helix.vectordb import wipe_collections
...
def factory_reset(keep_categories: list[str] | None = None, *, allow_credentials_wipe: bool = False) -> dict:
    ...
    if "scheduler_jobs" in wipe_set:
        from helix.scheduler import seed_helix_jobs

        seed_helix_jobs()

    if "system_docs" in wipe_set:
        from helix.workspace import _create_defaults
        ...
        with get_db() as conn:
            create_pending_task(
                ...
                "brain_invoke",
                {
                    ...
                    "message": (
                        "Helix was just factory-reset. You are the Brain — the sole orchestrator.\n\n"
...
def resolve_best_symbol_timeframe(strategy_id: str) -> tuple[str | None, str | None, float, dict]:
    ...
    from helix.policy import score_strategy
...
def verify_chroma_persistence(result_id: str) -> tuple[bool, str]:
    ...
    try:
        ...
        from helix.config import CHROMA_DIR
...
def close_bot_trade(
    ...
    *,
    ...
    """
    from helix.trade_state import close_trade_record
