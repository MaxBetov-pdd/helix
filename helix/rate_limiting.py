...
defense-in-depth for the opt-in HELIX_BIND_HOST/proxied scenarios (and a
...
catches floods. Disable with HELIX_RATE_LIMIT_ENABLED=0 for testing.
"""
...
log = logging.getLogger("helix.rate_limiting")
...
def rate_limit_enabled() -> bool:
    raw = str(os.environ.get("HELIX_RATE_LIMIT_ENABLED", "1")).strip().lower()
    return raw in {"1", "true", "yes", "on"}


def get_default_limit() -> str:
    return str(os.environ.get("HELIX_RATE_LIMIT_DEFAULT", _DEFAULT_LIMIT)).strip() or _DEFAULT_LIMIT
...
def install_rate_limiter(app) -> Optional[object]:
    ...
    if not rate_limit_enabled():
        log.info("Rate limiting disabled via HELIX_RATE_LIMIT_ENABLED")
        return None
    try:
