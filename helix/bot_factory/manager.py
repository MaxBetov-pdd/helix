...
def _build_isolated_env(bot_config: dict) -> dict[str, str]:
    ...
    Passes HELIX_HOME, BOT_ID, minimal system vars, the ChromaDB in-process
    ...
    for guard_var in (
        "HELIX_DISABLE_CHROMA_IN_PROCESS",
        "HELIX_DISABLE_CHROMA",
    ...
    try:
        from helix.auth.store import _ENV_ACCESS_TOKEN_KEYS, _ENV_BASE_URL_KEYS
    ...
    # credential lives on-disk (HELIX_HOME, already forwarded) still has a key
...
class BotManager:
    ...
    def start_bot(self, bot_id: str) -> dict:
        ...
        try:
            process = subprocess.Popen(
                [
                    sys.executable, "-m", "helix.bot_factory.runner",
                    "--bot-id", bot_id,
    ...
    def stop_bot(self, bot_id: str, timeout: float = 5.0) -> dict:
        ...
        if process and process.poll() is None:
            ...
            from helix.db import get_bot_status as _get_status
            ...
            if pid == os.getpid():
                ...
                try:
                    ...
                    try:
                        ...
                        # so a recycled PID that's merely some other helix process
                        # is never killed.
                        looks_like_this_bot = (
    ...
    def _reconcile_orphans(self) -> None:
        ...
        from helix.db import get_db, reconcile_orphaned_bot_trades
