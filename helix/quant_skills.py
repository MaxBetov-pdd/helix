...
log = logging.getLogger("helix.quant_skills")

SKILLS_DIR = HELIX_HOME / "quant-skills"
...
# `version` are top-level. All Helix domain fields nest under `metadata.helix`.
...
def _build_frontmatter(skill: QuantSkill, name: str) -> dict:
    ...
    helix_meta: dict = {"type": skill.skill_type}
    for k, v in skill.metadata.items():
        ...
        helix_meta[k] = str(v) if not isinstance(v, (list, dict)) else v

    fm: dict = {
        "name": name,
        "description": skill.description,
        "version": skill.version,
        "metadata": {"helix": helix_meta},
    }
    return fm
...
def write_skill(skill: QuantSkill, *, evidence_task_id: str | None = None, created_by: str = "system") -> Path:
    ...
    try:
        from helix.db import get_db  # local import to avoid circular at module load
        with get_db() as conn:
            conn.execute(
...
def _migrate_legacy_frontmatter(fm: dict) -> dict:
    ...
    Returns a normalized dict where Helix domain fields always live under
    `metadata.helix`. Pure function — does not touch disk.
    """
    raw_metadata = fm.get("metadata", {}) if isinstance(fm.get("metadata"), dict) else {}
    if "helix" in raw_metadata and isinstance(raw_metadata["helix"], dict):
        # Already v3.
        return fm
    # Legacy: nest everything currently under metadata into metadata.helix.
    fm = dict(fm)
    fm["metadata"] = {"helix": dict(raw_metadata)}
    return fm
...
def read_skill(name: str) -> QuantSkill | None:
    ...
    Tolerates BOTH the v3 envelope shape (Helix fields under `metadata.helix`)
    and the legacy shape (Helix fields directly under `metadata`). Legacy
    ...
    helix_meta = dict(fm.get("metadata", {}).get("helix", {}))
    skill_type = helix_meta.pop("type", "regime")

    # Promote top-level v3 keys back into metadata for the dataclass round-trip.
    metadata: dict = dict(helix_meta)
    if "version" in fm:
