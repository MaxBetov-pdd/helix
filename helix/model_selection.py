...
* CONNECTED — a provider is "connected" only if the operator explicitly added
  it in the app (persisted in the ``helix:connected-providers`` KV set) AND a
...
log = logging.getLogger("helix.model_selection")

_CONNECTED_PROVIDERS_KEY = "helix:connected-providers"
...
_ENFORCEMENT_KEY = "helix:model-selection-enforced"
...
def _provider_has_token(provider: str) -> bool:
    ...
    p = str(provider or "").strip().lower()
    try:
        from helix.auth.store import get_profile, get_token
...
def migrate_connected_from_profiles() -> set[str]:
    ...
    """
    try:
        from helix.auth.store import load_auth
    except Exception:
