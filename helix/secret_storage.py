...
log = logging.getLogger("helix.secret_storage")
_ENCRYPTION_PREFIX = "fernet:"
_KEY_FILE_NAME = ".helix_key"
...
def _preferred_key_path() -> Path:
    ...
    Windows: %LOCALAPPDATA%\\Helix\\.helix_key (not synced by OneDrive/Drive).
    Others:  $XDG_CONFIG_HOME/helix/.helix_key, else ~/.config/helix/.helix_key.
    """
    if sys.platform.startswith("win"):
        local_appdata = os.environ.get("LOCALAPPDATA")
        if local_appdata:
            return Path(local_appdata) / "Helix" / _KEY_FILE_NAME
    xdg = os.environ.get("XDG_CONFIG_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".config"
    return base / "helix" / _KEY_FILE_NAME


def _legacy_key_path() -> Path:
    return Path(cfg.HELIX_HOME) / _KEY_FILE_NAME
...
def _load_fernet_key() -> bytes:
    ...
    Priority:
      1. $HELIX_ENCRYPTION_KEY (explicit override).
    ...
    """
    configured = str(os.environ.get("HELIX_ENCRYPTION_KEY") or "").strip()
    if configured:
    ...
    with FileLock(str(lock_path), timeout=10):
        ...
        # This has happened before: a test with a tmp HELIX_HOME ran without
        # patching _preferred_key_path, so it wrote a fresh key to the real
        # %LOCALAPPDATA%/Helix/ location while the live auth.json stayed
        ...
        if _has_existing_encrypted_data():
            raise RuntimeError(
                ...
                f"HELIX_ENCRYPTION_KEY before continuing."
            )
