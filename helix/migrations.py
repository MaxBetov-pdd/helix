...
Usage (called once from db.init_db after legacy migrations):
    ...
    from helix.migrations import apply_pending
    apply_pending(conn)
...
log = logging.getLogger("helix.migrations")
...
def _m_2026_06_live_max_concurrent_default_bump(conn: sqlite3.Connection) -> None:
    ...
    persisted helix:settings blob — on existing installs that blob pins the
    ...
    """
    row = conn.execute("SELECT value FROM kv WHERE key = 'helix:settings'").fetchone()
    if row is None:
        return
    raw = row[0]
    try:
    ...
    conn.execute(
        "UPDATE kv SET value = ? WHERE key = 'helix:settings'",
        (json.dumps(settings),),
    )
