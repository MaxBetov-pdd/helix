...
const { getHelixAuthProvidersMock, updateSettingsSectionMock } = vi.hoisted(() => ({
	getHelixAuthProvidersMock: vi.fn(),
	updateSettingsSectionMock: vi.fn(),
}));

vi.mock('$lib/api', () => ({
	getHelixAuthProviders: getHelixAuthProvidersMock,
...
beforeEach(() => {
	getHelixAuthProvidersMock.mockReset();
	updateSettingsSectionMock.mockReset();
	getHelixAuthProvidersMock.mockResolvedValue([]);
