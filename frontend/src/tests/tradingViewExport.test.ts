...
describe('TradingView export', () => {
	it('builds a Pine v6 MACD strategy from the active backtest params', () => {
		...
		expect(result.pine).toContain('alertcondition(buy_signal, title = "Helix BUY"');
		expect(result.pine).toContain('alertcondition(sell_signal or window_forced_sell, title = "Helix SELL"');
