...
describe('API Client', () => {
	...
	describe('getSymbols', () => {
		...
		it('should include api/operator keys from local storage when configured', async () => {
			window.localStorage.setItem('helix_api_key', 'api-key-123');
			window.localStorage.setItem('helix_operator_key', 'op-key-456');
			mockFetch.mockResolvedValueOnce({
				ok: true,
	...
	describe('Helix API Contracts', () => {
		...
		it('should surface 404 for missing agent terminal', async () => {
			...
			await expect(api.getHelixAgentTerminal('missing-agent')).rejects.toMatchObject({
				name: 'ApiError',
				status: 404,
