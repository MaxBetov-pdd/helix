...
const apiMocks = vi.hoisted(() => ({
	getHelixAgents: vi.fn(),
	getHelixAgentDocuments: vi.fn(),
	updateHelixAgent: vi.fn(),
	updateHelixAgentDocument: vi.fn(),
	testHelixAgentDiscord: vi.fn(),
	getHelixSchedulerJobs: vi.fn(),
	updateHelixSchedulerJob: vi.fn(),
}));
...
beforeEach(() => {
	clearDirty();
	originalValues.set({});
	apiMocks.getHelixAgents.mockReset();
	apiMocks.getHelixAgentDocuments.mockReset();
	apiMocks.updateHelixAgent.mockReset();
	apiMocks.updateHelixAgentDocument.mockReset();
	apiMocks.testHelixAgentDiscord.mockReset();
	apiMocks.getHelixSchedulerJobs.mockReset();
	apiMocks.updateHelixSchedulerJob.mockReset();

	apiMocks.getHelixAgents.mockResolvedValue([
		{
			id: 'alpha',
			name: 'Alpha',
			role: 'trader',
	...
	apiMocks.getHelixAgentDocuments.mockResolvedValue({
		soul: 'SOUL for alpha',
		agents: 'AGENTS for alpha',
		role: 'ROLE for alpha',
	});
	apiMocks.getHelixSchedulerJobs.mockResolvedValue([
		{
			id: 'job-1',
...
describe('SettingsAgents section', () => {
	...
	it('loads the agent roster and selects the first agent by default', async () => {
		...
		expect(apiMocks.getHelixAgents).toHaveBeenCalled();
		expect(apiMocks.getHelixAgentDocuments).toHaveBeenCalledWith('alpha');
	...
	it('renders the scheduler jobs loaded from the backend', async () => {
		...
		expect(apiMocks.getHelixSchedulerJobs).toHaveBeenCalled();
		const text = target.textContent || '';
