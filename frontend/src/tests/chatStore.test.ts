...
describe('chatStore', () => {
	...
	it('tracks unread replies until the chat is marked as read', () => {
		...
		expect(sessionStorage.getItem('helix_brain_chat_unread')).toBe('3');

		markChatRead();

		expect(get(chatUnreadCount)).toBe(0);
		expect(sessionStorage.getItem('helix_brain_chat_unread')).toBe('0');
	});
	...
	it('clears unread state when the chat history is cleared', () => {
		...
		expect(get(chatUnreadCount)).toBe(0);
		expect(sessionStorage.getItem('helix_brain_chat')).toBe('[]');
	});
});
