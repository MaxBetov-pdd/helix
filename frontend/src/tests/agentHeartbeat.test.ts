...
vi.mock('$lib/api', () => ({
	getHelixLogs: vi.fn().mockResolvedValue([]),
	getHelixAgents: vi.fn().mockResolvedValue([
		{
			id: 'brain',
			name: 'Brain',
	...
	getHelixAgentTasks: vi.fn().mockResolvedValue([
...
describe('AgentHeartbeat with roster', () => {
	it('renders the roster pulled from getHelixAgents', async () => {
	...
	it('keeps the roster wrapper rendered even when no agents load', async () => {
		...
		(api.getHelixAgents as ReturnType<typeof vi.fn>).mockResolvedValueOnce([]);
		target = document.createElement('div');
		document.body.appendChild(target);
		instance = mount(AgentHeartbeat, { target, props: {} });
		await flush();
