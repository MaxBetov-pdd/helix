...
const { getSettingsAuditLogMock, getHelixDashboardMock } = vi.hoisted(() => ({
	getSettingsAuditLogMock: vi.fn(),
	getHelixDashboardMock: vi.fn(),
}));

vi.mock('$lib/api', () => ({
	getSettingsAuditLog: getSettingsAuditLogMock,
	getHelixDashboard: getHelixDashboardMock,
}));
...
beforeEach(() => {
	...
	getHelixDashboardMock.mockReset();
	getSettingsAuditLogMock.mockResolvedValue([]);
	getHelixDashboardMock.mockResolvedValue({ execution_mode: 'paper' });
});
