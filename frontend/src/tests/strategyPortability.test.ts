...
describe('buildExportFilename', () => {
	it('slugifies id + name', () => {
		expect(buildExportFilename('S00123', 'BTC RSI Momentum')).toBe('helix-S00123-btc-rsi-momentum.json');
	});

	it('falls back to id only when name is empty', () => {
		expect(buildExportFilename('S00123')).toBe('helix-S00123.json');
	});

	it('sanitizes unsafe id characters', () => {
		expect(buildExportFilename('S 00/123')).toBe('helix-S_00_123.json');
	});
});
...
describe('parseEnvelope', () => {
	...
	it('throws when helix_export metadata is missing', () => {
		const text = JSON.stringify({ strategy: {}, configuration: {} });
		expect(() => parseEnvelope(text)).toThrow(/not a Helix strategy export/i);
	});

	it('throws on an unsupported export kind', () => {
		const text = JSON.stringify({ helix_export: { kind: 'dataset', version: '1.0' } });
