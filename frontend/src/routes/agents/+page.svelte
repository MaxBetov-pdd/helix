<script lang="ts">
	...
	import {
		createHelixStrategyDeveloperAgent,
		deleteHelixAgent,
		dismissHelixAgentTask,
		getHelixAgents,
		getHelixAgentTasks,
		getHelixSchedulerJobs,
		getHelixLogs,
		getHelixAgentTerminal,
		getHelixAgentModelOptions,
		getHelixModelPolicy,
		updateHelixAgent,
		updateHelixSchedulerJob
	} from '$lib/api';
	import {
		listMCPServers,
		listMCPGrants,
	...
	import type { HelixAgent, HelixAgentTask, HelixAgentModelOption, HelixAgentUpdatePayload, HelixModelPolicyResponse, HelixProvider, HelixSchedulerJob } from '$lib/api';
	...
	type AgentProvider = HelixProvider;
	...
	const requiredCoreAgentIds = fallbackAgentDefBase
		...
		.map((agent) => agent.id);
	let modelPolicy: HelixModelPolicyResponse | null = null;
	let modelPresets: AgentModelPreset[] = [];
	let fallbackModelPresets: AgentModelPreset[] = [];
	let agentModelOptions: HelixAgentModelOption[] = [];

	let agents: HelixAgent[] = [];
	...
	let agentTasks: HelixAgentTask[] = [];
	let schedulerJobs: HelixSchedulerJob[] = [];
	...
	function resolveAgentModel(agent: HelixAgent): { provider: AgentProvider; modelId: string; key: string } {
	...
	async function dismissTaskAlert(task: HelixAgentTask, silent = false): Promise<void> {
		...
		try {
			await dismissHelixAgentTask(taskId, source);
	...
	function normalizeTaskKey(task: HelixAgentTask): string {
	...
	function isCompletedTask(task: HelixAgentTask): boolean {
		const status = (task.status ?? '').toLowerCase();
		return status === 'done' || status === 'completed' || status === 'reviewed';
	}

	function isErrorTask(task: HelixAgentTask): boolean {
		const status = (task.status ?? '').toLowerCase();
		return status === 'error' || status === 'failed';
	}

	function handleTaskCompletionAlert(nextTasks: HelixAgentTask[]) {
	...
	function parseAgentStatus(task: HelixAgentTask | undefined | null): string {
	...
	function latestTaskForAgent(agentId: string): HelixAgentTask | null {
		let best: HelixAgentTask | null = null;
		let bestTs = -1;
	...
	function toAgentCard(agent: HelixAgent): AgentCard | null {
	...
	function mergeAgentCards(runtimeAgents: HelixAgent[], runtimeTasks: HelixAgentTask[], runtimeLogs: AgentLogEntry[]): AgentCard[] {
	...
	async function fetchTerminal() {
		...
		terminalCalls = [];
		try {
			const data = await getHelixAgentTerminal(agentId);
	...
	async function handleRenameCommit(card: AgentCard) {
		...
		try {
			const updated = await updateHelixAgent(card.id, { name: draft });
	...
	async function handleAddDeveloper() {
		...
		try {
			...
			const created = await createHelixStrategyDeveloperAgent({ name });
	...
	async function handleRemoveDeveloper(card: AgentCard) {
		...
		try {
			await deleteHelixAgent(card.id);
	...
	async function handleEditCommit(card: AgentCard) {
		...
		try {
			...
			const payload: HelixAgentUpdatePayload = {
				name,
				instructions: editDraftInstructions
			};
			const updated = await updateHelixAgent(card.id, payload);
			agents = agents.map((item) => {
				const id = String(item.id ?? '').trim();
				return id === card.id ? { ...item, ...updated } : item;
			});
	...
	async function handleSchedulerSave(
		...
		try {
			...
			const result = await updateHelixSchedulerJob(jobId, scheduleType, payloadExpr, enabled);
			if (!result?.ok) {
