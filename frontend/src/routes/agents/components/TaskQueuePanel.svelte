<script lang="ts">
	import type { HelixAgentTask } from '$lib/api';

	type DateDisplayMode = 'relative' | 'absolute';
	type AccentColor = 'cyan' | 'green' | 'amber' | 'rose';
	type NormalizedTask = HelixAgentTask & {
		_agentId: string;
		_id: string;
	};

	export let tasks: HelixAgentTask[] = [];
	...
	export let onDismissTask: (task: HelixAgentTask) => void | Promise<void> = () => {};
