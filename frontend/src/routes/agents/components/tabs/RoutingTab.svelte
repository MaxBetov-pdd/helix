<script lang="ts">
	 ...
	 *    /api/model-policy (updateHelixModelPolicy) — one call.
	...
	import {
		...
		type HelixAgent,
		type HelixProvider,
		type HelixModelPolicyFallbackEntry,
		type HelixModelPolicyUpdatePayload,
	...
	function fromKey(key: string): HelixModelPolicyFallbackEntry | null {
		const sep = key.indexOf(':');
		if (sep <= 0) return null;
		return { provider: key.slice(0, sep), model_id: key.slice(sep + 1) };
	}
	function chainToKeys(chain: HelixModelPolicyFallbackEntry[] | undefined): string[] {
		return (chain ?? []).map((e) => toKey(e.provider, e.model_id)).filter(Boolean);
	}
	function keysToChain(keys: string[]): HelixModelPolicyFallbackEntry[] {
		return keys.map(fromKey).filter((e): e is HelixModelPolicyFallbackEntry => e !== null);
	}
		...
		try {
			...
			if (agentsRes.status === 'fulfilled') {
				const list = (agentsRes.value ?? []).filter(
					(a): a is HelixAgent => Boolean(String(a?.id ?? '').trim())
				);
				const rows: AgentRow[] = [];
	...
	): HelixModelPolicyUpdatePayload {
		const next: HelixModelPolicyUpdatePayload = { ...payload };
	...
	async function saveAll() {
		...
		try {
			...
			for (const row of agentRows) {
				...
				if (!entry) continue;
				await updateHelixAgentModel(row.id, {
					model: entry.provider as HelixProvider,
			...
			const fallback_chains: Record<string, HelixModelPolicyFallbackEntry[]> = {
			...
			const updated = await updateHelixModelPolicy(withDerivedPrimary({ fallback_chains }));
