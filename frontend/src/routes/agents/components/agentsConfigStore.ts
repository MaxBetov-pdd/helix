...
import {
	...
	type HelixModelPolicyResponse,
} from '$lib/api';

export interface AgentsConfigState {
	providers: HelixAuthProviderStatus[];
	authFile: string | null;
	modelOptions: HelixAgentModelOption[];
	enabledKeys: Set<string>;
	policy: HelixModelPolicyResponse | null;
	loading: boolean;
	error: string | null;
}
 ...
 */
export function isProviderConnected(p: HelixAuthProviderStatus): boolean {
...
export const agentsConfig = {
	...
	async load(opts: { refreshModels?: boolean } = {}): Promise<void> {
		...
		const [authRes, modelRes, policyRes] = await Promise.allSettled([
			getHelixAuthProviders(),
			getHelixAgentModelOptions(Boolean(opts.refreshModels)),
			getHelixModelPolicy(),
		]);
	...
	setPolicy(policy: HelixModelPolicyResponse): void {
		store.update((s) => ({ ...s, policy }));
	},
};

/** Providers the operator has connected (authorized spend). */
export const connectedProviders: Readable<HelixAuthProviderStatus[]> = derived(store, ($s) =>
 ...
 */
export const selectableModelOptions: Readable<HelixAgentModelOption[]> = derived(
