<script lang="ts">
	 ...
	 * /api/agents/{id}/documents/{doc}; GET via getHelixAgentDocuments), the
	...
	export let agent: HelixAgent;

	const dispatch = createEventDispatcher<{ close: void; saved: HelixAgent }>();
	...
	async function loadDocs() {
		...
		docsLoading = true;
		try {
			const res = await getHelixAgentDocuments(agentId);
	...
	async function saveAgent() {
		...
		try {
			...
			const updated = await updateHelixAgent(agentId, payload);
	...
	async function saveDoc(doc: DocKind) {
		...
		try {
			await updateHelixAgentDocument(agentId, doc, docs[doc]);
	...
	async function testDiscord() {
		...
		discordTesting = true;
		try {
			const result = await testHelixAgentDiscord(agentId, discordToken.trim() || undefined);
