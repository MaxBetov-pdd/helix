<script lang="ts">
	...
	// the engine defaults in helix/strategies/backtest.py _resolve_execution_controls
	...
	async function confirmPromotion(override = false): Promise<void> {
		...
		promoting = true;
		try {
			await promoteHelixStrategy(strategyId, target.key, {
	...
	async function archiveStrategyFromConfig(): Promise<void> {
		...
		promoting = true;
		try {
			await promoteHelixStrategy(strategyId, 'archived', {
				fromStatus: currentLifecycleStage,
