<script lang="ts">
	...
	import { connectHelixWs, disconnectHelixWs, helixWsConnected } from '$lib/stores/helixWebSocket';
	import { shouldMarkBackendDisconnected } from '$lib/utils/connectionHealth';
	import Sidebar from '$lib/components/Sidebar.svelte';
	import Toast from '$lib/components/Toast.svelte';
	import GlobalControlStrip from '$lib/components/helix/GlobalControlStrip.svelte';
	...
	const DESCRIPTION_OVERRIDES: Record<string, string> = {
		...
		'/diagnostics': 'Health checks, cost rollups, and resumable tasks for the Helix runtime.',
		'/integrations': 'Connect AI clients to Helix and manage external MCP tool servers for agents.',
		'/integrations/mcp': 'Connect AI clients to Helix and manage external MCP tool servers for agents.',
		'/settings': 'Configure execution, API keys, alerts, and platform preferences for Helix.',
	};
	...
	function resolvePageDescription(pathname: string): string {
		...
		if (pathname.startsWith('/integrations')) return 'Connect AI clients to Helix and manage external MCP tool servers for agents.';
		return DESCRIPTION_OVERRIDES[pathname] ?? 'Helix trading workspace.';
	}

	$: pageTitle = `${resolvePageTitle($page.url.pathname)} | Helix`;
	...
	async function attemptHealthCheck() {
		try {
			...
			const wsStillConnected = get(helixWsConnected);
	...
	onMount(() => {
		...
		if (typeof window !== 'undefined') {
			window.addEventListener('helix:reconnected', handleReconnect);
		}
	});
	...
	onDestroy(() => {
		...
		if (typeof window !== 'undefined') {
			window.removeEventListener('helix:reconnected', handleReconnect);
		}
	});
