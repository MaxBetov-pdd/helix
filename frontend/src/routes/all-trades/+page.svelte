<script lang="ts">
	...
	import { getHelixAllTrades, markHelixTradeFailed } from '$lib/api';
	import type { HelixTrade, HelixTradesPage } from '$lib/api';

	export let data: { initialPage: HelixTradesPage | null };

	const STATUSES = ['ALL', 'OPEN', 'CLOSED', 'FAILED'] as const;
	type StatusFilter = (typeof STATUSES)[number];
	const PAGE_SIZE = 200;

	let trades: HelixTrade[] = data.initialPage?.trades ?? [];
	...
	async function loadPage(reset = false): Promise<void> {
		...
		error = '';
		try {
			const page = await getHelixAllTrades({
	...
	async function handleMarkFailed(trade: HelixTrade): Promise<void> {
		...
		notice = '';
		try {
			await markHelixTradeFailed(tradeId);
	...
	function asTrade(row: unknown): HelixTrade {
		return row as HelixTrade;
	}
