...
import { getHelixLiveWebSocketUrl, getHelixLiveWebSocketUrls } from '$lib/api';

export const helixDaemonState = writable<Record<string, unknown> | null>(null);
export const helixLivePrices = writable<Record<string, number>>({});
export const helixPositionPnl = writable<Array<Record<string, unknown>>>([]);
export const helixRealtimeLogs = writable<Array<Record<string, unknown>>>([]);
export const helixLastTrade = writable<Record<string, unknown> | null>(null);
export const helixRealtimeEvents = writable<Array<Record<string, unknown>>>([]);

const _wsConnected = writable(false);
export const helixWsConnected = readonly(_wsConnected);
...
function refreshWsUrlCandidates(): void {
	...
	wsUrlCandidates = [getHelixLiveWebSocketUrl()];
}
...
function pickWsUrl(): string {
	...
	if (wsUrlCandidates.length === 0) return getHelixLiveWebSocketUrl();
	const idx = ((wsUrlIndex % wsUrlCandidates.length) + wsUrlCandidates.length) % wsUrlCandidates.length;
	return wsUrlCandidates[idx] ?? wsUrlCandidates[0] ?? getHelixLiveWebSocketUrl();
}
...
function handlePayload(data: Record<string, unknown>) {
	...
	switch (data.type) {
		case 'init':
			...
			helixDaemonState.set((data.data as Record<string, unknown>) ?? data);
			break;
		...
		case 'prices':
			helixLivePrices.set((data.prices ?? data) as Record<string, number>);
			break;

		case 'position_pnl':
			helixPositionPnl.set((data.positions ?? data.data ?? []) as Array<Record<string, unknown>>);
			break;

		case 'logs': {
			const entries = (data.entries ?? data.logs ?? []) as Array<Record<string, unknown>>;
			helixRealtimeLogs.update((current) => {
		...
		case 'trade':
			helixLastTrade.set(data);
			if (typeof window !== 'undefined') {
				window.dispatchEvent(new CustomEvent('helix:event', { detail: data }));
			}
			break;
		...
		case 'risk_alert': {
			helixRealtimeEvents.update((current) => {
...
function attachSocketListeners(socket: WebSocket, url: string): void {
	const onOpen = () => {
		...
		resetHeartbeat();
		if (typeof window !== 'undefined') {
			window.dispatchEvent(new CustomEvent('helix:connected'));
			if (wasReconnect) {
				window.dispatchEvent(new CustomEvent('helix:reconnected'));
			}
		}
	};
	...
	const onClose = (event: CloseEvent) => {
		...
		if (typeof window !== 'undefined') {
			window.dispatchEvent(new CustomEvent('helix:disconnected'));
		}
		ws = null;
	...
	const onError = () => {
		...
		console.warn('[HelixWS] websocket error', { url, readyState: socket.readyState });
	};
...
export function connectHelixWs() {
...
export function disconnectHelixWs() {
