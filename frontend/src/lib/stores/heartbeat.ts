...
async function refreshHeartbeat(): Promise<void> {
	try {
		...
		if (heartbeat.dashboard) helixDashboard.set(heartbeat.dashboard);
		if (heartbeat.risk) helixRisk.set(heartbeat.risk);
		if (heartbeat.sentiment) helixSentiment.set(heartbeat.sentiment);
		if (heartbeat.regime) helixRegime.set(heartbeat.regime);
		if (Array.isArray(heartbeat.open_trades)) helixOpenTrades.set(heartbeat.open_trades);
		if (heartbeat.scanner_state) helixScannerState.set(heartbeat.scanner_state);
		setNavIndicators(
