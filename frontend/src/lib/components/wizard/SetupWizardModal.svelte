<script lang="ts">
	...
	import { getHelixAuthProviders, reconcileAgentProviders, updateSettingsSection } from '$lib/api';
	...
	const STEPS: Step[] = [
		{ id: 'welcome', label: 'Welcome', critical: false, description: 'Get Helix set up.' },
	...
	async function refreshProviders() {
		try {
			const response = await getHelixAuthProviders();
	...
	function skipMessage(s: Step): string {
		if (s.id === 'trading') {
			return "Skip Trading basics? Helix won't be able to place paper or live orders until you connect an exchange in Settings.";
		}
		return "Skip AI providers? Helix won't be able to run research, propose strategies, or chat until you connect one in Settings.";
	}
	...
	async function finish() {
		...
		if (unsatisfied.length > 0) {
			...
			const ok = window.confirm(
				`You haven't finished: ${labels}. Helix won't run correctly until these are set up. Finish anyway?`
			);
			if (!ok) return;
		}
	...
	async function skipAll() {
		...
		if (unsatisfied.length > 0) {
			const labels = unsatisfied.map((s) => s.label).join(', ');
			const ok = window.confirm(
				`Skip setup? You haven't finished: ${labels}. Helix won't run correctly until these are set up.`
			);
			if (!ok) return;
		}
		try {
			...
			<section class="flex-1 min-w-0 flex flex-col">
				...
				{#if step.critical && !isSatisfied(step)}
					<div class="px-6 py-3 bg-amber-900/30 border-b border-amber-800 text-sm text-amber-200">
						{#if step.id === 'trading'}
							⚠ Without an exchange API connection, Helix can't place paper or live orders.
						{:else}
				...
				<div class="flex-1 min-h-0 overflow-y-auto px-6 py-4">
					{#if step.id === 'welcome'}
						<p class="text-sm text-gray-300 leading-relaxed">
							This wizard walks you through the minimum setup to run Helix.
					...
					{:else if step.id === 'ai'}
						<div class="mb-4 rounded border border-cyan-900 bg-cyan-950/30 px-4 py-3 text-sm text-cyan-100">
							...
							<ul class="mt-2 space-y-1 text-xs text-cyan-100/90 list-disc pl-5">
								<li>Cheapest model that reliably runs Helix's agents (~$0.10 / $0.40 per 1M input/output tokens) and has a <strong>free tier</strong> to try.</li>
