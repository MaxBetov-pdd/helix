<script lang="ts">
	...
	import { helixWsConnected } from '$lib/stores/helixWebSocket';
	import {
		helixDashboard,
		helixRisk,
		helixSentiment,
		helixRegime,
	} from '$lib/stores/helix';
	import { simulationActive, simulationPhase, simulationTime } from '$lib/stores/simulation';
	import {
		getSystemStatus,
		setSystemMode,
		resetTradingHalt,
		setHelixExecutionMode,
		triggerHelixEmergencyHalt,
	...
	$: executionMode = normalizeMode($helixDashboard?.execution_mode);
	$: hlNetwork = (() => {
		const raw = ($helixDashboard?.account?.network || '').toString().trim().toLowerCase();
		if (raw === 'mainnet' || raw === 'testnet') return raw;
		return executionMode === 'live' ? 'mainnet' : 'testnet';
	})();
	$: daemonStatus = $helixDashboard ? ($helixDashboard.daemon_running ? 'ACTIVE' : 'OFFLINE') : 'SYNCING';
	$: btcRegime = ($helixRegime as Record<string, Record<string, string>> | null)?.BTC?.regime || '--';
	$: ethRegime = ($helixRegime as Record<string, Record<string, string>> | null)?.ETH?.regime || null;
	$: solRegime = ($helixRegime as Record<string, Record<string, string>> | null)?.SOL?.regime || null;
	$: sentimentScore = typeof ($helixSentiment as Record<string, unknown> | null)?.composite === 'number' ? ($helixSentiment as Record<string, number>).composite : null;
	$: tradingAllowed = $helixDashboard?.trading_allowed ?? true;
	$: tradingReason = ($helixDashboard as Record<string, unknown> | null)?.trading_reason as string || 'OK';
	$: killSwitchActive = Boolean($helixRisk?.kill_switch_active || ($helixDashboard?.risk as Record<string, unknown> | undefined)?.kill_switch_active);
	$: modeSwitchTarget = executionMode === 'paper' ? 'live' : 'paper';
	$: wsConnected = $helixWsConnected;
	...
	async function confirmModal() {
		...
		try {
			if (modalAction === 'system-mode-change') {
				...
				await setHelixExecutionMode(modeSwitchTarget);
			} else if (modalAction === 'emergency-halt') {
				await triggerHelixEmergencyHalt();
...
{#if !wsConnected}
	<div class="bg-red-700/85 border-b border-red-500 px-4 py-1 text-[11px] uppercase tracking-wider text-white font-bold">
		Connection lost. Reconnecting to Helix websocket...
	</div>
{/if}
{#if $simulationActive}
...
<header class="border-b border-[#222] bg-[#0b0b0b] px-4 py-2 flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-wider">
	...
	<div class="flex min-w-0 flex-1 flex-wrap items-center justify-center gap-2">
		...
		<!-- Execution mode is display-only: Helix supports paper trading +
