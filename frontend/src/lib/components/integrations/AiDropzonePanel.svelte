<script lang="ts">
	...
	const SESSION_STORAGE_KEY = 'helix:ai-dropzone:active-session-id';
	...
	const MCP_CLIENTS: McpClient[] = [
		...
		// ── IDE / code editor ───────────────────────────────────
		{
			...
			note: 'Or run: claude mcp add helix -- python -m helix.mcp_server',
		},
		{
			id: 'cursor',
			name: 'Cursor',
	...
	async function submitRegister() {
		...
		// before code lands in helix/strategies/custom/.
	...
	function buildMcpConfig(format: ConfigFormat = selectedClient.format): string {
		...
		switch (format) {
			...
			default: {
				const cfg = { mcpServers: { helix: serverSpec() } };
				return JSON.stringify(cfg, null, 2);
			}
		}
	}
	...
	function buildCliCommand(): string {
		...
		return (
			`claude mcp add helix ` +
			`-e HELIX_API_URL=${backendUrl()} ` +
			`-e HELIX_API_KEY=your-api-key ` +
			`-e HELIX_OPERATOR_KEY=your-operator-key ` +
	...
	// `helix.agent` client/CLI, or any HTTP client.
	function httpCliSnippet(): string {
		const base = backendUrl();
		return [
			'# Any shell — Claude Code, Codex, CI. JSON in / JSON out.',
			'# (set HELIX_API_URL if the backend is not on this machine)',
			`export HELIX_API_URL=${base}`,
			'python -m helix.agent health            # or: helix-agent health',
			'python -m helix.agent list --status paper',
			'python -m helix.agent backtest --strategy S00719 --dataset BTC/USDT-1h --compact',
			'# write a strategy .py to helix/strategies/custom/, then run the full loop:',
			'python -m helix.agent enqueue --file /abs/path/strategy.py --dataset BTC/USDT-1h',
			'python -m helix.agent wait-paper --strategies S00719 --timeout 1800',
		].join('\n');
	}
	function httpPySnippet(): string {
		return [
			'from helix.agent import HelixAgentClient',
			'fc = HelixAgentClient()   # HELIX_API_URL / base_url override',
			'fc.health()',
	...
	function httpTsSnippet(): string {
		return [
			...
			'const v = await HelixAgent.enqueueCandidate("/abs/path/strategy.py", "BTC/USDT-1h");',
		].join('\n');
	}
			...
			<div class="flex items-center gap-4">
				...
				<div class="flex items-center gap-3 ml-4">
					<div class="flex items-center gap-1.5">
						...
						<span class="help-tip">?<span class="help-text">Helix HTTP API at /api. If this is red, MCP tool calls will also fail — MCP proxies through this API.</span></span>
					</div>
			...
			{#if showHttpConfig}
				<div class="px-4 pb-4 border-t border-[#222] pt-3 space-y-3">
					<p class="text-[11px] text-gray-400">
						...
						<code class="bg-[#111] px-1 rounded">helix.agent</code> client is stdlib-only (zero deps).
						Base URL:
					...
					<p class="text-[10px] text-gray-500">
						...
						<code class="bg-[#111] px-1 rounded">HELIX_API_KEY</code> /
						<code class="bg-[#111] px-1 rounded">HELIX_OPERATOR_KEY</code>. Full reference:
						<code class="bg-[#111] px-1 rounded">helix/agent/README.md</code>.
					</p>
				</div>
			{/if}
		</div>
			...
			<div class="lg:col-span-2 border border-[#222] rounded bg-[#0a0a0a]">
				...
				{#if !activeDetail}
					<div class="p-6 text-center text-xs text-gray-600">
						No session selected. Tell your assistant: <em class="text-gray-400">"Open a Helix session for X"</em>
					</div>
				{:else}
					<!-- Strategies -->
					<div class="px-4 py-3">
