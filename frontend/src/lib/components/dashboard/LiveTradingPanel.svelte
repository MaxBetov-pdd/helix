<script lang="ts">
	...
	import { getHelixDashboard, getHelixEquityHistory, getHelixRecentTrades, getHelixOpenTrades, getHelixScannerState } from '$lib/api';
	import type { HelixDashboardResponse, HelixEquityHistory, HelixScannerState, HelixTrade } from '$lib/api';
	import EquityChart from '$lib/components/EquityChart.svelte';

	let dashboard: HelixDashboardResponse | null = null;
	let equityData: HelixEquityHistory = { base: 0, curve: [] };
	let recentTrades: HelixTrade[] = [];
	let openTrades: HelixTrade[] = [];
	let scannerState: HelixScannerState | null = null;
	let loading = true;
	let dashboardError = '';
	...
	function normalizeTradesPayload(payload: unknown): HelixTrade[] {
		if (Array.isArray(payload)) return payload as HelixTrade[];
		const record = asRecord(payload);
		if (!record) return [];
		if (Array.isArray(record.trades)) return record.trades as HelixTrade[];
		if (Array.isArray(record.open_trades)) return record.open_trades as HelixTrade[];
		if (Array.isArray(record.recent_trades)) return record.recent_trades as HelixTrade[];
		if (Array.isArray(record.items)) return record.items as HelixTrade[];
		return [];
	}

	function normalizeEquityPayload(payload: unknown): HelixEquityHistory {
		const fallback: HelixEquityHistory = { base: 0, curve: [] };
		const record = asRecord(payload);
		...
		return {
			...
			curve: curve as HelixEquityHistory['curve'],
		};
	}
	...
	async function runFetchData() {
		try {
			const [dash, equity, recent, open, scanner] = await Promise.allSettled([
				getHelixDashboard(),
				getHelixEquityHistory(),
				getHelixRecentTrades(10),
				getHelixOpenTrades(),
				getHelixScannerState(),
			]);

			if (dash.status === 'fulfilled') {
				const normalizedDashboard = asRecord(dash.value);
				dashboard = normalizedDashboard ? (dash.value as HelixDashboardResponse) : null;
				dashboardError = '';
			} else {
				dashboard = null;
			...
			if (scanner.status === 'fulfilled') {
				...
				scannerState = normalizedScanner ? (scanner.value as HelixScannerState) : null;
			}
		} catch (err) {
	...
	function tradeDirection(trade: HelixTrade): string {
		return String(trade.direction ?? '').trim().toLowerCase() || 'long';
	}

	function tradeStatus(trade: HelixTrade): string {
		return String(trade.status ?? '').trim().toUpperCase() || 'UNKNOWN';
	}

	function tradeStrategyLabel(trade: HelixTrade): string {
	...
	function tradeSignalDataRecord(trade: HelixTrade): Record<string, unknown> {
		return asRecord(trade.signal_data) ?? {};
	}

	function tradeBadges(trade: HelixTrade): TradeBadge[] {
	...
	function isExchangeBackedTrade(trade: HelixTrade): boolean {
	...
	function computeLivePnlUsd(trade: HelixTrade): number | null {
	...
	function tradeDisplayPnl(trade: HelixTrade): number | null {
	...
	function tradeElapsed(trade: HelixTrade): string {
	...
	function tradeCurrentPrice(trade: HelixTrade): string {
		const asset = typeof trade.asset === 'string' ? trade.asset : '';
