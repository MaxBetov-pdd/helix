<script lang="ts">
	...
	import { helixLivePrices } from '$lib/stores/helixWebSocket';
	...
	export let dashboard: HelixDashboardResponse | null = null;
	...
	let helixEventUnsubscribe: (() => void) | null = null;
	...
	function getSelectedSessionStorageKey(): string {
		if (view === 'live') return 'helix.live.selectedSessionId';
		if (view === 'all') return 'helix.sessions.selectedSessionId';
		return 'helix.paper.selectedSessionId';
	}
	...
	function handleHelixRealtimeEvent(): void {
		if (!selectedSession || selectedSession.mode === 'replay') return;
	...
	onMount(async () => {
		livePriceUnsubscribe = helixLivePrices.subscribe((priceMap) => {
			applyRealtimePriceSnapshot(priceMap ?? {});
		});
		if (typeof window !== 'undefined') {
			const eventHandler = () => handleHelixRealtimeEvent();
			window.addEventListener('helix:event', eventHandler);
			helixEventUnsubscribe = () => window.removeEventListener('helix:event', eventHandler);
		}
	...
	onDestroy(() => {
		...
		helixEventUnsubscribe?.();
		helixEventUnsubscribe = null;
		stopLiveChartPolling();
		stopSelectedSessionSync();
		if (typeof window !== 'undefined') {
