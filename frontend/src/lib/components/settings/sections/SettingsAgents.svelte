<script lang="ts">
	...
	import {
		getHelixAgents,
		getHelixAgentDocuments,
		updateHelixAgent,
		updateHelixAgentDocument,
		testHelixAgentDiscord,
		getHelixSchedulerJobs,
		updateHelixSchedulerJob,
		getHelixAgentModelOptions,
		updateSettingsSection,
		getHelixAuthProviders,
		setHelixAuthProvider,
		deleteHelixAuthProvider,
		testHelixAuthProvider,
		startHelixAuthProviderOAuth,
		completeHelixAuthProviderOAuth,
		pollHelixAuthProviderOAuth,
		cancelHelixAuthProviderOAuth,
		type HelixAgent,
		type HelixSchedulerJob,
		type HelixAgentModelOption,
		type HelixAuthProviderStatus,
		type HelixAuthProviderOAuthStartResponse,
	} from '$lib/api';
	...
	let modelOptions: HelixAgentModelOption[] = [];
	...
	let authProviders: HelixAuthProviderStatus[] = [];
	...
	let providerOAuthState: Record<
		string,
		(HelixAuthProviderOAuthStartResponse & { code: string }) | null
	> = {};
	let providerOAuthStatus: Record<string, string> = {};

	function draftFromAgent(a: HelixAgent) {
		return {
	...
	async function selectAgent(id: string | null) {
		...
		try {
			agentDocs = await getHelixAgentDocuments(id);
		} catch (e) {
	...
	async function saveAgent() {
		...
		try {
			...
			await updateHelixAgent(selectedAgentId, payload);
	...
	async function saveAgentDoc(doc: 'soul' | 'agents' | 'role') {
		...
		agentError = null;
		try {
			await updateHelixAgentDocument(selectedAgentId, doc, agentDocs[doc]);
	...
	async function testAgentDiscord() {
		...
		agentError = null;
		try {
			const result = await testHelixAgentDiscord(
				selectedAgentId,
	...
	async function loadSchedulerJobs() {
		...
		try {
			schedulerJobs = await getHelixSchedulerJobs();
		} catch (e) {
	...
	async function loadModelOptions(refresh = false) {
		...
		try {
			const res = await getHelixAgentModelOptions(refresh);
	...
	async function loadAuthProviders() {
		...
		try {
			const res = await getHelixAuthProviders();
	...
	async function saveProviderToken(provider: string) {
		...
		try {
			await setHelixAuthProvider(provider, { api_key: token });
	...
	async function saveProviderBaseUrl(provider: string) {
		...
		setProviderError(provider, null);
		try {
			await setHelixAuthProvider(provider, { base_url: url });
			setProviderMessage(provider, 'Saved');
			await loadAuthProviders();
		} catch (e) {
	...
	async function testProvider(provider: string) {
		...
		setProviderError(provider, null);
		try {
			const res = await testHelixAuthProvider(provider);
			setProviderMessage(
				provider,
	...
	async function disconnectProvider(provider: string) {
		...
		setProviderError(provider, null);
		try {
			await deleteHelixAuthProvider(provider);
	...
	function schedulePoll(provider: string, state: string, intervalSeconds: number) {
		...
		POLL_TIMERS[provider] = setTimeout(async () => {
			try {
				const status = await pollHelixAuthProviderOAuth(provider, state);
	...
	async function startOAuth(provider: string) {
		...
		setProviderError(provider, null);
		try {
			const res = await startHelixAuthProviderOAuth(provider);
	...
	async function completeOAuth(provider: string) {
		...
		setProviderError(provider, null);
		try {
			await completeHelixAuthProviderOAuth(provider, {
				code: code || undefined,
				state: flow.state,
	...
	async function cancelOAuth(provider: string) {
		...
		if (flow?.state) {
			try {
				await cancelHelixAuthProviderOAuth(provider, flow.state);
			} catch {
				/* best-effort */
			}
		}
...
<div class="space-y-6">
	...
	>
		...
		{:else}
			{@const grouped = modelOptions.reduce<Record<string, HelixAgentModelOption[]>>(
				(acc, opt) => {
