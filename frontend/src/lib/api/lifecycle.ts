...
import { getHelixStrategiesQuery, promoteHelixStrategy } from './helix';
...
/**
 * Raw export envelope: the full container snapshot plus a `helix_export` meta
...
export type StrategyExportEnvelope = Record<string, unknown> & {
	helix_export?: Partial<StrategyExportMeta>;
};
...
export async function transitionStage(
	...
	const promoted = await promoteHelixStrategy(
		strategyId,
...
export async function getGraveyard(): Promise<{ active: ContainerStrategy[]; archived: ContainerStrategy[] }> {
	const rows = await getHelixStrategiesQuery();
...
export async function reviveFromGraveyard(strategyId: string): Promise<{ ok: boolean; strategy_id: string; from: string; to: string }> {
	const promoted = await promoteHelixStrategy(strategyId, 'researching', {
