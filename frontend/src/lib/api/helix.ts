...
// ============== Helix Classic Compatibility ==============

export interface HelixDashboardResponse {
...
export interface HelixRiskStatus {
...
export interface HelixRegimeSnapshot {
	[asset: string]: {
		regime?: string;
		confidence?: number;
		adx?: number;
...
export interface HelixEquityPoint {
	time: string;
	value: number;
	pnl: number;
	is_current?: boolean;
...
export interface HelixEquityHistory {
	base: number;
	curve: HelixEquityPoint[];
}

export interface HelixSentimentSnapshot {
	composite?: number;
	[key: string]: unknown;
}

export interface HelixTrade {
	id?: string;
	asset?: string;
	direction?: string;
	strategy?: string;
...
export interface HelixTradesPage {
	trades: HelixTrade[];
	total: number;
	limit: number;
	offset: number;
...
export interface HelixStrategyPerformance {
	strategy?: string;
	total_trades?: number;
	wins?: number;
	losses?: number;
...
export interface HelixAgentTask {
	id?: string | number;
	agent_id?: string;
	type?: string;
	title?: string;
...
export interface HelixSchedulerJob {
	id?: string | number;
	name?: string;
...
export interface HelixScannerState {
	strategies?: unknown[];
	[key: string]: unknown;
}
...
export interface HelixAgentDocumentsResponse {
	soul: string;
	agents: string;
	role: string;
}

export interface HelixAgentUpdatePayload {
	name?: string;
	role?: string;
	model?: string;
	model_id?: string | null;
...
export interface HelixAgentDocumentBody {
	content: string;
}

export interface HelixAgentDiscordTestResponse {
	status: string;
	agent_id: string;
	agent_name?: string;
	channel: string;
...
export interface HelixAgentModelOption {
	key: string;
...
export interface HelixAgentModelOptionsResponse {
	options?: HelixAgentModelOption[];
	providers?: Array<{
		provider: string;
...
export interface HelixAuthProviderStatus {
	provider: HelixProvider;
	configured: boolean;
	/**
...
export interface HelixAuthProviderProfilePayload {
	access_token?: string;
	api_key?: string;
...
export interface HelixAuthProviderOAuthStartResponse {
	provider: HelixProvider;
...
export type HelixAuthProviderOAuthStatus =
...
export interface HelixAuthProviderOAuthCompleteBody {
	code?: string;
	state: string;
	code_verifier?: string;
}

export interface HelixAuthProviderOAuthCompleteResponse {
	ok: boolean;
	provider: HelixProvider;
...
export interface HelixAuthProvidersResponse {
	providers: HelixAuthProviderStatus[];
	configure_command: string;
	status_command: string;
	auth_file: string;
}

export interface HelixModelPolicyFallbackEntry {
	provider: string;
	model_id: string;
}

export interface HelixModelPolicyResponse {
	primary_provider: 'minimax' | 'openai' | string;
	primary_model: string;
	provider_priority: string[];
	default_models: Record<string, string>;
	fallback_chains: Record<string, HelixModelPolicyFallbackEntry[]>;
}
 ...
 */
export interface HelixModelPolicyUpdatePayload {
...
export interface HelixAgentTerminalResponse {
	memory?: string;
	logs?: Array<{
		id?: number;
		level?: string;
...
export interface HelixBridgeStatus {
	available?: boolean;
	base_url?: string;
...
export async function getHelixStats(): Promise<Record<string, number>> {
	return fetchApi('/helix/stats');
}

export async function getHelixDashboard(): Promise<HelixDashboardResponse> {
	// Canonical endpoint. `/helix/dashboard` is a deprecated shim that proxies to
...
export async function getHelixRisk(): Promise<HelixRiskStatus> {
	// Canonical endpoint. `/helix/risk` is a deprecated shim that proxies to the
	// same control-plane handler and returns the identical shape.
	return fetchApi('/api/risk');
}

export async function getHelixRegime(): Promise<HelixRegimeSnapshot> {
	return fetchApi('/helix/regime');
}

export async function getHelixSentiment(): Promise<HelixSentimentSnapshot> {
	return fetchApi('/helix/sentiment');
}

export async function getHelixOpenTrades(): Promise<HelixTrade[]> {
	return fetchApi('/helix/trades/open');
}

export async function getHelixRecentTrades(limit = 20): Promise<HelixTrade[]> {
	return fetchApi(`/helix/trades/recent?limit=${limit}`);
}
...
export async function forceCloseHelixTrade(
	tradeId: string,
	...
	return fetchApi(`/helix/trades/${encodeURIComponent(tradeId)}/force-close`, {
		method: 'POST',
		body: JSON.stringify({ reason }),
	});
}

export async function getHelixAllTrades(
	opts: { status?: string; limit?: number; offset?: number } = {}
): Promise<HelixTradesPage> {
	const params = new URLSearchParams();
...
export async function markHelixTradeFailed(
	tradeId: string,
...
export async function getHelixScannerState(): Promise<HelixScannerState> {
	return fetchApi('/helix/scanner/state');
}

export async function getHelixStrategyPerformance(): Promise<HelixStrategyPerformance[]> {
	return fetchApi('/helix/strategies/performance');
}

export async function getHelixAgentTasks(): Promise<HelixAgentTask[]> {
	return fetchApi('/helix/agent-tasks');
}

export async function dismissHelixAgentTask(
...
export async function getHelixSchedulerJobs(): Promise<HelixSchedulerJob[]> {
	return fetchApi('/helix/scheduler');
}
...
export async function updateHelixSchedulerJob(
	jobId: string | number,
	scheduleType: string,
	scheduleExpr: string,
	enabled?: boolean
...
export async function getHelixAgents(): Promise<HelixAgent[]> {
	return fetchApi('/helix/agents');
}

export async function getHelixAgentDocuments(agentId: string): Promise<HelixAgentDocumentsResponse> {
	return fetchApi(`/helix/agents/${encodeURIComponent(agentId)}/documents`);
}

export async function updateHelixAgent(agentId: string, payload: HelixAgentUpdatePayload): Promise<HelixAgent> {
	return fetchApi(`/helix/agents/${encodeURIComponent(agentId)}`, {
		method: 'PATCH',
		body: JSON.stringify(payload),
	});
}

export interface HelixCreateStrategyDeveloperPayload {
	name: string;
	model?: HelixProvider;
	model_id?: string;
	instructions?: string;
...
export async function createHelixStrategyDeveloperAgent(
	payload: HelixCreateStrategyDeveloperPayload
): Promise<HelixAgent> {
	return fetchApi('/helix/agents/strategy-developers', {
		method: 'POST',
		body: JSON.stringify(payload),
	});
}

export async function deleteHelixAgent(agentId: string): Promise<{ ok: boolean; deleted_agent_id: string }> {
	return fetchApi(`/agents/${encodeURIComponent(agentId)}`, {
		method: 'DELETE',
	});
}

export async function updateHelixAgentDocument(agentId: string, document: 'soul' | 'agents' | 'role', content: string): Promise<{ ok: boolean }> {
	return fetchApi(`/helix/agents/${encodeURIComponent(agentId)}/documents/${document}`, {
		method: 'PUT',
		body: JSON.stringify({ content }),
	});
}

export async function testHelixAgentDiscord(
	agentId: string,
	discordToken?: string
): Promise<HelixAgentDiscordTestResponse> {
	const token = (discordToken ?? '').trim();
	return fetchApi(`/helix/agents/${encodeURIComponent(agentId)}/test-discord`, {
		method: 'POST',
		body: JSON.stringify(token ? { discord_token: token } : {}),
	});
}

export async function getHelixAgentModelOptions(refresh = false): Promise<HelixAgentModelOptionsResponse> {
	const params = refresh ? '?refresh=true' : '';
	return fetchApi(`/helix/agents/model-options${params}`);
}

export async function getHelixAuthProviders(): Promise<HelixAuthProvidersResponse> {
	return fetchApi('/helix/auth/providers');
}

export async function setHelixAuthProvider(
	provider: string,
	payload: HelixAuthProviderProfilePayload
): Promise<{ ok: boolean; provider: string }> {
	return fetchApi(`/helix/auth/providers/${encodeURIComponent(provider)}`, {
		method: 'POST',
		body: JSON.stringify(payload),
	});
}

export async function startHelixAuthProviderOAuth(provider: string): Promise<HelixAuthProviderOAuthStartResponse> {
	return fetchApi(`/helix/auth/providers/${encodeURIComponent(provider)}/oauth/start`, {
		method: 'POST'
	});
}

export async function completeHelixAuthProviderOAuth(
	provider: string,
	payload: HelixAuthProviderOAuthCompleteBody
): Promise<HelixAuthProviderOAuthCompleteResponse> {
	return fetchApi(`/helix/auth/providers/${encodeURIComponent(provider)}/oauth/complete`, {
		method: 'POST',
		body: JSON.stringify(payload)
	});
}

export async function pollHelixAuthProviderOAuth(
	provider: string,
	state: string
): Promise<HelixAuthProviderOAuthStatus> {
	return fetchApi(
		`/helix/auth/providers/${encodeURIComponent(provider)}/oauth/status?state=${encodeURIComponent(state)}`
	);
}

export async function cancelHelixAuthProviderOAuth(
	provider: string,
	state: string
): Promise<{ ok: boolean; provider: string }> {
	return fetchApi(
		`/helix/auth/providers/${encodeURIComponent(provider)}/oauth/cancel?state=${encodeURIComponent(state)}`,
		{ method: 'POST' }
	);
}

export async function deleteHelixAuthProvider(provider: string): Promise<{ ok: boolean; provider: string; removed?: boolean }> {
	return fetchApi(`/helix/auth/providers/${encodeURIComponent(provider)}`, {
		method: 'DELETE',
	});
}

export async function testHelixAuthProvider(provider: string): Promise<{ ok: boolean; provider: string; status: string; message?: string }> {
	return fetchApi(`/helix/auth/providers/${encodeURIComponent(provider)}/test`, {
		method: 'POST',
	});
}

export async function getHelixModelPolicy(): Promise<HelixModelPolicyResponse> {
	return fetchApi('/helix/model-policy');
}
 ...
 */
export async function updateHelixModelPolicy(
	payload: HelixModelPolicyUpdatePayload
): Promise<HelixModelPolicyResponse> {
	return fetchApi('/helix/model-policy', {
		method: 'PUT',
		body: JSON.stringify(payload),
	});
...
export async function updateHelixAgentModel(
	agentId: string,
	payload: { model: HelixProvider; model_id?: string }
): Promise<HelixAgent> {
	return fetchApi(`/helix/agents/${encodeURIComponent(agentId)}/model`, {
		method: 'PATCH',
		body: JSON.stringify(payload),
	});
}

export async function getHelixBacktestingStatus(): Promise<HelixBridgeStatus> {
	return fetchApi('/helix/backtesting/status');
}
...
export async function acknowledgeNotification(notificationId: number): Promise<{ ok: boolean; item: HelixNotification }> {
...
export async function resendNotification(notificationId: number): Promise<{ ok: boolean; item: HelixNotification }> {
...
): Promise<{ ok: boolean; item: HelixNotification }> {
...
export async function setHelixExecutionMode(mode: 'paper' | 'live' | 'mainnet'): Promise<{ ok: boolean; mode: string; previous: string }> {
	return fetchApi('/helix/execution-mode', {
		method: 'POST',
		body: JSON.stringify({ mode, confirm: true }),
	});
}

export async function triggerHelixEmergencyHalt(): Promise<{ ok: boolean; closed?: unknown[] }> {
	return fetchApi('/helix/emergency-halt', {
		method: 'POST',
		body: JSON.stringify({ confirm: true }),
	});
}

export async function resetHelixKillSwitch(): Promise<{ ok: boolean }> {
	return fetchApi('/helix/kill-switch/reset', {
		method: 'POST',
		body: JSON.stringify({ confirm: true }),
	});
}

export async function toggleHelixKillSwitch(enabled: boolean): Promise<{ ok: boolean; kill_switch_enabled: boolean }> {
	return fetchApi('/helix/kill-switch/toggle', {
		method: 'POST',
...
function resolveWsApiKey(): string {
	...
	try {
		const fromEnv = String((import.meta as { env?: Record<string, unknown> }).env?.VITE_HELIX_API_KEY ?? '').trim();
	...
	try {
		if (typeof window !== 'undefined') {
			return (window.localStorage.getItem('helix_api_key') ?? '').trim();
		}
	} catch {
		/* ignore */
	}
...
export function getHelixLiveWebSocketUrls(): string[] {
...
export function getHelixLiveWebSocketUrl(): string {
	const urls = getHelixLiveWebSocketUrls();
	return urls[0] ?? toLiveWsUrl(ACTIVE_API_BASE || API_BASE || '/api');
}

export async function getHelixEquityHistory(): Promise<HelixEquityHistory> {
	return fetchApi('/helix/equity-history');
}

export interface HelixLogEntry {
	level: string;
	source?: string | null;
	ts?: string | null;
	created_at?: string | null;
...
export async function getHelixLogs(limit = 100): Promise<HelixLogEntry[]> {
	return fetchApi(`/helix/logs?limit=${limit}`);
}

export async function getHelixAgentTerminal(agentId: string): Promise<HelixAgentTerminalResponse> {
	return fetchApi(`/agents/${encodeURIComponent(agentId)}/terminal`);
}

export async function getHelixEvolution(): Promise<Record<string, number>> {
	return fetchApi('/helix/evolution');
}

export async function getHelixStrategies(status?: string): Promise<Array<Record<string, unknown>>> {
	const params = status ? `?status=${encodeURIComponent(status)}` : '';
	return fetchApi(`/helix/strategies${params}`);
}

export interface HelixStrategyQuery {
	status?: string;
	owner?: string;
	limit?: number;
	offset?: number;
...
export async function getHelixStrategiesQuery(query: HelixStrategyQuery = {}): Promise<Array<Record<string, unknown>>> {
	const params = new URLSearchParams();
	...
	return fetchApi(`/helix/strategies${queryString ? `?${queryString}` : ''}`);
}
...
export async function promoteHelixStrategy(
	strategyId: string,
	toStatus: string,
	...
	const result = await fetchApi<{
		...
		to_status?: string;
		updated_at?: string;
	}>(`/helix/strategies/${encodeURIComponent(strategyId)}/promote`, {
		method: 'POST',
		body: JSON.stringify({
			to_status: toStatus,
...
export async function getSettings(): Promise<HelixSettings> {
...
export interface SystemHeartbeatResponse {
	dashboard: HelixDashboardResponse;
	risk: HelixRiskStatus;
	sentiment: HelixSentimentSnapshot;
	regime: HelixRegimeSnapshot;
	scanner_state: HelixScannerState;
	open_trades: HelixTrade[];
