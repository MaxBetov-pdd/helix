/**
 * Helix agent client (TypeScript) — a first-class typed client so the in-app
 * (Tauri) AI assistant or any browser/Node agent can drive the full Helix
 * strategy lifecycle over the same REST API the rest of the UI uses.
 *
 * This is the TS sibling of `helix/agent/client.py`. It reuses `fetchApi`
...
/** Quick-screen gate thresholds (mirrors helix/agent/client.py). */
...
export const HelixAgent = {
	...
	async runBacktest(strategyId: string, datasetId: string, opts: RunBacktestOpts = {}) {
		...
		return opts.compact ? HelixAgent.compactResult(res) : res;
	},
