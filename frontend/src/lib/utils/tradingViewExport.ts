...
function buildHeader(container: StrategyContainerPayload, params: Record<string, unknown>, run: StrategyContainerHistoryItem | null, source: PineExportSource): string {
	...
	return [
		'// ============================================================================',
		`// Helix TradingView export - ${container.strategy.id}`,
		'// ============================================================================',
		'// Paste this into TradingView Pine Editor, add it to the matching chart,',
		'// then compare Strategy Tester results against the Helix run below.',
		'//',
		...metricLines,
		`//   exporter       : ${source}`,
...
function buildWindow(run: StrategyContainerHistoryItem | null): string {
	...
	return [
		'use_window = input.bool(true, "Restrict to Helix verification window", group = "Date range")',
...
function strategyDeclaration(container: StrategyContainerPayload): string {
	return [
		'//@version=6',
		'strategy(',
		`     title              = "Helix ${pineString(container.strategy.id)} - ${pineString(container.strategy.name)}",`,
...
export function buildTradingViewExport(container: StrategyContainerPayload): TradingViewExport {
	...
	const pine = [
		...
		'alertcondition(buy_signal, title = "Helix BUY", message = "Helix BUY signal")',
		'alertcondition(sell_signal or window_forced_sell, title = "Helix SELL", message = "Helix SELL signal")',
		'bgcolor(in_window ? color.new(color.blue, 92) : na, title = "Helix verification window")',
		'',
	].join('\n');
	return {
