...
export function createRealtimeRefresh(
	...
	function syncFallbackPoller(): void {
		...
		const wsConnected = get(helixWsConnected);
		if (wsConnected) {
			stopFallbackPoller();
		} else {
			startFallbackPollerIfNeeded();
	...
	function start(): void {
		...
		if (typeof window !== 'undefined' && !wsEventHandler) {
			...
			window.addEventListener('helix:event', wsEventHandler);
		}

		if (typeof window !== 'undefined' && listenReconnect && !reconnectHandler) {
			reconnectHandler = () => scheduleRefresh();
			window.addEventListener('helix:reconnected', reconnectHandler);
		}
		...
		if (typeof window !== 'undefined' && !wsConnectedHandler) {
			...
			window.addEventListener('helix:connected', wsConnectedHandler);
		}

		if (typeof window !== 'undefined' && !wsDisconnectedHandler) {
			wsDisconnectedHandler = () => syncFallbackPoller();
			window.addEventListener('helix:disconnected', wsDisconnectedHandler);
		}
	...
	function stop(): void {
		...
		if (typeof window !== 'undefined' && wsEventHandler) {
			window.removeEventListener('helix:event', wsEventHandler);
			wsEventHandler = null;
		}

		if (typeof window !== 'undefined' && reconnectHandler) {
			window.removeEventListener('helix:reconnected', reconnectHandler);
			reconnectHandler = null;
		}

		if (typeof window !== 'undefined' && wsConnectedHandler) {
			window.removeEventListener('helix:connected', wsConnectedHandler);
			wsConnectedHandler = null;
		}

		if (typeof window !== 'undefined' && wsDisconnectedHandler) {
			window.removeEventListener('helix:disconnected', wsDisconnectedHandler);
