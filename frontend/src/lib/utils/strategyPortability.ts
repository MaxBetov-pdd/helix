...
export function buildExportFilename(displayId: string, name?: string): string {
	...
	const slug = (name || '')
		...
		.replace(/^-+|-+$/g, '')
		.slice(0, 48);
	return slug ? `helix-${id}-${slug}.json` : `helix-${id}.json`;
}
 ...
 * user-facing message when the payload is not a recognizable Helix export.
 */
export function parseEnvelope(text: string): ParsedEnvelope {
	...
	const meta = asRecord(env.helix_export);
	if (!env.helix_export || typeof env.helix_export !== 'object') {
		throw new Error('Missing "helix_export" metadata — this is not a Helix strategy export.');
	}
