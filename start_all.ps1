            ...
            Get-CimInstance Win32_Process -Filter "Name = 'python.exe'" -ErrorAction Stop |
                Where-Object {
                    ...
                    $cmd -match $escapedRepoRoot -and (
                        $cmd -match 'from helix\.bot import run_bot' -or
                        $cmd -match '-m\s+helix(\.cli)?\s+bot' -or
                        $cmd -match '-m\s+helix\s+bot'
                    )
                } |
        ...
        return @(
            Get-CimInstance Win32_Process -Filter "Name = 'python.exe'" -ErrorAction Stop |
                Where-Object {
                    $cmd = [string]$_.CommandLine
                    $cmd -match $escapedRepoRoot -and (
                        $cmd -match '-m\s+helix\s+daemon\s+start' -or
                        $cmd -match '-m\s+helix(\.cli)?\s+daemon\s+start'
                    )
                } |
                Select-Object -ExpandProperty ProcessId -Unique
        )
...
function Install-BackendDeps {
    ...
    try {
        ...
        & $Python -m pip uninstall -y helix | Out-Null
        ...
        # (it had a stale pandas pin and a spurious 'alembic' helix never imports).
        $declared = @(& $Python -m helix.preflight --print-deps 2>$null | Where-Object { $_ -and $_.Trim() })
...
function Test-BotTokenConfigured {
    ...
    $configPath = Join-Path $env:HELIX_HOME "config.json"
...
function Get-BotLockStatus {
    return Get-PythonLockStatus -ModuleName "helix.bot" -FunctionName "get_bot_lock_status"
}

function Get-DaemonLockStatus {
    return Get-PythonLockStatus -ModuleName "helix.daemon" -FunctionName "get_daemon_lock_status"
}
...
function Get-LabWorkerStatus {
    try {
        $json = & $python -c "import json; from helix.lab_worker_service import get_lab_worker_status; print(json.dumps(get_lab_worker_status()))" 2>$null
        if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace(($json -join ""))) {
            return $null
        }
...
function Start-BackendService {
    ...
    $proc = Start-LoggedProcess -FilePath $python -CommandArgs @("-m","uvicorn","--app-dir",$script:RepoRoot,"helix.api:app","--host",$backendHost,"--port",$backendPort.ToString(),"--workers",$backendWorkers.ToString()) `
...
function Ensure-BotService {
    ...
    if ($forceRestart -eq "0" -and $lockHealthy) {
        Write-Info "Reusing healthy Helix bot."
    ...
    if ($rootPids.Count -gt 0) {
        try {
            ...
            Write-WarnMessage "Could not stop existing Helix bot; leaving it alone so the app can still start. $($_.Exception.Message)"
    ...
    Write-Info "Starting Helix bot ..."
    $proc = Start-LoggedProcess -FilePath $python -CommandArgs @("-m","helix","bot","start") `
...
function Ensure-DaemonService {
    ...
    if ($forceRestart -eq "0" -and $lockHealthy) {
        Write-Info "Reusing healthy Helix daemon."
    ...
    if ($rootPids.Count -gt 0) {
        try {
            ...
            Write-WarnMessage "Could not stop existing Helix daemon; leaving it alone so the app can still start. $($_.Exception.Message)"
    ...
    Write-Info "Starting Helix daemon (data/risk loop) ..."
    $proc = Start-LoggedProcess -FilePath $python -CommandArgs @("-m","helix","daemon","start") `
...
function Ensure-LabWorkerService {
    ...
    $proc = Start-LoggedProcess -FilePath $python -CommandArgs @("-m","helix","lab","worker") `
...
function Stop-AllHelixProcesses {
    # Kill live Helix services tied to this repo before clearing stale lock files.
    ...
    $helixHome = if ($env:HELIX_HOME) { $env:HELIX_HOME } else { Join-Path $env:USERPROFILE ".helix" }
    foreach ($lockName in @("bot.lock", "daemon.lock")) {
        $lockPath = Join-Path $helixHome $lockName
...
$backendPort = if ([string]::IsNullOrWhiteSpace($env:HELIX_PORT)) { 8003 } else { [int]$env:HELIX_PORT }
$backendHost = if (-not [string]::IsNullOrWhiteSpace($env:HELIX_BIND_HOST)) {
    $env:HELIX_BIND_HOST.Trim()
} elseif (-not [string]::IsNullOrWhiteSpace($env:HELIX_HOST)) {
    $env:HELIX_HOST.Trim()
} else {
    "127.0.0.1"
}
...
$regimeLabEnabledRaw = if ($env:HELIX_ENABLE_REGIME_LAB) { $env:HELIX_ENABLE_REGIME_LAB.Trim() } else { "0" }
$regimeLabEnabled = @("1", "true", "yes", "on") -contains $regimeLabEnabledRaw.ToLowerInvariant()
$env:HELIX_ENABLE_REGIME_LAB = if ($regimeLabEnabled) { "1" } else { "0" }
if ([string]::IsNullOrWhiteSpace($env:VITE_ENABLE_REGIME_LAB)) {
    $env:VITE_ENABLE_REGIME_LAB = $env:HELIX_ENABLE_REGIME_LAB
}
...
if ([string]::IsNullOrWhiteSpace($env:HELIX_HOME)) {
    $env:HELIX_HOME = Join-Path $env:USERPROFILE ".helix"
    Write-Info "HELIX_HOME not set. Defaulting to $($env:HELIX_HOME)"
}
try {
    New-Item -Path $env:HELIX_HOME -ItemType Directory -Force -ErrorAction Stop | Out-Null
    $homeProbe = Join-Path $env:HELIX_HOME ".helix_write_probe"
    ...
    $fallbackHome = Join-Path $script:RepoRoot ".helix_home"
    Write-WarnMessage "Could not use HELIX_HOME '$($env:HELIX_HOME)': $($_.Exception.Message)"
    Write-WarnMessage "Falling back to $fallbackHome"
    $env:HELIX_HOME = $fallbackHome
    New-Item -Path $env:HELIX_HOME -ItemType Directory -Force -ErrorAction Stop | Out-Null
}
...
$botLog = Join-Path $logRoot "helix_bot.log"
$botErr = Join-Path $logRoot "helix_bot.err.log"
$labWorkerLog = Join-Path $logRoot "helix_lab_worker.log"
$labWorkerErr = Join-Path $logRoot "helix_lab_worker.err.log"
$daemonLog = Join-Path $logRoot "helix_daemon.log"
$daemonErr = Join-Path $logRoot "helix_daemon.err.log"

$backendHealth = "http://127.0.0.1:$backendPort/api/health"
$frontendRoot = "http://127.0.0.1:$frontendPort/"
if ([string]::IsNullOrWhiteSpace($env:HELIX_CLIENT_BASE)) { $env:HELIX_CLIENT_BASE = "http://127.0.0.1:$backendPort" }
...
Invoke-Checked -FilePath $python -CommandArgs @("-c","from helix.db import init_db; init_db(); print('db_initialized')")
...
Invoke-Checked -FilePath $python -CommandArgs @("-c","from helix.bot import seed_default_agents; r = seed_default_agents(); print('agents_seeded created=' + str(len(r['created'])) + ' updated=' + str(len(r['updated'])) + ' total=' + str(r['total']))")
...
try {
    ...
    if (-not [bool]$watchdogClaim.claimed) {
        ...
        if ($null -ne $watchdogStatus -and [bool]$watchdogStatus.other_process_active) {
            Write-WarnMessage "Another Helix watchdog owner is already active ($activeOwnerName PID $activeOwnerPid); leaving services untouched."
        ...
        if ($null -ne $watchdogStatus -and [bool]$watchdogStatus.lock_held) {
            Write-WarnMessage "Another Helix watchdog owner appears to be active, but its owner metadata is unavailable; leaving services untouched."
            Add-StartupSummary -Service "watchdog" -Action "skipped" -Details "owner=unknown pid=unknown"
            Write-StartupSummary
            $startupCompleted = $true
            return
        }
        Throw-StartAllError "Could not acquire the Helix watchdog owner lock."
    }
    $script:WatchdogOwnerLockHeld = $true
    Add-StartupSummary -Service "watchdog" -Action "claimed" -Details "owner=start_all pid=$PID"

    # Always kill all existing Helix processes for a clean start
    Write-Info "Stopping all existing Helix processes..."
    Stop-AllHelixProcesses

    Write-Info "Starting Helix services..."
    ...
    # restart this session" — see helix/bot.py run_bot() LoginFailure handler.
    $script:ServiceState = @{}
                ...
                if (Test-CanRestartService -Name "daemon") {
                    ...
                    try {
                        $daemonProc = Start-LoggedProcess -FilePath $python `
                            -CommandArgs @("-m","helix","daemon","start") `
                            -WorkingDirectory $script:RepoRoot -StdOutPath $daemonLog -StdErrPath $daemonErr
