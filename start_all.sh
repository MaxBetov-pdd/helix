...
BACKEND_PORT="${HELIX_PORT:-8003}"
BACKEND_HOST="${HELIX_BIND_HOST:-${HELIX_HOST:-127.0.0.1}}"
BACKEND_WORKERS="${BACKEND_WORKERS:-2}"
FRONTEND_PORT="${FRONTEND_PORT:-5173}"
START_BOT="${START_BOT:-1}"
HELIX_ENABLE_REGIME_LAB="${HELIX_ENABLE_REGIME_LAB:-0}"
case "${HELIX_ENABLE_REGIME_LAB,,}" in
	1|true|yes|on) HELIX_ENABLE_REGIME_LAB="1" ;;
	*) HELIX_ENABLE_REGIME_LAB="0" ;;
esac
export HELIX_ENABLE_REGIME_LAB
export VITE_ENABLE_REGIME_LAB="${VITE_ENABLE_REGIME_LAB:-$HELIX_ENABLE_REGIME_LAB}"
START_LAB_WORKER="${START_LAB_WORKER:-$HELIX_ENABLE_REGIME_LAB}"
START_DAEMON="${START_DAEMON:-0}"
FORCE_RESTART="${FORCE_RESTART:-1}"
DB_PATH="${HELIX_DB_PATH:-$DIR/data/helix.db}"
TMP_DIR="$DIR/.tmp"
...
BOT_LOG="$LOG_DIR/helix_bot.log"
LAB_WORKER_LOG="$LOG_DIR/helix_lab_worker.log"
DAEMON_LOG="$LOG_DIR/helix_daemon.log"
...
HELIX_HEALTH_URL="http://127.0.0.1:${BACKEND_PORT}/api/health"

export HELIX_CLIENT_BASE="${HELIX_CLIENT_BASE:-http://127.0.0.1:${BACKEND_PORT}}"
...
kill_stale_bot() {
	local lock_file="${HELIX_HOME:-$HOME/.helix}/bot.lock"
...
get_lab_worker_status_line() {
	python3 -c 'from helix.lab_worker_service import get_lab_worker_status; s = get_lab_worker_status(); w = s.get("worker") or {}; print(f"{1 if s.get(\"active\") else 0}|{w.get(\"pid\") or \"\"}")' 2>/dev/null || true
}
...
BACKEND_MODULE="helix.api:app"
if ! module_available "helix.api"; then
...
if ! python3 -m helix.preflight; then
...
if [[ "$START_BOT" == "1" ]]; then
	info "Starting Helix Discord Bot..."
	BOT_MODULE="helix.bot"
	if ! module_available "$BOT_MODULE"; then
		die "Required bot module not found: ${BOT_MODULE}"
	fi
	python3 -c "from helix.bot import run_bot; run_bot()" > "$BOT_LOG" 2>&1 &
	BOT_PID=$!
	PIDS+=("$BOT_PID")
	sleep 1
...
if [[ "$START_LAB_WORKER" == "1" ]]; then
	...
	if [[ "$FORCE_RESTART" != "1" && "$active" == "1" && "$pid" =~ ^[0-9]+$ ]]; then
		...
		python3 -m helix lab worker > "$LAB_WORKER_LOG" 2>&1 &
		LAB_WORKER_PID=$!
		PIDS+=("$LAB_WORKER_PID")
		sleep 2
		status_line="$(get_lab_worker_status_line)"
...
if [[ "$START_DAEMON" == "1" ]]; then
	info "Starting Helix daemon (data/risk loop)..."
	DAEMON_MODULE="helix"
...
if ! wait_for_http "$HELIX_HEALTH_URL" "Backend"; then
	tail -n 80 "$BACKEND_LOG" || true
