...
#   schtasks /create /tn "HelixWatchdog" /tr "powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File C:\path\to\Helix\watchdog.ps1" /sc minute /mo 2 /rl highest
...
$BackendPort = 8003
if (-not [string]::IsNullOrWhiteSpace($env:HELIX_PORT)) { $BackendPort = [int]$env:HELIX_PORT }
...
try {
    ...
    if ($backendListeners.Count -eq 0 -or -not $backendHealthy) {
        ...
        $proc = Start-Process -FilePath $python `
            -ArgumentList @("-m","uvicorn","--app-dir",$RepoRoot,"helix.api:app","--host",$BackendHost,"--port",$BackendPort.ToString(),"--workers","1") `
...
$botLockFile = Join-Path $env:HELIX_HOME "bot.lock"
...
if (-not $botAlive) {
    ...
    if ($tokenOk) {
        ...
        $proc = Start-Process -FilePath $python -ArgumentList "-c `"from helix.bot import run_bot; run_bot()`"" `
...
# --- Check Daemon ---
$daemonAlive = $false
$daemonLockFile = Join-Path $env:HELIX_HOME "daemon.lock"
...
if (-not $daemonAlive) {
    try {
        $daemonProcs = Get-CimInstance Win32_Process -Filter "Name='python.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -match "helix.*daemon" }
...
if (-not $daemonAlive) {
    ...
    $proc = Start-Process -FilePath $python -ArgumentList @("-m","helix","daemon","start") `
...
$regimeLabFlag = if (-not [string]::IsNullOrWhiteSpace($env:HELIX_ENABLE_REGIME_LAB)) { $env:HELIX_ENABLE_REGIME_LAB.Trim().ToLowerInvariant() } else { "" }
...
if (-not $labWorkerAlive -and $regimeLabEnabled) {
    ...
    $labPidFile = Join-Path (Join-Path $env:HELIX_HOME "lab") "lab_worker.pid"
    ...
    $proc = Start-Process -FilePath $python -ArgumentList @("-m","helix","lab","worker") `
...
from helix.lab_db import get_lab_meta
