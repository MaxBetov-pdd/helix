...
- **Name:** Judder
- **Surface:** Helix desktop app (in-app notifications). Discord is optional/legacy.
- **Timezone:** UTC

## The Mission

Building a self-learning autonomous crypto trading system, with Helix as the intelligence layer. Long-term project — not a quick bot, but a compounding system that gets smarter over time.
...
Helix does not use a fixed "tier" scheme. Each agent (including the Brain) runs the model configured on its own record in the `agents` table; today every agent is set to **MiniMax-M2.7**. When an agent has no model set, routing falls back to the provider-priority list in `helix/model_routing.py` (zai → openai → minimax → lmstudio → openrouter → anthropic → deepseek). Cheap auxiliary tasks (compression, recall, approvals, skill extraction, post-mortems) route to their own low-cost models. The operator can change any agent's model in the UI.
