...
## How Helix Actually Runs

Helix is a **Tauri desktop app**. The Rust shell spawns the embedded Python backend as `python -m helix.api`. **There are no 24/7 OS services, no systemd units, no cron jobs, no scheduled tasks.** Everything is gated on the app being open:
...
Do **not** try to `systemctl`, `sc start`, or otherwise "start a service" — there are none, and on Windows those commands don't apply. To run Helix, open the app (or, for dev, `start_all.ps1`).
...
- `python -m helix daemon start` — run the data/risk daemon standalone.
- `python -m helix bot start` — run the optional Discord bot (legacy).
