...
Helix's source and project state live in a local git repo on this machine. Trading data and secrets are **not** committed.

## What is covered
- Source code and project state tracked in git.
- Secrets excluded via `.gitignore` (`.env`, keys, certs, `*.db`, the `.helix_home/` workspace).
...
## Safety checks
- Never commit `.env`, `*.db`, auth tokens, or anything under `.helix_home/`.
