...
Helix is self-hosted software that holds **your** exchange (testnet) and LLM
...
- **Alternatively:** email **MaxBetov-pdd@helix.app**.
...
- **Keep the API on localhost.** It is unauthenticated by default. If you set
  `HELIX_BIND_HOST` to a non-loopback address, the app requires `HELIX_API_KEY`
  (and ideally `HELIX_OPERATOR_KEY`) or it refuses to start. Never expose it to
  the internet without auth and a TLS-terminating proxy.
- **The agent shell tool is disabled by default** (`HELIX_ENABLE_SHELL_TOOL=0`).
