...
def _require_api_key(x_api_key: str = Header(default="")):
    ...
    server_key = os.environ.get("HELIX_COMPUTE_API_KEY", "").strip()
    if not server_key:
...
REMOTE_DATA_ROOT_ENV = "HELIX_REMOTE_ENGINE_DATA_ROOT"
DEFAULT_REMOTE_DATA_ROOT = "D:/HelixData/ohlcv"
...
app = FastAPI(
    title="Helix Compute Engine & Data Lake API",
    description="Dedicated execution and storage environment for Helix backtests.",
    version="1.0.0"
)
...
def _dataset_from_file(path: Path, fs_symbol: str, timeframe: str) -> dict[str, Any]:
    ...
    source_raw = keyvals.get(b"helix_source", b"remote")
...
async def get_market_data(
    ...
    # df = pd.read_parquet(f"D:/HelixData/{symbol}/{timeframe}.parquet")
...
async def run_remote_backtest(request: BacktestRequest):
    ...
    Future real execution should use the subprocess sandbox in helix/sandbox.py.
    """
...
if __name__ == "__main__":
    print("-" * 60)
    print("🚀 HELIX COMPUTE ENGINE & DATA LAKE STARTED")
