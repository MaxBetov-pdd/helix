...
During the multi-day soak the active gate store (``helix:pipeline_thresholds``)
...
the stale legacy ``helix:pipeline_thresholds`` key that nothing reads.
...
from helix.policy import DEFAULT_PIPELINE_CONFIG, load_pipeline_config, save_pipeline_config

ACTIVE_KEY = "helix:pipeline_thresholds"
LEGACY_KEY = "helix:pipeline_thresholds"
...
def main() -> None:
    ...
    # Drop the stale legacy key (load_pipeline_config reads the helix: key only).
