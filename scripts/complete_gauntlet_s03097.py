...
if __name__ == '__main__':
    import sqlite3
    from helix.config import HELIX_DB
    from helix.strategies.backtest import walk_forward, backtest_strategy
    ...
    kv = conn.execute("SELECT value FROM kv WHERE key='helix:pipeline_thresholds'").fetchone()
    ...
    conn.execute("UPDATE kv SET value=? WHERE key='helix:pipeline_thresholds'", (json.dumps(cfg),))
    conn.commit()
    ...
    from helix.policy import load_pipeline_config, _evaluate_gauntlet_gate
