...
async def _run_all_bots():
    ...
    try:
        ...
        for agent in agents:
            agent_bot = HelixBot(agent_id=agent["id"])
...
content = re.sub(
    ...
    "    try:\n        log.info(\"Starting Helix gateway (bot + scheduler + task processor) + agent bots\")\n        asyncio.run(_run_all_bots())",
    content
)
