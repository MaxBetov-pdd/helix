"""One-shot cutover: merge ~/.helix into ~/.helix and (optionally) delete ~/.helix.

Background: the pre-rename home was `~/.helix/` and used DB files named
`helix.db` / `helix_lab.db`. The canonical home is `~/.helix/` with
`helix.db` / `helix_lab.db`. The automatic migration in `helix.config`
refuses to overwrite files that already exist in the canonical home, so a
freshly-initialized empty `helix.db` blocks the real data from being picked
up.

This script does the cutover explicitly:

  1. Copies DB files with renames (helix.db -> helix.db, helix_lab.db ->
     helix_lab.db). If the canonical DB already exists it is backed up as
     `<name>.pre-helix-migration.bak` before being replaced. Replacement
     only happens when the legacy DB has at least as many strategies as the
     canonical one (never silently discards newer data).
  2. Merges every other file/dir from `~/.helix/` into `~/.helix/`,
  ...
  3. With `--cleanup`, removes `~/.helix/` after a successful merge.
     Without it, `~/.helix/` is left intact so you can verify first.

Usage:
    # 0. Stop the Helix backend (it holds helix.db open).
    python scripts/migrate_helix_home.py            # dry-safe: no delete
    python scripts/migrate_helix_home.py --cleanup  # also removes ~/.helix
...
HOME = Path.home()
LEGACY = HOME / ".helix"
CANONICAL = HOME / ".helix"

DB_RENAMES = {
    "helix.db": "helix.db",
    "helix.db-journal": "helix.db-journal",
    "helix.db-wal": "helix.db-wal",
    "helix.db-shm": "helix.db-shm",
    "helix_lab.db": "helix_lab.db",
    "helix_lab.db-journal": "helix_lab.db-journal",
    "helix_lab.db-wal": "helix_lab.db-wal",
    "helix_lab.db-shm": "helix_lab.db-shm",
}
...
def _backup_and_replace(src: Path, dst: Path) -> None:
    if dst.exists():
        backup = dst.with_suffix(dst.suffix + ".pre-helix-migration.bak")
        if backup.exists():
            backup.unlink()
        dst.rename(backup)
...
def main() -> int:
    ...
    parser.add_argument("--cleanup", action="store_true", help="Remove ~/.helix after a successful merge.")
    ...
    if args.cleanup:
        if not replaced:
            print("\n--cleanup requested but no DB was replaced; refusing to delete ~/.helix.")
    ...
    print("\nDone. Restart the Helix backend, then reload The Forge.")
    return 0
