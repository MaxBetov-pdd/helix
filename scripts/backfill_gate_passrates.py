...
*reads* strategies and *re-evaluates* the gate functions from helix.policy
...
Run from the repo root so the `helix` package imports:
...
# helix.util.normalize_stage (get_strategies already normalizes s["stage"]).
STAGE_TO_GATE = {
...
def main() -> int:
    ...
    try:
        ...
        print("FATAL: failed to import helix backend modules.", file=sys.stderr)
        print(
            "Run this from the repo root with the project's python so the "
            "`helix` package is importable, e.g.:",
            file=sys.stderr,
        )
    ...
    # ---------------------------------------------------------------- output
    print("=" * 88)
    print("HELIX GATE PASS-RATE BACKFILL  (READ-ONLY - no DB/KV writes, no promotions)")
    print("=" * 88)
