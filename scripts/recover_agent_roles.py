"""Recover missing or overwritten agent ROLE.md files from the unified Helix workspace.
...
- It mirrors recovered personas into the workspace:
  `~/.helix/workspace/agents/*/ROLE.md`.
- It updates `name` and `instructions` columns in `~/.helix/helix.db`.
  ...
  helix/brain.py, and overwriting it with ROLE.md prose silently removes
...
ROOT = Path.home()
TARGET_WORKSPACES = [ROOT / ".helix", ROOT / ".judex"]
TARGET_DBS = [
    ROOT / ".helix" / "helix.db",
    ROOT / ".judex" / "helix.db",
]
