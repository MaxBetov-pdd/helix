...
from helix.config import HELIX_DB, HELIX_HOME, ensure_dirs  # noqa: E402
from helix.db import get_db, init_db  # noqa: E402
from helix.vectordb import CHROMA_DIR, wipe_collections  # noqa: E402
...
def _default_backup_dir() -> Path:
    return HELIX_HOME / "backups" / f"container-reset-{_utc_stamp()}"
...
def backup_state(backup_dir: Path) -> dict[str, str | None]:
    ...
    db_backup_path = backup_dir / HELIX_DB.name
    _backup_sqlite(HELIX_DB, db_backup_path)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional backup directory. Default: <HELIX_HOME>/backups/container-reset-<UTC timestamp>",
    )
