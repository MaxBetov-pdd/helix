...
- Hyperliquid testnet held an open BTC long while Helix showed `0` open positions.
...
- [x] Add a startup recovery preflight in `helix.daemon.run()` before entering the main loop.
...
- [x] After restart, any live Hyperliquid position is visible in Helix within seconds.
