...
Helix is a local-first workspace for building, validating, supervising, and operating algorithmic trading strategies.

Core pieces:

- A FastAPI backend on `http://127.0.0.1:8003`
- A SvelteKit frontend on `http://127.0.0.1:5173`
- SQLite state stored under `HELIX_HOME`
- ChromaDB-backed memory and workspace context
- CCXT-based exchange adapters under `helix/exchange/`

Helix keeps research, approvals, runtime monitoring, and operational controls in one place instead of splitting them across notebooks, bots, and dashboards.
...
```powershell
python -m helix --help
python -m helix configure
python -m helix auth status
```
...
These map to the policy gates managed in `helix/policy.py`.
...
- If API keys are configured for browser access, pair `HELIX_API_KEY` with `VITE_HELIX_API_KEY`, and pair `HELIX_OPERATOR_KEY` with `VITE_HELIX_OPERATOR_KEY`.
- The live exchange code lives under `helix/exchange/` and should be treated as sensitive.
...
- `python -m helix` is the CLI entrypoint, not the backend server.
- If the frontend is up but API calls fail, verify `http://127.0.0.1:8003/api/health` first.
- If auth or workspace state seems missing, check the active `HELIX_HOME` path.
