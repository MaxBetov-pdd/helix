...
1. Backup `helix.db` and `chromadb/`.
...
```powershell
.venv\Scripts\python.exe scripts\reset_strategy_containers.py --yes --backup-dir C:\tmp\helix-reset-backup
```
...
```powershell
@'
from helix.vectordb import get_client
client = get_client()
...
1. Stop services.
2. Copy backup DB file back to `HELIX_HOME\helix.db`.
3. Replace `HELIX_HOME\chromadb` with the backup copy.
4. Restart services.
