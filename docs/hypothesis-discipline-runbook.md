...
1. **The agent isn't trying.** Check `helix-hypothesis-promotion-loop` is
...
3. Is the `helix-hypothesis-revisit-pass` job enabled? It's registered in a
...
Enable:
- `helix-hypothesis-promotion-loop` (5-min interval)
- `helix-hypothesis-verdict-loop` (5-min interval)
- `helix-hypothesis-revisit-pass` (daily — batch sweep still short-circuits
