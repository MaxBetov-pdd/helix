...
```bash
HELIX_GITHUB_WEBHOOK_SECRET=<long-random-secret>
HELIX_GITHUB_WEBHOOK_BRANCH=feature/quant-factory-upgrade
HELIX_GITHUB_WEBHOOK_REMOTE=origin
HELIX_GITHUB_WEBHOOK_REPO=/home/trestor/helix
HELIX_GITHUB_WEBHOOK_POST_PULL_CMD=
```

Optional post-pull command example:

```bash
HELIX_GITHUB_WEBHOOK_POST_PULL_CMD=systemctl --user restart helix-stack.service
```
...
4. Secret: same value as `HELIX_GITHUB_WEBHOOK_SECRET`
...
- Only push events for `HELIX_GITHUB_WEBHOOK_BRANCH` trigger pull.
- Updates run with:
