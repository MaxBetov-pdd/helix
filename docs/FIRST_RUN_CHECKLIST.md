...
Use this checklist when cloning Helix onto a new machine.
...
- [ ] `git clone https://github.com/MaxBetov-pdd659/Helix.git`
- [ ] `cd Helix`
...
- [ ] Review `.env.example`
- [ ] Keep `HELIX_EXECUTION_MODE=paper`
- [ ] Set `HELIX_ENCRYPTION_KEY` if you want portable encrypted secrets
- [ ] Set `HELIX_API_KEY` and `HELIX_OPERATOR_KEY` if the API will be used beyond localhost
- [ ] Log in to AI providers as needed:

```powershell
python -m helix configure
python -m helix auth login openai
python -m helix auth login minimax
...
- [ ] Runtime state path: `~/.helix` unless `HELIX_HOME` is set
- [ ] Remember that `python -m helix` opens the CLI; backend-only startup uses `python -m uvicorn --app-dir . helix.api:app`
