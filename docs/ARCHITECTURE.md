...
Helix is a local-first trading operations framework with three main layers:

1. A SvelteKit frontend for operator workflows
2. A FastAPI backend for APIs, orchestration, and compatibility routes
3. Local persistence and workspace state centered on SQLite and `HELIX_HOME`
...
graph TD
    ...
    API --> DB[("SQLite")]
    API --> MEM[("ChromaDB")]
    API --> WORK["workspace files under HELIX_HOME"]
...
The FastAPI app is assembled in `helix/api.py`.

Key app responsibilities:

- Register routers
- Apply CORS
- Apply `HelixV1CompatMiddleware`
- Run startup initialization through `helix.api_core._on_startup`
...
- `helix/routers/`: HTTP and websocket route definitions
- `helix/control_plane/`: operator-facing runtime behavior such as status, ops, approvals, and notifications
- `helix/api_domains/`: extracted API-facing domain logic and compatibility helpers
- `helix/strategies/`: strategy base classes, registry, optimization, and backtesting helpers
- `helix/api_core.py`: shared compatibility, startup, and legacy behavior that has not yet been fully extracted

### Compatibility layer

`HelixV1CompatMiddleware` and the legacy routes keep older `/api/helix/*` clients working while the current frontend and newer APIs use the newer route layout.
...
The core implementation lives in `helix/db.py`.

### Workspace and auth state

Runtime state defaults to `~/.helix` unless `HELIX_HOME` is set. Important files and directories include:
...
- Treat `helix/exchange/` as sensitive integration code
