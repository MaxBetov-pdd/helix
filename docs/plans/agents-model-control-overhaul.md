...
Choosing models in Helix is confusing and **unsafe**: the bot routinely calls
...
`helix/auth/store.py` (env keys at lines 38–46, `credential_status` ~496),
`helix/api_core.py` (the auth-provider Test/Save path that becomes "Connect").
...
`helix/model_routing.py` (defaults at 55–157; selectors at 361–539).
...
only from here. New code in `helix/ai.py` / `helix/model_routing.py`.
...
models.) Note for current systems: the persisted KV `helix:model-routing`
...
`helix:provider-health:{provider}` = `{state: ok|degraded|down, kind:
...
(`HELIX_BOT_OWNS_RUNTIME` unset; the Discord bot's gateway‑only branch at
...
4. Startup seeding creates global IDENTITY.md + per‑agent SOUL/AGENTS/ROLE.md for
   all 7 built‑ins (temp `HELIX_HOME`).
...
- **Routing/safety:** `helix/model_routing.py`, `helix/ai.py`,
  `helix/agents/runner.py`, `helix/agents/providers.py`, `helix/auth/store.py`,
  `helix/api_core.py`; offenders in `jobs/daily_learning.py`,
...
- **Surfacing:** `helix/health_monitor.py`, `helix/routers/health.py`,
  `helix/agents/provider_health.py`, `frontend/.../+layout.svelte`,
...
- **Identity files:** `helix/workspace.py`, `helix/agents/manager.py`,
  `helix/bot.py`, `helix/api.py`, `helix/context.py`, `helix/api_core.py`,
