...
from helix.policy import _evaluate_paper_gate, DEFAULT_PIPELINE_CONFIG
...
def test_oos_sharpe_much_higher_than_is_sharpe_rejects(helix_db):
...
def test_oos_sharpe_within_ratio_passes(helix_db):
...
def test_oos_sharpe_check_skipped_when_is_zero(helix_db):
...
def test_profit_factor_below_1_5_rejects(helix_db):
...
def test_profit_factor_between_1_5_and_2_0_passes_with_size_reduction(helix_db):
...
def test_profit_factor_above_2_0_passes_normally(helix_db):
...
def test_insufficient_paper_trades_rejects(helix_db):
...
def test_insufficient_paper_sample_precedes_static_pf_reject(helix_db):
...
def test_exactly_50_trades_passes(helix_db):
...
def test_negative_paper_return_rejects(helix_db):
...
def test_zero_paper_return_rejects(helix_db):
...
def test_paper_drawdown_exceeding_limit_rejects(helix_db):
...
def test_missing_strategy_rejects(helix_db):
...
def test_oos_profit_factor_used_when_available(helix_db):
...
def test_general_pf_fallback_when_no_oos(helix_db):
