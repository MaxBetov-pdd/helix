...
def _setup_isolated_auth(tmp_path, monkeypatch):
    ...
    from helix import db as helix_db_mod
    from helix import secret_storage
    from helix.auth import store as auth_store

    auth_file = tmp_path / "auth.json"
    key_file = tmp_path / ".helix_key"
    monkeypatch.setenv("HELIX_ENCRYPTION_KEY", Fernet.generate_key().decode("utf-8"))
    monkeypatch.setattr(cfg, "AUTH_FILE", auth_file)
    monkeypatch.setattr(cfg, "HELIX_HOME", tmp_path)
    monkeypatch.setattr(auth_store, "AUTH_FILE", auth_file)
    monkeypatch.setattr(auth_store, "LOCK_PATH", auth_file.with_suffix(".lock"))
    monkeypatch.setattr(helix_db_mod, "AUTH_FILE", auth_file)
    monkeypatch.setattr(helix_db_mod, "HELIX_HOME", tmp_path)
...
def test_factory_reset_defaults_preserve_credentials(helix_db, tmp_path, monkeypatch):
    ...
    result = helix_db_mod.factory_reset()  # None -> use default_keep
...
def test_factory_reset_explicit_empty_protects_credentials_by_default(helix_db, tmp_path, monkeypatch):
    ...
    result = helix_db_mod.factory_reset([])  # explicit wipe-all, but NO opt-in
...
def test_factory_reset_wipes_credentials_only_with_explicit_optin(helix_db, tmp_path, monkeypatch):
    ...
    result = helix_db_mod.factory_reset([], allow_credentials_wipe=True)
