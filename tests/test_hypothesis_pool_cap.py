...
def test_create_hypothesis_succeeds_below_cap(helix_db):
    _set_cap(5)
    for i in range(4):
        _make_hyp(i)
...
def test_at_cap_evicts_weakest_and_admits_new(helix_db):
...
def test_eviction_prefers_fewest_strategies_over_age(helix_db):
...
def test_disproven_hypotheses_do_not_count_against_cap(helix_db):
    _set_cap(5)
...
def test_proven_hypotheses_do_not_count_against_cap(helix_db):
    _set_cap(3)
...
def test_archived_hypotheses_do_not_count_against_cap(helix_db):
    _set_cap(3)
    a, _b, _c = (_make_hyp(i) for i in range(3))
    with get_db() as conn:
        conn.execute(
...
def test_default_cap_is_one_hundred(helix_db):
...
def test_derived_from_parent_is_protected_from_eviction(helix_db):
...
def test_protected_hypothesis_is_not_pool_pressure_victim(helix_db):
    _set_cap(2)
...
def test_pool_full_error_still_raised_if_no_eviction_possible(helix_db, monkeypatch):
...
def test_pool_full_error_if_defensive_eviction_does_not_archive(helix_db, monkeypatch):
