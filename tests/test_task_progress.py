"""Tests for helix.task_progress — checkpointing and resumable tasks."""
...
from helix.task_progress import (
    INTERRUPTED_STATUS,
    checkpoint,
    clear_checkpoints,
    list_checkpoints,
...
def test_checkpoint_and_read_roundtrip(helix_db):
...
def test_checkpoint_upserts_on_conflict(helix_db):
...
def test_read_missing_returns_none(helix_db):
    task_id = _create_task(display_id="T99103")
    assert read_checkpoint(task_id, "nope") is None


def test_invalid_task_id_raises(helix_db):
...
def test_invalid_key_raises(helix_db):
    task_id = _create_task(display_id="T99104")
    with pytest.raises(ValueError):
        checkpoint(task_id, "", "v")


def test_list_checkpoints_orders_by_updated_desc(helix_db):
...
def test_clear_checkpoints(helix_db):
...
def test_mark_interrupted_flips_only_running(helix_db):
...
def test_mark_interrupted_explicit_ids(helix_db):
...
def test_mark_interrupted_empty_list_no_op(helix_db):
    _create_task(display_id="T99112", status="running")
    assert mark_interrupted([]) == 0


def test_list_resumable_tasks_returns_only_interrupted(helix_db):
...
def test_resume_task_flips_status_to_pending(helix_db):
...
def test_resume_task_only_works_on_interrupted(helix_db):
...
def test_resume_task_preserves_checkpoints(helix_db):
...
def test_complex_payload_roundtrip(helix_db):
