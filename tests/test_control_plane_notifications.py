...
from helix.control_plane import notifications as control_plane_notifications
from helix.control_plane.models import NotificationPreferencesBody
from helix.notifications import emit_notification


def test_get_notifications_list_includes_items_stats_and_preferences(helix_db, monkeypatch):
    monkeypatch.setattr("helix.bot.send_sync", lambda *args, **kwargs: True)
...
def test_get_notifications_grouped_includes_groups_stats_and_preferences(helix_db, monkeypatch):
    monkeypatch.setattr("helix.bot.send_sync", lambda *args, **kwargs: True)
...
def test_get_notifications_list_passes_group_key_filter(helix_db, monkeypatch):
    monkeypatch.setattr("helix.bot.send_sync", lambda *args, **kwargs: True)
...
def test_post_notification_acknowledge_raises_404_for_missing_notification(helix_db):
...
def test_put_notifications_preferences_round_trips(helix_db):
