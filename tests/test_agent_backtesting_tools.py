...
from helix.agents.context import _current_strategy_id_var, reset_tool_context, set_tool_context
import helix.agents.tools_backtesting as tools_mod
from helix.agents.tools_backtesting import _persist_agent_verdict, _tool_backtesting, _tool_register_strategy, _tool_run_backtest
...
def test_agent_run_backtest_persists_result_and_syncs_strategy(helix_db, monkeypatch):
...
def test_agent_verdict_persistence_normalizes_gauntlet_aliases(helix_db):
    _insert_strategy(
        "s-agent-verdict",
        stage="gauntlet",
        metrics={
...
def test_gauntlet_promotion_requires_visible_optimization_or_walk_forward_artifact(helix_db):
    _insert_strategy(
...
def test_register_strategy_persists_runtime_type_for_current_strategy(helix_db, monkeypatch, tmp_path):
    _insert_strategy("s-runtime-type", stage="paper")

    monkeypatch.setattr(
        "helix.selfheal.validate_strategy_code",
        lambda code: {
            "valid": True,
            "code": code,
            "lint_issues": [],
    ...
    import helix.strategies.registry as registry_mod
    ...
    monkeypatch.setattr(
        "helix.strategies.intake.register_custom_strategy_file",
        lambda **_kwargs: {},
    )
        ...
        result = _tool_register_strategy(
            {
                ...
                "code": "from helix.strategies.base import BaseStrategy, Signal\n",
            }
        )
    finally:
        _current_strategy_id_var.reset(token)
...
def test_register_strategy_persists_agent_candidate_provenance(helix_db, monkeypatch, tmp_path):
...
def test_jbt_create_strategy_persists_agent_candidate_provenance_after_strict_client_create(helix_db, monkeypatch):
    ...
    monkeypatch.setattr("helix.backtesting.get_client", lambda: FakeClient())
...
def test_jbt_create_strategy_rejects_mismatched_hypothesis_and_crucible(helix_db, monkeypatch):
    created: list[dict] = []
    with get_db() as conn:
        conn.execute(
            """
