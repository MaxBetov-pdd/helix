...
def test_artifact_cached_columns_exist(helix_db):
    with get_db() as conn:
...
def test_add_artifact_without_content_leaves_cache_columns_null(helix_db):
...
def test_add_artifact_persists_cached_content(helix_db):
    hyp = _make_hypothesis()
...
def test_cached_content_truncated_above_cap(helix_db):
    hyp = _make_hypothesis()
    big = "x" * (600 * 1024)
    art = add_hypothesis_artifact(
        hypothesis_id=hyp["id"], source_type="blog", source_title="T",
...
def test_truncated_hash_matches_truncated_content(helix_db):
    hyp = _make_hypothesis()
    big = "x" * (600 * 1024)
    art = add_hypothesis_artifact(
        hypothesis_id=hyp["id"], source_type="blog", source_title="T",
