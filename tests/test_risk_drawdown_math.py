...
def test_drawdown_percent_tracks_high_water_mark(helix_db):
...
def test_kill_switch_triggers_exactly_at_drawdown_threshold(helix_db):
...
def test_daily_loss_halt_triggers_exactly_at_limit(helix_db):
...
def test_update_equity_persists_drawdown_and_daily_snapshot(helix_db):
    update_equity(10000.0)
