...
from helix.lab_db import create_lab_experiment, create_or_update_model_version, get_lab_experiment, list_lab_jobs
from helix.lab_models import LabJobState
from helix.lab_models import DispatchPaperIntentResponse, SelectorDecisionResponse
...
def test_lab_regime_worker_start_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.routers.lab_regime.start_lab_worker_process",
...
def test_lab_regime_worker_feed_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.lab_regime.read_lab_worker_feed", _fake_feed)
...
def test_lab_regime_pool_report_route(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.routers.lab_regime.inspect_strategy_pool",
...
def test_lab_regime_timeline_normalizes_core_regimes_and_uncertainty(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.lab_regime.get_model_version", lambda _model_version_id: _Model())
    monkeypatch.setattr("helix.routers.lab_regime.get_regime_segments", lambda **_kwargs: [_Segment()])
    monkeypatch.setattr("helix.routers.lab_regime.get_regime_labels", lambda **_kwargs: [_Label()])
    monkeypatch.setattr("helix.routers.lab_regime.Path.exists", lambda _self: True)
    monkeypatch.setattr(
        "helix.routers.lab_regime.pd.read_parquet",
...
def test_lab_regime_heatmap_marks_error_and_summary(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.lab_regime.get_model_version", lambda _model_version_id: _Model())
    monkeypatch.setattr(
        "helix.routers.lab_regime.list_strategy_regime_scores",
...
def test_lab_regime_selector_and_dispatch_endpoints(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.routers.lab_regime.decide_current_regime",
    ...
    monkeypatch.setattr(
        "helix.routers.lab_regime.dispatch_paper_intent",
...
def test_lab_regime_router_not_mounted_when_feature_is_dormant(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setenv("HELIX_ENABLE_REGIME_LAB", "1")
