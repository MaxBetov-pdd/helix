...
def test_api_resolve_exchange_testnet_uses_settings_when_creds_unavailable(helix_db, monkeypatch):
    monkeypatch.setattr(trading_domain, "get_execution_mode", lambda: "live")
    monkeypatch.setattr(
        trading_domain,
        "kv_get",
        lambda key, default=None: {"hyperliquid_testnet": True} if key == "helix:settings" else (default or {}),
    )
...
def test_api_resolve_exchange_testnet_prefers_creds_when_present(helix_db, monkeypatch):
...
def test_daemon_get_testnet_uses_settings_when_creds_unavailable(monkeypatch):
    monkeypatch.setattr(
        daemon,
        "kv_get",
        lambda key, default=None: {"hyperliquid_testnet": False} if key == "helix:settings" else (default or {}),
    )
