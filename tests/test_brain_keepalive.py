...
from helix import system_pause
...
def test_keepalive_seeds_when_brain_silent(helix_db, monkeypatch):
...
def test_keepalive_seeds_after_long_silence(helix_db, monkeypatch):
    monkeypatch.setattr(system_pause, "is_autonomy_paused", lambda: False)
...
def test_keepalive_noop_when_recent_cycle(helix_db, monkeypatch):
    monkeypatch.setattr(system_pause, "is_autonomy_paused", lambda: False)
...
def test_keepalive_noop_when_work_pending(helix_db, monkeypatch):
    monkeypatch.setattr(system_pause, "is_autonomy_paused", lambda: False)
...
def test_keepalive_respects_autonomy_pause(helix_db, monkeypatch):
