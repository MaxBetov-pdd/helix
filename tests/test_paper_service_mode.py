...
def test_start_paper_service_high_activity_enables_test_flags(helix_db, monkeypatch):
    import helix.api_core as core

    monkeypatch.setattr("helix.scheduler.apply_runtime_scheduler_overrides", lambda: 0)
    monkeypatch.setattr("helix.scanner.run_scan", lambda execute_positions=True: {})
    ...
    with get_db() as conn:
        rows = conn.execute(
            "SELECT id, enabled FROM scheduler_jobs WHERE id IN ('helix-scanner-signal', 'helix-scanner-hourly')"
        ).fetchall()
    by_id = {row["id"]: int(row["enabled"]) for row in rows}
    assert by_id.get("helix-scanner-signal") == 1
    assert by_id.get("helix-scanner-hourly") == 1


def test_stop_paper_service_restores_settings_and_disables_jobs(helix_db, monkeypatch):
    import helix.api_core as core
    ...
    by_id = {row["id"]: int(row["enabled"]) for row in rows}
    assert by_id.get("helix-scanner-signal") == 0
    assert by_id.get("helix-scanner-hourly") == 0
