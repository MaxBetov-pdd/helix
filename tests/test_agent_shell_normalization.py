from helix.agents.context import _normalize_legacy_paths


def test_normalize_legacy_paths_translates_windows_ls_and_home_alias():
    result = _normalize_legacy_paths(
        "ls -la ~/.helix/workspace/",
        is_windows=True,
        home=r"C:\Users\trader",
    )

    assert result == "dir C:/Users/trader/.helix/workspace/"


def test_normalize_legacy_paths_translates_windows_null_redirection():
    result = _normalize_legacy_paths(
        'cd ~/.helix && ls -la 2>/dev/null || echo "No .helix directory"',
        is_windows=True,
        home=r"C:\Users\trader",
    )

    assert "cd C:/Users/trader/.helix" in result
...
def test_normalize_legacy_paths_translates_unix_find_to_powershell():
    result = _normalize_legacy_paths(
        'find ~/.helix/workspace -name "*S00038*" -o -name "*post*mortem*" 2>/dev/null | head -20',
        is_windows=True,
        home=r"C:\Users\trader",
    )

    assert result.startswith("powershell -NoProfile -Command ")
    assert "Get-ChildItem -Path 'C:\\Users\\trader\\.helix\\workspace'" in result
...
def test_normalize_legacy_paths_keeps_posix_commands_but_repairs_legacy_home():
    ...
    assert result == "ls -la ~/.helix/workspace/"
