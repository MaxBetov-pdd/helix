...
from helix.health_monitor import (
    ComponentStatus,
    DataCheck,
    HealthAlert,
    HealthMonitor,
...
_EMIT_TARGET = "helix.notifications.emit_notification"
...
class TestCheckScheduler:
    def test_green_with_enabled_jobs(self):
        ...
        with patch("helix.scheduler.get_enabled_jobs", return_value=jobs):
            with patch(
                "helix.db.kv_get",
    ...
    def test_red_with_no_jobs(self):
        with patch("helix.scheduler.get_enabled_jobs", return_value=[]):
            result = check_scheduler()
    ...
    def test_red_with_stale_scheduler_heartbeat(self):
        ...
        with patch("helix.scheduler.get_enabled_jobs", return_value=jobs):
            with patch(
                ...
                with patch("helix.scheduler.get_last_tick_at", return_value=None):
                    result = check_scheduler()
                assert result.state == State.RED
...
class TestCheckBots:
    def test_no_bots_is_green(self):
        with patch("helix.db.get_running_bots", return_value=[]):
            results = check_bots()
    ...
    def test_stale_bot_heartbeat(self):
        ...
        with patch("helix.db.get_running_bots", return_value=bots):
            results = check_bots()
            assert len(results) == 1
...
class TestCheckLabWorker:
    def test_no_worker_is_green(self):
        with patch("helix.lab_db.get_lab_meta", return_value={}):
    ...
    def test_active_stale_worker(self):
        ...
        with patch("helix.lab_db.get_lab_meta", return_value=meta):
            result = check_lab_worker()
            assert result.state == State.RED
...
class TestCheckPipelineConsistency:
    ...
    def test_check_handles_exception(self):
        with patch("helix.db.get_db", side_effect=Exception("db locked")):
            results = check_pipeline_consistency()
            assert any(not r.passed for r in results)


class TestCheckSqliteHealth:
    def test_healthy_db(self, helix_db):
        result = check_sqlite_health()
        assert result.passed

    def test_handles_exception(self):
        with patch("helix.db.get_db", side_effect=Exception("locked")):
            result = check_sqlite_health()
...
class TestHealthMonitor:
    ...
    def test_poll_loop_survives_check_failure(self):
        ...
        async def _run():
            ...
            with patch("helix.health_monitor.check_scheduler", side_effect=Exception("boom")):
                await monitor.start()
    ...
    def test_poll_loop_observes_and_alerts_in_manual_mode(self):
        ...
        async def _run():
            ...
            with patch("helix.health_monitor.autonomous_runtime_allowed", return_value=False), \
                 patch("helix.health_monitor.check_ai_providers", return_value=down) as ai_check, \
                 patch("helix.health_monitor._dispatch_alerts") as dispatch, \
                 patch("helix.health_monitor._attempt_recovery") as recovery:
                await monitor.start()
    ...
    def test_poll_loop_auto_recovers_in_autonomous_mode(self):
        ...
        async def _run():
            ...
            with patch("helix.health_monitor.autonomous_runtime_allowed", return_value=True), \
                 patch("helix.health_monitor.check_ai_providers", return_value=down), \
                 patch("helix.health_monitor._dispatch_alerts"), \
                 patch("helix.health_monitor._attempt_recovery") as recovery:
                await monitor.start()
                await asyncio.sleep(0.15)
                await monitor.stop()
...
class TestWatchdogCrashIsVisible:
    def test_data_freshness_crash_returns_amber_not_green(self):
        from helix.health_monitor import check_data_freshness

        with patch("helix.data_manager.data_manager_stats", side_effect=Exception("boom")):
            result = check_data_freshness()
        assert result.state == State.AMBER
        assert "Check failed" in result.message

    def test_pipeline_throughput_crash_returns_amber_not_green(self):
        with patch("helix.lab_db.get_lab_meta", side_effect=Exception("boom")):
...
class TestAutoRecovery:
    def test_recovery_records_attempt(self):
        from helix.health_monitor import _attempt_recovery
...
class TestPipelineThroughput:
    ...
    def test_green_when_worker_stopped(self):
        with patch("helix.lab_db.get_lab_meta") as mock_meta:
    ...
    def test_red_when_frozen(self):
        ...
        with patch("helix.lab_db.get_lab_meta", side_effect=_mock_meta):
            with patch("helix.lab_db.list_lab_jobs", return_value=[MagicMock()]):
                result = check_pipeline_throughput()
                assert result.state == State.RED
...
class TestPipelineThroughputRecovery:
    ...
    def test_frozen_pipeline_triggers_kill(self):
        async def _run():
            ...
            with patch(_EMIT_TARGET):
                with patch("helix.health_monitor._kill_lab_worker_processes", return_value=1) as mock_kill:
                    with patch("helix.lab_worker_service._reconcile_orchestrator_state"):
