"""Tests for helix.maintenance terminal queue-row pruning (failed_retention_hours).
...
def test_default_window_is_noop_for_recent_terminal_rows(helix_db):
...
def test_run_db_maintenance_default_settings_does_not_prune_recent(helix_db):
...
def test_zero_disables_pruning(helix_db):
...
def test_old_definitive_terminal_rows_are_pruned(helix_db):
...
def test_null_completed_at_is_never_pruned(helix_db):
...
def test_interrupted_rows_are_never_pruned(helix_db):
...
def test_inflight_rows_are_never_pruned(helix_db):
...
def test_failed_uses_longer_window_than_definitive(helix_db):
...
def test_failed_with_nonrecoverable_error_pruned_past_failed_window(helix_db):
...
def test_failed_with_recoverable_error_is_never_pruned(helix_db):
...
def test_recovery_protected_rows_do_not_block_other_deletions(helix_db):
...
def test_agent_tasks_fts_stays_consistent_after_prune(helix_db):
