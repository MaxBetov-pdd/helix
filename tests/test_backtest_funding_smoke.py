...
def test_funding_direction_signal_runs_on_enriched_df(tmp_path):
    ...
    with patch("helix.scanner.fetch_hyperliquid_funding_rate", return_value=0.0001):
        from helix.scanner import check_funding_direction_signal
...
def test_backtest_strategy_accepts_datetime_index_for_funding(helix_db, monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.strategies.sentiment.get_funding_for_backtest",
