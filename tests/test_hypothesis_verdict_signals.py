...
from helix.hypothesis_verdict import (
...
def test_signals_empty_window_is_researching(helix_db):
    _set_discipline()
    h = _hyp()
...
def test_signals_high_hit_high_diversity_proven(helix_db):
    _set_discipline(
...
def test_signals_high_hit_low_diversity_researching(helix_db):
    _set_discipline(
        verdict_hit_rate_threshold=0.5,
...
def test_signals_low_hit_full_window_disproven(helix_db):
    _set_discipline(
...
def test_signals_low_hit_partial_window_researching(helix_db):
...
def test_signals_all_children_archived_disproven_short_window(helix_db):
...
def test_signals_one_dead_one_alive_not_disproven_yet(helix_db):
...
def test_signals_single_dead_child_below_floor_is_researching(helix_db):
...
def test_signals_legacy_lifecycle_paper_eligible_counts_as_pass(helix_db):
...
def test_single_cell_thesis_proven_by_one_robust_child(helix_db):
...
def test_gauntlet_stage_children_are_not_counted_as_passing(helix_db):
...
def test_write_verdict_memo_uses_signals_floor(helix_db):
    ...
    with patch("helix.hypothesis_verdict._call_llm", return_value=fake_llm):
...
def test_write_verdict_memo_disproven_floor_overrides_llm(helix_db):
    _set_discipline(
        verdict_hit_rate_threshold=0.5,
        verdict_min_diversity_cells=2,
        verdict_rolling_window=4,
...
def test_write_verdict_memo_proven_can_downgrade(helix_db):
...
def test_claim_verdict_from_verdict_mapping():
    from helix.hypothesis_verdict import _claim_verdict_from_verdict
...
def test_write_verdict_memo_captures_claim_ruling(helix_db):
    ...
    with patch("helix.hypothesis_verdict._call_llm", return_value=fake):
        result = write_verdict_memo(h["id"])
    assert result["ok"]
    memo = result["hypothesis"]["verdict_memo"]
...
def test_write_verdict_memo_defaults_claim_verdict_when_omitted(helix_db):
...
def test_eligible_when_child_changed_after_memo(helix_db):
    ...
    from helix.hypothesis_verdict import _eligible_hypothesis_ids
...
def test_not_eligible_when_memo_newer_than_children(helix_db):
