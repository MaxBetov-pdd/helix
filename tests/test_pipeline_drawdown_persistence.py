...
def test_explicit_quick_screen_drawdown_survives_stale_paper_gate_alias(helix_db):
...
def test_gauntlet_drawdown_edit_persists(helix_db):
    from helix.policy import load_pipeline_config, save_pipeline_config
...
def test_legacy_paper_gate_still_back_maps_without_explicit_quick_screen(helix_db):
    ...
    kv_set("helix:pipeline_thresholds", {"paper_gate": {"max_drawdown_pct": 0.22}})
    cfg = load_pipeline_config()
