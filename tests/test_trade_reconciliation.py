...
from helix.api_domains.trading import _close_stale_open_trades
from helix.db import get_db
from helix.trade_state import close_trade_record, mark_trade_pending_close_reconcile
...
def test_stale_close_uses_price_map_and_writes_pnl(helix_db):
    _insert_open_trade(
        "t-reconcile-1",
        asset="BTC",
        direction="long",
...
def test_stale_close_falls_back_to_fill_exit_price(helix_db):
    _insert_open_trade(
        "t-reconcile-2",
        asset="ETH",
        direction="short",
...
def test_stale_close_marks_trade_incomplete_when_no_price_is_available(helix_db):
    _insert_open_trade(
        "t-reconcile-3",
        asset="SOL",
        direction="long",
...
def test_close_trade_record_preserves_fill_exit_price_over_signal_price(helix_db):
    _insert_open_trade(
        "t-reconcile-4",
        asset="BTC",
        direction="long",
...
def test_mark_trade_pending_close_reconcile_keeps_trade_open_until_confirmed(helix_db):
    _insert_open_trade(
        "t-reconcile-5",
        asset="BTC",
        direction="long",
