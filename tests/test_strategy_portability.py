...
def _isolate_custom_dir(monkeypatch, tmp_path):
    """Point helix.strategies.custom at an empty temp dir + reset the registry so
...
def test_export_import_round_trip_creates_new_quick_screen(helix_db, monkeypatch, tmp_path):
    _isolate_custom_dir(monkeypatch, tmp_path)
    sid, _ = _make_macd_container()
...
def test_import_warns_history_not_replayed(helix_db, monkeypatch, tmp_path):
    _isolate_custom_dir(monkeypatch, tmp_path)
    sid, _ = _make_macd_container()
    env = lifecycle.build_container_export(sid)
...
def test_import_rejects_missing_envelope(helix_db):
    with pytest.raises(HTTPException) as exc:
        lifecycle.import_strategy_container({"strategy": {}, "configuration": {}})
    assert exc.value.status_code == 400


def test_import_rejects_non_object_payload(helix_db):
    with pytest.raises(HTTPException) as exc:
        lifecycle.import_strategy_container("not-a-dict")
    assert exc.value.status_code == 400


def test_import_rejects_unsupported_version(helix_db):
    env = {
        "helix_export": {"kind": "strategy_container", "version": "9.9"},
        "configuration": {
            "type": "macd",
            "symbol": "BTC",
            "timeframe": "1h",
...
def test_import_unregistered_type_without_source_hints_reexport(helix_db, monkeypatch, tmp_path):
...
def test_export_bundles_source_code_for_code_class(helix_db, monkeypatch, tmp_path):
    ...
    sys.modules.pop(f"helix.strategies.custom.{type_name}", None)
...
def test_code_class_round_trip_registers_on_fresh_machine(helix_db, monkeypatch, tmp_path):
    temp_custom_dir = _isolate_custom_dir(monkeypatch, tmp_path)
...
def test_code_class_round_trip_handles_string_strategy_class(helix_db, monkeypatch, tmp_path):
...
def test_code_class_import_rejects_unsafe_source(helix_db, monkeypatch, tmp_path):
    _isolate_custom_dir(monkeypatch, tmp_path)
    env = {
        "helix_export": {"kind": "strategy_container", "version": "1.0", "source_strategy_id": "S1"},
