...
def test_backtest_results_table_has_strategy_fk(helix_db):
    with get_db() as conn:
...
def test_create_strategy_container_uses_canonical_s_id(helix_db):
    with get_db() as conn:
...
def test_backtest_runs_backfills_backtest_results_on_migration(helix_db):
...
def test_create_strategy_container_defaults_symbol_when_missing(helix_db):
    with get_db() as conn:
        strategy_id, _, _ = create_strategy_container(
            conn=conn,
            name="missing-symbol",
...
def test_init_db_repairs_legacy_generic_strategy_identity(helix_db):
    with get_db() as conn:
        strategy_id, _, _ = create_strategy_container(
            conn=conn,
            name="legacy-generic",
...
def test_trades_schema_exposes_runtime_read_columns(helix_db):
    required_columns = {
        "display_id",
        "strategy_name",
        "symbol",
...
def test_auto_assign_best_symbol_updates_timeframe_context(helix_db):
    now_iso = datetime.now(timezone.utc).isoformat()
    with get_db() as conn:
        strategy_id, _, _ = create_strategy_container(
            conn=conn,
