...
from helix import brain as brain_mod
from helix import brain_decisions as bd
from helix.brain import BrainDecision, BrainTaskAction, BrainTransitionAction
...
def test_record_decision_writes_row(helix_db):
...
def test_situation_summary_truncates_at_cap(helix_db):
...
def test_update_action_taken_round_trip(helix_db):
    decision_id = bd.record_decision(
        cycle_id="c1",
        situation_summary="s",
        decision_json={"actions": []},
...
def test_link_agent_task_sets_brain_decision_id(helix_db):
...
def test_execute_brain_actions_writes_one_row_per_call(helix_db, monkeypatch):
    _ensure_test_agent("quant-researcher")
...
def test_execute_brain_actions_rejects_unknown_agent(helix_db, monkeypatch):
...
def test_execute_brain_actions_allows_known_agent(helix_db, monkeypatch):
...
def test_execute_brain_actions_uses_ambient_cycle_context(helix_db, monkeypatch):
    _ensure_test_agent("quant-researcher")
...
def test_execute_brain_actions_records_with_no_actions(helix_db):
...
def test_transition_action_records_decision_too(helix_db, monkeypatch):
    monkeypatch.setattr(
        brain_mod,
        "transition_stage",
