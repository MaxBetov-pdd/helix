...
def test_stale_lock_recovery_backstops_leaked_background_job(helix_db, monkeypatch):
    import helix.scheduler as sched
    ...
    with get_db() as conn:
        conn.execute(
            ...
            "VALUES ('helix-gauntlet-step-loop', 'gauntlet step loop', 'interval', '120', 'gauntlet_step_loop', 1, ?)",
            (old,),
        )
        conn.commit()
    sched._SCHEDULER_BACKGROUND_JOB_IDS.add("helix-gauntlet-step-loop")
    ...
    assert recovered >= 1
    with get_db() as conn:
        rs = conn.execute("SELECT running_since FROM scheduler_jobs WHERE id='helix-gauntlet-step-loop'").fetchone()["running_since"]
    assert rs is None  # lock cleared -> job can run again
    assert "helix-gauntlet-step-loop" not in sched._SCHEDULER_BACKGROUND_JOB_IDS


def test_stale_lock_recovery_skips_live_background_job(helix_db, monkeypatch):
    import helix.scheduler as sched
    ...
    try:
        ...
        sched._SCHEDULER_BACKGROUND_JOB_IDS.discard("helix-gauntlet-step-loop")


# --- Fix 3: tick wall-clock budget ----------------------------------------

def test_tick_respects_wall_clock_deadline(helix_db, monkeypatch):
    import helix.gauntlet.engine as engine
...
def test_validation_optimization_resubmits_stale_running_result(helix_db, monkeypatch):
...
def test_validation_optimization_keeps_polling_fresh_running_result(helix_db, monkeypatch):
    from helix.gauntlet.tasks import run_validation_optimization
    ...
    called = {"n": 0}
    monkeypatch.setattr("helix.gauntlet.tasks._submit_optimization", lambda body: called.__setitem__("n", called["n"] + 1))
