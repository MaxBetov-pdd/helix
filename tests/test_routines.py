...
Covers ``helix.control_plane.routines`` validation, round-trip, and the
...
from helix.control_plane import routines as r
...
def test_create_rejects_invalid_cron(helix_db) -> None:
    init_db()
    with pytest.raises(r.RoutineValidationError):
        _make(cron_expr="not a cron")


def test_create_rejects_empty_cron(helix_db) -> None:
    init_db()
    with pytest.raises(r.RoutineValidationError):
        _make(cron_expr="   ")


def test_create_rejects_empty_name(helix_db) -> None:
    init_db()
    with pytest.raises(r.RoutineValidationError):
        _make(name="   ")


def test_create_rejects_empty_prompt(helix_db) -> None:
    init_db()
    with pytest.raises(r.RoutineValidationError):
        _make(prompt="")


def test_create_rejects_invalid_context(helix_db) -> None:
    init_db()
    with pytest.raises(r.RoutineValidationError):
...
def test_create_and_get_round_trip(helix_db) -> None:
    init_db()
...
def test_get_routine_missing_returns_none(helix_db) -> None:
    init_db()
    assert r.get_routine(999999) is None


def test_get_by_name_round_trip(helix_db) -> None:
    init_db()
...
def test_list_routines_returns_all_then_filtered(helix_db) -> None:
    init_db()
    _make(name="r1")
...
def test_update_routine_changes_fields(helix_db) -> None:
    init_db()
    routine_id = _make()
...
def test_update_routine_rejects_bad_cron(helix_db) -> None:
    init_db()
    routine_id = _make()
    with pytest.raises(r.RoutineValidationError):
...
def test_update_routine_rejects_empty_name(helix_db) -> None:
    init_db()
    routine_id = _make()
    with pytest.raises(r.RoutineValidationError):
...
def test_set_routine_enabled_toggles(helix_db) -> None:
    init_db()
    routine_id = _make()
...
def test_delete_routine_removes_row(helix_db) -> None:
    init_db()
    routine_id = _make()
...
def test_record_routine_run_writes_status(helix_db) -> None:
    init_db()
    routine_id = _make()
...
def test_record_routine_run_records_error(helix_db) -> None:
    init_db()
    routine_id = _make()
...
def test_preview_schedule_returns_n_iso_timestamps(helix_db) -> None:
    init_db()
...
def test_preview_schedule_clamps_count(helix_db) -> None:
    init_db()
...
def test_preview_schedule_rejects_invalid_cron(helix_db) -> None:
    init_db()
    with pytest.raises(r.RoutineValidationError):
...
def test_skills_none_round_trip(helix_db) -> None:
    init_db()
...
def test_skills_already_json_string_passthrough(helix_db) -> None:
    init_db()
