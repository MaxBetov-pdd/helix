...
WORKSPACE_DIR bindings (captured at import time) to the per-test HELIX_HOME.
The conftest `_isolate_helix_home` fixture only patches `cfg.WORKSPACE_DIR`,
which does not reach the `from helix.config import WORKSPACE_DIR` aliases that
...
def _pin_workspace_dir(home):
    ...
    the per-test home so read/write_workspace touch the temp dir, not ~/.helix.
    """
    ...
    import helix.workspace as ws_mod
    import helix.agents.manager as mgr_mod
...
def test_create_agent_seeds_three_per_agent_identity_files(helix_db, _isolate_helix_home):
    home = _isolate_helix_home
    with _pin_workspace_dir(home) as ws_dir:
        ...
        assert "Helix" in (agent_dir / "SOUL.md").read_text(encoding="utf-8")


def test_build_agent_documents_returns_per_agent_content(helix_db, _isolate_helix_home):
    home = _isolate_helix_home
    with _pin_workspace_dir(home):
        ...
        # workspace dir.
        from helix.api_core import _build_agent_documents
...
def test_ensure_identity_files_is_idempotent_and_self_heals(helix_db, _isolate_helix_home):
    """Self-heal recreates only MISSING files and never clobbers real content."""
    home = _isolate_helix_home
    with _pin_workspace_dir(home) as ws_dir:
        from helix.agents.manager import create_agent, ensure_agent_identity_files
...
def test_put_agent_document_writes_per_agent_not_global(helix_db, _isolate_helix_home):
    home = _isolate_helix_home
    with _pin_workspace_dir(home) as ws_dir:
        from helix.agents.manager import create_agent
