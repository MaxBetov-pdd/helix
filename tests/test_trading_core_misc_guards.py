...
from helix.sim import data_pump
...
def test_collect_compat_paper_sessions_ignores_trades_from_prior_strategy_incarnation(helix_db, monkeypatch):
...
def test_collect_compat_paper_sessions_surfaces_hedged_positions(helix_db, monkeypatch):
...
def test_force_transition_records_audit_and_warning_activity(helix_db, monkeypatch):
    _insert_strategy("S-FORCE-AUDIT", stage="rejected")
    activity_events: list[tuple[str, str, str]] = []
    monkeypatch.setattr(
        "helix.brain.log_activity",
