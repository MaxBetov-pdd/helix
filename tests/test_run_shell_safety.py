...
from helix.agents import tools_core


@pytest.fixture(autouse=True)
def _enable_shell_tool(monkeypatch):
    """The shell tool is disabled by default; these guard tests exercise the
    behavior that applies once an operator opts in via HELIX_ENABLE_SHELL_TOOL."""
    monkeypatch.setenv("HELIX_ENABLE_SHELL_TOOL", "1")
...
def test_run_shell_disabled_by_default(monkeypatch):
    ...
    monkeypatch.delenv("HELIX_ENABLE_SHELL_TOOL", raising=False)
...
def test_run_shell_blocks_denylisted_program_in_pipeline(monkeypatch):
    ...
    monkeypatch.delenv("HELIX_SHELL_STRICT_ALLOWLIST", raising=False)
...
def test_run_shell_strict_allowlist_blocks_unknown_program(monkeypatch):
    ...
    monkeypatch.setenv("HELIX_SHELL_STRICT_ALLOWLIST", "1")
