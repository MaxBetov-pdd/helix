...
from helix.control_plane import ops
...
def test_process_task_queues_delegates_processing_to_live_bot_worker(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.db.recover_stale_running_tasks",
...
def test_process_task_queues_falls_back_to_local_runtime_when_bot_worker_is_down(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.runtime_worker.process_agent_tasks_once",
...
def test_process_task_queues_falls_back_to_local_runtime_when_bot_lock_is_stale(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.runtime_worker.process_brain_tasks_once",
        lambda: asyncio.sleep(0, result=1),
    )
...
def test_bot_consumes_queued_processing_request(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot.kv_get", lambda key, default=None: store.get(key, default))
    monkeypatch.setattr("helix.bot.kv_set", lambda key, value: store.__setitem__(key, value))
