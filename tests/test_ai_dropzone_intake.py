...
import helix.brain as brain_mod
from helix.db import create_strategy_container, get_db
from helix.strategies import custom as custom_pkg
from helix.strategies import intake as intake_mod
from helix.strategies import registry
from helix.strategy_lifecycle import StrategyPromoteBody, promote_strategy, read_strategies
...
def _write_custom_strategy(
    ...
    *,
    ...
    if embedded_hypothesis_id:
        lines.extend(
            [
                f'HELIX_HYPOTHESIS_ID = "{embedded_hypothesis_id}"',
                "",
            ]
        )
    lines.extend(
...
def test_register_custom_strategy_file_creates_quick_screen_ai_row(helix_db, monkeypatch, tmp_path):
    ...
    sys.modules.pop("helix.strategies.custom.btc_ai_dropzone_wave_test", None)
...
def test_scan_custom_strategies_registers_active_file_once(helix_db, monkeypatch, tmp_path):
    temp_custom_dir = tmp_path / "custom"
    temp_custom_dir.mkdir()
    strategy_file = temp_custom_dir / "btc_ai_dropzone_wave_test.py"
    _write_custom_strategy(strategy_file)
...
def test_scan_custom_strategies_skips_archived_modules_by_default(helix_db, monkeypatch, tmp_path):
    temp_custom_dir = tmp_path / "custom"
    temp_custom_dir.mkdir()
    ...
    registry.reset()
    importlib.invalidate_caches()
    sys.modules.pop("helix.strategies.custom.btc_active_wave", None)
    sys.modules.pop("helix.strategies.custom.btc_archived_wave_s00123", None)
...
def test_register_custom_strategy_file_rejects_duplicate_db_strategy(helix_db, monkeypatch, tmp_path):
    temp_custom_dir = tmp_path / "custom"
    temp_custom_dir.mkdir()
    strategy_file = temp_custom_dir / "btc_ai_dropzone_wave_test.py"
    _write_custom_strategy(strategy_file)
...
def test_read_strategies_exposes_ai_dropzone_provenance_and_backtest_flag(helix_db):
    with get_db() as conn:
...
def test_promote_strategy_blocks_unbacktested_ai_dropzone_rows(helix_db):
    with get_db() as conn:
        strategy_id, _, _ = create_strategy_container(
            conn=conn,
            name="ignored",
...
def test_promote_strategy_allows_ai_dropzone_rows_after_backtest(helix_db, monkeypatch):
    with get_db() as conn:
        strategy_id, _, _ = create_strategy_container(
            conn=conn,
            name="ignored",
...
def test_promote_strategy_keeps_non_ai_rows_unchanged(helix_db, monkeypatch):
    with get_db() as conn:
        strategy_id, _, _ = create_strategy_container(
            conn=conn,
            name="ignored",
...
def test_auto_intake_recent_files_requires_embedded_hypothesis_id(helix_db, monkeypatch, tmp_path):
    temp_custom_dir = tmp_path / "custom"
    temp_custom_dir.mkdir()
    ...
    registry.reset()
    importlib.invalidate_caches()
    sys.modules.pop("helix.strategies.custom.btc_recent_auto_intake", None)
...
def test_auto_intake_recent_files_links_embedded_hypothesis_id(helix_db, monkeypatch, tmp_path):
    ...
    registry.reset()
    importlib.invalidate_caches()
    sys.modules.pop("helix.strategies.custom.btc_recent_hypothesis_linked", None)
