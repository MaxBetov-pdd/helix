...
def test_robustness_floor_default_permissive(helix_db, monkeypatch):
...
def test_robustness_floor_operator_reenforced(helix_db, monkeypatch):
...
def test_mc_dd_ceiling_default(helix_db, monkeypatch):
...
def test_mc_dd_ceiling_operator_tightened(helix_db, monkeypatch):
...
def test_trade_count_floor_blocks_tiny_sample(helix_db, monkeypatch):
...
def test_trade_count_floor_allows_modest_sample(helix_db, monkeypatch):
...
def test_trade_count_floor_operator_reenforced(helix_db, monkeypatch):
...
def test_f4_wfa_fold_floor_fires_when_not_required(helix_db, monkeypatch):
...
def test_f4_jitter_floor_fires_when_not_required(helix_db, monkeypatch):
...
def test_floored_gate_still_passes_legit_strategy(helix_db, monkeypatch):
    _stub_prereqs(monkeypatch, {
...
def test_mc_dd_setting_percent_normalized_and_wired(helix_db, monkeypatch):
