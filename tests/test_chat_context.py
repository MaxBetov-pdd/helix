...
def test_store_conversation_default_source_is_ui_chat(monkeypatch):
    ...
    asyncio.run(
        ctx.store_conversation(
            ...
            ai_response="It is mid-gauntlet with a Sharpe near 1.4 on the OOS window. — Helix",
        )
    )
...
def test_store_conversation_explicit_ui_chat_source(monkeypatch):
    ...
    asyncio.run(
        ctx.store_conversation(
            ...
            "Two candidates in gauntlet and one paper strategy. — Helix",
            source="ui_chat",
        )
    )
    assert captured.metadata["source"] == "ui_chat"
...
def test_store_conversation_discord_backward_compat(monkeypatch):
    ...
    asyncio.run(
        ctx.store_conversation(
            ...
            "Equity is flat, no open positions, regime is range-bound. — Helix",
        )
    )
...
def test_store_conversation_explicit_discord_source(monkeypatch):
    ...
    asyncio.run(
        ctx.store_conversation(
            ...
            "No, drawdown stayed under 2% the whole session. — Helix",
            source="discord",
        )
    )
    assert captured.metadata["source"] == "discord"
...
def test_run_brain_task_chat_uses_act_toolset_and_persists(helix_db, monkeypatch):
    ...
    monkeypatch.setattr("helix.context.build_chat_context", lambda: "ctx")
    monkeypatch.setattr("helix.context.store_conversation", _fake_store_conversation)
    monkeypatch.setattr("helix.brain.resolve_brain_provider_model", lambda p, m: ("openai", "gpt-5.2"))
    ...
    assert "Helix" in stored["ai_response"]
