"""Tests for the Helix risk module — kill-switch, daily loss, mode-aware limits."""

import json
from datetime import timedelta
from unittest.mock import patch

from helix.db import get_db, kv_set

from helix.exchange.risk import (
    _get_risk_limits,
    _MAINNET_LIMITS,
    _TESTNET_LIMITS,
...
from helix.sim.clock import get_now
from helix.system_pause import set_system_paused
...
class TestModeAwareRiskLimits:
    ...
    def test_testnet_limits(self):
        with patch("helix.config.get_execution_mode", return_value="paper"):
    ...
    def test_live_uses_testnet_limits(self):
        with patch("helix.config.get_execution_mode", return_value="live"):
            limits = _get_risk_limits()
        assert limits == _TESTNET_LIMITS

    def test_mainnet_limits_are_tighter(self):
        with patch("helix.config.get_execution_mode", return_value="mainnet"):
            limits = _get_risk_limits()
    ...
    def test_canonical_risk_aliases_override_legacy_fields(self, helix_db):
        kv_set(
            "helix:settings",
            {
...
class TestKillSwitch:
    ...
    def test_kill_switch_triggers_on_drawdown(self, helix_db):
    ...
    def test_kill_switch_stays_active(self, helix_db):
    ...
    def test_kill_switch_blocks_trading(self, helix_db):
        update_equity(10000.0)
    ...
    def test_kill_switch_reset(self, helix_db):
        update_equity(10000.0)
    ...
    def test_kill_switch_reset_prevents_retrigger(self, helix_db):
    ...
    def test_kill_switch_reset_clears_daily_halt(self, helix_db):
        """Reset should clear daily_loss_halt to avoid blocking after reset."""
        from helix.db import kv_get, kv_set
    ...
    def test_paper_to_exchange_rebaselines_automatically(self, helix_db):
    ...
    def test_paper_to_exchange_any_wallet_amount(self, helix_db):
    ...
    def test_paper_to_exchange_tiny_wallet(self, helix_db):
    ...
    def test_exchange_to_exchange_still_triggers(self, helix_db):
    ...
    def test_no_trigger_within_threshold(self, helix_db):
        update_equity(10000.0)
...
class TestKillSwitchToggle:
    ...
    def test_disable_prevents_auto_trigger(self, helix_db):
    ...
    def test_reenable_allows_trigger(self, helix_db):
    ...
    def test_toggle_persists_in_kv(self, helix_db):
    ...
    def test_disabled_still_tracks_drawdown(self, helix_db):
...
class TestDailyLossLimit:
    ...
    def test_daily_loss_triggers(self, helix_db):
    ...
    def test_daily_loss_blocks_trading(self, helix_db):
        update_equity(10000.0)
...
class TestManualPause:
    ...
    def test_manual_pause_blocks_and_resumes_trading(self, helix_db):
...
class TestRecoveryGate:
    ...
    def test_recovery_active_blocks_trading(self, helix_db):
        kv_set(
            "daemon_state",
            {
    ...
    def test_operator_pause_remains_blocking_after_recovery_clears(self, helix_db):
    ...
    def test_risk_status_includes_recovery_fields(self, helix_db):
        kv_set(
            "daemon_state",
            {
                "recovery_active": True,
...
class TestCanOpen:
    ...
    def test_blocks_when_kill_switch_active(self, helix_db):
        update_equity(10000.0)
    ...
    def test_ungrouped_asset_is_singleton_group_not_bypass(self, helix_db):
    ...
    def test_ungrouped_asset_blocks_duplicate(self, helix_db):
    ...
    def test_rejects_excessive_risk(self, helix_db):
        update_equity(10000.0)
        allowed, risk, reason = can_open("BTC", "long", "test_strat", risk_pct=0.05)
        assert allowed is False
        assert "exceeds" in reason.lower()

    def test_blocks_when_max_concurrent_positions_reached(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1})
    ...
    def test_blocks_strategy_during_loss_cooldown(self, helix_db):
        kv_set("helix:settings", {"cooldown_after_loss_hours": 4})
...
class TestPaperLivePositionScoping:
    ...
    def test_paper_sessions_do_not_block_each_other(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1, "paper_max_concurrent_positions": 0})
    ...
    def test_paper_two_strategies_can_hold_same_asset(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1, "paper_max_concurrent_positions": 0})
    ...
    def test_paper_long_and_short_same_asset_across_sessions(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1, "paper_max_concurrent_positions": 0})
        _seed_position("p-a", "BTC", "long", "strat-A", "paper_challenger")
        allowed, _risk, reason = can_open(
    ...
    def test_paper_same_strategy_cannot_double_open_same_asset(self, helix_db):
        kv_set("helix:settings", {"paper_max_concurrent_positions": 0})
        _seed_position("p-a", "BTC", "long", "strat-A", "paper_challenger")
        allowed, _risk, reason = can_open(
    ...
    def test_paper_respects_per_session_cap_when_set(self, helix_db):
        # A non-zero paper cap applies within a single session (per-strategy scope).
        kv_set("helix:settings", {"paper_max_concurrent_positions": 1})
        _seed_position("p-a", "BTC", "long", "strat-A", "paper_challenger")
        allowed, _risk, reason = can_open(
    ...
    def test_live_positions_still_globally_capped(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1})
    ...
    def test_live_keeps_one_net_position_per_asset(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 5})
    ...
    def test_paper_position_does_not_consume_live_slot(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1, "paper_max_concurrent_positions": 0})
        _seed_position("p-a", "ETH", "long", "strat-A", "paper_challenger")
        allowed, _risk, reason = can_open(
    ...
    def test_live_position_does_not_block_paper_session(self, helix_db):
        kv_set("helix:settings", {"max_concurrent_positions": 1, "paper_max_concurrent_positions": 0})
    ...
    def test_live_risk_display_excludes_paper_positions(self, helix_db):
...
class TestRiskStatus:
    ...
    def test_includes_execution_mode(self, helix_db):
        update_equity(10000.0)
        status = get_risk_status()
...
class TestCloseAllPositionsPartialFailure:
    ...
    def test_successful_close_marks_trade_closed(self, helix_db):
        ...
        fake_hl = types.ModuleType("helix.exchange.hyperliquid")
        fake_hl.get_positions = lambda: mock_positions
        fake_hl.close_position = lambda coin, size, side, **kwargs: {"close_price": 50000}

        with patch.dict(sys.modules, {"helix.exchange.hyperliquid": fake_hl}):
    ...
    def test_failed_close_leaves_trade_open(self, helix_db):
        from helix.db import get_db
        from datetime import datetime, timezone
        import sys
        import types
    ...
    def test_error_response_marks_trade_pending_reconcile(self, helix_db):
        from helix.db import get_db
        from datetime import datetime, timezone
        import sys
        import types
        ...
        with patch.dict(sys.modules, {"helix.exchange.hyperliquid": fake_hl}):
            with patch("helix.exchange.risk.time.sleep", lambda *_args, **_kwargs: None):
                results = close_all_positions()
    ...
    def test_transient_close_error_retries_then_closes_trade(self, helix_db):
        from helix.db import get_db
        from datetime import datetime, timezone
        import sys
        import types
