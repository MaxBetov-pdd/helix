...
def test_paper_stage_strategy_projects_as_compat_session(helix_db):
...
def test_open_paper_trade_projects_position(helix_db):
...
def test_closed_paper_trade_projection_uses_exit_fallbacks(helix_db):
...
def test_paper_session_reports_closed_trade_performance_metrics(helix_db):
...
def test_flat_paper_session_reports_trade_mode_from_default_params(helix_db):
    _insert_strategy(
...
def test_paper_session_prefers_scanner_canonical_decision_params(helix_db):
    _insert_strategy(
...
def test_paper_session_surfaces_scanner_blocked_reason(helix_db):
...
def test_paper_session_does_not_mark_static_regime_mismatch_as_gated(helix_db):
...
def test_paper_session_marks_non_regime_blocked_status(helix_db):
