...
def test_new_gate_floor_knobs_have_unchanged_defaults(helix_db):
...
def test_quick_screen_pf_floor_rejects_at_default(helix_db):
    with get_db() as conn:
...
def test_quick_screen_pf_floor_honors_operator_knob(helix_db):
...
def test_quick_screen_pf_floor_can_be_tightened(helix_db):
...
def test_gauntlet_oos_pf_floor_rejects_at_default(helix_db, monkeypatch):
...
def test_gauntlet_oos_pf_floor_honors_operator_knob(helix_db, monkeypatch):
    _stub_gauntlet_prerequisites(monkeypatch)
...
def test_gauntlet_oos_pf_floor_can_be_tightened(helix_db, monkeypatch):
    _stub_gauntlet_prerequisites(monkeypatch)
...
def test_paper_gate_pf_floor_rejects_at_default(helix_db):
    with get_db() as conn:
...
def test_paper_gate_pf_floor_honors_operator_knob(helix_db):
...
def test_paper_gate_pf_reduction_threshold_honors_operator_knob(helix_db):
...
def test_oos_is_ratio_check_fires_on_nested_metrics(helix_db):
    with get_db() as conn:
...
def test_oos_is_ratio_check_fires_on_doubly_nested_metrics(helix_db):
...
def test_oos_is_ratio_within_limit_passes_on_nested_metrics(helix_db):
    with get_db() as conn:
...
def test_oos_is_ratio_limit_honors_operator_knob(helix_db):
...
def test_flat_is_oos_sharpe_keys_still_work_as_fallback(helix_db):
    with get_db() as conn:
...
def test_paper_gate_fails_closed_on_missing_stage_changed_at(helix_db):
    with get_db() as conn:
...
def test_paper_gate_fails_closed_on_unparseable_stage_changed_at(helix_db):
    with get_db() as conn:
        _insert_strategy(
...
def test_paper_gate_unknown_entry_time_is_evidence_absence_not_quality(helix_db):
...
def test_check_paper_trades_window_falls_back_to_created_at(helix_db):
