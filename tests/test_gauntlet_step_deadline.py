...
def test_stops_after_one_step_when_deadline_already_passed(helix_db):
...
def test_advances_multiple_steps_without_a_deadline(helix_db):
    wf = create_or_get_workflow(
        strategy_id=_strategy(), created_by="pytest", settings_snapshot=build_settings_snapshot()
    )
    ran = []
...
def test_future_deadline_does_not_curtail_the_visit(helix_db):
    wf = create_or_get_workflow(
        strategy_id=_strategy(), created_by="pytest", settings_snapshot=build_settings_snapshot()
    )
    ran = []
