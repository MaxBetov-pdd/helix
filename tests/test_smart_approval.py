...
Tests the ``helix.control_plane.smart_approval`` module:
...
from helix.control_plane import smart_approval as sa
from helix.db import create_approval, get_approval, init_db
...
def test_force_escalate_on_live_trade(helix_db, monkeypatch) -> None:
...
def test_force_escalate_on_real_money(helix_db, monkeypatch) -> None:
    init_db()
    _aux_returns(monkeypatch, {"recommendation": "auto_approve",
...
def test_force_hold_on_oversized_payload(helix_db, monkeypatch) -> None:
...
def test_aux_unreachable_defaults_to_hold(helix_db, monkeypatch) -> None:
    init_db()
...
def test_malformed_response_defaults_to_hold(helix_db, monkeypatch) -> None:
    init_db()
...
def test_invalid_recommendation_value_defaults_to_hold(helix_db, monkeypatch) -> None:
    init_db()
...
def test_classify_persists_columns_to_approvals_row(helix_db, monkeypatch) -> None:
    init_db()
    _aux_returns(monkeypatch, {"recommendation": "auto_approve",
...
def test_apply_smart_decision_auto_approves_in_smart_mode(helix_db, monkeypatch) -> None:
    init_db()
    _aux_returns(monkeypatch, {"recommendation": "auto_approve",
    ...
    monkeypatch.setattr(
        "helix.control_plane.approval_modes.get_mode",
...
def test_apply_smart_decision_no_op_in_manual_mode(helix_db, monkeypatch) -> None:
...
def test_apply_smart_decision_returns_hold_for_missing_row(helix_db) -> None:
    init_db()
