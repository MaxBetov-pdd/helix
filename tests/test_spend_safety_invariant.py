...
def test_call_single_fails_closed_when_nothing_connected(helix_db, monkeypatch):
...
def test_call_single_blocks_other_provider_when_one_connected(helix_db, monkeypatch):
    ms.enable_enforcement()
...
def test_enforcement_off_does_not_block(helix_db, monkeypatch):
