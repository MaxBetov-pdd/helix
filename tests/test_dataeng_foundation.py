...
def test_legacy_symbol_helpers_delegate_without_behavior_change():
    from helix.data import symbol_to_ccxt, symbol_to_fs
...
def test_data_engine_settings_defaults_and_roundtrip(helix_db):
    from helix import api_core
...
def test_datahub_candles_matches_legacy_load_parquet_with_flag(helix_db, monkeypatch, tmp_path):
    from helix import api_core
...
def test_datahub_candles_supports_range_and_projection(monkeypatch, tmp_path):
    from helix import data as data_mod
    from helix.dataeng.hub import DataHub
...
def test_datahub_enrich_matches_legacy_data_manager(helix_db, tmp_path, monkeypatch):
    from helix import api_core
    from helix.data_manager import DataManager, _save_stream_parquet

    monkeypatch.setattr("helix.data_manager.FUNDING_DIR", tmp_path / "funding")
    monkeypatch.setattr("helix.data_manager.OI_DIR", tmp_path / "oi")
    monkeypatch.setattr("helix.data_manager.DERIVATIVES_DIR", tmp_path / "derivatives")
    monkeypatch.setattr("helix.data_manager.MACRO_DIR", tmp_path / "macro")
...
def test_datahub_quality_matches_legacy_compute_data_quality(helix_db, monkeypatch, tmp_path):
    from helix import api_core
    from helix import data as data_mod
