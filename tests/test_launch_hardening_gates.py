...
def test_quick_screen_rejects_low_trade_count(helix_db):
    with get_db() as conn:
        _insert_strategy(
            conn, "qs-few",
...
def test_quick_screen_min_trades_clears_at_sufficient_sample(helix_db):
...
def test_quick_screen_min_trades_honors_operator_knob(helix_db):
...
def test_paper_gate_rejects_weak_forward_edge(helix_db):
...
def test_paper_gate_sharpe_floor_skipped_for_zero_variance(helix_db):
...
def test_required_tests_guard_restores_walk_forward():
    ...
    from helix.policy import _normalize_pipeline_config
...
def test_paper_gate_forward_floors_disabled_when_zeroed(helix_db):
