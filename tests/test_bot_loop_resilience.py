...
This test inspects the source of helix.bot to assert the invariant. It is
...
import helix.bot as bot_module
...
def test_queue_draining_loops_have_error_handlers():
    ...
    assert not missing, f"expected tasks.loop methods missing from helix.bot: {missing}"
...
def test_bot_runtime_loops_are_api_owned_by_default(monkeypatch):
    monkeypatch.delenv("HELIX_BOT_OWNS_RUNTIME", raising=False)
    assert bot_module._bot_owns_runtime_loops() is False

    monkeypatch.setenv("HELIX_BOT_OWNS_RUNTIME", "1")
