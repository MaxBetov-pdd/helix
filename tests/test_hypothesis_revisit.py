...
from helix.db import get_approval, get_db, kv_set
from helix.hypotheses import create_hypothesis
from helix.hypothesis_graduation import graduate_hypothesis
from helix.hypothesis_revisit import force_revisit, run_revisit_pass
...
def test_run_revisit_pass_promotes_due_hypothesis(helix_db):
...
def test_run_revisit_pass_skips_protected_graduated_crucible(helix_db):
    _set_discipline(active_pool_cap=10, revisit_interval_days=30)
    h = _hyp(0)
    graduate_hypothesis(h["id"])
    past = datetime.now(timezone.utc) - timedelta(days=1)
...
def test_run_revisit_pass_skips_if_not_due(helix_db):
    _set_discipline(active_pool_cap=10, revisit_interval_days=30)
    h = _hyp(0)
    graduate_hypothesis(h["id"])
...
def test_run_revisit_pass_stops_on_pool_full(helix_db):
...
def test_run_revisit_pass_respects_cap_mid_pass(helix_db):
...
def test_force_revisit_succeeds(helix_db):
    _set_discipline(active_pool_cap=10, revisit_interval_days=30)
    h = _hyp(0)
    graduate_hypothesis(h["id"])
    _clear_protection(h["id"])
...
def test_force_revisit_protected_graduated_crucible_requires_approval(helix_db):
    _set_discipline(active_pool_cap=10, revisit_interval_days=30)
    h = _hyp(0)
    graduate_hypothesis(h["id"])
...
def test_force_revisit_raises_value_error_when_not_graduated(helix_db):
...
def test_force_revisit_raises_value_error_for_unknown_id(helix_db):
    with pytest.raises(ValueError, match="unknown"):
        force_revisit("does-not-exist")


def test_force_revisit_evicts_weakest_when_pool_full(helix_db):
...
def test_force_revisit_raises_when_no_eviction_possible(helix_db, monkeypatch):
    ...
    import helix.hypotheses as hyp_module
...
def test_revisit_does_not_count_disproven_in_active_cap(helix_db):
    """Disproven hypotheses don't occupy the cap — a graduated one can still revive."""
    from helix.hypotheses import update_hypothesis_status
