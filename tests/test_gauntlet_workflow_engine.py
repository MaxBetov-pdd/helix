...
from helix.gauntlet.engine import (
    block_step,
    cancel_workflow,
    claim_next_step,
    complete_step,
...
def test_claim_next_step_respects_dependencies(helix_db):
...
def test_retry_step_requeues_retryable_block_without_failing_workflow(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_cancel_workflow_cancels_open_steps(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_recover_stale_running_steps_marks_retryable_runtime_block(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_resume_workflow_preserves_running_async_step(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_list_active_workflow_ids_returns_only_non_terminal(helix_db):
...
def test_tick_advances_pending_workflow_one_step(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_tick_skips_terminal_workflows(helix_db):
    sid = _strategy()
    cancelled = create_or_get_workflow(
        strategy_id=sid,
        created_by="pytest",
...
def test_tick_isolates_per_workflow_failures(helix_db):
...
def test_gate_contention_block_is_never_drained_and_requeues_with_reset_attempts(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_gate_contention_reason_code_nested_in_transition_payload_is_recognised(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_transient_block_at_legacy_max_attempts_is_retried_not_drained(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_transient_block_drains_terminal_only_at_transient_attempt_cap(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_requeue_backoff_defers_recently_blocked_step(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_resume_workflow_does_not_redispatch_fresh_running_step(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_resume_workflow_redispatches_stale_running_step(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_resume_workflow_still_polls_fresh_running_step_with_poll_handle(helix_db):
...
def test_overlapping_drivers_cannot_double_execute_a_step(helix_db):
...
def test_running_step_heartbeat_prevents_stale_recovery(helix_db):
    workflow = create_or_get_workflow(
        strategy_id=_strategy(),
        created_by="pytest",
        settings_snapshot=build_settings_snapshot(),
...
def test_block_step_serializes_numpy_payload(helix_db):
...
def test_complete_step_serializes_numpy_output(helix_db):
    import json as _json
...
def test_resume_workflow_records_outcome_even_when_payload_write_fails(helix_db):
