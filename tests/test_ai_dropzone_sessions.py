...
we rewrite helix.strategies.custom.__path__ into tmp_path so the intake
...
def test_create_session_assigns_sequential_ids(helix_db):
...
def test_list_sessions_includes_strategy_counts(helix_db):
...
def test_close_session_marks_closed_and_stamps_ended_at(helix_db):
...
def test_get_session_and_session_exists(helix_db):
...
def test_register_custom_strategy_file_tags_session(helix_db, monkeypatch, tmp_path):
...
def test_register_custom_strategy_file_rejects_unknown_session(helix_db, monkeypatch, tmp_path):
    temp_custom_dir = tmp_path / "custom"
    temp_custom_dir.mkdir()
    strategy_file = temp_custom_dir / "btc_ai_dropzone_wave_test.py"
    _write_custom_strategy(strategy_file)
...
def test_get_session_detail_surfaces_runs_via_config_json(helix_db):
