...
from helix.gauntlet.engine import resume_workflow
from helix.gauntlet.settings import build_settings_snapshot
from helix.gauntlet.status import get_strategy_gauntlet_status
from helix.gauntlet.store import create_or_get_workflow, get_workflow_detail, update_step_status
from helix.strategy_lifecycle import LifecycleCreateBody, create_lifecycle_strategy


def test_lifecycle_strategy_creation_starts_quick_screen_workflow(helix_db):
    kv_set("helix:pipeline:settings", {"gauntlet_auto_quick_screen_enabled": True})
...
def test_quick_screen_runtime_error_blocks_without_rejecting_strategy(helix_db, monkeypatch):
    kv_set("helix:pipeline:settings", {"gauntlet_auto_quick_screen_enabled": True})
    created = create_lifecycle_strategy(
        LifecycleCreateBody(
    ...
    monkeypatch.setattr("helix.gauntlet.tasks._submit_backtest", _raise_runtime)
...
def test_trade_mode_unsupported_is_terminal_not_retryable():
    ...
    from helix.gauntlet.tasks import _classify_exception
...
def test_quick_screen_pass_advances_to_gate(helix_db, monkeypatch):
    kv_set("helix:pipeline:settings", {"gauntlet_auto_quick_screen_enabled": True})
    created = create_lifecycle_strategy(
        LifecycleCreateBody(
            name="RSI Quick Pass",
    ...
    monkeypatch.setattr("helix.gauntlet.tasks._submit_backtest", _fake_submit)
...
def test_validation_optimization_resubmits_after_server_restart(helix_db, monkeypatch):
    ...
    monkeypatch.setattr("helix.gauntlet.tasks._submit_optimization", _fake_submit)

    from helix.gauntlet.tasks import run_validation_optimization
...
def test_validation_optimization_blocks_on_genuine_failure(helix_db, monkeypatch):
...
def test_validation_optimization_uses_best_sweep_timeframe(helix_db, monkeypatch):
    strategy_id = _created_strategy_for_workflow()
    workflow = create_or_get_workflow(strategy_id=strategy_id, created_by="pytest", settings_snapshot=build_settings_snapshot())
...
def test_apply_optimized_defaults_updates_strategy_params_and_records_artifact(helix_db, monkeypatch):
    strategy_id = _created_strategy_for_workflow()
    workflow = create_or_get_workflow(strategy_id=strategy_id, created_by="pytest", settings_snapshot=build_settings_snapshot())
    detail = get_workflow_detail(workflow["id"])
    opt_step = next(step for step in detail["steps"] if step["step_key"] == "validation_optimization")
    ...
    from helix.strategies.optimization_acceptance import AcceptanceDecision
    ...
    monkeypatch.setattr(
        "helix.strategies.optimization_acceptance.apply_optimized_params_if_accepted", _force_accept
    )

    from helix.gauntlet.tasks import run_apply_optimized_defaults
...
def test_confirmation_backtest_uses_applied_defaults(helix_db, monkeypatch):
    strategy_id = _created_strategy_for_workflow()
    workflow = create_or_get_workflow(strategy_id=strategy_id, created_by="pytest", settings_snapshot=build_settings_snapshot())
    detail = get_workflow_detail(workflow["id"])
    ...
    from helix.gauntlet.tasks import run_confirmation_backtest
...
def test_regime_split_adapter_rejects_vacuous_pass_payload(helix_db, monkeypatch):
    strategy_id = _created_strategy_for_workflow()
    workflow = create_or_get_workflow(strategy_id=strategy_id, created_by="pytest", settings_snapshot=build_settings_snapshot())
    detail = get_workflow_detail(workflow["id"])
    ...
    monkeypatch.setattr(
        "helix.gauntlet.tasks._run_regime_split",
    ...
    monkeypatch.setattr(
        "helix.gauntlet.tasks._required_tests",
        lambda _wf: ["walk_forward", "parameter_jitter", "cost_stress", "regime_split"],
    )

    from helix.gauntlet.tasks import run_regime_split
...
def test_non_required_test_failure_does_not_fail_the_gate(helix_db, monkeypatch):
...
def test_paper_promotion_gate_uses_unified_status_and_transition(helix_db, monkeypatch):
    strategy_id = _created_strategy_for_workflow()
    workflow = create_or_get_workflow(
    ...
    monkeypatch.setattr("helix.gauntlet.tasks._transition_to_paper", _fake_transition)
    gate_step = next(item for item in detail["steps"] if item["step_key"] == "paper_promotion_gate")

    from helix.gauntlet.tasks import run_paper_promotion_gate
