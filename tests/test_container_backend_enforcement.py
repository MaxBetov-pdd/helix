...
def test_create_lifecycle_strategy_resolves_executable_type(helix_db):
...
def test_create_lifecycle_strategy_routes_uncertified_payload_to_research_only(helix_db):
    created = create_lifecycle_strategy(
        LifecycleCreateBody(
...
def test_read_strategies_honors_offset_for_paged_graveyard_loads(helix_db):
...
def test_backtest_submit_recovers_type_from_task_audit(helix_db, monkeypatch):
    ...
    import helix.vectordb as vectordb_mod
...
def test_backtest_submit_skips_task_audit_lookup_when_strategy_is_already_executable(
    ...
    import helix.api_core as api_core_mod
    import helix.strategies.backtest as bt_mod
    import helix.vectordb as vectordb_mod
...
def test_backtest_submit_uses_strategy_leverage_when_request_omits_it(helix_db, monkeypatch):
...
def test_read_strategies_uses_best_backtest_metrics(helix_db):
...
def test_read_strategies_penalizes_legacy_overflow_drawdown(helix_db):
    strategy_id = _seed_strategy(symbol="BTC", strategy_type="rsi_momentum")
...
def test_read_strategies_uses_archive_era_backtest_for_terminal_rows(helix_db):
    strategy_id = _seed_strategy(symbol="BTC", strategy_type="rsi_momentum")
...
def test_strategy_container_history_caps_legacy_overflow_drawdown(helix_db):
    strategy_id = _seed_strategy(symbol="BTC", strategy_type="rsi_momentum")
...
def test_submit_backtest_requires_existing_strategy_id(helix_db):
...
def test_submit_backtest_does_not_warn_for_honored_body_execution_controls(helix_db, monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.strategies.backtest.backtest_strategy",
...
def test_submit_backtest_still_warns_for_strategy_param_risk_fields(helix_db, monkeypatch):
...
def test_submit_backtest_translates_simple_macd_rule_blob_params(helix_db, monkeypatch):
...
def test_submit_optimization_requires_existing_strategy_id(helix_db):
    with pytest.raises(HTTPException) as exc:
...
def test_backtest_submit_persists_sqlite_before_index(helix_db, monkeypatch):
    strategy_id = _seed_strategy(symbol="ETH", strategy_type="rsi_momentum")
...
def test_manual_gauntlet_preserve_result_keeps_zero_trade_history(helix_db, monkeypatch):
    strategy_id = _seed_strategy(symbol="BTC", strategy_type="rsi_momentum")
...
def test_auto_trash_sweep_keeps_preserved_manual_history_rows(helix_db):
    strategy_id = _seed_strategy(symbol="BTC", strategy_type="rsi_momentum")
    ...
    from helix.api_core import _auto_trash_failed_local_backtests
...
def test_auto_trash_sweep_recovers_preserved_rows_already_in_trash(helix_db):
    strategy_id = _seed_strategy(symbol="BTC", strategy_type="rsi_momentum")
...
def test_strategy_container_endpoint_returns_unified_payload(helix_db):
...
def test_strategy_container_prefers_canonical_backtests_over_placeholder_legacy_rows(helix_db):
