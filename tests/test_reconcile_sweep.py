...
import helix.exchange.hyperliquid as hl_mod
...
def test_open_exchange_position_is_not_closed_as_flat(monkeypatch, helix_db):
...
def test_flat_exchange_closes_locally(monkeypatch, helix_db):
...
def test_zero_size_wrapper_position_counts_as_flat(monkeypatch, helix_db):
...
def test_retry_close_passes_size_side_and_testnet(monkeypatch, helix_db):
...
def test_retry_close_short_uses_buy_side_and_exchange_size_fallback(monkeypatch, helix_db):
...
def test_retry_close_error_dict_keeps_trade_open(monkeypatch, helix_db):
...
def test_age_grace_honored_via_pending_close_reconcile_at(monkeypatch, helix_db):
...
def test_aged_trade_is_swept_after_grace(monkeypatch, helix_db):
...
def test_legacy_requested_at_key_still_honored(monkeypatch, helix_db):
...
def test_local_only_paper_trade_closed_as_local_paper_close(monkeypatch, helix_db):
...
def test_paper_trade_with_exchange_order_id_is_reconciled_normally(monkeypatch, helix_db):
...
def test_exchange_unreachable_leaves_trade_open(monkeypatch, helix_db):
...
def test_subaccount_short_not_ghost_closed_when_master_flat(monkeypatch, helix_db):
...
def test_non_pending_trades_are_ignored(monkeypatch, helix_db):
