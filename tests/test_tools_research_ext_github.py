...
def _with_registry(monkeypatch, cfg):
    monkeypatch.setattr("helix.agents.tools_research._current_research_contract", _all_pass_contract)
    monkeypatch.setattr("helix.agents.tools_research._resolve_source_registry", lambda t: cfg)


def test_discover_blocked_when_github_not_allowed(monkeypatch):
    monkeypatch.setattr(
        "helix.agents.tools_research._current_research_contract",
...
def test_discover_happy_path(monkeypatch):
    ...
    with patch("helix.research_sources.github.search_github_repos") as m:
...
def test_discover_pat_from_registry_passed_to_client(monkeypatch):
    ...
    captured = {}
    with patch("helix.research_sources.github._client") as cl, patch("helix.research_sources.github.search_github_repos") as sr:
...
def test_inspect_happy_path(monkeypatch):
    _with_registry(monkeypatch, {"orgs": ["quantopian"], "rate_limit_per_min": 60, "personal_access_token": None})
    with patch("helix.research_sources.github.inspect_github_repo") as m:
...
def test_inspect_registry_error(monkeypatch):
    from helix.research_sources._registry import RegistryError
    monkeypatch.setattr("helix.agents.tools_research._current_research_contract", _all_pass_contract)
    def _raise(_t): raise RegistryError("orgs malformed")
    monkeypatch.setattr("helix.agents.tools_research._resolve_source_registry", _raise)
    res = json_loads(_tool_inspect_github_repo({"full_name": "a/b"}))
    assert res["ok"] is False
