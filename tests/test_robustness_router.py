...
from helix.db import create_strategy_container, get_db, init_db
from helix.routers import robustness as robustness_router
...
def test_load_strategy_row_uses_db_context_manager(helix_db):
...
def test_walk_forward_persists_result_row_and_payload(helix_db, monkeypatch):
    strategy_id = _create_strategy()

    monkeypatch.setattr(
        "helix.strategies.backtest.walk_forward",
        lambda **_kwargs: {
            "splits": [
                {
                    "split": 1,
...
def test_submit_monte_carlo_returns_placeholder_and_finalizes(helix_db, monkeypatch):
    strategy_id = _create_strategy()

    monkeypatch.setattr(
        "helix.api_core.get_backtest_result",
...
def test_monte_carlo_requires_persisted_trade_rows(helix_db, monkeypatch):
    strategy_id = _create_strategy()
...
def test_regime_split_fails_when_regime_labeling_is_unavailable(helix_db, monkeypatch):
    strategy_id = _create_strategy()
    ...
    monkeypatch.setattr("helix.strategies.backtest.load_backtest_candles", lambda **_kwargs: candles)
    monkeypatch.setattr("helix.strategies.backtest._detect_entry_regime", lambda _candles: "")
...
def test_walk_forward_rejects_zero_trade_analysis(helix_db, monkeypatch):
    strategy_id = _create_strategy()
...
def test_param_jitter_rejects_zero_trade_baseline(helix_db, monkeypatch):
    strategy_id = _create_strategy()
    ...
    monkeypatch.setattr(
        "helix.strategies.backtest.load_backtest_candles",
...
def test_param_jitter_fast_fails_low_trade_baseline(helix_db, monkeypatch):
...
def test_cost_stress_rejects_zero_trade_reruns(helix_db, monkeypatch):
    strategy_id = _create_strategy()
...
def test_param_jitter_reruns_disable_strategy_state_sync(helix_db, monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.backtest.load_backtest_candles", lambda **_kwargs: _flat_candles())
    ...
    monkeypatch.setattr("helix.strategies.backtest.backtest_strategy", _capture_backtest)
...
def test_cost_stress_reruns_disable_strategy_state_sync(helix_db, monkeypatch):
...
def test_recalculate_robustness_score_reconciles_gauntlet_to_paper(helix_db):
    strategy_id = _create_strategy()
    with get_db() as conn:
        conn.execute(
...
def test_low_sample_monte_carlo_and_failed_optimization_do_not_promote(helix_db):
