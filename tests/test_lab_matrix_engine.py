...
from helix.lab_matrix_engine import (
    MATRIX_JOB_TYPE,
    StrategyCandidate,
...
def test_candidate_payload_uses_true_forward_oos(monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_matrix_engine.backtest_strategy", _fake_backtest_strategy)
...
def test_load_strategy_candidates_merges_registry_and_managed_sources(monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_matrix_engine.discover", lambda: None)
    monkeypatch.setattr("helix.lab_matrix_engine.get_all", lambda: {"REG-1": _RegistryStrategy()})
    monkeypatch.setattr(
        "helix.lab_matrix_engine.list_strategy_pool_candidates",
        lambda **_kwargs: [
            {
                "strategy_id": "MAN-1",
...
def test_matrix_job_pools_short_segments_into_regime_windows(helix_db, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "helix.lab_matrix_engine._load_strategy_candidates",
    ...
    monkeypatch.setattr("helix.lab_matrix_engine.backtest_strategy", _fake_backtest_strategy)
    monkeypatch.setattr("helix.lab_matrix_engine.load_parquet", lambda *_args, **_kwargs: execution_snapshot.copy())
...
def test_load_strategy_candidates_expands_registry_trade_mode_variants(monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_matrix_engine.discover", lambda: None)
    monkeypatch.setattr("helix.lab_matrix_engine.get_all", lambda: {"REG-VAR": _VariantRegistryStrategy()})
    monkeypatch.setattr("helix.lab_matrix_engine.list_strategy_pool_candidates", lambda **_kwargs: [])
