...
class TestDuplicateOpenIndexM1:
    def test_unique_index_rejects_second_same_key_open(self, helix_db):
        _insert_open_trade("MT-DUP-1")
        import sqlite3
        with pytest.raises(sqlite3.IntegrityError):
            _insert_open_trade("MT-DUP-2")  # same strategy/asset/direction, OPEN

    def test_different_direction_same_asset_is_allowed(self, helix_db):
    ...
    def test_migration_dedups_preexisting_open_duplicates(self, helix_db):
        from helix.migrations import _m_2026_06_unique_open_trade
        with get_db() as conn:
...
class TestPendingOpenSlotReleaseM2:
    def test_rebuild_skips_pending_open_without_exchange_order(self, helix_db):
        from helix.exchange.risk import sync_from_trades
    ...
    def test_rebuild_readopts_once_fill_recorded(self, helix_db):
        from helix.exchange.risk import sync_from_trades
        _insert_open_trade(
            "MT-FILLED", asset="SOL",
...
class TestCrossBookGuardM7:
    ...
    def test_same_account_short_circuits_false(self, monkeypatch):
        ...
        monkeypatch.setattr("helix.exchange.books.book_address", lambda b, settings=None: "0xSAME")
    ...
    def _patch_addrs(self, monkeypatch):
        ...
        monkeypatch.setattr("helix.exchange.books.book_address", lambda b, settings=None: addrs.get(b))
    ...
    def test_position_alone_does_not_block(self, monkeypatch):
        ...
        import helix.scanner as sc
        self._patch_addrs(monkeypatch)
        monkeypatch.setattr("helix.exchange.hyperliquid.get_open_orders", lambda testnet=True, account_address=None: [])
        cross, _ = sc._opposite_book_would_cross("BTC", "long")
...
class TestKillSwitchSlippageM8:
    ...
    def test_residual_helper(self):
        from helix.exchange.risk import _close_residual_size
    ...
    def test_no_fill_ioc_detected_as_error_so_escalation_fires(self):
        ...
        from helix.exchange.risk import _close_result_error
        no_fill = {
            "status": "ok",
...
class TestDailyHaltOpenPathM9:
    def test_fires_halt_when_daily_loss_exceeded(self, helix_db, monkeypatch):
        import helix.exchange.risk as risk
        from helix.db import kv_set
        from helix.sim.clock import sim_kv_key, get_today
    ...
    def test_no_halt_when_within_limit(self, helix_db, monkeypatch):
        import helix.exchange.risk as risk
        from helix.db import kv_set
        from helix.sim.clock import sim_kv_key, get_today
        _orig = dict(risk._get_risk_limits())
    ...
    def test_missing_baseline_seeds_and_no_false_halt(self, helix_db):
        import helix.exchange.risk as risk
...
class TestCancelByOidM10:
    def test_only_listed_oid_cancelled(self, monkeypatch):
        ...
        cancelled_oids = []
        monkeypatch.setattr(
            "helix.exchange.hyperliquid.cancel_order",
