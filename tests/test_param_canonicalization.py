...
from helix.strategies.params import (
    canonicalize_params,
...
def test_backtest_strategy_rejects_non_executable_rule_blob_params(helix_db, monkeypatch):
    monkeypatch.setattr(
        backtest_mod,
...
def test_backtest_strategy_rejects_invalid_williams_r_thresholds(helix_db, monkeypatch):
    monkeypatch.setattr(
        backtest_mod,
        "load_backtest_candles",
        lambda **_kwargs: (_ for _ in ()).throw(AssertionError("load_backtest_candles should not run")),
...
def test_backtest_strategy_rejects_invalid_stochastic_thresholds(helix_db, monkeypatch):
    monkeypatch.setattr(
        backtest_mod,
        "load_backtest_candles",
        lambda **_kwargs: (_ for _ in ()).throw(AssertionError("load_backtest_candles should not run")),
...
def test_update_strategy_default_params_rejects_invalid_williams_r_thresholds(helix_db):
    with get_db() as conn:
        conn.execute(
            """
...
def test_update_strategy_default_params_rejects_invalid_stochastic_thresholds(helix_db):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO strategies
...
def test_update_strategy_default_params_merges_partial_update_into_existing_params(helix_db):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO strategies
...
def test_update_strategy_default_params_syncs_timeframe_from_pinned_backtest(helix_db):
