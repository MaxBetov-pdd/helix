...
def test_loop_triggers_on_n_strategy_threshold(helix_db):
...
def test_loop_skips_hypothesis_under_threshold(helix_db):
    from helix.hypothesis_verdict import run_verdict_loop
    hyp = _hyp()
    _add_children(hyp["id"], 1)  # below _N_TRIGGER=2
    with patch("helix.hypothesis_verdict._call_llm") as m:
        result = run_verdict_loop()
    assert hyp["id"] not in result["processed_ids"]
    m.assert_not_called()


def test_loop_triggers_on_staleness(helix_db):
    from helix.hypothesis_verdict import run_verdict_loop
    hyp = _hyp()
...
def test_loop_skips_disproven(helix_db):
    from helix.hypothesis_verdict import run_verdict_loop
...
def test_loop_sweeps_stranded_proven(helix_db):
