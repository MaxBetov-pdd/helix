...
from helix import db as helix_db
from helix import quant_skills as qs


@pytest.fixture
def isolated_skills(tmp_path, monkeypatch):
    """Redirect SKILLS_DIR + HELIX_HOME so SKILL.md and history both land in tmpdir."""
    skills_dir = tmp_path / "quant-skills"
    skills_dir.mkdir()
    (skills_dir / "_hypotheses").mkdir()
    (skills_dir / "_archived").mkdir()

    monkeypatch.setattr("helix.quant_skills.SKILLS_DIR", skills_dir)
    monkeypatch.setattr("helix.quant_skills.HYPOTHESES_DIR", skills_dir / "_hypotheses")
    monkeypatch.setattr("helix.quant_skills.ARCHIVED_DIR", skills_dir / "_archived")

    # Fresh DB
    db_dir = tempfile.mkdtemp()
    monkeypatch.setenv("HELIX_HOME", db_dir)
    if hasattr(helix_db, "_DB_PATH"):
        helix_db._DB_PATH = None  # type: ignore[attr-defined]
    if hasattr(helix_db, "_init_db_done"):
        helix_db._init_db_done = False  # type: ignore[attr-defined]
    helix_db.init_db()
    yield skills_dir
...
def test_legacy_frontmatter_reads_as_v1(isolated_skills, tmp_path):
    """A pre-Phase-3 SKILL.md (no `version`, Helix keys at top of metadata)
...
def test_round_trip_legacy_to_v3(isolated_skills):
    ...
    assert "helix:" in raw  # nested under metadata.helix now
