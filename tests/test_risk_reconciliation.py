...
def test_reconcile_exchange_positions_closes_ghost_trade_with_price(helix_db, monkeypatch):
    _insert_open_trade(
        "t-risk-reconcile-1",
        asset="BTC",
        direction="long",
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 110.0})
...
def test_reconcile_exchange_positions_marks_incomplete_when_price_missing(helix_db, monkeypatch):
    _insert_open_trade(
        "t-risk-reconcile-2",
        asset="ETH",
        direction="short",
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_positions", lambda testnet=True: {"positions": []})
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {})
    monkeypatch.setattr(risk_mod, "log_activity", lambda *_args, **_kwargs: None)
...
def test_reconcile_exchange_positions_confirms_pending_close_and_cancels_reduce_only_orders(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_positions", lambda testnet=True: {"positions": []})
    monkeypatch.setattr("helix.exchange.hyperliquid.get_open_orders", lambda testnet=True: [{"coin": "BTC", "oid": 101, "reduceOnly": True}])
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 108.0})
    monkeypatch.setattr(
