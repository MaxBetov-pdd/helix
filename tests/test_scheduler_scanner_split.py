...
def test_seed_jobs_include_signal_and_execution_scanners(helix_db):
    kv_set(
        "helix:settings",
        {
    ...
    assert "helix-scanner-signal" in jobs
    assert "helix-scanner-hourly" in jobs
    assert "helix-crucible-planner" in jobs

    signal_job = jobs["helix-scanner-signal"]
    exec_job = jobs["helix-scanner-hourly"]
    crucible_job = jobs["helix-crucible-planner"]
    ...
    assert "helix-coding-daily" not in jobs
    assert jobs["helix-ideation-daily"].get("enabled") == 0


def test_runtime_overrides_update_scanner_cadence(helix_db):
    kv_set(
        "helix:settings",
        {
            "throughput_auto_scheduler_control": True,
    ...
    jobs = _job_map()
    assert str(jobs["helix-scanner-signal"].get("schedule_expr")) == str(7 * 60 * 1000)
    assert str(jobs["helix-scanner-hourly"].get("schedule_expr")) == str(21 * 60 * 1000)


def test_runtime_overrides_only_touch_managed_jobs(helix_db):
    kv_set(
        "helix:settings",
        {
            "throughput_auto_scheduler_control": True,
    ...
    jobs = _job_map()
    assert str(jobs["helix-scanner-signal"].get("schedule_expr")) == str(4 * 60 * 1000)
    assert str(jobs["helix-scanner-hourly"].get("schedule_expr")) == str(9 * 60 * 1000)
    assert str(jobs["custom-monitoring"].get("schedule_expr")) == "123000"


def test_reconcile_helix_jobs_removes_legacy_helix_jobs(helix_db):
    seed_helix_jobs()
    add_job(
    ...
    assert "helix-testing-cycle" in _job_map()

    result = reconcile_helix_jobs()

    jobs = _job_map()
    assert result["removed"] >= 1
    assert "helix-testing-cycle" not in jobs
    assert "helix-testing-cycle" in jobs
    assert "custom-monitoring" in jobs


def test_reconcile_helix_jobs_is_stable_after_seed(helix_db):
    seed_helix_jobs()

    result = reconcile_helix_jobs()

    assert result == {"removed": 0, "added": 0}
    assert "helix-orphan-type-scan" in _job_map()


def test_seed_jobs_keeps_single_daily_learning_schedule(helix_db):
    kv_set("helix:settings", {"throughput_auto_scheduler_control": True})
    seed_helix_jobs()

    jobs = _job_map()
    daily_learning = jobs["helix-daily-learning"]
...
def test_run_job_dispatches_scanner_signal_kind(monkeypatch, helix_db):
    calls = {"count": 0}
    ...
    import helix.scanner as scanner_mod
...
def test_autonomy_backpressure_ignores_restart_recovery_failures(helix_db):
    import helix.scheduler as scheduler_mod
...
def test_autonomy_backpressure_counts_genuine_recent_agent_failures(helix_db):
    import helix.scheduler as scheduler_mod
...
def test_scanner_jobs_are_exempt_from_backpressure_gate():
    ...
    assert "helix-scanner-signal" not in scheduler_mod._AUTONOMY_BACKPRESSURE_JOB_IDS
    assert "helix-scanner-hourly" not in scheduler_mod._AUTONOMY_BACKPRESSURE_JOB_IDS
...
def test_tick_runs_scanner_jobs_despite_active_autonomy_backpressure(monkeypatch, helix_db):
...
def test_tick_skips_intake_job_when_autonomy_backpressure_is_active(monkeypatch, helix_db):
    import helix.scheduler as scheduler_mod

    now = datetime.now(timezone.utc)
    job = {
        "id": "helix-auto-intake",
        "name": "Auto Intake",
        "schedule_type": "interval",
        "schedule_expr": "60000",
        "timezone": "UTC",
