...
import helix.api_domains.data as data_domain
from helix.dataeng.settings import (
    DataEngineSettings,
...
def _patch_executor(monkeypatch, tasks, backfill):
    monkeypatch.setattr("helix.dataeng.catchup.CatchUpPlanner", _FakePlanner)
    _FakePlanner.tasks = tasks
    monkeypatch.setattr("helix.dataeng.catalog.Catalog", _FakeCatalog)
    _FakeCatalog.scan_calls = 0
    monkeypatch.setattr("helix.data.backfill_ohlcv_gaps", backfill)
    # The action log touches the DB; keep the unit test isolated from it.
    monkeypatch.setattr("helix.data._log_data_action", lambda *a, **k: None)
...
def test_catchup_advances_past_completed_batch(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("helix.dataeng.catalog.default_data_root", lambda: lake)
    monkeypatch.setattr(
        "helix.dataeng.catalog.default_catalog_path", lambda: tmp_path / "catalog.duckdb"
    )
    monkeypatch.setattr("helix.data._log_data_action", lambda *a, **k: None)
    data_domain._catchup_stalled.clear()
    ...
    monkeypatch.setattr("helix.data.backfill_ohlcv_gaps", fake_backfill)
...
def test_catchup_job_is_a_registered_default():
    """The job id must be in the default set, else reconcile_helix_jobs would
    delete it as a stale helix- row on every startup."""
    from helix import scheduler

    assert "helix-data-engine-catchup" in scheduler._DEFAULT_JOB_IDS
