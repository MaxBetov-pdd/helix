...
from helix.hypothesis_graduation import (
    graduate_hypothesis,
    is_canonical,
)
...
def test_graduate_sets_manager_state_and_timestamps(helix_db):
...
def test_graduate_picks_one_canonical_per_cell(helix_db):
    h = _hyp()
...
def test_graduate_ignores_non_qualifying_stages(helix_db):
...
def test_graduate_with_no_qualifying_children_makes_no_canonicals(helix_db):
    h = _hyp()
...
def test_graduate_is_idempotent(helix_db):
...
def test_graduation_frees_active_pool_slot(helix_db):
...
def test_canonical_strategy_blocks_archive(helix_db):
...
def test_canonical_strategy_blocks_reject(helix_db):
    from helix.brain import transition_stage
...
def test_reopening_terminal_strategy_clears_stale_terminal_metrics(helix_db):
    from helix.brain import transition_stage
...
def test_verdict_proven_triggers_graduation(helix_db):
    """End-to-end: write_verdict_memo with floor='proven' graduates the
    hypothesis and flags canonicals."""
    from helix.hypothesis_verdict import write_verdict_memo
