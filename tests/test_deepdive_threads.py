...
def test_create_first_thread_for_strategy(helix_db):
...
def test_get_existing_active_thread_idempotent(helix_db):
...
def test_archive_then_create_returns_fresh(helix_db):
...
def test_get_thread_returns_none_for_unknown(helix_db):
