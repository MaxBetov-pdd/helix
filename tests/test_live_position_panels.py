...
import helix.api_domains.trading as t
...
def test_live_indicators_delegates_with_resolved_timeframe(helix_db, monkeypatch):
    ...
    monkeypatch.setattr("helix.api_domains.paper.get_paper_session_indicators", _fake)
...
def test_live_indicators_explicit_timeframe_wins(helix_db, monkeypatch):
    _seed_strategy("S-IND2", timeframe="4h")
    captured: dict = {}
    monkeypatch.setattr(
        "helix.api_domains.paper.get_paper_session_indicators",
...
def test_live_markers_delegates(helix_db, monkeypatch):
    _seed_strategy("S-MK")
    captured: dict = {}
    ...
    monkeypatch.setattr("helix.api_domains.paper.get_paper_session_markers", _fake)
    out = t.read_live_markers("S-MK", limit=100)
    assert captured == {"session_id": "S-MK", "limit": 100, "include_generated": False}
    assert out["entries"] == []


def test_live_signals_builds_pending_from_scanner_snapshot(helix_db):
...
def test_live_signals_no_snapshot_is_empty(helix_db):
...
def test_empty_strategy_id_rejected(helix_db):
