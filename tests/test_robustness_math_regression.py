...
from helix.policy import DEFAULT_PIPELINE_CONFIG, load_pipeline_config
...
def test_load_pipeline_config_preserves_robustness_thresholds(helix_db):
...
def test_coerce_trade_return_pct_is_normalized_from_percent_points():
    from helix.routers.robustness import _coerce_trade_return_ratio
...
def _run_mc_with_trades(trades: list[dict], *, n_simulations: int = 400) -> dict:
    ...
    """
    from helix.routers import robustness as rb
...
def test_monte_carlo_on_percent_point_trades_produces_bounded_equity(helix_db):
...
def test_monte_carlo_verdict_uses_profitable_paths_and_drawdown_cap(helix_db):
