...
def test_lifecycle_gauntlet_status_delegates_to_unified_projection(helix_db):
    ...
    from helix.routers.lifecycle import read_gauntlet_status
...
def test_gauntlet_router_can_create_and_resume_workflow(helix_db, monkeypatch):
    strategy_id = _strategy()
    monkeypatch.setattr(
        "helix.gauntlet.engine.resume_workflow",
        lambda workflow_id, max_steps=1: {"ok": True, "workflow_id": workflow_id, "steps_run": max_steps},
    )

    from helix.routers.gauntlet import create_strategy_workflow, resume_gauntlet_workflow
