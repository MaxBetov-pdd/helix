...
def test_stage_to_outcome_maps_terminals(helix_db):
...
def test_backfill_marks_success_on_live_graduated(helix_db):
...
def test_backfill_marks_failure_on_archived(helix_db):
...
def test_backfill_skips_intermediate_stage(helix_db):
...
def test_backfill_idempotent_first_terminal_wins(helix_db, caplog):
    ...
    caplog.set_level(logging.WARNING, logger="helix.brain_decisions")
...
def test_backfill_noops_when_strategy_has_no_decision_link(helix_db):
...
def test_transition_stage_triggers_backfill(helix_db, monkeypatch):
