...
from helix.lab_orchestrator import ORCHESTRATOR_JOB_TYPE, get_orchestrator_status, update_orchestrator_config
from helix.lab_regime_engine import MODEL_REBUILD_JOB_TYPE, SEGMENT_BUILD_JOB_TYPE
from helix.lab_models import LabJobState
from helix.lab_worker_service import (
...
def test_process_claimed_lab_job_supports_model_rebuild(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.lab_worker_service.run_model_rebuild_job",
        lambda payload: {
            "status": "ok",
...
def test_process_claimed_lab_job_supports_segment_build(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.lab_worker_service.run_segment_build_job",
        lambda payload: {
            "status": "ok",
...
def test_non_matrix_job_heartbeat_loop_refreshes_claim(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.lab_worker_service.heartbeat_lab_job",
        lambda *args, **kwargs: beats.append(dict(kwargs)),
    )
    monkeypatch.setattr("helix.lab_worker_service._write_worker_status", lambda **_kwargs: None)
