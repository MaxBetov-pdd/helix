...
from helix.logging_config import (
...
def test_env_int_parses_positive_ints(monkeypatch):
    monkeypatch.setenv("HELIX_LOG_MAX_BYTES", "1048576")
    assert _env_int("HELIX_LOG_MAX_BYTES", 999) == 1048576


def test_env_int_falls_back_on_empty(monkeypatch):
    monkeypatch.delenv("HELIX_LOG_MAX_BYTES", raising=False)
    assert _env_int("HELIX_LOG_MAX_BYTES", 123) == 123


def test_env_int_falls_back_on_invalid(monkeypatch):
    monkeypatch.setenv("HELIX_LOG_MAX_BYTES", "not-a-number")
    assert _env_int("HELIX_LOG_MAX_BYTES", 999) == 999


def test_env_int_rejects_zero_and_negative(monkeypatch):
    monkeypatch.setenv("HELIX_LOG_MAX_BYTES", "0")
    assert _env_int("HELIX_LOG_MAX_BYTES", 999) == 999
    monkeypatch.setenv("HELIX_LOG_MAX_BYTES", "-5")
    assert _env_int("HELIX_LOG_MAX_BYTES", 999) == 999
...
def test_setup_rotates_when_threshold_exceeded(tmp_path, reset_root_logger):
    ...
    logger = logging.getLogger("helix.test_rotation")
    for i in range(50):
...
def test_setup_env_override_for_max_bytes(tmp_path, reset_root_logger, monkeypatch):
    monkeypatch.setenv("HELIX_LOG_MAX_BYTES", "12345")
