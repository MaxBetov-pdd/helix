...
from helix.brain_decisions import link_agent_task, record_decision
from helix.brain_memory import MAX_MEMORY_CHARS
...
def test_get_memory_returns_metadata(helix_db):
...
def test_put_memory_overwrites(helix_db):
    client = TestClient(app)
...
def test_put_memory_rejects_oversize_with_422(helix_db):
    client = TestClient(app)
...
def test_memory_history_lists_mutations(helix_db):
    client = TestClient(app)
...
def test_list_decisions_returns_pagination_envelope(helix_db):
...
def test_list_decisions_filters_by_cycle_id(helix_db):
...
def test_list_decisions_filters_by_outcome(helix_db):
...
def test_list_decisions_filters_by_strategy_and_action_type(helix_db):
...
def test_get_decision_returns_full_row_with_linked_tasks(helix_db):
...
def test_get_decision_404_when_missing(helix_db):
    client = TestClient(app)
...
def test_recall_endpoint_success(helix_db, monkeypatch):
    fake = {
        "summary": "synthesized",
    ...
    monkeypatch.setattr(
        "helix.recall.recall_similar_situation",
        lambda *a, **kw: fake,
    )
...
def test_recall_endpoint_clamps_limit_high(helix_db):
    client = TestClient(app)
...
def test_recall_endpoint_handles_underlying_exception(helix_db, monkeypatch):
    def boom(*_a, **_kw):
        raise RuntimeError("aux wedged")

    monkeypatch.setattr("helix.recall.recall_similar_situation", boom)
