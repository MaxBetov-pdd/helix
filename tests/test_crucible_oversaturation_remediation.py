...
def test_eviction_picker_ignores_dead_strategies(helix_db):
    from helix.hypotheses import _pick_weakest_active_hypothesis
...
def test_refine_gets_reserved_budget_when_develop_budget_is_saturated(helix_db):
...
def test_unstarted_ageout_archives_only_idle_never_started_proposals(helix_db):
...
def test_unstarted_ageout_dry_run_makes_no_changes(helix_db):
    from helix.crucibles import get_crucible
    from helix.hypothesis_cleanup import run_unstarted_ageout_pass
...
def test_count_unstarted_active_only_counts_proposed_zero_live(helix_db):
    from helix.hypotheses import count_unstarted_active_hypotheses
...
# 5. archive_reason is never silently NULL ----------------------------------------
def test_archive_without_reason_records_sentinel(helix_db):
    from helix.crucibles import get_crucible
...
def test_create_hypothesis_tool_throttles_when_backlog_saturated(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
    kv_set("helix:settings", {"research_settings": {"hypothesis_discipline": {"max_unrefined_active": 1}}})
...
def test_create_hypothesis_tool_rejects_exact_title_dup_of_active(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
...
def test_create_hypothesis_tool_rejects_remint_of_recently_disproven(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
...
def test_create_hypothesis_tool_allows_remint_after_disproven_lookback(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
...
def test_create_hypothesis_tool_rejects_near_duplicate_title(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
    create_hypothesis(
...
def test_create_hypothesis_tool_allows_novel_title(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
    _crucible("researching")
...
def test_create_hypothesis_tool_dedup_exempts_derived_creates(helix_db, monkeypatch):
    tools_research = _as_strategy_developer(monkeypatch)
