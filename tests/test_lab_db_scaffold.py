...
def test_init_lab_db_creates_expected_tables():
    ...
    assert cfg.HELIX_LAB_DB.exists()

    with sqlite3.connect(str(cfg.HELIX_LAB_DB)) as conn:
        rows = conn.execute(
...
def test_lab_write_guard_rejects_non_lab_connection(helix_db):
    with sqlite3.connect(str(cfg.HELIX_DB)) as conn:
        try:
...
def test_lab_db_module_does_not_use_production_get_db():
    source = Path("helix/lab_db.py").read_text(encoding="utf-8")
