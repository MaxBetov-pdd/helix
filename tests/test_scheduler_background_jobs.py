...
def test_tick_does_not_block_due_queue_on_long_evolution_job(monkeypatch, helix_db):
...
def test_tick_does_not_block_due_queue_on_promotion_loop(monkeypatch, helix_db):
    now = datetime.now(timezone.utc)
    _insert_scheduler_job(
        "test-promotion-loop",
        (now - timedelta(minutes=10)).isoformat(),
...
def test_run_sync_job_timeout_registers_zombie_thread(helix_db):
...
def test_timed_out_job_keeps_lock_until_thread_exits(helix_db):
...
def test_completed_job_with_no_zombie_clears_lock_normally(helix_db):
...
def test_absolute_recovery_ceiling_covers_largest_per_kind_window(helix_db):
...
def test_skip_due_job_preserves_zombie_held_lock(helix_db):
...
def test_gauntlet_step_loop_runs_via_tracked_sync_job(monkeypatch, helix_db):
...
def test_tick_skips_due_gauntlet_job_while_zombie_thread_alive(monkeypatch, helix_db):
