...
from helix import brain_memory, brain_inject
from helix.diagnostics import check_brain_cache_hit_rate


def _set_memory(helix_db, body: str) -> None:
    brain_memory.set_memory(body, mutated_by="test")


def test_empty_memory_still_emits_well_formed_fence(helix_db):
...
def test_user_message_places_fence_before_situational_text(helix_db):
...
def test_extract_leading_user_text_stops_at_boundary(helix_db):
    msg = brain_inject.build_user_message(
...
def test_consecutive_cycles_with_identical_memory_share_leading_text(helix_db):
    _set_memory(helix_db, "stable memory body")
...
def test_prompt_hash_stable_when_memory_unchanged(helix_db):
    _set_memory(helix_db, "stable")
...
def test_prompt_hash_changes_when_memory_mutates(helix_db):
    _set_memory(helix_db, "first")
    msg_a = brain_inject.build_user_message("x", brain_inject.get_memory_body_for_injection())
    _set_memory(helix_db, "second")
...
def test_prompt_hash_changes_when_system_changes(helix_db):
    _set_memory(helix_db, "stable")
...
def test_record_cache_observation_counts_hits_and_misses(helix_db):
    h1 = "a" * 64
    h2 = "b" * 64
...
def test_diagnostics_check_reports_rate_after_two_cycles(helix_db):
    _set_memory(helix_db, "a")
...
def test_diagnostics_check_passes_with_no_history(helix_db):
    res = check_brain_cache_hit_rate()
    assert res.status == "pass"
    assert res.detail.get("comparisons") == 0


def test_fence_constants_are_unique_and_searchable(helix_db):
