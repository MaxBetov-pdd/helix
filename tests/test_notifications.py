...
def test_agent_completion_stores_without_discord(monkeypatch):
    init_db()
    sent = []
    monkeypatch.setattr("helix.bot.send_sync", lambda *args, **kwargs: sent.append((args, kwargs)) or True)
...
def test_trade_alert_delivers_and_records_delivery(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot.send_sync", fake_send)
...
def test_failed_delivery_does_not_block_reemission(monkeypatch):
    ...
    init_db()
    monkeypatch.setattr("helix.bot.send_sync", lambda *args, **kwargs: False)
    first = emit_notification(
        "system_degraded",
        source="daemon",
        title="Scanner execution stale",
...
def test_digest_uses_thread_delivery(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot.send_thread_sync", fake_thread)
...
def test_digest_thread_delivery_marks_failed_when_all_attempts_fail(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot.send_thread_sync", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("thread create failed")))
    monkeypatch.setattr("helix.bot.send_sync", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("fallback send failed")))
