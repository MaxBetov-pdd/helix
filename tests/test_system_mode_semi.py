...
from helix.db import create_task_container, get_db
from helix.system_mode_policy import (
    autonomous_hardening_allowed,
    autonomous_hypothesis_generation_allowed,
)


def test_semi_mode_policy_allows_hardening_but_blocks_generation(helix_db):
...
def test_semi_mode_allows_system_hardening_tasks_to_be_claimed(helix_db):
    from helix.db import claim_pending_agent_tasks
...
def test_semi_mode_blocks_agent_hypothesis_creation_tool(helix_db):
    from helix.agents.tools_research import _tool_create_hypothesis
...
def test_stop_start_system_preserves_manual_mode(helix_db):
    from helix.system_pause import get_system_mode, is_system_paused
...
def test_stop_start_system_preserves_semi_auto_mode(helix_db):
...
def test_resume_generation_in_manual_mode_stays_manual_and_does_not_thaw(helix_db):
    from helix.system_pause import get_system_mode, is_generation_paused
...
def test_generation_toggle_still_moves_between_semi_auto_and_auto(helix_db):
    from helix.system_pause import get_system_mode
...
def test_fresh_install_default_manual_survives_stop_start(helix_db):
