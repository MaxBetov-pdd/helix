...
def test_autonomous_research_completion_queues_follow_through_for_fresh_hypothesis(helix_db, monkeypatch):
    queued: list[dict] = []
...
def test_autonomous_research_completion_skips_duplicate_follow_through_tasks(helix_db, monkeypatch):
    monkeypatch.setattr(runner, "_agent_is_strategy_developer", lambda _agent_id: True)
    monkeypatch.setattr(
        runner,
        "_list_hypotheses_for_follow_through",
...
def test_crucible_planner_completion_skips_legacy_follow_through(helix_db, monkeypatch):
    queued: list[dict] = []
