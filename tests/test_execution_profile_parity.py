...
from helix.api_core import _execution_profile_parity_warnings as warn


def test_clean_risk_based_profile_is_silent(helix_db):
...
def test_full_sizing_warns(helix_db):
    msgs = warn({"sizing_mode": "full"})
    assert any("not used live" in m for m in msgs)


def test_missing_sizing_mode_in_a_set_profile_treated_as_full(helix_db):
...
def test_risk_above_per_trade_cap_warns(helix_db):
...
def test_trailing_and_time_stops_have_no_live_equivalent(helix_db):
