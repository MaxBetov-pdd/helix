...
def test_legacy_helix_get_routes_dashboard(monkeypatch):
    monkeypatch.setattr(
        "helix.api_domains.legacy.control_plane_status.get_dashboard",
    ...
    response = client.get("/api/helix/dashboard")
...
def test_legacy_model_policy_route_delegates(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.legacy.legacy_domain.put_legacy_model_policy", _fake_put_legacy_model_policy)

    app = FastAPI()
    app.include_router(legacy_router)
    client = TestClient(app)

    response = client.put("/api/helix/model-policy", json={"provider_priority": ["openai", "minimax"]})
...
def test_legacy_agent_patch_route_delegates(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.legacy.legacy_domain.legacy_patch_agent", _fake_patch)

    app = FastAPI()
    app.include_router(legacy_router)
    client = TestClient(app)

    response = client.patch("/api/helix/agents/brain", json={"name": "Brain 2"})
...
def test_legacy_websocket_mount_delegates(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.routers.legacy.legacy_domain.legacy_websocket_endpoint",
    ...
    with client.websocket_connect("/api/helix/ws/live") as websocket:
