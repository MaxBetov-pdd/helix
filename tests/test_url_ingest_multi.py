...
from helix.hypotheses import list_hypothesis_artifacts
...
def test_from_urls_combines_into_one_crucible(helix_db):
    control_plane_ops.update_system_mode("auto")
    with patch("helix.api_domains.hypotheses.fetch_preview", side_effect=_fake_fetch_preview):
...
def test_from_urls_dedupes_identical_urls(helix_db):
    control_plane_ops.update_system_mode("auto")
    with patch("helix.api_domains.hypotheses.fetch_preview", side_effect=_fake_fetch_preview):
        client = TestClient(app)
        r = client.post(
...
def test_from_urls_all_fail_creates_nothing(helix_db):
    ...
    with patch("helix.api_domains.hypotheses.fetch_preview", side_effect=_all_fail):
        client = TestClient(app)
        r = client.post(
            "/api/hypotheses/from_urls",
...
def test_from_urls_empty_list_rejected(helix_db):
    client = TestClient(app)
