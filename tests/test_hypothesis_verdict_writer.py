...
def test_write_verdict_memo_happy_path(helix_db):
    ...
    with patch("helix.hypothesis_verdict._call_llm", return_value=fake_response):
...
def test_write_verdict_memo_disproven_path(helix_db):
    from helix.hypothesis_verdict import write_verdict_memo
    hyp = _hyp()
...
def test_write_verdict_memo_researching_keeps_status(helix_db):
    from helix.hypothesis_verdict import write_verdict_memo
    hyp = _hyp()
...
def test_write_verdict_memo_malformed_json_leaves_status(helix_db):
    from helix.hypothesis_verdict import write_verdict_memo
    hyp = _hyp()
    with patch("helix.hypothesis_verdict._call_llm", return_value="not json at all"):
        result = write_verdict_memo(hyp["id"])
...
def test_write_verdict_memo_llm_exception_leaves_status(helix_db):
    from helix.hypothesis_verdict import write_verdict_memo
    hyp = _hyp()
    with patch("helix.hypothesis_verdict._call_llm", side_effect=RuntimeError("rate limit")):
        result = write_verdict_memo(hyp["id"])
    assert result["ok"] is False
    assert result["error_code"] == "llm_call_failed"


def test_write_verdict_memo_missing_hypothesis_returns_error(helix_db):
    from helix.hypothesis_verdict import write_verdict_memo
...
def test_write_verdict_memo_invalid_verdict_value_rejected(helix_db):
    from helix.hypothesis_verdict import write_verdict_memo
    hyp = _hyp()
