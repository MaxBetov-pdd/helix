...
def test_run_backtest_pulls_strategy_id_from_context_and_calls_backtest(helix_db, monkeypatch):
    from helix.agents import tools_deepdive
    sid = _seed_strategy()
    ...
    monkeypatch.setattr("helix.strategies.backtest.backtest_strategy", fake_backtest)
...
def test_run_backtest_uses_runtime_type_over_type(helix_db, monkeypatch):
    from helix.agents import tools_deepdive
...
def test_run_backtest_without_session_raises(helix_db):
    from helix.agents import tools_deepdive
    tools_deepdive.clear_deepdive_strategy()
...
def test_run_backtest_propagates_backtest_error(helix_db, monkeypatch):
    from helix.agents import tools_deepdive
    sid = _seed_strategy()
    tools_deepdive.set_deepdive_strategy(sid)
    monkeypatch.setattr(
