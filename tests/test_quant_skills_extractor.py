...
gates partial dicts."""
import helix.quant_skills_extractor as qse
...
def test_extract_insight_recovers_prose_wrapped_response(monkeypatch):
    monkeypatch.setattr(
        "helix.model_routing.get_auxiliary_routing",
        lambda _task: {"provider": "openai", "model_id": "gpt-x"},
    )
    monkeypatch.setattr(
        "helix.ai.call_ai_sync",
...
def test_extract_insight_returns_none_on_unparseable(monkeypatch):
    ...
    monkeypatch.setattr("helix.ai.call_ai_sync", lambda **_kwargs: "not json at all, sorry")
    assert qse.extract_insight({"metrics": {"total_trades": 5}}, []) is None
