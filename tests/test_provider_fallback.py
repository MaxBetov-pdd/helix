...
def test_get_backup_ai_provider_precedence(monkeypatch):
    ...
    monkeypatch.delenv("HELIX_BACKUP_AI_PROVIDER", raising=False)
    ...
    monkeypatch.setenv("HELIX_BACKUP_AI_PROVIDER", "  MiniMax ")
...
def test_resolve_backup_provider(monkeypatch):
    from helix.agents import runner
    import helix.config as config
    import helix.model_routing as mr
...
# --------------------------------------------------------------------------- #
def test_agents_settings_validates_backup_provider(helix_db):
...
def test_is_credential_failure():
    from helix.runtime_worker import _is_credential_failure
    from helix.auth.store import CredentialError
...
def test_credential_failure_pauses_routine_and_alerts(monkeypatch):
    ...
    from helix.auth.store import CredentialError
    import helix.control_plane.routines as routines
    import helix.notifications as notifications
...
def test_call_single_openrouter_hits_openrouter_endpoint():
    ...
    async def _run():
        with patch("helix.ai.get_token", return_value="sk-or-key"):
...
def test_call_single_still_rejects_truly_unknown_provider():
    ...
    with patch("helix.ai.get_token", return_value="tok"):
...
def _setup_tool_loop(monkeypatch, impls: dict, executed: list):
    ...
    import helix.agents.providers as providers_mod
    import helix.auth.store as auth_store
    import helix.billing_guard as billing_guard
    from helix.agents import runner
...
def test_tool_call_fallback_does_not_replay_after_tool_executed(monkeypatch):
    ...
    from helix.agents.providers import ProviderResponse, ToolCall
