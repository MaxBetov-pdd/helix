...
def test_strict_reject_fires_on_cost_stress_survival(helix_db):
...
def test_strict_reject_fires_on_regime_consistency(helix_db):
...
def test_strict_reject_fires_on_wfa_degradation(helix_db):
...
def test_strict_reject_passes_clean_strategy(helix_db):
...
def test_paper_gate_allows_strategy_that_fails_strict_live(helix_db):
