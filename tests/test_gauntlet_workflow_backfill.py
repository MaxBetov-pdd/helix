...
from helix.gauntlet.engine import backfill_missing_quick_screen_workflows
...
def test_backfill_resets_passed_workflow_demoted_to_gauntlet(helix_db):
...
def test_backfill_creates_fresh_when_only_old_version_exists(helix_db):
...
def test_backfill_still_covers_quick_screen_with_no_workflow(helix_db):
    _insert_strategy("qs-fresh", stage="quick_screen")
    backfill_missing_quick_screen_workflows(limit=10)
    assert _active_wf("qs-fresh") is not None


def test_backfill_leaves_active_workflow_untouched(helix_db):
...
def test_backfill_does_not_reset_queued_or_blocked_workflow(helix_db):
...
def test_backfill_does_not_reset_failed_gate_workflow(helix_db):
...
def test_transition_to_gauntlet_resets_passed_workflow(helix_db):
