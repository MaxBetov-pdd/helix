...
from helix.lab_db import get_model_version, upsert_lab_experiment
from helix.lab_models import ModelRebuildRequest
...
def test_snapshot_rebuilds_when_experiment_window_changes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("helix.lab_regime_engine.parquet_path", lambda *_args, **_kwargs: source_path)
    monkeypatch.setattr("helix.lab_regime_engine.compute_checksum", lambda *_args, **_kwargs: "checksum-test")
    monkeypatch.setattr("helix.lab_regime_engine.load_parquet", lambda *_args, **_kwargs: frame.copy())
...
def test_model_rebuild_persists_classifier_metadata(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("helix.lab_regime_engine.parquet_path", lambda *_args, **_kwargs: source_path)
    monkeypatch.setattr("helix.lab_regime_engine.compute_checksum", lambda *_args, **_kwargs: "checksum-gmm")
    monkeypatch.setattr("helix.lab_regime_engine.load_parquet", lambda *_args, **_kwargs: frame.copy())
