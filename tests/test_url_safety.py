"""Phase 5 / P5-T02 — SSRF guard at ``helix/security/url_safety.py``.

The blocklist used to live in ``helix/research_sources/_http.py``; Phase 5
...
class TestValidatePublicUrlWithDns:
    def test_rejects_when_any_resolved_ip_is_blocked(self) -> None:
        ...
        with mock.patch("helix.security.url_safety.socket.getaddrinfo", return_value=fake):
            with pytest.raises(UnsafeUrlError):
    ...
    def test_dns_failure_raises(self) -> None:
        with mock.patch(
            "helix.security.url_safety.socket.getaddrinfo",
...
def test_research_sources_http_reexports_unsafe_url_error() -> None:
    ...
    from helix.research_sources._http import UnsafeUrlError as Legacy
...
def test_underscore_aliases_present() -> None:
    ...
    from helix.security import url_safety as us
