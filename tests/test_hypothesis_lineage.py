...
from helix.hypothesis_lineage import (
...
def test_sibling_table_returns_active_children(helix_db):
    h = _hyp()
...
def test_sibling_table_excludes_archived_and_rejected(helix_db):
    h = _hyp()
...
def test_sibling_table_empty_when_no_children(helix_db):
    h = _hyp()
...
def test_canonical_coverage_map_only_counts_canonicals(helix_db):
    h = _hyp()
...
def test_canonical_coverage_map_empty_for_no_canonicals(helix_db):
    h = _hyp()
...
def test_create_strategy_rejects_parent_from_different_hypothesis(helix_db):
    h_a = _hyp(0)
    h_b = _hyp(1)
...
def test_create_strategy_rejects_unknown_parent(helix_db):
    h = _hyp()
...
def test_create_strategy_accepts_parent_in_same_hypothesis(helix_db):
    h = _hyp()
