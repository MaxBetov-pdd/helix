...
from helix.scheduler import run_job, seed_helix_jobs
...
def test_seed_helix_jobs_enables_crucible_planner_and_disables_superseded_jobs(helix_db):
    seed_helix_jobs()

    planner = _scheduler_row("helix-crucible-planner")
    ...
    assert _scheduler_row("helix-ideation-daily")["enabled"] == 0
    # The Daily Coding Cycle is retired — not seeded at all (not merely disabled).
    with get_db() as conn:
        coding_row = conn.execute(
            "SELECT id FROM scheduler_jobs WHERE id = 'helix-coding-daily'"
        ).fetchone()
    assert coding_row is None

    assert _scheduler_row("helix-hypothesis-promotion-loop")["enabled"] == 1


def test_run_job_dispatches_crucible_planner_kind(monkeypatch, helix_db):
    calls: list[int] = []
...
def test_brain_assign_research_cycle_delegates_to_crucible_planner(monkeypatch, helix_db):
    calls: list[int] = []
