...
def test_migration_adds_hypothesis_columns(helix_db) -> None:
    with get_db() as conn:
...
def test_migration_adds_strategy_columns(helix_db) -> None:
    with get_db() as conn:
...
def test_migration_creates_indexes(helix_db) -> None:
    with get_db() as conn:
...
def test_hyp_legacy_seeded_archived(helix_db) -> None:
    with get_db() as conn:
        row = conn.execute(
...
def test_orphan_strategies_backfilled_to_legacy(helix_db) -> None:
    ...
    from helix.migrations import _m_2026_04_hypothesis_refinement_loop
    with get_db() as conn:
...
def test_migration_is_idempotent_when_re_applied(helix_db) -> None:
...
def test_migration_recorded_in_schema_migrations(helix_db) -> None:
    with get_db() as conn:
        row = conn.execute(
...
def test_revisit_count_defaults_to_zero(helix_db) -> None:
    with get_db() as conn:
        row = conn.execute(
...
def test_canonical_defaults_to_zero_for_new_strategies(helix_db) -> None:
