...
def test_create_backtesting_strategy_infers_macd_type(helix_db):
    from helix.routers.backtesting import create_backtesting_strategy
...
def test_create_backtesting_strategy_routes_rule_blob_payload_to_research_only(helix_db):
    from helix.routers.backtesting import create_backtesting_strategy
    from helix.hypotheses import create_hypothesis
...
def test_create_backtesting_strategy_maps_orb_variant_to_research_only(helix_db):
    from helix.routers.backtesting import create_backtesting_strategy
    from helix.hypotheses import create_hypothesis
...
def test_create_backtesting_strategy_rejects_unknown_hypothesis_id(helix_db):
    from helix.routers.backtesting import create_backtesting_strategy
...
def test_post_backtesting_run_local_infers_type_from_strategy_context(helix_db):
    from helix.api_core import post_backtesting_run
    ...
    fake_backtest_module = types.ModuleType("helix.strategies.backtest")
    fake_backtest_module.backtest_strategy = _fake_backtest_strategy

    with patch("helix.api_core.kv_get", return_value={"remote_engine_enabled": False}), patch.dict(
        sys.modules,
        {"helix.strategies.backtest": fake_backtest_module},
    ):
...
def test_post_backtesting_run_local_resolves_decorated_strategy_name(helix_db):
    from helix.api_core import post_backtesting_run
...
def test_post_backtesting_run_local_forwards_body_execution_controls(helix_db, monkeypatch):
    from helix.api_core import post_backtesting_run
    import helix.backtesting as backtesting_mod
    import helix.strategies.backtest as bt_mod
    ...
    monkeypatch.setattr("helix.api_core._write_backtest_result_artifacts", lambda *args, **kwargs: None)
    monkeypatch.setattr("helix.api_core.auto_assign_best_symbol", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("helix.vectordb.store_backtest_result", lambda **_kwargs: None)

    with patch("helix.api_core.kv_get", return_value={"remote_engine_enabled": False}):
        response = post_backtesting_run(
            {
                "strategy_id": "S00140",
                "dataset_id": "dataset-11-BTC/USDT-1h",
...
def test_post_backtesting_run_local_persists_result_rows_for_agent_flow(helix_db, monkeypatch):
    from helix.api_core import post_backtesting_run
...
def test_post_backtesting_run_local_sanitizes_nonfinite_metrics_for_json_response(helix_db, monkeypatch):
    from helix.api_core import post_backtesting_run
...
def test_get_backtest_result_sanitizes_stored_infinite_profit_factor(helix_db):
...
def test_get_backtesting_runs_sanitizes_nested_nonfinite_metrics(helix_db):
    from helix.api_core import get_backtesting_runs
...
def test_get_backtest_results_lists_sqlite_rows_without_touching_chroma(helix_db, monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.api_core._chroma_backtest_records",
