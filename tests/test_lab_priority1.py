...
def test_pid_lock_acquire_and_release(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_worker_service.HELIX_HOME", tmp_path)
    from helix.lab_worker_service import acquire_pid_lock, release_pid_lock, _pid_file_path
...
def test_pid_lock_stale_pid_reclaimed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_worker_service.HELIX_HOME", tmp_path)
    from helix.lab_worker_service import acquire_pid_lock, _pid_file_path
...
def test_pid_lock_reclaims_exited_windows_process_handle(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_worker_service.os.name", "nt")
    monkeypatch.setattr("helix.lab_worker_service.psutil.Process", lambda _pid: (_ for _ in ()).throw(__import__("psutil").NoSuchProcess(_pid)))
...
def test_feature_flag_regime_lab():
    """Feature flag reads HELIX_ENABLE_REGIME_LAB correctly."""
    from helix.lab_features import regime_lab_enabled

    original = os.environ.get("HELIX_ENABLE_REGIME_LAB")
    try:
        os.environ["HELIX_ENABLE_REGIME_LAB"] = "1"
        assert regime_lab_enabled() is True

        os.environ["HELIX_ENABLE_REGIME_LAB"] = "true"
        assert regime_lab_enabled() is True

        os.environ["HELIX_ENABLE_REGIME_LAB"] = "0"
        assert regime_lab_enabled() is False

        os.environ["HELIX_ENABLE_REGIME_LAB"] = ""
        assert regime_lab_enabled() is False

        del os.environ["HELIX_ENABLE_REGIME_LAB"]
        assert regime_lab_enabled() is False
    finally:
        if original is not None:
            os.environ["HELIX_ENABLE_REGIME_LAB"] = original
        elif "HELIX_ENABLE_REGIME_LAB" in os.environ:
            del os.environ["HELIX_ENABLE_REGIME_LAB"]
...
def test_feature_flag_no_longer_falls_back_to_orchestrator_db(monkeypatch):
    ...
    from helix.lab_db import set_lab_meta
    from helix.lab_features import regime_lab_enabled
    from helix.lab_orchestrator import ORCHESTRATOR_CONFIG_META_KEY

    original = os.environ.get("HELIX_ENABLE_REGIME_LAB")
    try:
        if "HELIX_ENABLE_REGIME_LAB" in os.environ:
            del os.environ["HELIX_ENABLE_REGIME_LAB"]
...
def test_quiesce_regime_lab_disables_orchestrator_and_fails_active_jobs(monkeypatch):
    ...
    from helix.lab_models import LabJobState
    from helix.lab_orchestrator import ORCHESTRATOR_CONFIG_META_KEY, ORCHESTRATOR_STATUS_META_KEY
...
def test_lab_worker_start_refuses_when_feature_is_dormant(monkeypatch):
    ...
    monkeypatch.setenv("HELIX_ENABLE_REGIME_LAB", "0")

    from helix.lab_worker_service import start_lab_worker_process
...
def test_recover_stale_lab_jobs():
    ...
    from helix.lab_models import LabJobState
    from helix.lab_regime_engine import MODEL_REBUILD_JOB_TYPE
...
def test_detect_champion_changes_identifies_new_champion():
    ...
    from helix.lab_matrix_engine import _detect_champion_changes
