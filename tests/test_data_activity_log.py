...
from helix.api_domains.data import get_data_activity
...
def test_execute_data_engine_backfill(monkeypatch):
    ...
    from helix import data as ddata
    from helix.api_domains import data as dd
    from helix.dataeng import catchup
...
def test_get_stream_rows_funding(monkeypatch):
    ...
    from helix import data_manager as dmmod
    from helix.api_domains import data as dd
...
def test_activity_filters_to_data_source(helix_db):
    _clear_activity()
...
def test_activity_event_shape_and_actions(helix_db):
    _clear_activity()
...
def test_log_data_action_writes_data_source(helix_db):
    _clear_activity()
...
def test_log_data_action_never_raises(helix_db):
...
import helix.data as d  # noqa: E402
...
def test_dataset_delete_logs(helix_db, monkeypatch, tmp_path):
    _clear_activity()
...
def test_csv_upload_logs(helix_db, monkeypatch, tmp_path):
    _clear_activity()
    monkeypatch.setattr(d, "DATA_DIR", tmp_path / "ohlcv")
    csv = (
...
def test_csv_upload_drops_unclosed_current_bar(helix_db, monkeypatch, tmp_path):
...
def test_orphan_scan_clean_does_not_log(helix_db, monkeypatch, tmp_path):
    _clear_activity()
    monkeypatch.setattr(d, "DATA_DIR", tmp_path / "ohlcv")
...
def test_orphan_scan_and_cleanup_log_on_drift(helix_db, monkeypatch, tmp_path):
    import os
    import time
...
def test_cleanup_never_deletes_fresh_tmp_or_unreadable(helix_db, monkeypatch, tmp_path):
