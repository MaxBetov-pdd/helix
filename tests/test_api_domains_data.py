...
def test_get_data_health_includes_stream_freshness(monkeypatch):
    monkeypatch.setattr(
        "helix.data.compute_data_health",
        lambda: {"ok": True, "datasets": [], "coverage": {}},
    )
    monkeypatch.setattr(
        "helix.data_manager.data_manager_stats",
        lambda: {
            "funding": {
                "total_calls": 1,
                "total_errors": 0,
    ...
    monkeypatch.setattr("helix.data_manager._now_iso", lambda: "2026-04-23T00:01:00+00:00")
...
def test_get_dataset_detail_stub_translates_missing_file(monkeypatch):
    monkeypatch.setattr(data_domain, "_remote_data_engine_config", lambda: (False, ""))
    monkeypatch.setattr("helix.data.get_dataset_detail", lambda symbol, timeframe: (_ for _ in ()).throw(FileNotFoundError("missing dataset")))
...
def test_export_and_ohlcv_wrappers_preserve_payload_shape(monkeypatch):
    monkeypatch.setattr(data_domain, "_remote_data_engine_config", lambda: (False, ""))
    monkeypatch.setattr(
        "helix.data.export_dataset_bytes",
        lambda **kwargs: (b"csv-data", "text/csv", "btc-1h.csv"),
    )
    monkeypatch.setattr(
        "helix.data.dataset_ohlcv",
