...
from helix.routers.webhooks import _reset_webhook_replay_cache, router as webhooks_router
...
def test_github_webhook_accepts_valid_push(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.webhooks._webhook_secret", lambda: "secret")
    monkeypatch.setattr("helix.routers.webhooks._target_branch", lambda: "main")
    monkeypatch.setattr(
        "helix.routers.webhooks._git_pull",
        lambda **kwargs: {"fetch_output": "ok", "pull_output": "ok"},
    )
    monkeypatch.setattr("helix.routers.webhooks._run_post_pull", lambda command, repo_path: "")
...
def test_validate_git_ref_rejects_shell_metacharacters():
    ...
    import pytest
    from helix.routers.webhooks import _validate_git_ref
...
def test_validate_git_remote_rejects_shell_metacharacters():
    import pytest
    from helix.routers.webhooks import _validate_git_remote
