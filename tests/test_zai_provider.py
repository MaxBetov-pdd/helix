...
def test_get_default_model_for_zai():
    with patch("helix.model_routing.kv_get", side_effect=_mock_kv_get):
...
def test_auth_store_reads_anthropic_env_for_zai():
    from helix.auth.store import get_profile
    ...
    with patch.dict(
        ...
        with patch("helix.auth.store.load_auth", return_value={"version": 1, "profiles": {}}):
...
def test_call_single_dispatches_to_zai():
    from helix.ai import _call_single

    mock_response = "Hello from GLM"

    async def _run():
        with patch("helix.ai._call_zai", new_callable=AsyncMock, return_value=mock_response) as mock_call:
            with patch("helix.ai.get_token", return_value="fake-zai-token"):
...
def test_call_zai_uses_reasoning_content_when_content_empty():
    from helix.ai import _call_zai
    ...
    async def _run():
        with patch("helix.ai.get_profile", return_value={"base_url": "https://api.z.ai/api/paas/v4"}):
            with patch("helix.ai.httpx.AsyncClient") as MockClient:
...
def test_call_zai_supports_anthropic_compatible_base_url():
    ...
    async def _run():
        with patch("helix.ai.get_profile", return_value={"base_url": "https://api.z.ai/api/anthropic"}):
            with patch("helix.ai.httpx.AsyncClient") as MockClient:
                mock_client = MagicMock()
                mock_client.__aenter__ = AsyncMock(return_value=mock_client)
                mock_client.__aexit__ = AsyncMock(return_value=False)
...
def test_agent_provider_factory_returns_zai_provider():
    from helix.agents.providers import ZAIProvider, get_provider
...
def test_agent_zai_provider_uses_openai_compatible_endpoint():
    from helix.agents.providers import ZAIProvider
    ...
    async def _run():
        ...
        with patch("helix.agents.providers.get_profile", return_value={"base_url": "https://api.z.ai/api/paas/v4"}):
            with patch("helix.agents.providers.httpx.AsyncClient") as MockClient:
                mock_client = MagicMock()
                mock_client.__aenter__ = AsyncMock(return_value=mock_client)
                mock_client.__aexit__ = AsyncMock(return_value=False)
                mock_client.post = AsyncMock(side_effect=_post)
...
def test_agent_zai_provider_uses_anthropic_compatible_endpoint():
    ...
    async def _run():
        provider = ZAIProvider()
        with patch("helix.agents.providers.get_profile", return_value={"base_url": "https://api.z.ai/api/anthropic"}):
            with patch("helix.agents.providers.httpx.AsyncClient") as MockClient:
                mock_client = MagicMock()
                mock_client.__aenter__ = AsyncMock(return_value=mock_client)
                mock_client.__aexit__ = AsyncMock(return_value=False)
...
def test_zai_discovery_endpoints_registered():
    from helix.api_core import _MODEL_DISCOVERY_ALT_ENDPOINTS
...
def test_zai_discovery_headers_registered():
    from helix.api_core import _MODEL_DISCOVERY_HEADERS
...
def test_zai_model_should_belong():
    from helix.api_core import _discovery_model_should_belong
...
def test_zai_endpoint_detection_success():
    ...
    with patch("helix.api_core.httpx.Client") as MockClient:
        mock_client = MagicMock()
...
def test_build_auth_provider_payload_zai():
    from helix.api_core import _build_auth_provider_payload
    from unittest.mock import patch

    with patch("helix.api_core.get_profile", return_value=None):
...
def test_zai_full_provider_round_trip():
    ...
    with patch("helix.api_core.get_profile", return_value=None), \
         patch("helix.model_routing.kv_get", side_effect=_mock_kv_get):
...
def test_model_routing_migrates_legacy_priority_when_anthropic_env_present():
    from helix.model_routing import get_model_routing
    ...
    with patch.dict("os.environ", {"ANTHROPIC_AUTH_TOKEN": "env-zai-token"}, clear=False):
        with patch("helix.model_routing.kv_get", return_value=legacy_policy):
            with patch("helix.model_routing.kv_set", side_effect=lambda key, value: captured.update({"key": key, "value": value})):
                migrated = get_model_routing()

    assert migrated["provider_priority"][0] == "zai"
    assert captured["key"] == "helix:model-routing"
