...
from helix.diagnostics import check_brain_fence_strips


def test_strip_returns_input_unchanged_when_no_fence(helix_db):
...
def test_strip_removes_simple_fence_block(helix_db):
...
def test_strip_removes_multiple_fence_blocks(helix_db):
...
def test_strip_handles_attributes_on_open_tag(helix_db):
...
def test_strip_handles_multiline_block(helix_db):
    text = "intro\n<brain-context>\nline1\nline2\nline3\n</brain-context>\noutro"
    cleaned, count = strip_brain_context_fences(text)
...
def test_strip_is_idempotent(helix_db):
...
def test_strip_handles_empty_input(helix_db):
    assert strip_brain_context_fences("") == ("", 0)
    assert strip_brain_context_fences(None) == ("", 0)


def test_sanitize_increments_counter_and_returns_clean_text(helix_db):
...
def test_sanitize_no_fence_does_not_bump_counter(helix_db):
    before = fence_strip_count()
...
def test_sanitize_logs_warning(helix_db, caplog):
    caplog.set_level(logging.WARNING, logger="helix.sanitize")
    sanitize_operator_input(
...
def test_diagnostics_check_reports_strip_count(helix_db):
...
def test_case_insensitive_open_tag(helix_db):
