...
from helix.db import claim_pending_agent_tasks, claim_pending_tasks, create_task_container, get_db
...
def test_manual_mode_freezes_autonomous_backlog_and_auto_mode_thaws_it(helix_db):
    with get_db() as conn:
...
def test_thaw_manual_mode_backlog_skips_duplicate_agent_tasks(helix_db):
    with get_db() as conn:
        first = conn.execute(
            """
...
def test_reconcile_thaws_stale_paused_manual_when_mode_is_auto(helix_db):
...
def test_reconcile_is_noop_when_auto_and_no_stuck_rows(helix_db):
...
def test_claim_pending_agent_tasks_in_manual_mode_only_claims_user_work(helix_db):
...
def test_claim_pending_brain_tasks_in_manual_mode_only_claims_user_work(helix_db):
    control_plane_ops.update_system_mode("manual")
