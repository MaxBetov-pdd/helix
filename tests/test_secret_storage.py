...
def test_load_auth_preserves_undecryptable_profiles(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HELIX_ENCRYPTION_KEY", valid_key)
    valid_access = auth_store.encrypt_secret("valid-token")

    monkeypatch.setenv("HELIX_ENCRYPTION_KEY", invalid_key)
...
def test_key_migration_copies_legacy_to_preferred(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HELIX_ENCRYPTION_KEY", raising=False)
    legacy_file = tmp_path / "legacy" / ".helix_key"
    preferred_file = tmp_path / "preferred" / ".helix_key"
