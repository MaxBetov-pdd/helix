...
from helix.crucibles import (
    get_crucible,
    is_crucible_protected,
    mark_crucible_viable,
    request_dethrone_approval,
)
from helix.db import _backfill_proven_hypothesis_protection, get_approval, get_db
from helix.hypothesis_graduation import graduate_hypothesis
from helix.hypotheses import create_hypothesis, update_hypothesis_status
...
def test_created_hypothesis_has_default_crucible_protection_fields(helix_db):
...
def test_mark_crucible_viable_sets_protection_fields(helix_db):
    hypothesis = _make_crucible()
...
def test_update_hypothesis_status_to_proven_protects_crucible(helix_db):
    hypothesis = _make_crucible()
...
def test_graduate_hypothesis_protects_viable_crucible(helix_db):
    hypothesis = _make_crucible()
...
def test_migration_backfills_existing_proven_crucibles_as_protected(helix_db):
    hypothesis = _make_crucible()
...
def test_request_dethrone_approval_marks_contested_and_creates_approval(helix_db):
    hypothesis = _make_crucible()
    mark_crucible_viable(
        hypothesis["id"],
        evidence_id="BT-123",
...
def test_request_dethrone_approval_reuses_existing_pending_approval(helix_db):
    hypothesis = _make_crucible()
    mark_crucible_viable(
        hypothesis["id"],
        evidence_id="BT-123",
...
def test_request_dethrone_approval_rejects_unprotected_crucible(helix_db):
    hypothesis = _make_crucible()
...
def test_strategy_table_has_crucible_provenance_columns(helix_db):
    with get_db() as conn:
...
def test_derive_crucible_status_lifecycle():
    ...
    from helix.crucibles import EXPANDED_MIN_STRATEGIES, derive_crucible_status
...
def test_detail_and_summary_payloads_expose_crucible_status_and_protection(helix_db):
...
def test_detail_payload_includes_child_gauntlet_status(helix_db):
    """1c/Phase-2: Forge rows show each candidate's real gauntlet (proof) status."""
    from helix.api_domains.hypotheses import get_hypothesis_detail_payload
