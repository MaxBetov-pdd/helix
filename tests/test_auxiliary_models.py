...
import helix.model_routing as model_routing
...
def test_default_routing_includes_all_auxiliary_kinds(helix_db):
...
def test_get_auxiliary_routing_returns_recall_default(helix_db):
...
def test_get_auxiliary_routing_returns_skill_extraction_default(helix_db):
    routing = get_auxiliary_routing("skill_extraction")
    assert routing["provider"] == "openrouter"
    assert routing["model_id"] == "anthropic/claude-3-5-sonnet"


def test_get_auxiliary_routing_unknown_kind_falls_back_to_priority_zero(helix_db):
...
def test_update_one_auxiliary_key_preserves_others(helix_db):
    policy = get_model_routing()
...
def test_legacy_policy_missing_auxiliary_block_reads_with_defaults(helix_db):
...
def test_bad_shaped_auxiliary_entry_falls_back_to_default(helix_db):
...
def test_per_task_base_url_and_api_key_round_trip(helix_db):
    policy = get_model_routing()
...
def test_unsupported_provider_in_auxiliary_is_rejected(helix_db):
...
def test_aux_routing_diverts_only_to_connected_and_callable_provider(helix_db, monkeypatch):
...
def test_aux_routing_does_not_divert_to_env_only_unselected_provider(helix_db, monkeypatch):
...
def test_aux_routing_divert_emits_loud_runtime_health_event(helix_db, monkeypatch):
    ...
    from helix import provider_runtime_health as prh
...
def test_aux_routing_with_mocked_auth_profiles_resolves_to_available_provider(helix_db, monkeypatch):
    """Exercise the REAL credential probe against mocked auth profiles
    (helix.auth.store.get_token) rather than a stubbed predicate. Divert still
    ...
    def _get_token(provider: str) -> str:
        if provider in ("openai", "minimax"):
            return "tok"
        raise ValueError(f"No auth profile for {provider}. Run: helix auth login {provider}")

    monkeypatch.setattr("helix.auth.store.get_token", _get_token)
...
def test_aux_routing_keeps_openrouter_when_it_has_credentials(helix_db, monkeypatch):
...
def test_aux_routing_explicit_api_key_is_never_diverted(helix_db, monkeypatch):
...
def test_aux_routing_nothing_credentialed_keeps_routed_provider(helix_db):
...
def test_auxiliary_block_is_persisted_after_update(helix_db):
    policy = get_model_routing()
    policy["auxiliary"]["recall"] = {
        "provider": "openai",
        "model_id": "gpt-4o",
