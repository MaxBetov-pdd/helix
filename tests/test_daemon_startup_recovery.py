...
from helix import daemon
from helix.db import get_db
from helix.exchange.risk import reconcile_exchange_positions
...
def test_reconcile_can_adopt_missing_exchange_position_by_stop_order_id(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
    ...
    monkeypatch.setattr(
        "helix.exchange.hyperliquid.get_open_orders",
        lambda testnet=True: [
            {
                "coin": "BTC",
                "oid": 49994803084,
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 68613.0})
...
def test_reconcile_can_adopt_missing_exchange_position_by_entry_order_id(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_open_orders", lambda testnet=True: [])
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 68613.0})
    monkeypatch.setattr(
        "helix.exchange.hyperliquid.place_protective_stop",
...
def test_reconcile_requires_operator_when_recovery_match_is_ambiguous(helix_db, monkeypatch):
    with get_db() as conn:
...
def test_startup_recovery_preflight_adopts_exchange_position_before_entries_resume(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_positions", lambda testnet=True: positions_payload)
    monkeypatch.setattr("helix.exchange.hyperliquid.get_open_orders", lambda testnet=True: open_orders_payload)
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 68613.0})
...
def test_startup_recovery_preflight_restores_missing_portfolio_positions(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_positions", lambda testnet=True: positions_payload)
    monkeypatch.setattr("helix.exchange.hyperliquid.get_open_orders", lambda testnet=True: open_orders_payload)
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 70000.0})
...
def test_startup_recovery_preflight_preserves_pending_close_trade_until_exchange_flat(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_startup_recovery_preflight_blocks_ambiguous_match_candidates(helix_db, monkeypatch):
    with get_db() as conn:
...
def test_startup_recovery_preflight_restores_prior_stop_for_recovered_position(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_startup_recovery_preflight_places_emergency_stop_for_recovered_position(helix_db, monkeypatch):
    positions_payload = {
        "positions": [
            {
                "position": {
...
def test_startup_recovery_preflight_repairs_missing_stop_for_existing_open_trade(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_startup_recovery_preflight_blocks_unprotected_recovered_position(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_startup_recovery_preflight_resolves_inverse_orphan_when_exchange_is_flat(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
    ...
    monkeypatch.setattr("helix.exchange.hyperliquid.get_positions", lambda testnet=True: {"positions": []})
    monkeypatch.setattr("helix.exchange.hyperliquid.get_open_orders", lambda testnet=True: [])
    monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 70000.0})
