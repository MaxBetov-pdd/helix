from helix.crucibles import get_crucible, mark_crucible_viable
...
def test_archive_unprotected_crucible_still_archives(helix_db):
...
def test_archive_protected_crucible_creates_approval_instead(helix_db):
    crucible = _make_crucible()
...
def test_trash_protected_crucible_creates_approval_with_trash_intent(helix_db):
    crucible = _make_crucible()
    mark_crucible_viable(
        crucible["id"],
        evidence_id="BT-901",
...
def test_status_demotion_of_protected_crucible_creates_approval_instead(helix_db):
    crucible = _make_crucible()
    mark_crucible_viable(
        crucible["id"],
        evidence_id="BT-904",
...
def test_status_demotion_of_unprotected_crucible_still_updates(helix_db):
    crucible = _make_crucible()
...
def test_bulk_trash_mixed_unprotected_and_protected_returns_approval(helix_db):
...
def test_repeated_archive_and_trash_reuse_pending_approval(helix_db):
    crucible = _make_crucible()
    mark_crucible_viable(
        crucible["id"],
        evidence_id="BT-903",
...
def test_active_pool_eviction_skips_protected_crucibles(helix_db):
    kv_set(
        "helix:settings",
