...
def test_paper_test_bypass_still_evaluates_paper_gate(helix_db):
    ...
    kv_set("helix:settings", {"paper_test_bypass_gates_enabled": True})
...
def test_testing_mode_still_evaluates_paper_gate(helix_db):
    """testing_mode (pipeline_thresholds) likewise must not skip the paper gate."""
    kv_set("helix:pipeline_thresholds", {"testing_mode": True})
...
def test_canonical_blocks_archive_for_automated_system_actor(helix_db):
...
def test_canonical_archivable_by_decay_tracker(helix_db):
...
def test_default_skips_slot_guard_for_paper(helix_db):
...
def test_default_skips_duplicate_tournament_at_gauntlet_entry(helix_db):
...
def test_decay_kill_switch_force_archives_live_strategy(helix_db):
...
def test_non_privileged_actor_force_is_downgraded(helix_db):
