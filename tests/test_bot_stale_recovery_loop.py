...
def test_stale_recovery_minutes_clamps_below_safe_runtime(monkeypatch):
    monkeypatch.setattr(
        "helix.db.kv_get",
        lambda key, default=None: {"task_stale_recovery_minutes": 7} if key == "helix:settings" else default,
    )
    monkeypatch.setattr("helix.bot.load_config", lambda: {})
...
def test_maybe_recover_stale_tasks_skips_when_inflight(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot._stale_recovery_minutes", lambda: 10)
    monkeypatch.setattr("helix.db.recover_stale_running_tasks", fake_recover_stale_running_tasks)
...
def test_maybe_recover_stale_tasks_throttles(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot._stale_recovery_minutes", lambda: 1)
    monkeypatch.setattr("helix.bot.time.time", lambda: clock["now"])
    monkeypatch.setattr("helix.db.recover_stale_running_tasks", fake_recover_stale_running_tasks)
    monkeypatch.setattr("helix.db.STALE_RECOVERY_FAIL_AGENTS", ("execution-trader",))
...
def test_should_queue_bootstrap_brain_cycle_skips_recent_bootstrap_tasks(helix_db):
...
def test_should_queue_bootstrap_brain_cycle_allows_new_task_after_cooldown(helix_db):
    from helix.db import get_db
...
def test_bot_pid_probe_tolerates_windows_access_denied(monkeypatch):
    ...
    monkeypatch.setattr("helix.bot.os.name", "nt")
    monkeypatch.setitem(sys.modules, "ctypes", _Ctypes)

    from helix import bot as bot_mod

    assert bot_mod._is_pid_running(12345) is True


def test_bot_process_pending_tasks_bootstrap_dispatches_strategy_developer_research_without_llm(helix_db, monkeypatch):
    from helix.db import get_db
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (
                501,
                '{"source":"bootstrap","message":"Helix just started."}',
    ...
    monkeypatch.setattr("helix.db.claim_pending_tasks", lambda *args, **kwargs: [{"id": 501, "payload": '{"source":"bootstrap","message":"Helix just started."}'}])
    monkeypatch.setattr("helix.brain.assign_research_cycle", lambda: delegated.append("research"))
    monkeypatch.setattr("helix.agents.runner._call_with_tools", _unexpected_call)
