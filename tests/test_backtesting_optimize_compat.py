...
from helix.backtesting import BacktestingClient
from helix.db import get_db
from helix.routers import strategies as strategies_router
from helix.routers import verdict as verdict_router
from helix.strategies.optimizer import _get_param_space, grid_search, optimize_strategy
...
def test_post_backtesting_optimize_parses_dataset_id_and_ranges(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.strategies.core.post_optimization_submit", _fake_submit)
    monkeypatch.setattr("helix.routers.strategies.asyncio.to_thread", _fake_to_thread)
...
def test_post_backtesting_verdict_forwards_payload(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.strategies.verdict_routes.execute_verdict", _fake_execute)
    monkeypatch.setattr("helix.routers.strategies.asyncio.to_thread", _fake_to_thread)
...
def test_post_backtesting_run_offloads_sync_execution(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.strategies.core.post_backtesting_run", _fake_run)
    monkeypatch.setattr("helix.routers.strategies.asyncio.to_thread", _fake_to_thread)
...
def test_native_verdict_route_offloads_sync_execution(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.verdict.execute_verdict", _fake_execute)
    monkeypatch.setattr("helix.routers.verdict.asyncio.to_thread", _fake_to_thread)
...
def test_execute_verdict_resolves_dataset_id_to_latest_matching_result(helix_db):
...
def test_execute_verdict_dataset_resolution_stays_scoped_to_strategy(helix_db):
...
def test_execute_verdict_resolves_decorated_strategy_id_to_canonical_result(helix_db):
...
def test_execute_verdict_falls_back_to_strategy_metrics_when_result_row_missing(helix_db):
...
def test_execute_verdict_direct_result_id_lookup_still_works(helix_db):
...
def test_backtesting_client_run_verdict_falls_back_to_native_route_on_404(monkeypatch):
    ...
    monkeypatch.setattr("helix.backtesting.httpx.Client", _DummyClient)
    monkeypatch.setattr("helix.backtesting.httpx.post", _fallback_post)
...
def test_backtesting_client_sends_helix_api_and_operator_keys(monkeypatch):
    from helix.backtesting import BacktestingClient
    ...
    monkeypatch.setenv("HELIX_API_KEY", "api-key-123")
    monkeypatch.setenv("HELIX_OPERATOR_KEY", "operator-key-456")
    monkeypatch.setattr("helix.backtesting.httpx.Client", _DummyClient)
...
def test_optimize_strategy_uses_explicit_parameter_ranges(monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.optimizer.grid_search", _fake_grid_search)
    monkeypatch.setattr(
        "helix.strategies.optimizer.walk_forward",
        lambda **_kwargs: {"verdict": "PASS", "degradation": 0.1},
    )
    monkeypatch.setattr("helix.api_core.get_settings", lambda: {"backtest_duration_days": 365})
    monkeypatch.setattr("helix.vectordb.store_backtest_result", lambda **_kwargs: None)
...
def test_optimize_strategy_honors_execution_ranges_objective_window_and_base_params(monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.optimizer.grid_search", _fake_grid_search)
    monkeypatch.setattr("helix.strategies.optimizer.walk_forward", _fake_walk_forward)
    monkeypatch.setattr("helix.api_core.get_settings", lambda: {"backtest_duration_days": 365})
    monkeypatch.setattr("helix.vectordb.store_backtest_result", lambda **_kwargs: None)
...
def test_grid_search_sanitizes_legacy_leverage_axes(monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.optimizer.backtest_strategy", _fake_backtest_strategy)
...
def _isolate_param_space_lookups(monkeypatch, *, registry_obj=None):
    ...
    monkeypatch.setattr("helix.strategies.registry.discover", lambda: None)
    monkeypatch.setattr("helix.strategies.registry.get", lambda _sid: registry_obj)
    monkeypatch.setattr(
        "helix.strategies.registry.resolve_runtime_type",
        lambda *_a, **_k: (None, {}),
    )
    monkeypatch.setattr("helix.strategies.registry._TYPE_MAP", {})
    monkeypatch.setattr(
        "helix.strategies.optimizer.pkgutil",
...
def test_optimize_strategy_surfaces_grid_timeout(monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.optimizer.grid_search", _raise_timeout)
    monkeypatch.setattr("helix.api_core.get_settings", lambda: {"backtest_duration_days": 365})
