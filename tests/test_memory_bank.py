...
from helix.api_domains import memory as memory_domain
...
def test_workspace_memory_items_are_sectioned_and_deterministic(helix_db, monkeypatch):
...
def test_daily_agent_logs_are_compacted_with_capped_signal_sections(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
...
def test_search_memory_records_handles_chroma_degraded(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    (workspace_dir / "memory").mkdir(parents=True, exist_ok=True)
...
def test_memory_overview_reports_narrative_browse_mode(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    (workspace_dir / "memory").mkdir(parents=True, exist_ok=True)
    (workspace_dir / "memory" / "MEMORY.md").write_text(
...
def test_memory_overview_drops_chroma_rows_when_vector_layer_disabled(helix_db, monkeypatch):
...
def test_memory_overview_keeps_chroma_row_when_vector_layer_enabled(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    (workspace_dir / "memory").mkdir(parents=True, exist_ok=True)
    monkeypatch.delenv("HELIX_DISABLE_CHROMA_IN_PROCESS", raising=False)
    monkeypatch.setattr(memory_domain, "_run_chroma_subprocess", lambda payload, timeout=60: {"ok": True, "collections": {}})
    monkeypatch.setattr(memory_domain, "_browse_narrative_items_sync", lambda limit: [])
...
def test_memory_overview_browses_live_narratives_by_default(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    (workspace_dir / "memory").mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(memory_domain, "_run_chroma_subprocess", lambda payload, timeout=60: {"ok": True, "collections": {}})
    monkeypatch.setattr(
...
def test_search_memory_records_reports_live_narrative_matches(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    (workspace_dir / "memory").mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(memory_domain, "_run_chroma_subprocess", lambda payload, timeout=60: {"ok": True, "collections": {}})
    monkeypatch.setattr(
...
def test_apply_memory_action_hide_toggles_narrative(helix_db, monkeypatch):
...
def test_memory_maintenance_preview_reports_old_daily_logs(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    daily_dir = workspace_dir / "agents" / "strategy-developer" / "memory"
    daily_dir.mkdir(parents=True, exist_ok=True)
...
def test_memory_maintenance_preview_skips_annotated_daily_logs(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
...
def test_memory_maintenance_run_writes_summary_and_hides_raw_daily_log(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    daily_dir = workspace_dir / "agents" / "strategy-developer" / "memory"
    daily_dir.mkdir(parents=True, exist_ok=True)
    (daily_dir / "2026-01-01.md").write_text(
...
def test_memory_router_search_and_annotation_round_trip(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    (workspace_dir / "memory").mkdir(parents=True, exist_ok=True)
    (workspace_dir / "memory" / "MEMORY.md").write_text(
...
def test_memory_router_maintenance_preview_and_dry_run(helix_db, monkeypatch):
    workspace_dir = _pin_workspace(monkeypatch)
    daily_dir = workspace_dir / "agents" / "brain" / "memory"
    daily_dir.mkdir(parents=True, exist_ok=True)
    (daily_dir / "2026-01-01.md").write_text(
