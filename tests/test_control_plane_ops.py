...
from helix.db import get_db, kv_get, kv_set
from helix.exchange.risk import is_trading_allowed


def test_stop_system_sets_pause_state_and_start_system_clears_it(helix_db):
...
def test_pause_strategy_generation_does_not_toggle_trading_pause(helix_db):
...
def test_trading_halt_reset_clears_pause_and_risk_gates(helix_db):
    kv_set(
        "system_state",
        {
            "paused": True,
...
def test_trading_halt_reset_reports_when_recovery_still_blocks_entries(helix_db):
    kv_set(
        "system_state",
        {
            "paused": True,
...
def test_manual_exchange_reconcile_updates_daemon_and_operator_recovery_state(helix_db, monkeypatch):
    kv_set(
        "daemon_state",
        {
            "running": True,
    ...
    monkeypatch.setattr("helix.exchange.risk.sync_from_trades", lambda: sync_calls.append("sync") or 1)
    ...
    monkeypatch.setattr("helix.exchange.risk.reconcile_all_books", _fake_reconcile_all_books)
...
def test_recovery_batch_rollback_pauses_and_rebuilds_positions(helix_db):
    kv_set(
        "daemon_state",
        {
            "running": True,
