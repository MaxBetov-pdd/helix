...
from helix.db import create_strategy_container, get_db, kv_set
...
def test_settings_snapshot_normalizes_gauntlet_gate_aliases(helix_db):
    kv_set(
...
def test_strategy_gauntlet_status_unifies_workflow_steps_and_robustness_results(helix_db):
...
def test_strategy_gauntlet_status_is_strict_json_serializable_with_nonfinite_metrics(helix_db):
    strategy_id = _create_strategy()
...
def test_settings_pipeline_save_updates_policy_thresholds_and_workflow_settings(helix_db):
    from helix.api_core import PipelineSettingsUpdateBody, get_settings, put_pipeline_settings
...
def test_json_dumps_sanitizes_non_finite_floats(helix_db):
    ...
    from helix.gauntlet.store import _json_dumps
...
def test_rescrub_json_text_repairs_legacy_infinity_rows(helix_db):
    from helix.gauntlet.store import _rescrub_json_text
...
def test_gauntlet_status_payload_is_strict_json_with_poisoned_step(helix_db):
