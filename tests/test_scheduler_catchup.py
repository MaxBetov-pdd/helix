"""Tests for helix.scheduler.apply_startup_catchup — collapse missed cycles."""

from datetime import datetime, timedelta, timezone

from helix.db import get_db
from helix.scheduler import apply_startup_catchup
...
def test_no_jobs_returns_zero_counts(helix_db):
    summary = apply_startup_catchup()
    assert summary["total_jobs"] == 0
    assert summary["fast_forwarded"] == 0


def test_fresh_job_not_touched(helix_db):
...
def test_recently_overdue_job_not_collapsed(helix_db):
...
def test_long_overdue_job_collapsed_to_one_run(helix_db):
...
def test_disabled_jobs_ignored(helix_db):
...
def test_job_with_null_next_run_skipped(helix_db):
