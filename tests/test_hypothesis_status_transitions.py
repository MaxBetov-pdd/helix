...
def test_update_status_transitions_state(helix_db):
    hyp = _real_hyp()
...
def test_update_status_writes_history_row(helix_db):
    hyp = _real_hyp()
...
def test_update_status_history_accumulates(helix_db):
    hyp = _real_hyp()
...
def test_update_status_rejects_invalid_state(helix_db):
    hyp = _real_hyp()
    with pytest.raises(ValueError):
        update_hypothesis_status(hyp["id"], new_status="bogus", memo={}, by="x")


def test_update_status_missing_hypothesis_raises(helix_db):
    with pytest.raises(ValueError):
