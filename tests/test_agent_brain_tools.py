...
from helix.agents.tools_brain import _tool_create_strategy


def test_tool_create_strategy_returns_duplicate_error_without_keyerror(monkeypatch):
    monkeypatch.setattr(
        "helix.ai.normalize_provider_and_model",
        lambda model, model_id: (model, model_id),
    )
    monkeypatch.setattr(
        "helix.agents.tools_research.assert_hypothesis_spawn_allowed",
        lambda hypothesis_id: None,
    )
    monkeypatch.setattr(
        "helix.brain.create_strategy",
...
def test_tool_create_strategy_allows_research_only_payload(monkeypatch):
    ...
    monkeypatch.setattr("helix.brain.create_strategy", _fake_create_strategy)
...
def test_tool_create_strategy_rejects_agent_context_without_planner_task(helix_db):
...
def test_bootstrap_brain_cannot_assign_quant_researcher_research_task(monkeypatch, helix_db):
    from helix.agents import tools_brain
...
def test_non_bootstrap_brain_can_still_assign_quant_researcher_support_task(monkeypatch, helix_db):
    from helix.agents import tools_brain
    from helix.agents.manager import create_agent
    from helix.db import get_db
