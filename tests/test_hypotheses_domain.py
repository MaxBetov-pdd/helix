...
from helix.db import factory_reset, get_db, kv_set


def test_create_hypothesis_persists_first_class_fields_and_lists(helix_db):
    from helix.hypotheses import create_hypothesis, get_hypothesis
...
def test_add_hypothesis_artifact_persists_source_provenance(helix_db):
    from helix.hypotheses import add_hypothesis_artifact, create_hypothesis
...
def test_hypothesis_manager_state_defaults_and_lifecycle_transitions(helix_db):
    from helix.hypotheses import (
        archive_hypothesis,
        create_hypothesis,
        restore_hypothesis,
        trash_hypothesis,
...
def test_list_hypotheses_filters_by_view_search_and_sort(helix_db):
    from helix.hypotheses import archive_hypothesis, create_hypothesis, list_hypotheses, trash_hypothesis
...
def test_bulk_hypothesis_lifecycle_mutations_only_touch_requested_ids(helix_db):
    from helix.hypotheses import (
...
def test_record_data_gap_rolls_up_repeated_requests_and_links_to_strategy_or_hypothesis(helix_db):
    from helix.hypotheses import create_hypothesis, list_ranked_data_gaps, record_data_gap
...
def test_factory_reset_pipeline_data_wipes_hypothesis_tables(helix_db):
    from helix.hypotheses import add_hypothesis_artifact, create_hypothesis, record_data_gap
...
def test_reset_friendly_schema_includes_hypotheses_tables_and_strategy_backlink(helix_db):
    with get_db() as conn:
...
def test_record_data_gap_requires_hypothesis_or_strategy_link(helix_db):
    from helix.hypotheses import record_data_gap
...
def test_data_gap_links_reject_orphan_rows_at_db_layer(helix_db):
    with get_db() as conn:
        conn.execute(
            """
...
def test_deleting_hypothesis_nulls_strategy_and_child_backlinks(helix_db):
...
def test_get_hypothesis_spawn_stats_uses_live_research_settings(helix_db):
    from helix.hypotheses import create_hypothesis, get_hypothesis_spawn_stats
...
def test_brain_create_strategy_rejects_unknown_hypothesis_id(helix_db):
