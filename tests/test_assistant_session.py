...
import helix.assistant_db as adb
import helix.assistant_session as asess
...
def test_thread_reuse_and_monotonic_seq(helix_db):
...
def test_strategy_scoped_thread_is_separate(helix_db):
...
def test_run_turn_dispatches_create_strategy_from_nl(helix_db, monkeypatch):
...
def test_run_turn_gates_confirm_actions(helix_db, monkeypatch):
    thread = adb.create_or_get_active_thread("global", None)
    calls = {"n": 0}
...
def test_confirm_action_reject_does_not_execute(helix_db, monkeypatch):
    thread = adb.create_or_get_active_thread("global", None)
...
def test_no_action_tools_when_actions_disabled(helix_db):
...
def test_cost_cap_blocks_turn(helix_db, monkeypatch):
...
def test_assistant_create_strategy_end_to_end(helix_db):
    ...
    from helix.agents.tools_assistant import _tool_assistant_create_strategy
...
def test_assistant_create_strategy_rejects_unknown_family(helix_db):
    from helix.agents.tools_assistant import _tool_assistant_create_strategy
    from helix.db import get_db
...
def test_page_context_block_renders_entity_and_summary():
    from helix.assistant_context import _format_page_context
