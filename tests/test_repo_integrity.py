...
DUPLICATE_SCAN_ROOTS = (
    REPO_ROOT / "helix" / "routers",
    REPO_ROOT / "helix" / "strategies" / "builtin",
)
...
@pytest.mark.parametrize("module_name", _module_names("helix.routers"))
def test_router_modules_import_and_expose_router(module_name: str, helix_db):
    module = importlib.import_module(f"helix.routers.{module_name}")

    assert isinstance(module.router, APIRouter)


@pytest.mark.parametrize("module_name", _module_names("helix.strategies.builtin"))
def test_builtin_strategy_modules_import(module_name: str):
    importlib.import_module(f"helix.strategies.builtin.{module_name}")
