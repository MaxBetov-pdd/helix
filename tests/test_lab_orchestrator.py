...
from helix.lab_db import get_lab_experiment, get_lab_job
from helix.lab_db import replace_regime_segments, upsert_lab_experiment, upsert_regime_program, upsert_snapshot_manifest
...
def test_maybe_enqueue_due_continuous_cycle_honors_enabled_schedule(monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_orchestrator._now", lambda: fixed_now)
...
def test_run_orchestrator_cycle_job_reuses_active_program_model(monkeypatch):
    ...
    from helix.lab_db import create_or_update_model_version, update_regime_program as _update_program
