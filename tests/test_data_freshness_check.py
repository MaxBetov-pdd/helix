...
def test_green_and_full_score_when_fresh(helix_db):
    _reset_stats()
...
def test_red_on_repeated_stream_failures(helix_db):
    _reset_stats()
    for _ in range(3):
        try:
...
def test_amber_when_last_success_is_stale(helix_db):
    _reset_stats()
    dm._record_collection("ohlcv", None, 100, True)
    with dm._stats_lock:
...
def test_staleness_sla_is_operator_overridable(helix_db):
    from helix.db import kv_set
    from helix.health_monitor import _stream_staleness_sla_minutes

    assert _stream_staleness_sla_minutes("ohlcv") == 60  # default
    kv_set("helix:settings", {"staleness_thresholds": {"ohlcv": 5}})
