...
from helix.lab_intent_dispatch import dispatch_paper_intent
from helix.lab_models import DispatchPaperIntentRequest, SelectorDecideRequest
from helix.lab_selector import decide_current_regime
...
def test_selector_blocks_uncertain_regime_on_low_confidence(monkeypatch):
    ...
    monkeypatch.setattr("helix.lab_selector._resolve_market_frame", lambda *_args, **_kwargs: pd.DataFrame())
    monkeypatch.setattr("helix.lab_selector.compute_features_from_frame", lambda *_args, **_kwargs: pd.DataFrame({"x": [1]}))
    monkeypatch.setattr(
        "helix.lab_selector.classify_features",
...
def test_dispatch_routes_to_paper_and_persists_feedback(helix_db):
    model_id = _bootstrap_model()
...
def test_dispatch_uses_base_strategy_id_but_preserves_candidate_key_metadata(helix_db):
    model_id = _bootstrap_model()
    event = create_selection_event(
        symbol="BTC/USDT",
        timeframe="1h",
...
def test_dispatch_requires_selection_event_id(helix_db):
    model_id = _bootstrap_model()
