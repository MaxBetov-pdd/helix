...
def test_data_schema_injected_when_present():
    ...
    schema = "## Core Columns\n- timestamp\n- close"
    with patch("helix.context.read_workspace", side_effect=_make_read_workspace_mock(schema)):
        with patch("helix.context._get_chroma_recall", return_value=""):
            # Patch at the source (helix.db.get_db) not helix.context.get_db — context.py
            # imports get_db locally inside the function body, so there is no module-level
            # name to intercept at helix.context.get_db.
            with patch("helix.db.get_db", side_effect=Exception("no db")):
...
def test_data_schema_absent_does_not_raise():
    ...
    with patch("helix.context.read_workspace", side_effect=_make_read_workspace_mock(None)):
        with patch("helix.context._get_chroma_recall", return_value=""):
            # See first test for why we patch helix.db.get_db and not helix.context.get_db
            with patch("helix.db.get_db", side_effect=Exception("no db")):
                from helix.context import build_agent_context
                result = build_agent_context(
                    agent_id="quant-researcher",
...
def test_build_agent_context_still_includes_chroma_recall_when_task_present():
    ...
    with patch("helix.context.read_workspace", side_effect=_make_read_workspace_mock(None)):
        with patch("helix.context._get_chroma_recall", return_value=chroma_recall):
            with patch("helix.db.get_db", side_effect=Exception("no db")):
                from helix.context import build_agent_context
                result = build_agent_context(
                    agent_id="quant-researcher",
...
def test_build_agent_context_includes_strategy_diversity_guard_when_saturated():
    with patch("helix.context.read_workspace", side_effect=_make_read_workspace_mock(None)):
        with patch("helix.context._get_chroma_recall", return_value=""):
            with patch(
                "helix.context.render_strategy_diversity_guard",
...
def test_build_agent_context_uses_utc_daily_memory_paths_when_enabled():
    ...
    with patch("helix.context.read_workspace", side_effect=_mock_read_workspace):
        with patch("helix.context._get_chroma_recall", return_value=""):
            with patch("helix.context._utc_today", return_value=date(2026, 4, 14)):
                with patch("helix.context._utc_yesterday", return_value=date(2026, 4, 13)):
                    with patch("helix.db.get_db", side_effect=Exception("no db")):
                        from helix.context import build_agent_context
                        build_agent_context(
                            agent_id="quant-researcher",
