...
def test_run_scan_signal_only_skips_execution(monkeypatch, helix_db):
...
def test_run_scan_with_execution_applies_actions(monkeypatch, helix_db):
    monkeypatch.setattr(scanner_mod, "_load_deployed_strategies", lambda: {"S-STUB": {"asset": "BTC"}})
    monkeypatch.setattr(scanner_mod, "_load_live_price_cache", lambda: ({}, None))
    monkeypatch.setattr(scanner_mod, "sync_from_trades", lambda: None)
    monkeypatch.setattr(scanner_mod, "_evaluate_signal_matrix", lambda *args, **kwargs: _stub_signals())
...
def test_run_scan_execution_request_degrades_to_signal_only_by_policy(monkeypatch, helix_db):
    monkeypatch.setattr(scanner_mod, "_load_deployed_strategies", lambda: {"S-STUB": {"asset": "BTC"}})
    monkeypatch.setattr(scanner_mod, "_load_live_price_cache", lambda: ({}, None))
    monkeypatch.setattr(scanner_mod, "sync_from_trades", lambda: None)
    monkeypatch.setattr(scanner_mod, "_evaluate_signal_matrix", lambda *args, **kwargs: _stub_signals())
...
def test_run_scan_signal_only_preserves_prior_execution_summary(monkeypatch, helix_db):
    kv_set(
        "scanner_state",
        {
            "execution_summary": {
...
def test_manage_positions_queues_trade_execution_when_fast_path_disabled(monkeypatch, helix_db):
...
def test_manage_positions_executes_paper_stage_locally_by_default(monkeypatch, helix_db):
    fills: list[dict] = []
    opened: dict = {}
...
def test_manage_positions_preserves_blocked_signal_reason(monkeypatch, helix_db):
