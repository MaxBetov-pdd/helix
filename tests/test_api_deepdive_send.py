...
from helix.deepdive_db import create_or_get_active_thread
...
def test_send_streams_events_in_order(thread, client, monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.deepdive.run_turn", fake_run_turn)
...
def test_send_unknown_thread_returns_404(client, helix_db):
...
def test_send_propagates_error_event(thread, client, monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.deepdive.run_turn", errored_run_turn)
...
def test_send_to_archived_thread_returns_409(thread, client):
    from helix.deepdive_db import archive_thread
