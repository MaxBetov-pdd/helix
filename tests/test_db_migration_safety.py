...
from helix import migrations as mig
...
def test_failing_gauntlet_schema_init_sets_flag(monkeypatch):
    ...
    init_db()
    import helix.gauntlet.store as gstore
...
def test_rebuild_keeps_legacy_table_when_rows_unrescued(helix_db):
    with get_db() as conn:
...
def test_rebuild_drops_legacy_table_when_all_rows_rescued(helix_db):
    with get_db() as conn:
        conn.execute("INSERT INTO strategies (id, name) VALUES ('S1', 'Strat 1')")
...
def test_rebuild_keeps_legacy_table_when_schema_unrecognizable(helix_db):
    with get_db() as conn:
        conn.execute("DROP TABLE IF EXISTS backtest_results")
...
def test_second_rebuild_stashes_previously_kept_legacy_table(helix_db):
...
def test_rebuild_drops_empty_leftover_legacy_table(helix_db):
    with get_db() as conn:
        conn.execute("INSERT INTO strategies (id, name) VALUES ('S1', 'Strat 1')")
