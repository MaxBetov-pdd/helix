...
from helix.api_domains import trading as trading_domain
from helix.db import get_db
from helix.exchange import risk as risk_mod
...
def test_fail_unfilled_open_trade_marks_failed_and_releases(helix_db):
...
def test_fail_unfilled_open_trade_skips_filled_position(helix_db):
...
def test_fail_unfilled_open_trade_is_idempotent(helix_db):
...
def test_report_execution_failure_open_cleans_phantom_trade(helix_db, monkeypatch):
...
def test_report_execution_failure_close_leaves_real_position_open(helix_db, monkeypatch):
...
def test_read_all_trades_lists_across_statuses_with_filter_and_paging(helix_db):
...
def test_mark_trade_failed_clears_unfilled_phantom(helix_db):
...
def test_mark_trade_failed_refuses_filled_position(helix_db):
...
def test_mark_trade_failed_handles_missing_and_nonopen(helix_db):
...
def test_execute_trade_intent_blocks_open_when_trading_disallowed(helix_db, monkeypatch):
...
def test_execute_trade_intent_rejects_oversized_open(helix_db, monkeypatch):
...
def test_reconcile_exchange_positions_flags_duplicate_sqlite_trades(helix_db, monkeypatch):
