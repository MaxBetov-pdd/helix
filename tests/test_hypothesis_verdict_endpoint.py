...
def test_verdict_endpoint_triggers_writer(helix_db):
...
def test_verdict_endpoint_falls_back_to_math_floor_on_llm_failure(helix_db):
    ...
    with patch("helix.hypothesis_verdict._call_llm", side_effect=RuntimeError("boom")):
        client = TestClient(app)
        r = client.post(f"/api/hypotheses/{hyp['id']}/verdict")
    assert r.status_code == 200
    body = r.json()
...
def test_verdict_endpoint_missing_hypothesis_404(helix_db):
    client = TestClient(app)
