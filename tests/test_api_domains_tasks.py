...
def test_get_agent_tasks_merges_agent_and_global_rows(helix_db):
...
def test_get_agent_tasks_filters_stale_brain_callback_history(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    with get_db() as conn:
        ...
        hidden_done_id = conn.execute(
            ...
            INSERT INTO tasks
                (type, payload, status, priority, created_at, completed_at, error)
            VALUES
                ('brain_invoke', '{"source":"bootstrap","message":"Helix just started."}', 'done', 1, ?, ?, NULL)
            """,
            (now, now),
        ).lastrowid
...
def test_dismiss_agent_task_hides_failed_row_and_keeps_audit(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    _ensure_agent()
    with get_db() as conn:
...
def test_dismiss_global_task_hides_failed_row(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    with get_db() as conn:
        task_id = conn.execute(
            """
...
def test_dismiss_running_task_is_rejected(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    _ensure_agent()
    with get_db() as conn:
        task_id = conn.execute(
...
def test_get_task_containers_filters_and_audit_lookup(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    _ensure_agent()
    with get_db() as conn:
        conn.execute(
...
def test_get_task_containers_sanitizes_nonfinite_payload_values(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    _ensure_agent()
    with get_db() as conn:
        conn.execute(
...
def test_get_pipeline_errors_and_activity_stub(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    _ensure_agent()
    with get_db() as conn:
        conn.execute(
...
def test_seed_pipeline_creates_missing_strategies(monkeypatch, helix_db):
    created: list[str] = []
    promoted: list[str] = []
