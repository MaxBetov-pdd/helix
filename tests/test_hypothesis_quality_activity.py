...
from helix.api_domains.hypotheses import (
    QUALITY_ENRICHED,
    QUALITY_PLACEHOLDER,
    QUALITY_PRODUCTIVE,
    QUALITY_RESEARCHING,
...
def test_is_placeholder_operator_seed_with_unspecified(helix_db):
    hyp = _placeholder_hyp()
    assert _is_placeholder_hypothesis(hyp) is True


def test_is_placeholder_after_enrichment_false(helix_db):
    hyp = _placeholder_hyp()
...
def test_is_placeholder_non_operator_seed_never_placeholder(helix_db):
...
def test_list_surface_exposes_quality_field(helix_db):
    _placeholder_hyp()
    _real_hyp()
...
def test_list_filter_by_quality_placeholder(helix_db):
    _placeholder_hyp()
    _real_hyp()
    client = TestClient(app)
...
def test_list_unknown_quality_filter_ignored(helix_db):
    _placeholder_hyp()
    _real_hyp()
    client = TestClient(app)
...
def test_detail_exposes_quality_and_agent_activity_empty(helix_db):
    hyp = _real_hyp()
    client = TestClient(app)
...
def test_detail_agent_activity_lists_recent_task(helix_db):
...
def test_update_endpoint_overwrites_fields(helix_db):
    hyp = _placeholder_hyp()
    client = TestClient(app)
    r = client.post(
...
def test_update_endpoint_unknown_id_returns_404(helix_db):
    client = TestClient(app)
    r = client.post("/api/hypotheses/HYP-fake/update", json={"title": "x"})
    assert r.status_code == 404


def test_update_endpoint_rejects_empty_required_field(helix_db):
    hyp = _placeholder_hyp()
    client = TestClient(app)
...
def test_research_endpoint_enqueues_new_task(helix_db):
    hyp = _placeholder_hyp()
    add_hypothesis_artifact(
...
def test_research_endpoint_idempotent_when_task_already_running(helix_db):
    from helix.system_pause import set_system_mode
...
def test_research_endpoint_unknown_id_returns_404(helix_db):
    client = TestClient(app)
...
def test_list_summary_exposes_artifact_source_tags(helix_db):
    hyp = _real_hyp()
    add_hypothesis_artifact(
...
def test_list_summary_source_tags_empty_when_no_artifacts(helix_db):
    hyp = _real_hyp()
    client = TestClient(app)
    row = [r for r in client.get("/api/hypotheses").json()["hypotheses"] if r["id"] == hyp["id"]][0]
...
def test_detail_exposes_source_tags_from_artifacts(helix_db):
    hyp = _real_hyp()
    add_hypothesis_artifact(
...
def test_detail_includes_latest_result_from_backtest(helix_db):
