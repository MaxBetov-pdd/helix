...
def test_promotion_loop_dispatches_top_k_by_promise(helix_db):
    ...
    with patch("helix.brain.assign_task", return_value=999) as m:
...
def test_promotion_loop_skips_proposed_hypotheses(helix_db):
    from helix.hypothesis_promotion import run_promotion_loop
...
def test_promotion_loop_respects_cooldown(helix_db):
    from helix.hypothesis_promotion import run_promotion_loop
    h = _hyp()
...
def test_promotion_loop_respects_global_cap(helix_db):
    """When MAX_IN_FLIGHT is hit, dispatches nothing new."""
    from helix.hypothesis_promotion import run_promotion_loop
    _hyp(); _hyp(); _hyp()
    with patch("helix.hypothesis_promotion._current_in_flight_task_count", return_value=99):
        with patch("helix.brain.assign_task") as m:
