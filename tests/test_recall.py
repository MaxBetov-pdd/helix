...
from helix import recall as recall_mod
...
def test_recall_empty_query_returns_empty(helix_db):
...
def test_recall_fts_only_returns_hits_when_aux_disabled(helix_db, monkeypatch):
...
def test_recall_rerank_uses_llm_scores(helix_db, monkeypatch):
...
def test_recall_summary_failure_yields_empty_summary_with_hits(helix_db, monkeypatch):
...
def test_recall_scope_decisions_excludes_tasks(helix_db, monkeypatch):
...
def test_recall_scope_tasks_excludes_decisions(helix_db, monkeypatch):
    monkeypatch.setattr(recall_mod, "_call_aux_llm", lambda *_a, **_kw: "")
    _seed_decision("ZARFZIPZAR a unique decision")
...
def test_recall_scope_all_returns_both_sources(helix_db, monkeypatch):
    monkeypatch.setattr(recall_mod, "_call_aux_llm", lambda *_a, **_kw: "")
...
def test_recall_writes_cost_row_to_agent_tasks(helix_db, monkeypatch):
    monkeypatch.setattr(recall_mod, "_call_aux_llm", lambda *_a, **_kw: "")
...
def test_recall_query_with_quote_does_not_crash_fts(helix_db, monkeypatch):
...
def test_recall_limit_respected(helix_db, monkeypatch):
    monkeypatch.setattr(recall_mod, "_call_aux_llm", lambda *_a, **_kw: "")
    for i in range(20):
...
def test_recall_aux_model_label_is_provider_colon_model(helix_db, monkeypatch):
    monkeypatch.setattr(recall_mod, "_call_aux_llm", lambda *_a, **_kw: "")
...
def test_recall_latency_recorded(helix_db, monkeypatch):
    monkeypatch.setattr(recall_mod, "_call_aux_llm", lambda *_a, **_kw: "")
