...
from helix.hypotheses import archive_hypothesis, create_hypothesis, restore_hypothesis, trash_hypothesis
...
def test_archive_records_reason_and_restore_clears_it(helix_db):
    h = _hyp()
...
def test_trash_records_reason(helix_db):
    h = _hyp()
    trash_hypothesis(h["id"], reason="operator_trash")
    assert get_crucible(h["id"])["archive_reason"] == "operator_trash"


def test_disproven_verdict_attributes_archive_reason(helix_db):
    kv_set("helix:settings", {"research_settings": {"hypothesis_discipline": {
    ...
    with patch("helix.hypothesis_verdict._call_llm", return_value=json.dumps({"verdict": "researching", "rationale": "r"})):
