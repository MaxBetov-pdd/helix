...
from helix.agents.runner import _build_tool_ledger
from helix.db import log_tool_call


def test_ledger_empty_for_unknown_task(helix_db):
    text, trace = _build_tool_ledger("T99999")
    assert text == ""
    assert trace == []


def test_ledger_empty_for_blank_task_id(helix_db):
    text, trace = _build_tool_ledger("")
    assert text == ""
    assert trace == []


def test_ledger_marks_artifact_tool_failure(helix_db):
    task_id = "T00123"
...
def test_ledger_marks_artifact_tool_success(helix_db):
    task_id = "T00124"
    log_tool_call(
        task_id,
        agent_id="strategy-developer",
...
def test_ledger_warns_when_no_artifact_tools_ran(helix_db):
...
def test_ledger_separates_artifact_from_auxiliary(helix_db):
    task_id = "T00126"
