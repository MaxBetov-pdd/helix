...
def test_aged_zero_child_hypothesis_is_NOT_auto_disproven(helix_db):
...
def test_recent_hypothesis_unchanged(helix_db):
    from helix.hypothesis_cleanup import cleanup_stale_hypotheses
    hyp = _bare_hyp()
...
def test_endpoint_returns_zero(helix_db):
...
def test_dry_run_returns_zero_preview(helix_db):
    from helix.hypothesis_cleanup import cleanup_stale_hypotheses
    hyp = _bare_hyp()
    _age_created_at(hyp["id"], days_ago=30)
