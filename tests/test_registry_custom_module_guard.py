...
def test_forbidden_import_raises_and_is_not_imported(monkeypatch, tmp_path):
    ...
    assert "helix.strategies.custom.evil_os_mod" not in sys.modules
...
def _count_scans(monkeypatch):
    ...
    import helix.sandbox.ast_guard as guard
    ...
    monkeypatch.setattr("helix.sandbox.ast_guard.scan_source", _spy)
    return calls
