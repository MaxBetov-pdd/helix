...
from helix.trading_smoke import collect_trading_plane_smoke


def test_collect_trading_plane_smoke_passive_ok(helix_db, monkeypatch):
    monkeypatch.setattr(
        "helix.trading_smoke.get_account_value",
    ...
    monkeypatch.setattr(
        "helix.trading_smoke.get_positions",
        lambda testnet=True: {"positions": [{"position": {"coin": "BTC", "szi": "0"}}]},
    )
    monkeypatch.setattr("helix.trading_smoke.get_open_orders", lambda testnet=True: [])
    monkeypatch.setattr(
        "helix.trading_smoke.get_all_mids",
        lambda testnet=True: {"SOL": 150.0, "ETH": 3400.0},
    )
    monkeypatch.setattr("helix.trading_smoke.log_activity", lambda *args, **kwargs: None)
...
def test_collect_trading_plane_smoke_active_order_path(helix_db, monkeypatch):
    monkeypatch.setattr(
        "helix.trading_smoke.get_account_value",
        lambda testnet=True, require_connection=True: {"accountValue": 5000.0, "totalMarginUsed": 0.0},
    )
    monkeypatch.setattr("helix.trading_smoke.get_positions", lambda testnet=True: {"positions": []})
    monkeypatch.setattr("helix.trading_smoke.get_open_orders", lambda testnet=True: [])
    monkeypatch.setattr("helix.trading_smoke.get_all_mids", lambda testnet=True: {"SOL": 150.0})
    monkeypatch.setattr(
        "helix.trading_smoke.market_order",
    ...
    monkeypatch.setattr(
        "helix.trading_smoke.close_position",
        lambda asset, size, side="sell", testnet=True: {
            "close_price": 149.5,
            "order_id": "close-1",
        },
    )
    monkeypatch.setattr("helix.trading_smoke.cancel_all_orders", lambda asset=None, testnet=True: [])
    monkeypatch.setattr("helix.trading_smoke.log_activity", lambda *args, **kwargs: None)
...
def test_collect_trading_plane_smoke_active_close_failure_retains_trade(helix_db, monkeypatch):
    monkeypatch.setattr(
        "helix.trading_smoke.get_account_value",
        lambda testnet=True, require_connection=True: {"accountValue": 5000.0, "totalMarginUsed": 0.0},
    )
    ...
    monkeypatch.setattr(
        "helix.trading_smoke.cancel_all_orders",
