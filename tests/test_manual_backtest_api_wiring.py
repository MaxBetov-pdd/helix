...
def test_send_visual_strategy_to_forge(helix_db):
...
def test_send_code_strategy_to_forge(helix_db):
...
def test_send_to_forge_rejects_unregistered_type(helix_db):
...
def test_per_spec_rule_engine_ids_resolve_and_segregate(helix_db):
...
def test_send_to_forge_rejects_bad_mode(helix_db):
    with pytest.raises(core.HTTPException):
