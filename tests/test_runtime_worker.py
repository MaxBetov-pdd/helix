...
def test_process_agent_tasks_once_claims_and_runs(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.db.claim_pending_agent_tasks",
        lambda agent_id, limit=None: (
            claim_limits.append((agent_id, limit)) or claimed_by_agent.get(agent_id, [])[: limit or None]
        ),
    )
    monkeypatch.setattr(
        "helix.runtime_worker._run_agent_task",
        _fake_run_agent_task,
    )
    monkeypatch.setattr(
        "helix.runtime_worker._recover_durable_completed_develop_candidate_tasks",
        lambda: 0,
    )
    monkeypatch.setattr(
        "helix.runtime_worker._preempt_research_for_waiting_develop_candidate_tasks",
        lambda: 0,
    )
    monkeypatch.setattr(
        "helix.runtime_worker._headless_task_processing_allowed",
        lambda: True,
    )
...
def test_process_agent_tasks_once_keeps_claiming_with_long_task(monkeypatch):
    from helix import runtime_worker as rw
    ...
    async def _exercise():
        ...
        monkeypatch.setattr("helix.db.get_db", lambda: _Conn())
        monkeypatch.setattr("helix.db.claim_pending_agent_tasks", _claim)
        monkeypatch.setattr("helix.runtime_worker._run_agent_task", _fake_run_agent_task)
        monkeypatch.setattr(
            "helix.runtime_worker._recover_durable_completed_develop_candidate_tasks",
            lambda: 0,
        )
        ...
        monkeypatch.setattr("helix.runtime_worker._terminal_agent_task_ids", lambda _task_ids: set())
        monkeypatch.setattr("helix.runtime_worker._headless_task_processing_allowed", lambda: True)
...
def test_process_agent_tasks_once_recovers_completed_develop_candidate(helix_db, monkeypatch):
    from helix import runtime_worker as rw
    ...
    async def _exercise():
        ...
        setattr(active, "_helix_agent_id", "strategy-developer")
        setattr(active, "_helix_agent_task_id", 99002)
        monkeypatch.setattr(rw, "_active_agent_tasks", {active}, raising=False)
        monkeypatch.setattr(rw, "_agent_claim_lock", None, raising=False)
        monkeypatch.setattr(rw, "_agent_claim_cursor", 0, raising=False)
        monkeypatch.setattr("helix.db.claim_pending_agent_tasks", lambda _agent_id, limit=None: [])
        monkeypatch.setattr("helix.runtime_worker._headless_task_processing_allowed", lambda: True)
...
def test_process_agent_tasks_once_preempts_stale_research_for_strategy_creation(helix_db, monkeypatch):
    from helix import runtime_worker as rw
    from helix.db import get_db
    ...
    async def _exercise():
        ...
        active = asyncio.create_task(_blocked_runner())
        setattr(active, "_helix_agent_id", "strategy-developer")
        setattr(active, "_helix_agent_task_id", 99003)
        monkeypatch.setattr(rw, "_active_agent_tasks", {active}, raising=False)
        monkeypatch.setattr(rw, "_agent_claim_lock", None, raising=False)
        monkeypatch.setattr(rw, "_agent_claim_cursor", 0, raising=False)
        monkeypatch.setattr("helix.runtime_worker._run_agent_task", _fake_run_agent_task)
        monkeypatch.setattr("helix.runtime_worker._headless_task_processing_allowed", lambda: True)
        monkeypatch.setattr("helix.system_mode_policy.is_manual_mode", lambda: False)
...
def test_refine_crucible_research_is_not_preempted(helix_db):
...
def test_crucible_planner_backtest_task_runs_deterministically(helix_db, monkeypatch):
    from helix import runtime_worker as rw
    from helix.db import get_db
    ...
    monkeypatch.setattr("helix.evolution.run_backtest_validation", _fake_run_backtest_validation)
    monkeypatch.setattr("helix.agents.runner.run_agent_task", _unexpected_agent_runner)
...
def test_process_brain_tasks_once_skips_when_bot_runtime_active(monkeypatch):
    ...
    monkeypatch.setattr("helix.runtime_worker._bot_runtime_active", lambda: True)
    monkeypatch.setattr("helix.db.claim_pending_tasks", _claim_pending_tasks)
...
def test_process_brain_tasks_once_claims_operator_chat_when_autonomy_paused(monkeypatch):
    ...
    monkeypatch.setattr("helix.runtime_worker._bot_runtime_active", lambda: False)
    monkeypatch.setattr("helix.system_pause.is_autonomy_paused", lambda: True)
    monkeypatch.setattr("helix.system_mode_policy.reconcile_manual_mode_backlog", lambda: None)
    monkeypatch.setattr("helix.db.claim_pending_tasks", _claim_pending_tasks)
...
def test_bot_runtime_active_requires_fresh_task_worker_heartbeat(monkeypatch):
    monkeypatch.setattr(
        "helix.runtime_worker._get_bot_lock_status",
...
def test_get_bot_task_worker_status_reports_missing_heartbeat(monkeypatch):
    monkeypatch.setattr("helix.db.kv_get", lambda _key, default=None: default)
...
def test_get_api_task_worker_status_requires_agent_and_brain_heartbeats(monkeypatch):
    ...
    monkeypatch.setattr("helix.db.kv_get", _kv_get)
...
def test_agent_task_timeout_resolver_uses_shared_defaults(monkeypatch):
    from helix.runtime_worker import _resolve_agent_task_timeout_seconds

    monkeypatch.delenv("HELIX_AGENT_TASK_TIMEOUT_SECONDS", raising=False)
    monkeypatch.setattr("helix.db.kv_get", lambda _key, default=None: default)
...
def test_agent_task_timeout_resolver_honors_settings(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        } if key == "helix:settings" else default,
    )
