...
from helix.runtime_worker import process_brain_tasks_once


def test_headless_brain_transient_failure_requeues_task(helix_db, monkeypatch):
...
def test_headless_brain_transient_failure_exhausts_after_budget(helix_db, monkeypatch):
    from helix import runtime_worker
...
def test_headless_brain_timeout_requeues_task(helix_db, monkeypatch):
    from helix import runtime_worker
...
def test_headless_brain_agent_callback_timeout_suppresses_retry_and_reviews_task(helix_db, monkeypatch):
    from helix import runtime_worker
...
def test_direct_brain_chat_rate_limit_returns_retryable_error(monkeypatch):
    ...
    monkeypatch.setattr("helix.agents.runner._call_with_tools", _raise_rate_limit)
    monkeypatch.setattr("helix.brain.resolve_brain_provider_model", lambda provider, model: ("openai", "gpt-5.2"))
    monkeypatch.setattr("helix.context.build_chat_context", lambda: "context")
