...
from helix.exchange import books
...
class TestBooksResolution:
    def test_disabled_routes_everything_to_main(self, helix_db):
        kv_set("helix:settings", {"live_books_enabled": False})
    ...
    def test_long_routes_to_long_book(self, helix_db):
    ...
    def test_active_addresses_include_both_books(self, helix_db):
        _books_settings(hyperliquid_long_book_address=LONG_ADDR)
        pairs = dict(books.active_book_addresses())
        assert pairs.get("long") == LONG_ADDR
        assert pairs.get("short") == SHORT_ADDR

    def test_status_reports_long_only(self, helix_db):
        _books_settings(hyperliquid_short_book_address="")
...
class TestCanOpenBookScoping:
    def test_long_and_short_same_asset_coexist_across_books(self, helix_db):
        _books_settings()
    ...
    def test_same_asset_same_book_still_one_net_position(self, helix_db):
        _books_settings()
        _seed_live_position("l-1", "BTC", "long", "trend-strat", "long")
    ...
    def test_live_cap_is_per_book(self, helix_db):
...
class TestReconcilerBookSafetyGuard:
    ...
    def test_master_pass_does_not_ghost_close_short_book_trade(self, helix_db, monkeypatch):
        _books_settings()
        _seed_open_trade("T-SHORT-1", "BTC", "short", "scalp", "short")
        monkeypatch.setattr(
            "helix.exchange.risk._snapshot_exchange_state", self._empty_snapshot
        )
    ...
    def test_short_pass_considers_short_book_trade(self, helix_db, monkeypatch):
        _books_settings()
    ...
    def test_disabled_books_still_sweep_leftover_book_trade(self, helix_db, monkeypatch):
...
class TestLongOnlyVolumeGateSurfacing:
    def test_status_note_explains_100k_volume_gate(self, helix_db):
        _books_settings(hyperliquid_short_book_address="")
        status = books.live_books_status()
        assert status["long_only"] is True
    ...
    def test_long_only_notification_is_throttled(self, helix_db, monkeypatch):
        from helix import scanner

        calls = []
        monkeypatch.setattr(
            "helix.notifications.emit_notification",
...
class TestSizeQuantization:
    ...
    def test_rounds_down_to_szdecimals(self, monkeypatch):
        import helix.exchange.hyperliquid as hl
    ...
    def test_close_fails_open_to_raw_size_when_precision_unknown(self, monkeypatch):
        ...
        monkeypatch.setattr("helix.sim.clock.is_sim_active", lambda: False)
...
class TestHTierSafety:
    ...
    def test_h8_open_refused_when_halt_fires_after_can_open(self, monkeypatch):
        import helix.scanner as sc
        monkeypatch.setattr("helix.sim.clock.is_sim_active", lambda: False)
        monkeypatch.setattr(sc, "_resolve_trade_vault_address", lambda tid, strict=False: None)
        monkeypatch.setattr("helix.exchange.risk.is_trading_allowed", lambda: (False, "Kill-switch active"))
    ...
    def test_h9_emits_trade_failed_notification(self, helix_db, monkeypatch):
        import helix.scanner as sc
        import helix.brain as brain
        calls = []
        monkeypatch.setattr(
            "helix.notifications.emit_notification",
...
class TestLeverageB2:
    ...
    def test_margin_mode_toggle_defaults_isolated(self, helix_db):
        from helix.db import kv_set
        from helix.exchange.hyperliquid import configured_margin_is_cross
        kv_set("helix:settings", {"hyperliquid_use_cross_margin": False})
        assert configured_margin_is_cross() is False
        kv_set("helix:settings", {"hyperliquid_use_cross_margin": True})
...
class TestProtectionCoverageB3:
    ...
    def test_take_profit_alone_is_not_protected(self):
        from helix.exchange.risk import _summarize_position_protection
    ...
    def test_geometry_fallback_when_no_tpsl(self):
        from helix.exchange.risk import _order_is_stop_loss
...
class TestRecoveryStopRouting:
    def test_repair_places_protective_stop_on_the_sub_account(self, helix_db, monkeypatch):
        _books_settings()
        captured = {}
        ...
        monkeypatch.setattr("helix.exchange.hyperliquid.place_protective_stop", _fake_place)
...
class TestFillLedgerRecoveryH4:
    ...
    def test_recovers_weighted_exit_and_fee(self, monkeypatch):
        import helix.exchange.risk as risk
        fills = [
        ...
        monkeypatch.setattr("helix.exchange.hyperliquid.get_user_fills", lambda *a, **k: fills)
    ...
    def test_no_fills_returns_none(self, monkeypatch):
        import helix.exchange.risk as risk
        monkeypatch.setattr("helix.exchange.hyperliquid.get_user_fills", lambda *a, **k: [])
        out = risk._recover_exit_from_fills(
            "BTC", {"direction": "long", "opened_at": self._OPENED, "size": 1.0}, testnet=True, account_address=None
        )
        assert out is None
...
class TestFundingNetPnlH6:
    ...
    def test_funding_folded_into_net(self, helix_db):
        import json
        import helix.scanner as sc
    ...
    def test_no_funding_leaves_net_at_gross_minus_fees(self, helix_db):
        import json
        import helix.scanner as sc
    ...
    def test_funding_read_from_signal_data_on_default_close_path(self, helix_db):
...
class TestLiquidationMonitorH7:
    ...
    def test_critical_alert_when_close_to_liq(self, helix_db, monkeypatch):
        import helix.daemon as dmn
        calls = []
        monkeypatch.setattr(dmn, "_get_testnet", lambda: True)
        monkeypatch.setattr("helix.exchange.books.books_enabled", lambda: False)
        monkeypatch.setattr(
            "helix.exchange.hyperliquid.get_positions",
            lambda testnet=True, **k: {"positions": [
                {"position": {"coin": "BTC", "szi": "0.1", "liquidationPx": "96"}}
            ]},
        )
        monkeypatch.setattr("helix.exchange.hyperliquid.get_all_mids", lambda testnet=True: {"BTC": 100.0})
        monkeypatch.setattr(
            "helix.notifications.emit_notification",
    ...
    def test_no_alert_when_far_from_liq(self, helix_db, monkeypatch):
        import helix.daemon as dmn
        calls = []
        monkeypatch.setattr(dmn, "_get_testnet", lambda: True)
        monkeypatch.setattr("helix.exchange.books.books_enabled", lambda: False)
