...
def test_recall_tool_returns_envelope_on_success(helix_db, monkeypatch):
    from helix.agents import tools_brain  # noqa: F401 — registers tool
    ...
    import helix.agents.tools_brain as tb
    monkeypatch.setattr(
...
def test_recall_tool_missing_query_returns_envelope(helix_db):
    import helix.agents.tools_brain as tb
...
def test_recall_tool_clamps_limit_high(helix_db, monkeypatch):
    captured: dict = {}
    ...
    monkeypatch.setattr("helix.recall.recall_similar_situation", spy)
...
def test_recall_tool_clamps_limit_low(helix_db, monkeypatch):
    captured: dict = {}
...
def test_recall_tool_normalizes_unknown_scope_to_all(helix_db, monkeypatch):
    captured: dict = {}
...
def test_recall_tool_handles_underlying_exception(helix_db, monkeypatch):
    def boom(*_a, **_kw):
