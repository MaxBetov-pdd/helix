...
from helix.api_domains import paper as paper_domain
from helix.control_plane import ops as control_plane_ops
from helix import soak
from helix.db import create_approval, get_db, kv_set
from helix.routers.ops import router as ops_router
from helix.routers.status import router as status_router
from helix.scheduler import _DEFAULT_JOB_IDS
...
def _patch_core_views(monkeypatch) -> None:
    ...
    monkeypatch.setattr("helix.vectordb._check_chroma_available", lambda: True)


def test_collect_backend_soak_report_ok(helix_db, monkeypatch):
...
def test_collect_backend_soak_report_warns_on_stale_agent_tasks(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_warns_on_failed_tasks(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_ignores_historical_failed_tasks(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_prioritizes_stale_queue_summary(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_humanizes_runtime_summary(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_warns_on_scheduler_errors(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
    ...
    with get_db() as conn:
        conn.execute(
            ...
            SET last_status = 'error',
                ...
                next_run_at = ?
            WHERE id = 'helix-slippage-monitor'
            """,
            (stale_due,),
        )
    ...
    assert "helix-slippage-monitor" in scheduler["details"]["failed_jobs"]
    assert "helix-slippage-monitor" in scheduler["details"]["overdue_jobs"]


def test_collect_backend_soak_report_ignores_historical_scheduler_failures(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
    ...
    assert "helix-slippage-monitor" not in scheduler["details"]["failed_jobs"]
    assert "helix-slippage-monitor" in scheduler["details"]["historical_failed_jobs"]


def test_collect_backend_soak_report_warns_on_clustered_openai_rate_limits(helix_db, monkeypatch, tmp_path):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
    ...
    bot_log = tmp_path / "helix_bot.err.log"
    bot_log.write_text(
        "\n".join(
            [
                "2026-03-10 12:00:00,000 [helix.agents.runner] WARNING openai/gpt-5.2 tool-call path: 429 rate limited (round 1), cooldown 35.0s",
                "2026-03-10 12:02:00,000 [helix.agents.runner] WARNING openai/gpt-5.2 tool-call path: 429 rate limited (round 1), cooldown 35.0s",
                "2026-03-10 12:04:00,000 [helix.agents.runner] WARNING openai/gpt-5.2 tool-call path: 429 rate limited (round 1), cooldown 35.0s",
                "2026-03-10 12:06:00,000 [helix.agents.runner] WARNING openai/gpt-5.2 tool-call path: 429 rate limited (round 1), cooldown 35.0s",
            ]
        ),
        encoding="utf-8",
    )
...
def test_collect_backend_soak_report_includes_queue_previews(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_warns_when_vector_store_degraded(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
    monkeypatch.setattr("helix.vectordb._check_chroma_available", lambda: False)
...
def test_collect_backend_soak_report_includes_operator_actions(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_fails_when_execution_scanner_is_stale(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_fails_strict_hyperliquid_probe(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
    _seed_runtime_state()
    _patch_core_views(monkeypatch)
...
def test_collect_backend_soak_report_flags_stale_runtime(helix_db, monkeypatch):
    _seed_scheduler_jobs()
    _seed_agents()
...
def test_system_soak_report_route_delegates(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.status.collect_backend_soak_report", _fake_collect_backend_soak_report)
...
def test_post_signal_scan_now_runs_signal_only(monkeypatch):
    monkeypatch.setattr("helix.db.init_db", lambda: None)
    ...
    monkeypatch.setattr("helix.scanner.run_scan", _fake_run_scan)
...
def test_post_execution_scan_now_respects_trading_gate(helix_db, monkeypatch):
    from helix.system_pause import set_system_paused

    monkeypatch.setattr("helix.db.init_db", lambda: None)
    monkeypatch.setattr(control_plane_ops, "log_activity", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "helix.scanner.run_scan",
...
def test_post_exchange_reconcile_now_wraps_result(monkeypatch):
    ...
    monkeypatch.setattr(control_plane_ops, "kv_set", lambda key, value: operator_state.update(value) if key == "ops_manual_action_state" and isinstance(value, dict) else None)
    monkeypatch.setattr("helix.exchange.risk.sync_from_trades", lambda: 0)
    monkeypatch.setattr(
        "helix.exchange.risk.reconcile_all_books",
        lambda *args, **kwargs: {
            "sqlite_open": 2,
            "exchange_open": 1,
            "synced": False,
...
def test_system_manual_action_routes_delegate(monkeypatch):
    ...
    monkeypatch.setattr("helix.routers.ops.control_plane_ops.post_signal_scan_now", _fake_signal)
    monkeypatch.setattr("helix.routers.ops.control_plane_ops.post_execution_scan_now", _fake_execution)
    monkeypatch.setattr("helix.routers.ops.control_plane_ops.post_exchange_reconcile_now", _fake_reconcile)
