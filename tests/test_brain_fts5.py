...
from helix.db import FTS5_TABLES, get_db, rebuild_fts5_indices
...
def test_brain_decisions_insert_makes_row_searchable(helix_db):
...
def test_brain_decisions_update_reflects_new_body(helix_db):
...
def test_brain_decisions_delete_removes_from_index(helix_db):
...
def test_agent_tasks_insert_makes_row_searchable(helix_db):
...
def test_agent_tasks_update_description_and_output(helix_db):
...
def test_agent_tasks_delete_removes_from_index(helix_db):
...
def test_task_audit_log_insert_makes_row_searchable(helix_db):
...
def test_task_audit_log_update_reflects_new_summary(helix_db):
...
def test_task_audit_log_delete_removes_from_index(helix_db):
...
def test_rebuild_fts5_indices_repairs_after_simulated_drift(helix_db):
...
def test_fts5_rebuild_cli_command_runs(helix_db):
    """`helix fts5-rebuild` must exit 0 and print per-table counts."""
