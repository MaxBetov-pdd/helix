...
from helix.control_plane import approvals as control_plane_approvals
from helix.control_plane.models import ApprovalDecisionBody, ApprovalHandoffBody, ApprovalTroubleshootBody
from helix.db import create_approval, get_approval, get_db, kv_get
...
def test_post_approve_approval_requeues_blocked_task(helix_db):
...
def test_post_deny_approval_marks_blocked_task_failed_with_reason(helix_db):
...
def test_post_revise_approval_preserves_response_contract(helix_db):
    approval_id = create_approval(
        "code_change",
        target_type="strategy",
        target_id="S00044",
...
def test_post_handoff_approval_updates_owner(helix_db):
    approval_id = create_approval(
        "code_change",
        target_type="strategy",
        target_id="S00045",
...
def test_get_approvals_list_enriches_linked_task(helix_db):
...
def test_post_troubleshoot_approval_creates_diagnosis_task(helix_db):
...
def test_post_troubleshoot_approval_reuses_existing_task(helix_db):
...
def test_get_approval_context_includes_linked_and_troubleshoot_details(helix_db):
...
def test_approve_dethrone_recommendation_transitions_strategy(helix_db):
    with get_db() as conn:
        conn.execute(
            """
...
def test_deny_dethrone_recommendation_sets_cooldown(helix_db):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO strategies
    ...
    cooldown = kv_get("helix:dethrone:cooldown:s-dethrone-deny")
...
def test_approve_promotion_recommendation_transitions_strategy(helix_db, monkeypatch):
    """Approving a promotion approval advances the strategy through the gate."""
    monkeypatch.setattr(
        "helix.brain.verify_backtest_exists_for_stage_transition",
        lambda *_args, **_kwargs: (True, "ok"),
    )
    monkeypatch.setattr(
        "helix.brain.evaluate_promotion",
        lambda *_args, **_kwargs: (True, "ok"),
    )
    with get_db() as conn:
        conn.execute(
...
def test_promotion_approval_gate_detects_gauntlet_to_paper(helix_db):
    from helix.brain import _requires_operator_promotion_approval
    from helix.db import kv_set

    # Semi mode (manual): every capital-promotion transition requires a click.
    kv_set("helix:settings", {"auto_approve_promotions": "false"})
    kv_set("helix:pipeline:settings", {"promotion_mode": "manual"})
    ...
    kv_set("helix:pipeline:settings", {"promotion_mode": "auto"})
    ...
    kv_set("helix:settings", {"auto_approve_promotions": "true"})
    assert _requires_operator_promotion_approval("gauntlet", "paper") is False
    assert _requires_operator_promotion_approval("paper", "live_graduated") is False


def test_concurrent_approve_and_deny_returns_409_on_loser(helix_db):
...
def _make_protected_crucible() -> str:
    ...
    from helix.crucibles import mark_crucible_viable
...
def test_approve_crucible_dethrone_archives_and_clears_protection(helix_db):
    from helix.crucibles import get_crucible
    from helix.hypotheses import archive_hypothesis
...
def test_deny_crucible_dethrone_restores_protection(helix_db):
    from helix.crucibles import get_crucible
    from helix.hypotheses import archive_hypothesis
