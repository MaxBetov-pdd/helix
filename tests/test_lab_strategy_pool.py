...
from helix.lab_strategy_pool import list_strategy_pool_candidates
...
def test_strategy_pool_reads_active_and_graveyard_candidates(helix_db):
...
def test_strategy_pool_all_managed_includes_backtesting(helix_db):
...
def test_strategy_pool_skips_local_backtest_incompatible_candidates(helix_db):
    _insert_strategy(
        strategy_id="S_BAD",
        name="Unsupported",
...
def test_lab_strategy_pool_module_keeps_production_reads_read_only():
    source = Path("helix/lab_strategy_pool.py").read_text(encoding="utf-8")
    assert "from helix.db import get_db" not in source
    assert "helix.db.get_db" not in source
