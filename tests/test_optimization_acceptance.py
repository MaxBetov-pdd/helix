...
from helix.strategies.optimization_acceptance import (
...
def _patch_walk_forward(baseline, candidate):
    ...
    return patch("helix.strategies.backtest.walk_forward", mock), mock
...
def test_resolver_does_not_source_legacy_top_level_fields():
    ...
    sources a profile."""
    from helix.strategies.backtest import execution_controls_from_params as resolve
...
def test_bakeoff_error_retains_baseline():
    ...
    with patch("helix.strategies.backtest.walk_forward", mock):
        decision = evaluate_optimization_candidate(
            strategy_id="S", asset="BTC", strategy_type="rsi_momentum",
            current_params={"rsi_entry": 30}, candidate_params={"rsi_entry": 35},
            optimization_metrics=_OK_OPT,
...
def test_param_locked_strategy_not_mutated():
    with patch("helix.brain.stage_is_param_locked", return_value=True):
...
def test_gauntlet_uses_optimization_acceptance_chokepoint(helix_db):
    from helix.gauntlet import tasks as gt

    _insert_strategy("SOPT_G")
    spy = MagicMock(return_value={"applied": False, "code": "worse_oos", "reason": "spy reject", "decision": {}})

    with patch("helix.gauntlet.tasks._latest_step_output", return_value={"best_params": {"rsi_entry": 99}, "result_id": None}), \
         patch("helix.strategies.optimization_acceptance.apply_optimized_params_if_accepted", spy):
...
def test_evolution_uses_optimization_acceptance_chokepoint(helix_db):
    from helix.api_core import _persist_backtest_result_row
    from helix.evolution import _execute_gauntlet_step
    ...
    with patch("helix.strategies.optimization_acceptance.apply_optimized_params_if_accepted", spy):
...
def test_unvalidated_optimization_never_writes_params(helix_db):
    ...
    with patch("helix.strategies.backtest.walk_forward", side_effect=AssertionError("bake-off must not run for an unvalidated candidate")):
        result = _execute_gauntlet_step(
