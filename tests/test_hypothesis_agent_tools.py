...
def test_research_tools_create_hypothesis_and_attach_artifacts_and_gaps(helix_db, monkeypatch):
    from helix.system_pause import set_system_mode

    tools_research = importlib.import_module("helix.agents.tools_research")
...
def test_create_hypothesis_blocked_inside_candidate_task(helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
    from helix.agents.context import reset_tool_context, set_tool_context
    ...
    assert "helix_create_strategy" in result["guidance"]
    with get_db() as conn:
        count = conn.execute(
...
def test_create_hypothesis_allowed_for_propose_crucible_task(helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
    from helix.agents.context import reset_tool_context, set_tool_context
    from helix.db import get_db
    from helix.system_pause import set_system_mode
...
def test_strategy_developer_hypothesis_creation_normalizes_lane_and_source(monkeypatch, helix_db):
    from helix.system_pause import set_system_mode
...
def test_only_strategy_developer_role_gets_create_hypothesis_tool(helix_db):
    from helix.agents.manager import create_agent
    from helix.agents.tool_registry import get_tools_for_agent
...
def test_custom_strategy_agent_normalizes_origin_role_to_strategy_developer(monkeypatch, helix_db):
    from helix.agents.manager import create_agent
    from helix.system_pause import set_system_mode
...
def test_only_strategy_developer_role_gets_strategy_creation_tools(helix_db):
    from helix.agents.manager import create_agent
    from helix.agents.tool_registry import get_tools_for_agent
    ...
    assert "helix_create_strategy" in default_strategy_tools
    assert "register_strategy" in default_strategy_tools
    assert "helix_create_strategy" in custom_strategy_tools
    assert "register_strategy" in custom_strategy_tools
    assert "helix_create_strategy" not in quant_tools
    assert "register_strategy" not in quant_tools


def test_strategy_developer_can_discover_and_inspect_youtube_benchmarks(monkeypatch, helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
...
def test_discover_youtube_benchmarks_passes_query_through(monkeypatch, helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
...
def test_youtube_benchmark_tools_reject_non_benchmarking_lane(monkeypatch, helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
...
def test_inspect_youtube_video_rejects_non_benchmarking_lane_with_compact_error(monkeypatch, helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
...
def test_youtube_benchmark_tools_return_compact_error_when_helper_unavailable(monkeypatch, helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
    ...
    def _unavailable(*args, **kwargs):
        raise ImportError("helix.research_sources.youtube is unavailable")
...
def test_inspect_youtube_video_preserves_channel_name_and_unavailable_transcript_metadata(monkeypatch, helix_db):
    tools_research = importlib.import_module("helix.agents.tools_research")
...
def test_attach_hypothesis_artifact_tool_forwards_cached_content(helix_db):
    import json
    from helix.agents.tools_research import _tool_attach_hypothesis_artifact
    from helix.hypotheses import create_hypothesis, list_hypothesis_artifacts
...
def test_attach_hypothesis_artifact_tool_without_cached_content_still_works(helix_db):
    import json
    from helix.agents.tools_research import _tool_attach_hypothesis_artifact
    from helix.hypotheses import create_hypothesis, list_hypothesis_artifacts
...
def test_extrapolate_strategy_spec_tool_reconstructs_from_cached_artifact(helix_db, monkeypatch):
    import json

    import helix.strategy_extrapolation as se
    from helix.agents.tools_research import (
        _tool_attach_hypothesis_artifact,
        _tool_extrapolate_strategy_spec,
    )
    from helix.hypotheses import create_hypothesis, list_hypothesis_data_gaps
...
def test_extrapolate_strategy_spec_tool_errors_without_cached_artifact(helix_db):
    import json

    from helix.agents.tools_research import _tool_extrapolate_strategy_spec
...
def test_extrapolate_strategy_spec_tool_role_gated(helix_db):
    from helix.agents.manager import create_agent
    from helix.agents.tool_registry import get_tools_for_agent
