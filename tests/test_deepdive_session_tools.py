...
from helix.deepdive_db import create_or_get_active_thread, list_messages
...
def test_tool_call_round_trip_persists_tool_message(thread, monkeypatch):
    from helix import deepdive_session
...
def test_dispatch_rejects_non_deepdive_tool(helix_db):
    """Security boundary: only tools with 'deepdive' in permissions can be dispatched."""
    from helix.deepdive_session import _dispatch_tool
...
def test_dispatch_unknown_tool(helix_db):
    from helix.deepdive_session import _dispatch_tool
