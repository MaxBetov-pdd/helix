"""Tests for helix.data_manager — DataManager, collectors, and enrich()."""
...
from helix.data_manager import (
    DataManager,
    FundingCollector,
    OICollector,
    OHLCVCollector,
...
def test_loaded_df_is_safe_to_mutate(tmp_path):
    from helix.data_manager import _save_stream_parquet, _load_stream_parquet
    import pandas as pd
    df = pd.DataFrame({
...
def test_import_fails_loudly_when_pyarrow_missing(monkeypatch):
    ...
    # subsequent tests that do `patch("helix.data_manager.X", ...)` would
    ...
    original = sys.modules.get("helix.data_manager")
    try:
        # Make pyarrow fail on fresh import
        sys.modules.pop("helix.data_manager", None)
        monkeypatch.setitem(sys.modules, "pyarrow", None)
        with pytest.raises((ImportError, ModuleNotFoundError)):
            importlib.import_module("helix.data_manager")
    finally:
        if original is not None:
            sys.modules["helix.data_manager"] = original
...
def test_ohlcv_collector_returns_zero_on_empty_fetch():
    ...
    with patch("helix.data.fetch_ohlcv_chunked", return_value=pd.DataFrame()):
        with patch("helix.data.load_parquet", return_value=None):
            with patch("helix.data._get_dataset_lock", return_value=threading.Lock()):
...
def test_ohlcv_collector_does_not_hold_dataset_lock_during_fetch():
    ...
    with patch("helix.data.fetch_ohlcv_chunked", side_effect=_fake_fetch):
        with patch("helix.data.dataset_last_timestamp_ms", return_value=last_ms):
            with patch("helix.data._get_dataset_lock", return_value=dataset_lock):
                result = collector.collect("BTC-USDT", "1h")
...
def test_ohlcv_collector_raises_on_failure():
    ...
    with patch("helix.data.dataset_last_timestamp_ms", return_value=None):
        with patch("helix.data.parquet_path", return_value=_NoFile("first-time-no-file.parquet")):
            with patch("helix.data.fetch_ohlcv_chunked", side_effect=RuntimeError("db error")):
...
def test_funding_collector_creates_file(tmp_path):
    ...
    with patch("helix.data_manager.FUNDING_DIR", tmp_path / "funding"):
        with patch("helix.data_manager._get_futures_exchange") as mock_ex:
...
def test_funding_collector_incremental(tmp_path):
    ...
    with patch("helix.data_manager.FUNDING_DIR", funding_dir):
        with patch("helix.data_manager._get_futures_exchange") as mock_ex:
            mock_ex.return_value.fetch_funding_rate_history.return_value = _mock_funding_rows(3)
...
def test_funding_collector_raises_on_failure():
    collector = FundingCollector()
    with patch("helix.data_manager._get_futures_exchange", side_effect=RuntimeError("no exchange")):
...
def test_oi_collector_creates_file(tmp_path):
    ...
    with patch("helix.data_manager.OI_DIR", tmp_path / "oi"):
        with patch("helix.data_manager._get_futures_exchange") as mock_ex:
...
def test_lsr_collector_uses_shared_session(monkeypatch, tmp_path):
    from helix.data_manager import LongShortRatioCollector
    calls = {"n": 0}
    ...
    monkeypatch.setattr("helix.data_manager.DERIVATIVES_DIR", tmp_path)
...
def test_rest_collector_shares_http_and_combine_logic():
    from helix.data_manager import LongShortRatioCollector, TakerVolumeCollector
    import helix.data_manager as dm
...
def test_btc_dominance_uses_floor_hour_aligned_timestamp(monkeypatch, tmp_path):
    from helix.data_manager import BtcDominanceCollector
    monkeypatch.setattr("helix.data_manager.MACRO_DIR", tmp_path)

    class FakeResp:
        def raise_for_status(self): pass
        def json(self): return {"data": {"market_cap_percentage": {"btc": 55.5}}}

    monkeypatch.setattr("helix.data_manager._http_session", lambda: type("S", (), {
    ...
    monkeypatch.setattr("helix.data_manager.pd.Timestamp.now", staticmethod(_fake_now))
...
def test_enrich_no_files_returns_original(tmp_path):
    ...
    with patch("helix.data_manager.FUNDING_DIR", tmp_path / "funding"), \
         patch("helix.data_manager.OI_DIR", tmp_path / "oi"), \
         patch("helix.data_manager.DERIVATIVES_DIR", tmp_path / "derivatives"), \
         patch("helix.data_manager.MACRO_DIR", tmp_path / "macro"):
...
def test_enrich_reads_parquet_once_for_repeated_calls(tmp_path, monkeypatch):
    from helix.data_manager import _save_stream_parquet
    import pandas as pd
    monkeypatch.setattr("helix.data_manager.FUNDING_DIR", tmp_path)
    ...
    calls = {"n": 0}
    orig = __import__("helix.data_manager", fromlist=["_load_stream_parquet"])._load_stream_parquet
    def counting(p):
        calls["n"] += 1
        return orig(p)
    monkeypatch.setattr("helix.data_manager._load_stream_parquet", counting)
    base = pd.DataFrame({
...
def test_get_active_symbols_empty_db():
    ...
    dm = DataManager()
    with patch("helix.db.get_db") as mock_db:
        mock_conn = MagicMock()
...
def test_get_active_symbols_db_error_returns_empty():
    ...
    dm = DataManager()
    with patch("helix.db.get_db", side_effect=RuntimeError("db unavailable")):
        symbols = dm.get_active_symbols()
    assert symbols == set()
...
def test_get_active_symbols_filters_to_supported_keepalive_pairs(tmp_path):
    ...
    with patch("helix.db.get_db", return_value=mock_conn):
        with patch("helix.data.DATA_DIR", tmp_path / "ohlcv"):
            symbols = dm.get_active_symbols()

    assert symbols == {"BTC-USDT", "ETH-USDT", "SOL-USDT"}


def test_fetch_active_timeframes_matches_slash_dash_and_bare(helix_db):
    ...
    from helix.db import create_strategy_container, get_db
...
def test_collect_ohlcv_processes_staleness_selected_pairs(helix_db, monkeypatch):
...
def test_singleton_is_data_manager_instance():
    ...
    from helix.data_manager import get_data_manager
...
def test_backfill_summary_includes_skip_reason_when_probe_returns_none(tmp_path):
    ...
    from helix.data_manager import DataManager
    dm = DataManager()

    with patch("helix.data_manager.bv_client") as mock_bv:
...
def test_collect_funding_fallback_excludes_non_futures(monkeypatch, tmp_path):
    monkeypatch.setattr("helix.data.DATA_DIR", tmp_path)
...
def test_collect_liquidations_disabled_by_default(monkeypatch):
    monkeypatch.delenv("HELIX_ENABLE_LIQUIDATIONS", raising=False)
...
def test_collect_liquidations_runs_when_enabled(monkeypatch):
    monkeypatch.setenv("HELIX_ENABLE_LIQUIDATIONS", "1")
    monkeypatch.setattr(data_manager, "get_active_symbols", lambda: set())
    out = data_manager.collect_liquidations()
...
class TestHttpSession:
    def test_session_has_retry_adapter_for_https(self):
        from helix.data_manager import _http_session
        sess = _http_session()
...
class TestParquetCache:
    def test_cache_hit_does_not_reread_until_mtime_changes(self, tmp_path, monkeypatch):
        from helix.data_manager import _parquet_read_cache, _save_stream_parquet
        import helix.data_manager as dm
        import pandas as pd
        df = pd.DataFrame({
    ...
    def test_cache_returns_none_for_missing(self, tmp_path):
        from helix.data_manager import _parquet_read_cache
    ...
    def test_parquet_read_cache_returns_independent_copy(self, tmp_path):
        from helix.data_manager import _save_stream_parquet, _parquet_read_cache
        import pandas as pd
        df = pd.DataFrame({
            "timestamp": pd.to_datetime(["2026-01-01"], utc=True),
            "x": [1.0],
...
class TestMergeAsofParquet:
    def test_merge_pulls_backward_and_fills_default(self, tmp_path, monkeypatch):
        from helix.data_manager import _merge_asof_parquet, _save_stream_parquet
        import pandas as pd
    ...
    def test_returns_df_unchanged_when_missing(self, tmp_path):
        from helix.data_manager import _merge_asof_parquet
        import pandas as pd
        base = pd.DataFrame({
            "timestamp": pd.to_datetime(["2026-01-01"], utc=True),
            "close": [1.0],
...
class TestCombineAndSave:
    def test_merges_dedupes_returns_rows_added(self, tmp_path):
        from helix.data_manager import _combine_and_save, _load_stream_parquet
        import pandas as pd
    ...
    def test_first_write_with_no_existing(self, tmp_path):
        from helix.data_manager import _combine_and_save
        import pandas as pd
...
class TestValidateStreamDf:
    def test_drops_future_timestamps(self):
        from helix.data_manager import _validate_stream_df
        import pandas as pd
...
def test_stream_locks_do_not_grow_indefinitely():
    import gc
    from helix.data_manager import _get_stream_lock, _stream_locks
...
def test_data_manager_is_lazy():
    # Do NOT pop / reimport helix.data_manager here. Prior versions did, but
    ...
    # patch("helix.data_manager.FUNDING_DIR", ...) calls silently missed.
    import helix.data_manager as mod
...
def test_funding_collector_drops_future_rows(monkeypatch, tmp_path):
    from helix.data_manager import FundingCollector
    ...
    monkeypatch.setattr("helix.data_manager.FUNDING_DIR", tmp_path)
    monkeypatch.setattr(
        "helix.data_manager._get_futures_exchange",
        lambda: type(
            "E",
            (),
...
def test_oi_collector_drops_negative_open_interest(monkeypatch, tmp_path):
    from helix.data_manager import OICollector
    ...
    monkeypatch.setattr("helix.data_manager.OI_DIR", tmp_path)
    monkeypatch.setattr(
        "helix.data_manager._get_futures_exchange",
        lambda: type(
            "E",
...
def test_lsr_collector_drops_negative_ratio(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "helix.data_manager._http_session",
...
def test_macro_incremental_uses_minimal_period(monkeypatch, tmp_path):
    from helix.data_manager import MacroCollector, _save_stream_parquet
    monkeypatch.setattr("helix.data_manager.MACRO_DIR", tmp_path)
...
def test_macro_cold_start_uses_one_year(monkeypatch, tmp_path):
    from helix.data_manager import MacroCollector
    monkeypatch.setattr("helix.data_manager.MACRO_DIR", tmp_path)
    captured = {}
...
def _reset_data_manager_stats():
    from helix.data_manager import _stats, _stats_lock
    with _stats_lock:
        _stats.clear()


def test_data_manager_stats_tracks_last_success(monkeypatch, tmp_path):
    from helix.data_manager import data_manager_stats
...
def test_collect_ohlcv_all_pairs_failing_records_failure(monkeypatch):
    ...
    from helix.data_manager import DataManager, data_manager_stats
    _reset_data_manager_stats()
    dm = DataManager()
