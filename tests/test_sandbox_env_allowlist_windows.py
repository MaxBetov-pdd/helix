...
every parent var (ANTHROPIC_API_KEY, HELIX_HL_API_SECRET, HELIX_ENCRYPTION_KEY,
...
def test_windows_run_code_strips_secret_env_vars(monkeypatch):
    secrets = {
        ...
        "HELIX_HL_API_SECRET": "0x" + "a" * 64,
        "HELIX_ENCRYPTION_KEY": "ZmVybmV0LWtleS1ub3QtbGVhaw==",
        "HELIX_OPERATOR_KEY": "operator-token",
...
def test_windows_run_code_preserves_pythonpath_and_blas(monkeypatch):
    ...
    env = captured["env"]
    # repo root is on PYTHONPATH so the child can import helix.*
