...
from helix.db import SCHEMA_VERSION, get_db, init_db
...
def test_approvals_has_phase5_columns(helix_db) -> None:
    init_db()
    with get_db() as conn:
...
def test_agent_toolset_overrides_table_exists(helix_db) -> None:
    init_db()
    with get_db() as conn:
        row = conn.execute(
...
def test_agent_toolset_overrides_composite_pk(helix_db) -> None:
...
def test_brain_routines_table_exists(helix_db) -> None:
    init_db()
    with get_db() as conn:
        row = conn.execute(
...
def test_brain_routines_name_unique(helix_db) -> None:
    init_db()
    with get_db() as conn:
        conn.execute(
