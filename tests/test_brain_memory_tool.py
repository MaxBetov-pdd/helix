...
def _load_tools_brain():
    ...
    return importlib.import_module("helix.agents.tools_brain")


def test_memory_tool_registered_for_brain(helix_db):
    _load_tools_brain()
...
def test_memory_tool_not_visible_to_quant_agents(helix_db):
    _load_tools_brain()
    from helix.agents.tool_registry import get_tools_for_agent
...
def test_memory_tool_view_returns_initial_state(helix_db):
...
def test_memory_tool_add_then_view_roundtrip(helix_db, monkeypatch):
    tools_brain = _load_tools_brain()
    monkeypatch.setattr(
        tools_brain,
...
def test_memory_tool_replace_overwrites(helix_db):
    tools_brain = _load_tools_brain()
...
def test_memory_tool_remove_strips_substring(helix_db):
    tools_brain = _load_tools_brain()
...
def test_memory_tool_remove_missing_needle(helix_db):
    tools_brain = _load_tools_brain()
...
def test_memory_tool_cap_violation_returns_envelope(helix_db):
    tools_brain = _load_tools_brain()
    out = json.loads(
...
def test_memory_tool_invalid_action(helix_db):
    tools_brain = _load_tools_brain()
...
def test_memory_tool_add_missing_content_rejected(helix_db):
    tools_brain = _load_tools_brain()
...
def test_memory_tool_remove_missing_needle_param_rejected(helix_db):
    tools_brain = _load_tools_brain()
