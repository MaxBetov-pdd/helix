...
def test_remote_resolution_returns_none_when_unconfigured(helix_db):
    """No env var + no settings → None."""
    from helix.api_core import _resolve_backtest_results_remote_api

    empty_settings = {"remote_engine_enabled": False, "remote_engine_url": ""}
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("HELIX_BACKTEST_RESULTS_REMOTE_API", None)
        with patch("helix.api_core._load_settings_payload", return_value=empty_settings):
            result = _resolve_backtest_results_remote_api()
    assert result is None


def test_remote_resolution_reads_env_var(helix_db):
    """Env var set → returns normalized URL."""
    from helix.api_core import _resolve_backtest_results_remote_api

    with patch.dict(os.environ, {"HELIX_BACKTEST_RESULTS_REMOTE_API": "10.0.0.5:9050"}):
        result = _resolve_backtest_results_remote_api()
    assert result == "http://10.0.0.5:9050/api"


def test_remote_resolution_falls_back_to_settings(helix_db):
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("HELIX_BACKTEST_RESULTS_REMOTE_API", None)
        with patch("helix.api_core._load_settings_payload", return_value=settings):
            result = _resolve_backtest_results_remote_api()
    assert result == "http://192.168.1.100:9050/api"


def test_remote_resolution_ignores_disabled_settings(helix_db):
...
def test_remote_unreachable_raises_503(helix_db):
    """Remote configured + unreachable → HTTPException(503)."""
    from fastapi import HTTPException
    from helix.api_core import get_backtest_results

    with patch("helix.api_core._is_remote_configured", return_value=True), \
         patch("helix.api_core._resolve_backtest_results_remote_api", return_value="http://dead-host:9050/api"), \
         patch("helix.api_core._fetch_remote_backtest_summaries", return_value=[]), \
         patch("helix.api_core._is_remote_backtest_results_available", return_value=False):
...
def test_local_only_when_unconfigured(helix_db):
    ...
    with patch("helix.api_core._is_remote_configured", return_value=False), \
         patch("helix.api_core._chroma_backtest_records", return_value=fake_records), \
         patch("helix.api_core.get_db") as mock_db:
        mock_conn = MagicMock()
...
def test_remote_detail_unreachable_raises_503(helix_db):
    """Remote configured + unreachable on detail → HTTPException(503)."""
    from fastapi import HTTPException
    from helix.api_core import get_backtest_result

    with patch("helix.api_core._is_remote_configured", return_value=True), \
         patch("helix.api_core._resolve_backtest_results_remote_api", return_value="http://dead-host:9050/api"), \
         patch("helix.api_core._fetch_remote_backtest_detail", return_value=None), \
         patch("helix.api_core._is_remote_backtest_results_available", return_value=False):
        with pytest.raises(HTTPException) as exc_info:
...
def test_monthly_return_derived_from_total(helix_db):
    """total_return=50%, monthly=None, duration ~6mo → correct geometric monthly."""
    from helix.api_core import _normalize_backtest_summary
...
def test_monthly_return_sentinel_filtered(helix_db):
...
def test_vectordb_stores_sentinel_for_missing_monthly():
    ...
    with patch("helix.vectordb._upsert") as mock_upsert:
...
def test_coerce_backtest_summary_filters_sentinel(helix_db):
    """_coerce_backtest_summary_payload filters -999.0 sentinels."""
    from helix.api_core import _coerce_backtest_summary_payload
...
def test_leaderboard_does_not_use_total_as_monthly(helix_db):
    """Leaderboard monthly_return_pct defaults to 0.0, not total_return."""
    from helix.api_domains.analytics import _dashboard_leaderboard_entries
    ...
    with patch("helix.api_domains.analytics.get_strategies", return_value=[fake_strategy]):
...
def test_backtesting_status_reports_remote_error(helix_db):
    """Status endpoint reports remote_error when remote is configured but unreachable."""
    from helix.api_core import get_backtesting_status

    with patch("helix.api_core._resolve_backtest_results_remote_api", return_value="http://dead-host:9050/api"), \
         patch("helix.api_core._is_remote_backtest_results_available", return_value=False), \
         patch("helix.api_core.get_backtesting_runs", return_value={"runs": []}), \
         patch("helix.api_core.get_backtesting_outcomes", return_value={}):
