"""Tests for helix.lifecycle — graceful shutdown hook."""

from helix.db import get_db
from helix.lifecycle import mark_in_flight_tasks_interrupted
from helix.task_progress import INTERRUPTED_STATUS
...
def test_marks_running_tasks_only(helix_db):
...
def test_idempotent_second_call_is_no_op(helix_db):
...
def test_no_running_tasks_returns_zero(helix_db):
    _create_task("done", display_id="T99205")
    assert mark_in_flight_tasks_interrupted() == 0


def test_does_not_touch_interrupted_tasks(helix_db):
