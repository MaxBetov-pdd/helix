...
def test_malicious_top_level_import_is_rejected_before_import(helix_db, monkeypatch, tmp_path):
    d = _point_custom_dir(monkeypatch, tmp_path)
    f = d / "btc_guard_evil_test.py"
    f.write_text(_MALICIOUS, encoding="utf-8")
    sys.modules.pop("helix.strategies.custom.btc_guard_evil_test", None)
    ...
    assert "helix.strategies.custom.btc_guard_evil_test" not in sys.modules


def test_clean_strategy_still_registers(helix_db, monkeypatch, tmp_path):
    d = _point_custom_dir(monkeypatch, tmp_path)
    f = d / "btc_guard_ok_test.py"
    f.write_text(_CLEAN, encoding="utf-8")
    sys.modules.pop("helix.strategies.custom.btc_guard_ok_test", None)
