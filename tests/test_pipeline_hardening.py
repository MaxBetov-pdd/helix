...
import helix.policy as policy
from helix.api_core import read_lifecycle_strategy
from helix.brain import assign_task, escalate_to_engineer, promote_strategy as brain_promote_strategy, transition_stage
from helix.db import append_strategy_event, create_approval, create_task_container, get_db, init_db, kv_get, kv_set
from helix.evolution import check_paper_graduation, run_testing_step, run_weekly_review
from helix.monitoring import run_decay_tracker
from helix.policy import evaluate_promotion
from helix.scheduler import (
...
from helix.strategy_lifecycle import StrategyPromoteBody, promote_strategy as lifecycle_promote_strategy
from helix.strategies.backtest import _sync_strategy_metrics_and_promote_if_eligible
...
def _disable_readiness_gates():
    ...
    """
    from helix.db import kv_set
    kv_set("helix:pipeline:settings", {
...
def _readiness_gates_off(helix_db):
...
def test_transition_stage_updates_stage_changed_at_and_events(helix_db):
    _insert_strategy(
        "s-transition",
        stage="quick_screen",
...
def test_attempt_stage_promotion_reports_blocked_transition_when_transition_stage_stays_put(helix_db):
    _insert_strategy(
        "s-attempt-blocked",
        stage="quick_screen",
        owner="simulation-agent",
...
def test_transition_stage_supports_research_only_lane(helix_db):
...
def test_transition_stage_allows_forced_rejected_to_paper_without_backtests(helix_db):
...
def test_transition_stage_blocks_unforced_rejected_to_paper_without_backtests(helix_db):
...
def test_transition_stage_queues_operator_approval_for_automated_paper_archive(helix_db):
    _insert_strategy(
...
def test_transition_stage_force_archives_without_unbound_transition_locals(helix_db):
    _insert_strategy(
        "s-force-archive",
        stage="quick_screen",
        owner="simulation-agent",
...
def test_transition_stage_reuses_existing_dethrone_approval_for_automated_paper_demotion(helix_db):
    _insert_strategy(
...
def test_read_lifecycle_strategy_missing_raises_404(helix_db):
...
def test_transition_stage_keeps_paper_when_live_graduation_gate_fails(helix_db):
    _insert_strategy(
...
def test_brain_promote_strategy_returns_false_when_live_gate_blocks(helix_db):
    _insert_strategy(
...
def test_brain_promote_strategy_resolves_prefixed_display_name(helix_db, caplog):
    ...
    with caplog.at_level("ERROR", logger="helix.brain"):
...
def test_brain_promote_strategy_non_strategy_id_stays_not_found(helix_db):
...
def test_lifecycle_promote_strategy_returns_error_when_live_gate_blocks(helix_db):
    _insert_strategy(
        "s-api-paper-blocked",
        stage="paper",
        owner="risk-manager",
...
def test_gate_rejection_does_not_fetch_market_data(helix_db, monkeypatch):
    ...
    """
    import pandas as pd
    import helix.scanner as scanner
...
def test_gate_rejection_logging_never_blocks_on_held_write_lock(helix_db):
    ...
    """
    from helix.db import get_db_immediate, log_gate_rejection
...
def test_signal_result_logging_never_blocks_on_held_write_lock(helix_db):
    ...
    """
    from helix.db import get_db_immediate, record_signal_result
...
def test_create_approval_defers_phase5_when_handed_open_transaction(helix_db):
    ...
    from helix.control_plane.approval_modes import save_settings
    from helix.db import create_approval, get_db

    save_settings({"modes": {"strategy_dethrone_recommendation": "smart"}})

    with patch("helix.control_plane.smart_approval.apply_smart_decision") as auto_apply:
...
def test_create_approval_off_mode_auto_applies_on_conn_less_path(helix_db):
    ...
    """
    from helix.control_plane.approval_modes import save_settings
    from helix.db import create_approval, get_approval
...
def test_task_container_deduplicates_active_strategy_tasks(helix_db):
    init_db()
    ...
    from helix.system_pause import set_system_mode
...
def test_run_weekly_review_empty_contract(helix_db):
    result = run_weekly_review()
    assert result == {"retired": [], "top_performers": [], "total_deployed": 0}


def test_run_weekly_review_uses_recent_live_trade_metrics(helix_db):
    _insert_strategy(
        "s-weekly-live",
...
def test_paper_graduation_uses_live_paper_trades_not_backtest_metrics(helix_db):
    _insert_strategy(
        "s-paper-live",
        stage="paper_trading",
        owner="risk-manager",
...
def test_decay_tracker_handles_paper_trading_and_transitions_atomically(helix_db):
    _insert_strategy(
        "s-decay",
        stage="paper_trading",
        owner="risk-manager",
...
def test_decay_tracker_ignores_nonmatching_trade_sources_for_paper_stage(helix_db):
    _insert_strategy(
...
def test_scheduler_running_since_guard_prevents_overlap(helix_db):
    seed_helix_jobs()
    now = datetime.now(timezone.utc)
    job_id = "helix-testing-cycle"
...
def test_scheduler_job_takeover_window_uses_job_specific_timeout(helix_db):
    seed_helix_jobs()
    now = datetime.now(timezone.utc)

    with get_db() as conn:
        ideation = dict(conn.execute("SELECT * FROM scheduler_jobs WHERE id = ?", ("helix-ideation-daily",)).fetchone())
        testing = dict(conn.execute("SELECT * FROM scheduler_jobs WHERE id = ?", ("helix-testing-cycle",)).fetchone())
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (ideation_stale, "helix-ideation-daily"),
        )
        conn.execute(
            "UPDATE scheduler_jobs SET running_since = ? WHERE id = ?",
            (testing_fresh, "helix-testing-cycle"),
        )

    assert _try_mark_job_running("helix-ideation-daily", now, stale_seconds=ideation_stale_seconds) is True
    assert _try_mark_job_running("helix-testing-cycle", now, stale_seconds=testing_stale_seconds) is False


def test_recover_stale_scheduler_job_locks_uses_job_specific_timeout(helix_db):
    seed_helix_jobs()
    now = datetime.now(timezone.utc)
    ...
    with get_db() as conn:
        conn.execute(
            "UPDATE scheduler_jobs SET running_since = ? WHERE id = ?",
            ((now - timedelta(seconds=ideation_stale_seconds + 5)).isoformat(), "helix-ideation-daily"),
        )
        conn.execute(
            "UPDATE scheduler_jobs SET running_since = ? WHERE id = ?",
            ((now - timedelta(seconds=max(60, testing_stale_seconds - 30))).isoformat(), "helix-testing-cycle"),
        )
...
def test_reset_scheduler_job_locks_clears_inherited_running_since(helix_db):
    seed_helix_jobs()
    now = datetime.now(timezone.utc).isoformat()
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (now, "helix-ideation-daily", "helix-testing-cycle"),
        )
    ...
    with get_db() as conn:
        rows = conn.execute(
            ...
            ("helix-ideation-daily", "helix-testing-cycle"),
        ).fetchall()
...
def test_data_manager_scheduler_jobs_run_with_hard_timeout(monkeypatch):
    ...
    monkeypatch.setattr("helix.scheduler._run_sync_job", fake_run_sync_job)

    status, error = asyncio.run(
        run_job(
            {
                "id": "helix-data-ohlcv-keepalive",
...
def test_scheduler_hard_timeout_budget_expands_for_longer_jobs(helix_db):
    seed_helix_jobs()

    with get_db() as conn:
        daily_learning = dict(conn.execute(
            "SELECT * FROM scheduler_jobs WHERE id = ?",
            ("helix-daily-learning",),
        ).fetchone())
        data_keepalive = dict(conn.execute(
            "SELECT * FROM scheduler_jobs WHERE id = ?",
            ("helix-data-ohlcv-keepalive",),
        ).fetchone())
...
def test_scheduler_user_priority_deferral_window_is_bounded(helix_db):
    job_id = "helix-testing-cycle"
    now = datetime.now(timezone.utc)
    ...
    kv_set(
        f"helix:scheduler:deferring:{job_id}",
...
def test_testing_step_auto_promotes_with_existing_pass_metrics(helix_db):
    now = datetime.now(timezone.utc).isoformat()
    with get_db() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO agents (id, name, role, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
        ...
        conn.execute(
            ...
            ("helix:settings", json.dumps({"auto_approve_promotions": "true"})),
        )
...
def test_adaptive_pipeline_plan_scales_assignments_with_backlog(helix_db):
...
def test_static_pipeline_plan_respects_manual_limits_when_adaptive_disabled(helix_db):
    kv_set(
        "helix:settings",
        {
...
def test_pipeline_plan_parses_string_booleans_from_settings(helix_db):
    kv_set(
        "helix:settings",
        {
            "adaptive_pipeline_throughput_enabled": "false",
...
def test_evaluate_promotion_requires_verdict_evidence_for_paper(helix_db):
    _insert_strategy(
        "s-gauntlet-evidence",
        stage="gauntlet",
        owner="simulation-agent",
...
def test_evaluate_promotion_accepts_persisted_validation_artifacts_for_paper(helix_db):
    _insert_strategy(
...
def test_evaluate_promotion_blocks_failed_required_verdict_tests_for_paper(helix_db):
...
def test_evaluate_promotion_does_not_hard_block_failed_optional_verdict_tests(helix_db):
    kv_set(
        "helix:pipeline_thresholds",
        {
            "gauntlet": {
...
def test_evaluate_promotion_uses_required_tests_for_derived_robustness(helix_db):
...
def test_evaluate_promotion_still_blocks_failed_required_walk_forward(helix_db):
    kv_set(
        "helix:pipeline_thresholds",
        {
            "gauntlet": {
...
def test_evaluate_promotion_ignores_failed_legacy_verdict_when_artifacts_pass(helix_db):
    _insert_strategy(
...
def test_evaluate_promotion_blocks_unprofitable_robust_gauntlet_candidate(helix_db):
    _insert_strategy(
...
def test_testing_step_does_not_synthesize_gauntlet_evidence_from_validation_backtest(helix_db, monkeypatch):
    _insert_strategy(
        "s-code-first-pass",
        stage="gauntlet",
        owner="simulation-agent",
    ...
    monkeypatch.setattr(
        "helix.evolution._advance_gauntlet_readiness",
    ...
    monkeypatch.setattr("helix.evolution._run_backtest_validation_matrix_sync", _fake_validation_matrix)
...
def test_testing_step_keeps_gauntlet_stage_when_validation_backtest_is_not_artifact_evidence(helix_db, monkeypatch):
    _insert_strategy(
        "s-code-first-fail",
        stage="gauntlet",
        owner="simulation-agent",
...
def test_testing_step_dedupes_same_day_identical_gate_failures(helix_db, monkeypatch):
...
def test_testing_step_configured_threshold_still_dedupes_same_day_identical_failures(helix_db, monkeypatch):
    kv_set("helix:settings", {"pipeline_gate_failure_archive_attempts": 2})
    _insert_strategy(
...
def test_testing_step_archives_terminal_quick_screen_gate_failure_immediately(helix_db, monkeypatch):
    _insert_strategy(
...
def test_pipeline_hygiene_archives_existing_terminal_quick_screen_gate_failure(helix_db):
    _insert_strategy(
...
def test_terminal_stage_transition_cancels_pending_strategy_tasks(helix_db):
    _insert_strategy(
...
def test_quick_screen_rejection_does_not_queue_post_mortem_task(helix_db):
    _insert_strategy(
...
def test_assign_task_infers_strategy_id_and_cancels_terminal_work(helix_db):
    _insert_strategy(
        "S09999",
        stage="rejected",
        status="rejected",
...
def test_assign_task_does_not_infer_strategy_id_for_candidate_creation_siblings(helix_db):
    _insert_strategy(
        "S09999",
...
def test_repeated_paper_failures_do_not_auto_queue_dethrone_for_operator_owned_strategy(helix_db):
...
def test_repeated_failure_auto_archive_ignores_prior_stage_cycle_failures(helix_db):
    _insert_strategy(
...
def test_insufficient_paper_duration_does_not_create_dethrone_recommendation(helix_db):
    _insert_strategy(
...
def test_insufficient_paper_evidence_clears_stale_pending_dethrone_recommendation(helix_db):
    _insert_strategy(
...
def test_backtest_sync_updates_metrics_and_promotes_quick_screen_when_gate_passes(helix_db):
    _insert_strategy(
        "s-sync-promote",
        stage="quick_screen",
        owner="simulation-agent",
...
def test_backtest_sync_does_not_auto_promote_gauntlet_to_paper(helix_db):
    _insert_strategy(
        "s-sync-gauntlet",
        stage="gauntlet",
        owner="simulation-agent",
...
def test_backtest_sync_does_not_mutate_terminal_stage_metrics(helix_db):
    _insert_strategy(
        "s-sync-archived",
        stage="archived",
        status="archived",
...
def test_validation_backtest_does_not_sync_strategy_state(monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.backtest.backtest_strategy", _fake_backtest_strategy)
...
def test_request_fix_reports_to_triage_queue_not_engineer(helix_db):
    ...
    first = escalate_to_engineer(
        ...
        severity="critical",
        context={"affected_apis": ["helix_run_verdict"], "error": "HTTP 404"},
    )
...
def test_resolve_initial_stage_certified_returns_quick_screen():
    from helix.strategies.certification import certify_execution_strategy, resolve_initial_stage
...
def test_resolve_initial_stage_uncertified_returns_research_only():
    from helix.strategies.certification import StrategyExecutionCertification, resolve_initial_stage
    from helix.strategies.params import ParamCanonicalizationMeta
...
def test_transition_quick_screen_to_gauntlet_blocked_without_backtest(helix_db):
...
def test_transition_quick_screen_to_gauntlet_allowed_with_backtest(helix_db):
...
def test_demotion_thrash_redirects_to_research_only_after_3(helix_db):
...
def test_migration_snapshot_saves_and_restores(helix_db):
    from helix.db import save_migration_snapshot, restore_migration_snapshot
...
def test_gauntlet_migration_demotes_strategies_without_backtest(helix_db):
    from helix.brain import run_gauntlet_backtest_migration
    # Clear the migration flag if set
    kv_set("helix:migration:gauntlet_backtest_demotion_done", None)
...
def test_gauntlet_migration_skips_strategies_with_backtest(helix_db):
    from helix.brain import run_gauntlet_backtest_migration
    kv_set("helix:migration:gauntlet_backtest_demotion_done", None)
...
def test_gauntlet_migration_runs_once_only(helix_db):
    from helix.brain import run_gauntlet_backtest_migration
    kv_set("helix:migration:gauntlet_backtest_demotion_done", None)
...
def test_stale_gauntlet_no_backtest_48h_demoted(helix_db):
    from helix.evolution import _run_stale_cleanup
...
def test_stale_quick_screen_no_activity_7d_archived(helix_db):
    from helix.evolution import _run_stale_cleanup
...
def test_research_recovery_on_edit_promotes_if_certified(helix_db):
    from helix.brain import try_research_recovery
...
def test_research_recovery_on_edit_debounce_5min(helix_db):
    from helix.api_core import _try_research_recovery_on_edit
    ...
    assert kv_get("helix:recert_debounce:s-debounce") is not None
...
def test_research_recovery_sweep_max_3_per_cycle(helix_db):
    from helix.evolution import _sweep_research_recovery
...
def test_research_recovery_sweep_oldest_first(helix_db):
    from helix.evolution import _sweep_research_recovery
...
def test_research_only_30d_inactive_archived(helix_db):
    from helix.evolution import _sweep_research_recovery
...
def test_failure_tier_classification():
    from helix.strategies.certification import classify_failure_tier
...
def test_default_ideation_interval_is_120(helix_db):
    from helix.api_core import _DEFAULT_SETTINGS_PAYLOAD
    assert _DEFAULT_SETTINGS_PAYLOAD["ideation_interval_minutes"] == 120


def test_default_coding_interval_is_60(helix_db):
    from helix.api_core import _DEFAULT_SETTINGS_PAYLOAD
    assert _DEFAULT_SETTINGS_PAYLOAD["coding_interval_minutes"] == 60


def test_default_testing_interval_is_60(helix_db):
    from helix.api_core import _DEFAULT_SETTINGS_PAYLOAD
    assert _DEFAULT_SETTINGS_PAYLOAD["testing_interval_minutes"] == 60


def test_default_assignments_per_cycle_is_3(helix_db):
    from helix.api_core import _DEFAULT_SETTINGS_PAYLOAD
    assert _DEFAULT_SETTINGS_PAYLOAD["pipeline_assignments_per_cycle"] == 3


def test_backlog_throttle_activates_above_30_gauntlet(helix_db):
    """With drain mode on (default), all candidates are processed regardless of gauntlet count."""
    from helix.evolution import _resolve_pipeline_execution_plan
...
def test_backlog_throttle_inactive_below_30_gauntlet(helix_db):
    from helix.evolution import _resolve_pipeline_execution_plan
...
def test_gauntlet_overflow_alert_after_7_days(helix_db):
    from helix.evolution import _check_gauntlet_overflow_alert
    ...
    old_time = (datetime.now(timezone.utc) - timedelta(days=8)).isoformat()
    kv_set("helix:alert:gauntlet_overflow_start", old_time)
...
def test_ideation_prompt_congestion_warning_when_gauntlet_above_30(helix_db):
    ...
    from helix.evolution import _strategy_stage
    by_status = {}
    for s in strategies:
...
def test_brain_research_recovery_flag_default_false():
    from helix.lab_features import brain_research_recovery_enabled
    assert brain_research_recovery_enabled() is False


def test_mutation_audit_log_records_changes(helix_db):
    from helix.db import log_mutation_audit
...
def test_scheduler_circuit_breaker_records_tick_success(helix_db):
    """Successful tick resets consecutive error counter."""
    from helix.scheduler import _record_scheduler_tick_success, _record_scheduler_tick_failure
...
def test_scheduler_circuit_breaker_increments_errors(helix_db):
    """Tick failures increment the consecutive error counter."""
    from helix.scheduler import _record_scheduler_tick_failure
    for i in range(5):
...
def test_cpu_gate_force_claim_after_timeout():
    ...
    import helix.lab_worker_service as lws
...
def test_heartbeat_returns_abort_event():
    ...
    with patch("helix.lab_worker_service.heartbeat_lab_job"):
        with patch("helix.lab_worker_service._write_worker_status"):
...
def test_claim_next_lab_job_retry_on_lock():
    ...
    from helix.lab_db import _CLAIM_MAX_RETRIES, _CLAIM_RETRY_BASE_SECONDS
...
def test_pipeline_saturation_blocks_when_over_threshold(helix_db):
    """Pipeline saturation blocks strategy creation above threshold."""
    from helix.lab_features import is_pipeline_saturated, PIPELINE_SATURATION_THRESHOLD
...
def test_pipeline_saturation_allows_when_under_threshold(helix_db):
    """Pipeline is not saturated when under threshold."""
    from helix.lab_features import is_pipeline_saturated
...
def test_pipeline_saturation_ignores_archived(helix_db):
...
def test_pipeline_saturation_ignores_research_only(helix_db):
...
def test_pipeline_saturation_hysteresis(helix_db):
    """Once saturated, must drop below resume threshold to unsaturate."""
    from helix.lab_features import is_pipeline_saturated, PIPELINE_SATURATION_THRESHOLD, PIPELINE_RESUME_THRESHOLD
...
def test_brain_create_strategy_blocked_when_saturated(helix_db):
    """brain.create_strategy() refuses new strategies when pipeline saturated."""
    from helix.hypotheses import create_hypothesis
    from helix.lab_features import PIPELINE_SATURATION_THRESHOLD
    ...
    from helix.brain import create_strategy
...
def test_sweep_archives_zero_trade_gauntlet(helix_db):
    """Zero-trade gauntlet strategies older than 2 days get archived."""
    from helix.evolution import _sweep_pipeline_hygiene
...
def test_sweep_archives_negative_sharpe_gauntlet(helix_db):
...
def test_sweep_archives_stale_no_fitness_gauntlet(helix_db):
...
def test_sweep_keeps_healthy_gauntlet(helix_db):
...
def test_sweep_archives_stale_untested_quick_screen(helix_db):
...
def test_sweep_archives_tested_garbage_quick_screen(helix_db):
...
def test_sweep_respects_cooldown(helix_db):
...
def test_duplicate_gate_blocks_weaker_challenger(helix_db):
    """A challenger that does NOT beat the incumbent stays blocked as a duplicate."""
    kv_set("helix:settings", {"paper_slot_competition_enabled": True})
...
def test_duplicate_gate_allows_materially_better_challenger(helix_db):
...
def test_materially_better_challenger_queues_dethrone_for_incumbent(helix_db):
    ...
    """
    kv_set("helix:settings", {"auto_approve_dethrone": False, "paper_slot_competition_enabled": True})
...
def test_insufficient_evidence_clear_preserves_challenger_dethrone(helix_db):
...
def test_paper_gate_blocks_challenger_while_incumbent_holds_slot(helix_db):
...
def test_auto_approve_dethrone_frees_the_slot(helix_db):
    ...
    kv_set("helix:settings", {"auto_approve_dethrone": True, "paper_slot_competition_enabled": True})
...
def test_hygiene_archives_unpromotable_gauntlet_after_grace(helix_db, monkeypatch):
...
def test_hygiene_keeps_promotable_gauntlet_strategy(helix_db, monkeypatch):
