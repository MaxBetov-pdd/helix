...
def test_read_open_trades_auto_skips_exchange_verification_in_paper_mode(helix_db, monkeypatch):
    calls = {"extract": 0}
    trade = _sample_trade()
...
def test_read_open_trades_auto_verifies_exchange_in_live_mode(helix_db, monkeypatch):
    calls = {"extract": 0}
    trade = _sample_trade()
...
def test_read_open_trades_explicit_verification_overrides_paper_mode(helix_db, monkeypatch):
    calls = {"extract": 0}
    trade = _sample_trade()
...
def test_read_open_trades_verifies_pending_open_reconcile_even_in_paper_mode(helix_db, monkeypatch):
    calls = {"extract": 0}
    trade = {
        **_sample_trade(),
        "fill_entry_price": None,
...
def test_read_open_trades_auto_verifies_exchange_during_recovery_in_paper_mode(helix_db, monkeypatch):
    calls = {"extract": 0}
...
def test_read_open_trades_auto_verifies_exchange_when_reconciliation_issues_exist(helix_db, monkeypatch):
    calls = {"extract": 0}
    trade = _sample_trade()
...
def test_read_open_trades_repairs_missing_portfolio_row_for_recovered_trade(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_read_open_trades_preserves_exchange_mark_and_roe_fields(helix_db, monkeypatch):
    monkeypatch.setattr(trading_domain, "get_execution_mode", lambda: "paper")
    monkeypatch.setattr(trading_domain, "get_open_trades", lambda: [])
    monkeypatch.setattr(trading_domain, "_resolve_exchange_testnet", lambda: True)
    monkeypatch.setattr(trading_domain, "_pending_execution_trade_ids", lambda: set())
...
def test_force_close_trade_closes_exchange_backed_position(helix_db, monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.exchange.hyperliquid.close_position",
...
def test_force_close_route_raises_on_failure(helix_db, monkeypatch):
    ...
    from helix.routers import trading as trading_router
