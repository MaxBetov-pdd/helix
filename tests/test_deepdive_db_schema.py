...
def test_deepdive_threads_table_exists(helix_db):
    with get_db() as conn:
        cur = conn.execute(
...
def test_deepdive_messages_table_exists(helix_db):
    with get_db() as conn:
        cur = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
            ("deepdive_messages",),
...
def test_deepdive_threads_columns(helix_db):
    with get_db() as conn:
...
def test_deepdive_messages_columns(helix_db):
    with get_db() as conn:
...
def test_deepdive_indices_exist(helix_db):
    with get_db() as conn:
        rows = conn.execute(
