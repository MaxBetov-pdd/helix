...
def test_pick_blocked_until_min_strategies_reached(helix_db):
...
def test_pick_allowed_after_min_strategies_reached(helix_db):
...
def test_strategies_before_pick_do_not_count(helix_db):
...
def test_round_robin_distributes_picks(helix_db):
...
def test_run_promotion_loop_logs_no_eligible(helix_db, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="helix.hypothesis_promotion"):
        with patch("helix.brain.assign_task") as mock:
...
def test_disproven_strategies_still_count_for_depth(helix_db):
