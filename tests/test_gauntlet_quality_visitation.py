...
def test_fresh_workflows_ordered_by_sharpe_desc(helix_db):
...
def test_null_or_empty_metrics_sort_last_among_fresh(helix_db):
...
def test_stale_low_quality_floats_ahead_of_fresh_high_quality(helix_db):
...
def test_within_stale_tier_strict_oldest_first(helix_db):
...
def test_backfill_creates_higher_sharpe_first_under_limit(helix_db):
