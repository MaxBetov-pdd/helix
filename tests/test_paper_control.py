"""Tests for the manual paper-position controls (helix/api_domains/paper_control.py).
...
import helix.api_domains.paper_control as pc
...
def test_close_paper_position(helix_db):
...
def test_manual_close_reason_is_not_synthetic(helix_db):
    _insert_paper_strategy()
...
def test_partial_close_keeps_residual(helix_db):
    _insert_paper_strategy()
...
def test_open_manual_position(helix_db):
    _insert_paper_strategy()
    _set_mid("BTC", 100.0)
...
def test_open_manual_rejects_when_already_open(helix_db):
...
def test_open_manual_rejects_inverted_stop(helix_db):
    from fastapi import HTTPException
...
def test_adjust_stop_loss_and_take_profit(helix_db):
    _insert_paper_strategy()
...
def test_set_manual_pause(helix_db):
    _insert_paper_strategy()
...
def test_flip_position(helix_db):
    _insert_paper_strategy()
...
def _enable_books(*, long_addr: str | None = None, short_addr: str | None = None) -> None:
    ...
    kv_set("helix:settings", settings)


def test_trade_is_live_classification(helix_db):
...
def test_live_close_places_order_and_clean_reason(helix_db, monkeypatch):
...
def test_live_open_respects_gate(helix_db, monkeypatch):
    from fastapi import HTTPException
...
def test_live_open_places_market_order_and_registers(helix_db, monkeypatch):
    import helix.exchange.hyperliquid as hl
...
def test_live_adjust_stop_places_protective_stop(helix_db, monkeypatch):
    import helix.exchange.hyperliquid as hl
...
def test_live_open_routes_to_short_book(helix_db, monkeypatch):
    import helix.exchange.hyperliquid as hl
...
def test_live_open_long_only_skips_short(helix_db, monkeypatch):
    from fastapi import HTTPException
...
def test_live_close_routes_to_trade_book(helix_db, monkeypatch):
    import helix.exchange.hyperliquid as hl
...
def test_route_close_position(helix_db):
    _insert_paper_strategy()
...
def test_route_open_position_validation(helix_db):
    _insert_paper_strategy()
    _set_mid("BTC", 100.0)
...
def test_route_partial_close(helix_db):
    _insert_paper_strategy()
