...
from helix.brain import _SYSTEM_FORCE_ACTORS, _USER_ACTORS, transition_stage
from helix.db import get_db
from helix.evolution import _archive_terminal_quick_screen_gate_failure
from helix.gauntlet.engine import demote_failed_gate_strategies
from helix.gauntlet.store import WORKFLOW_DEFINITION_VERSION, create_or_get_workflow
from helix.gauntlet.tasks import run_quick_screen_gate
...
def test_auto_archive_goes_through_transition_stage_with_terminal_cleanup(helix_db):
...
def test_auto_archive_never_archives_canonical_strategy(helix_db):
...
def test_auto_archive_succeeds_under_ghost_protection(helix_db):
...
def test_quick_screen_gate_reports_failed_gate_when_guardrails_block(helix_db):
...
def test_quick_screen_gate_reports_blocked_runtime_when_backtest_evidence_missing(helix_db):
...
def test_quick_screen_gate_passes_and_advances_stage_with_real_evidence(helix_db):
...
def test_evolution_terminal_archive_succeeds_under_ghost_protection(helix_db):
...
def test_evolution_terminal_archive_still_respects_canonical_guard(helix_db):
...
def test_restored_strategy_survives_demote_sweep(helix_db):
...
def test_restore_resets_cancelled_workflow_so_strategy_is_not_stranded(helix_db):
...
def test_restore_retires_old_version_failed_workflow_instead_of_resetting(helix_db):
...
def test_quick_screen_guardrails_defer_in_testing_mode(helix_db):
    ...
    from helix.brain import _quick_screen_overfitting_guardrails
...
def test_quick_screen_guardrails_still_hard_reject_without_testing_mode(helix_db):
    from helix.brain import _quick_screen_overfitting_guardrails
    from helix.policy import load_pipeline_config, save_pipeline_config
...
def test_quick_screen_guardrail_floors_are_wired_settings(helix_db):
...
def test_quick_screen_data_quality_hold_survives_testing_mode(helix_db):
    ...
    from helix.brain import _quick_screen_overfitting_guardrails
    from helix.metrics_integrity import DATA_QUALITY_HOLD_PREFIX
    from helix.policy import load_pipeline_config, save_pipeline_config
