...
import helix.source_reconciliation as sr
...
def test_ts_close_frame_aligns_across_representations():
    ...
    from helix.data import reconcile_close_prices
...
def _patch_sources(monkeypatch, lake_closes, hl_closes, source="binance"):
    ...
    import helix.market_data as md
...
def test_gate_disabled_allows(helix_db):
    _seed_strategy()
...
def test_gate_missing_fail_open(helix_db):
    _seed_strategy()
...
def test_gate_missing_blocks_when_block_when_missing(helix_db):
    _seed_strategy()
...
def test_gate_blocks_high_divergence(helix_db):
    _seed_strategy()
...
def test_gate_allows_low_divergence(helix_db):
    _seed_strategy()
...
def test_gate_insufficient_overlap_treated_as_missing(helix_db):
    _seed_strategy()
...
def test_gate_stale_payload_fails_open(helix_db):
    _seed_strategy()
...
def test_resolve_min_overlap_reads_setting(monkeypatch):
    ...
    import helix.dataeng.settings as de
...
def test_evaluate_promotion_blocks_paper_on_divergence(helix_db):
    ...
    kv_set("helix:settings", _settings(max_pct=2.0))
    ok, reason = evaluate_promotion("S-DIV", "gauntlet", "paper")
    assert ok is False
    assert "divergence" in reason.lower()


def test_evaluate_promotion_divergence_gate_inert_when_disabled(helix_db):
    ...
    kv_set("helix:settings", _settings(enabled=False))
