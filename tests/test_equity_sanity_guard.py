...
def test_garbage_equity_does_not_arm_kill_switch(helix_db):
...
def test_corrupted_hwm_self_heals_on_next_good_tick(helix_db):
...
def test_nan_and_nonpositive_samples_rejected(helix_db):
...
def test_sustained_large_jump_self_heals_after_streak(helix_db):
...
def test_real_drawdown_still_fires_kill_switch(helix_db):
