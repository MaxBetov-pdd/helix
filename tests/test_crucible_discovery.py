...
def test_discovery_disabled_by_default(helix_db):
    res = run_crucible_discovery()
    assert res["created"] is False
    assert res["reason"] == "disabled"


def test_discovery_enabled_dispatches_benchmarking_task(helix_db):
    _enable()
    res = run_crucible_discovery()
...
def test_discovery_dedups_open_task(helix_db):
    _enable()
...
def test_discovery_autonomous_mode_is_stamped(helix_db):
...
def test_force_bypasses_disabled_setting(helix_db):
...
def test_force_still_dedups(helix_db):
...
def test_discovery_task_lists_known_crucibles_in_description(helix_db):
