"""Tests for helix.diagnostics — health checks + snapshot."""
...
from helix.task_progress import mark_interrupted
...
def test_check_database_passes_after_init(helix_db):
    result = check_database()
    assert result.status == PASS
    assert "schema" in result.summary


def test_check_database_does_not_run_init(monkeypatch, helix_db):
    def _boom():
        raise AssertionError("diagnostics must not initialize schema")

    monkeypatch.setattr("helix.db.init_db", _boom)
    result = check_database()
    assert result.status == PASS


def test_check_resumable_tasks_pass_when_none(helix_db):
    result = check_resumable_tasks()
    assert result.status == PASS
    assert result.detail["count"] == 0


def test_check_resumable_tasks_warn_when_present(helix_db):
...
def test_check_recent_costs_aggregates(helix_db):
...
def test_check_recent_costs_outside_window_excluded(helix_db):
...
def test_check_recent_truncations_zero(helix_db):
    result = check_recent_truncations()
    assert result.status == PASS
    assert result.detail["count_24h"] == 0


def test_check_scheduler_freshness_warn_when_no_runs(helix_db):
    result = check_scheduler_freshness()
    assert result.status == WARN


def test_snapshot_aggregates(helix_db):
    payload = snapshot()
...
def test_snapshot_overall_warn_with_resumable(helix_db):
...
def test_doctor_cli_runs_and_returns_json(helix_db):
    runner = CliRunner()
...
def test_doctor_cli_human_output(helix_db):
    runner = CliRunner()
    result = runner.invoke(cli, ["doctor"])
    assert result.exit_code in (0, 2)
    assert "helix doctor" in result.output
