...
- helix.hypotheses.update_hypothesis
...
def test_update_hypothesis_patches_only_supplied_fields(helix_db):
...
def test_update_hypothesis_all_fields(helix_db):
    hyp = _base_hypothesis()
    updated = update_hypothesis(
        hyp["id"],
        title="T",
...
def test_update_hypothesis_no_fields_returns_current(helix_db):
    hyp = _base_hypothesis()
...
def test_update_hypothesis_missing_id_raises(helix_db):
    with pytest.raises(ValueError):
        update_hypothesis("HYP-nonexistent", title="x")


def test_update_hypothesis_empty_required_fields_raises(helix_db):
    hyp = _base_hypothesis()
    with pytest.raises(ValueError):
...
def test_update_hypothesis_fields_tool_partial_update(helix_db):
    from helix.agents.tools_research import _tool_update_hypothesis_fields
...
def test_update_hypothesis_fields_tool_unknown_id_returns_error(helix_db):
    from helix.agents.tools_research import _tool_update_hypothesis_fields
    out = json.loads(_tool_update_hypothesis_fields({
...
def test_update_hypothesis_fields_tool_ignores_immutable_fields(helix_db):
...
def test_list_hypothesis_artifacts_tool_returns_cached_content(helix_db):
    ...
    from helix.agents.tools_research import _tool_list_hypothesis_artifacts
...
def test_list_hypothesis_artifacts_tool_missing_id_returns_error(helix_db):
    from helix.agents.tools_research import _tool_list_hypothesis_artifacts
...
def test_from_url_enqueues_strategy_developer_research_task(helix_db):
    fake_preview = {
        "status": "ok",
    ...
    with patch(
        "helix.research_sources.url_ingest.inspect_youtube_video",
...
def test_from_url_in_manual_mode_enqueues_operator_research(helix_db):
...
def test_from_url_enqueue_failure_does_not_break_paste(helix_db):
    ...
    with patch(
        "helix.research_sources.url_ingest.blog.inspect_blog_article",
        return_value=fake_preview,
    ), patch("helix.brain.assign_task", side_effect=_raise):
        client = TestClient(app)
        r = client.post(
            "/api/hypotheses/from_url",
...
def test_preview_url_does_not_enqueue_task(helix_db):
...
def test_manual_create_in_manual_mode_defers_research_but_explicit_research_still_runs(helix_db):
    control_plane_ops.update_system_mode("manual")
    client = TestClient(app)
