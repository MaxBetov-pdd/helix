...
from helix.phantom_recovery import handle_phantom_repair_completion, submit_phantom_replay_sync
from helix.strategy_lifecycle import get_strategy_container, read_strategies
...
def test_begin_phantom_recovery_creates_single_claim_row(helix_db):
...
def test_read_and_detail_schedule_inline_recovery_once_via_db_claim(monkeypatch, helix_db):
    ...
    monkeypatch.setattr("helix.phantom_recovery._INLINE_PHANTOM_RECOVERY_EXECUTOR", FakeExecutor())
...
def test_read_strategies_refuses_claim_if_backtest_arrives_before_claim(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
    ...
    monkeypatch.setattr("helix.phantom_recovery._schedule_allowed", _stale_outer_guard)
...
def test_read_strategies_allows_claim_if_placeholder_backtest_arrives_before_claim(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_read_strategies_blocks_malformed_b1abc_backtest_id_before_claim(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_get_strategy_container_refuses_claim_if_strategy_leaves_active_lane_before_claim(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_read_strategies_reclaims_stale_replay_running_claim(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_read_and_detail_keep_fresh_replay_running_claim_deduped(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_read_and_detail_skip_existing_backtest_results(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_get_strategy_container_uses_full_backtest_check_when_history_window_is_truncated(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_read_and_detail_skip_non_active_stages(monkeypatch, helix_db):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
def test_read_and_detail_skip_terminal_recovery_rows(monkeypatch, helix_db, terminal_status):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
...
@pytest.mark.parametrize("terminal_status", ["healed", "exhausted"])
def test_schedule_inline_phantom_recovery_cannot_reopen_terminal_rows_when_outer_guard_is_stale(monkeypatch, helix_db, terminal_status):
    submissions: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
    ...
    monkeypatch.setattr("helix.phantom_recovery._INLINE_PHANTOM_RECOVERY_EXECUTOR", FakeExecutor())
    monkeypatch.setattr("helix.phantom_recovery._schedule_allowed", lambda strategy_id: True)
    ...
    from helix.phantom_recovery import schedule_inline_phantom_recovery
...
def test_submit_phantom_replay_sync_marks_healed_on_success(monkeypatch, helix_db):
    with get_db() as conn:
        conn.execute(
            "INSERT INTO strategies (id, name, type, symbol, timeframe, params, stage, status, created_at, updated_at) "
            "VALUES ('S20002', 'BTC-RSI-S20002', 'rsi_momentum', 'BTC', '1h', '{}', 'gauntlet', 'gauntlet', datetime('now'), datetime('now'))"
    ...
    monkeypatch.setattr("helix.phantom_recovery._submit_phantom_replay_backtest", _fake_submit)
...
def test_submit_phantom_replay_sync_moves_to_repair_pending_on_submission_error(monkeypatch, helix_db):
    with get_db() as conn:
        conn.execute(
            "INSERT INTO strategies (id, name, type, symbol, timeframe, params, stage, status, created_at, updated_at) "
    ...
    monkeypatch.setattr("helix.phantom_recovery._submit_phantom_replay_backtest", _raise_submit)
    monkeypatch.setattr("helix.phantom_recovery._assign_phantom_repair_task", lambda strategy_id, reason: 9001)
...
def test_submit_phantom_replay_sync_moves_to_repair_pending_without_result_id(monkeypatch, helix_db):
    with get_db() as conn:
        conn.execute(
            "INSERT INTO strategies (id, name, type, symbol, timeframe, params, stage, status, created_at, updated_at) "
    ...
    monkeypatch.setattr(
        "helix.phantom_recovery._submit_phantom_replay_backtest",
        lambda strategy_id: {"status": "accepted", "warning": "replay_returned_without_result_id"},
    )
    monkeypatch.setattr("helix.phantom_recovery._assign_phantom_repair_task", lambda strategy_id, reason: 9002)
...
def test_handle_phantom_repair_completion_queues_final_retry(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.phantom_recovery.schedule_final_retry",
...
def test_handle_phantom_repair_completion_exhausts_without_fix(monkeypatch):
    ...
    monkeypatch.setattr(
        "helix.phantom_recovery.mark_phantom_recovery_exhausted",
...
def test_mark_phantom_recovery_healed_stores_result_id(helix_db):
    _insert_strategy("S12345")
...
def test_begin_phantom_recovery_does_not_reopen_after_terminal_cycle(helix_db, prior_status):
...
def test_missing_strategy_recovery_helpers_return_false(helix_db):
    claimed = begin_phantom_recovery(
        "S99999",
        trigger="read_strategies",
        next_status="replay_running",
...
def test_mark_phantom_recovery_healed_requires_existing_recovery_row(helix_db):
...
def test_mark_phantom_recovery_healed_rejects_missing_result_id(helix_db):
...
def test_mark_phantom_recovery_healed_rejects_foreign_result_id(helix_db):
...
def test_mark_phantom_recovery_healed_rejects_deleted_owned_result(helix_db):
...
def test_mark_phantom_recovery_healed_rejects_trashed_owned_result(helix_db):
...
def test_begin_phantom_recovery_rejects_invalid_status(helix_db):
...
def test_begin_phantom_recovery_rejects_terminal_statuses(helix_db, terminal_status):
...
def test_begin_phantom_recovery_is_atomic_across_concurrent_callers(helix_db):
...
def test_init_db_migrates_pre_recovery_schema_and_keeps_existing_data(_isolate_helix_home):
...
def test_janitor_finalizes_repair_with_dead_agent_task(helix_db, task_status):
    from helix.phantom_recovery import reclaim_wedged_phantom_recovery_states
...
def test_janitor_finalizes_repair_with_missing_agent_task(helix_db):
    from helix.phantom_recovery import reclaim_wedged_phantom_recovery_states
...
def test_janitor_leaves_alive_repair_task_alone(helix_db):
    from helix.phantom_recovery import reclaim_wedged_phantom_recovery_states
...
def test_janitor_skips_fresh_repair_claims(helix_db):
...
def test_janitor_redrives_stale_final_retry(monkeypatch, helix_db):
    ...
    import helix.phantom_recovery as pr
...
def test_janitor_leaves_fresh_final_retry_alone(monkeypatch, helix_db):
    import helix.phantom_recovery as pr
...
def test_sweep_batch_not_starved_by_wedged_rows(monkeypatch, helix_db):
