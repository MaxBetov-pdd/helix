"""Tests for unified Helix walk-forward validation."""
...
def test_walk_forward_insufficient_data_returns_error(monkeypatch, helix_db):
    def _short_candles(*_args, **_kwargs):
        return _fake_ohlcv(200)

    monkeypatch.setattr("helix.scanner.fetch_candles", _short_candles)
...
def test_walk_forward_returns_valid_structure(monkeypatch, helix_db):
    def _full_candles(*_args, **_kwargs):
        return _fake_ohlcv(1000)

    monkeypatch.setattr("helix.scanner.fetch_candles", _full_candles)
...
def test_walk_forward_accepts_explicit_timeframe(monkeypatch, helix_db):
    ...
    monkeypatch.setattr("helix.strategies.backtest.load_backtest_candles", _candles)
...
def test_walk_forward_gap_reduces_effective_oos(monkeypatch, helix_db):
    def _candles(*_args, **_kwargs):
        return _fake_ohlcv(1000)

    monkeypatch.setattr("helix.scanner.fetch_candles", _candles)
