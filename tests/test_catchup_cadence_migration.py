...
from helix.scheduler import add_job, migrate_data_engine_catchup_cadence

_JID = "helix-data-engine-catchup"
...
def test_catchup_migration_updates_old_default_only(helix_db):
...
def test_catchup_migration_leaves_custom_schedule_untouched(helix_db):
    add_job(job_id=_JID, name="Data Engine Catch-Up", schedule_type="interval",
...
def test_catchup_migration_noop_when_job_absent(helix_db):
    assert migrate_data_engine_catchup_cadence() is False
