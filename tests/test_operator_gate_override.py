...
import helix.strategy_lifecycle as sl
from helix.strategy_lifecycle import (
...
def _seed(helix_db, sid="S-OVR", stage="paper", source="manual"):
...
def _capture_transition(monkeypatch):
    ...
    monkeypatch.setattr("helix.brain.transition_stage", _fake)
    return captured


# --- promote_strategy -------------------------------------------------------

def test_force_is_neutered_for_live_without_override(helix_db, monkeypatch):
    sid = _seed(helix_db)
...
def test_override_forces_live_promotion(helix_db, monkeypatch):
    sid = _seed(helix_db)
    cap = _capture_transition(monkeypatch)
...
def test_force_still_works_for_noncapital_stage(helix_db, monkeypatch):
    sid = _seed(helix_db, stage="quick_screen")
    cap = _capture_transition(monkeypatch)
...
def test_lifecycle_override_forces_and_coerces_actor(helix_db, monkeypatch):
    sid = _seed(helix_db)
    cap = _capture_transition(monkeypatch)
    transition_lifecycle_strategy(
...
def test_lifecycle_no_override_neuters_force(helix_db, monkeypatch):
    sid = _seed(helix_db)
    cap = _capture_transition(monkeypatch)
    transition_lifecycle_strategy(
