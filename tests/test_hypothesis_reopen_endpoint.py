...
def test_reopen_flips_disproven_to_researching(helix_db):
...
def test_reopen_nonexistent_hypothesis_404(helix_db):
    client = TestClient(app)
    r = client.post("/api/hypotheses/HYP-nope/reopen", json={})
    assert r.status_code == 404


def test_reopen_already_researching_is_noop_with_200(helix_db):
