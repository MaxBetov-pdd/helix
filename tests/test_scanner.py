...
from helix.strategies.base import Signal
...
from helix.trade_state import mark_trade_pending_close_reconcile
...
def test_manage_positions_closes_on_take_profit_without_exit_signal(monkeypatch):
    ...
    monkeypatch.setattr("helix.config.get_execution_mode", lambda: "paper")
...
def test_execute_trade_intent_keeps_trade_open_when_close_fill_is_unconfirmed(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_manage_positions_blocks_same_bar_reopen_from_recent_closed_trade_in_db(helix_db, monkeypatch):
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO trades
...
def test_run_scan_bypasses_regime_filters_in_paper_mode(monkeypatch, helix_db):
    import helix.config as config_mod
    import helix.regime as regime_mod
...
def test_get_signal_instantiates_registered_type_when_strategy_id_is_not_active(monkeypatch):
    ...
    monkeypatch.setattr("helix.strategies.registry.get_active", lambda: {})
    monkeypatch.setattr("helix.strategies.registry._TYPE_MAP", {"fake": _FakeStrategy})
...
def test_enrich_scan_frame_passes_pair_form_to_data_manager(monkeypatch):
    ...
    from helix.data_manager import data_manager as dm
