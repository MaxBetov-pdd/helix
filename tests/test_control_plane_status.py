...
from helix.control_plane import status as control_plane_status


def test_get_system_status_reports_pause_state(helix_db, monkeypatch):
    monkeypatch.setattr(
...
def test_get_system_heartbeat_preserves_expected_keys(monkeypatch):
    ...
    monkeypatch.setattr("helix.api_domains.trading.read_open_trades", lambda verify_exchange=False: [])
    monkeypatch.setattr("helix.api_domains.tasks.get_agent_tasks", lambda: [])
    monkeypatch.setattr("helix.api_domains.data.get_datasets_stub", lambda remote_skip=False: [])
    monkeypatch.setattr("helix.api_domains.analytics.get_research_feed_metrics_stub", lambda: {"new_count": 0})
    monkeypatch.setattr("helix.api_domains.analytics.list_scanner_scans_stub", lambda limit=200: [])
    monkeypatch.setattr("helix.api_domains.data.get_data_ingestion_runs", lambda limit=25, offset=0, remote_skip=True: [])
    monkeypatch.setattr("helix.api_domains.paper.get_paper_sessions", lambda: [])
    monkeypatch.setattr("helix.db.get_strategies", lambda: [])
    monkeypatch.setattr("helix.control_plane.approvals.get_approvals_list", lambda status=None: [])
    monkeypatch.setattr(
        control_plane_status.core,
        "_load_settings_payload",
        lambda: {},
...
def test_get_system_heartbeat_includes_memory_nav_indicator(monkeypatch):
    ...
    monkeypatch.setattr("helix.db.get_strategies", lambda: [])
    monkeypatch.setattr("helix.control_plane.approvals.get_approvals_list", lambda status=None: [])
    monkeypatch.setattr("helix.api_domains.memory.get_memory_nav_indicator", lambda: {
        "kind": "status",
        "severity": "success",
        "label": "CANON",
...
def test_system_heartbeat_route_sanitizes_nonfinite_values(monkeypatch):
    from helix.routers import status as status_router
...
def test_get_dashboard_exposes_pause_and_recovery_state(helix_db, monkeypatch):
    monkeypatch.setattr(
        control_plane_status,
        "normalize_daemon_state",
        lambda write_back=True: {
