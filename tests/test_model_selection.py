...
from helix import model_selection as ms
...
def test_env_only_key_is_not_connected(helix_db, monkeypatch):
...
def test_connected_requires_both_record_and_token(helix_db, monkeypatch):
    _connect("gemini")
...
def test_mark_and_unmark(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
...
def test_allowed_pairs_intersect_connected_and_selected(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
...
def test_keyless_local_provider_usable_without_token(helix_db, monkeypatch):
    ...
    import helix.auth.store as store
...
def test_model_match_is_case_insensitive(helix_db, monkeypatch):
...
def test_resolve_route_fails_closed_when_nothing_selected(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
    ms.enable_enforcement()
...
def test_resolve_route_drops_unallowed_primary_and_fallbacks(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
    ms.enable_enforcement()
    _connect("gemini")
...
def test_resolve_route_passthrough_when_enforcement_off(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
...
def test_assert_callable_noop_when_enforcement_off(helix_db, monkeypatch):
    ms.disable_enforcement()
    _connect()
    ms.assert_callable("openai", "gpt-5.2", slot="t")  # must not raise


def test_assert_callable_blocks_unconnected_and_unselected(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
    ms.enable_enforcement()
    _connect("gemini")
    _set_enabled("gemini:gemini-2.5-flash-lite")
...
def test_agent_row_selection_is_allowed_when_connected(helix_db, monkeypatch):
    _all_tokens_present(monkeypatch)
    _connect("openrouter")
...
def test_migrate_seeds_connected_from_profiles(helix_db, monkeypatch):
    monkeypatch.setattr(
        ms, "load_auth",
